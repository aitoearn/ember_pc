...
allowed-tools: Bash, ember_create_url_parse_task
metadata:
  ember_argument_hint: 输入 URL、抽取目标（摘要/要点/全文清洗）、输出格式要求。
  ember_when_to_use: 用户提供链接并希望抽取正文、要点或可引用信息时使用。
  ember_version: 1.1.0
...
- 优先调用 `Bash` 执行 `ember task create url-parse --json` 创建任务。
- 若当前环境暂时无法执行 `ember` CLI，再回退到 `ember_create_url_parse_task`。
