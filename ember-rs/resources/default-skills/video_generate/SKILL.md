...
allowed-tools: Bash
metadata:
  ember_argument_hint: 输入主题、受众、平台、时长、画幅、风格、素材来源。
  ember_when_to_use: 用户要求生成视频，或将现有文稿改编为短视频。
  ember_version: 1.2.0
...
- 优先调用 `Bash` 执行 `ember media video generate --json` 或 `ember task create video --json` 创建真实任务。
- 若当前环境暂时无法执行 `ember` CLI，应报告视频生成绑定不可用并停止，不要回退旧视频 facade。
- `projectId` 必须来自当前工作区项目；不要虚构 providerId/model。
- 任务结果必须兼容 App Server `mediaTaskArtifact/video/create` 与 `ember task create video --json` 的任务文件契约。
- 禁止伪造“视频已生成完成”。
