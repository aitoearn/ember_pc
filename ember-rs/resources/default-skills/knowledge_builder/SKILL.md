...
metadata:
  ember_argument_hint: 输入知识包类型、pack name、项目根目录、来源目录或粘贴资料，并说明目标使用场景。
  ember_when_to_use: 用户需要把个人、品牌产品、组织流程、项目资料或增长策略整理成可确认的 Agent Knowledge 知识包时使用。
  ember_version: 1.2.1
  ember_execution_mode: prompt
  ember_surface: workbench
  ember_category: knowledge
  Ember_deprecated: "true"
  Ember_compat_delegate: "true"
  Ember_deprecated_reason: "current Knowledge v2 uses dedicated Builder Skills and document-first KnowledgePack outputs"
---
...
1. **不要生成 `compiled/brief.md`**；运行时视图由 Ember 从 `documents/` 派生 `compiled/splits/*`。
...
| pack type / ember template | current Builder Skill | family |
| --- | --- | --- |
...
当历史 / 未知 pack 迁移清单清零后，Ember 应删除或停用本 Skill；后续新增知识库模板只能通过新的专用 Builder Skill 包进入。
