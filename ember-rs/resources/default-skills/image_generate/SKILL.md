...
allowed-tools: ember_create_image_generation_task
metadata:
  ember_argument_hint: 输入主题、画面主体、风格、构图、数量、尺寸。
  ember_when_to_use: 用户显式使用 @配图/@修图/@重绘/@image，或在普通对话中已确认“调用画图/现在生成”时使用；封面需求优先交给 cover_generate。
  ember_version: 1.3.5
...
- 给出必要风险提示后，仍必须直接调用 `ember_create_image_generation_task`，并把原始需求、必要的风险提示或安全替代建议放入任务参数；最终是否拒绝、改写或降级由图片生成执行器/图片模型自己的策略决定，外层 Agent 和本 Skill 不要自行替图片模型下最终拒绝结论。
...
- 不要搜索 “Skill image_generate”、“ember media image generate --json”、“ember_create_image_generation_task” 之类目录信息；当前上下文已经明确要求执行图片任务。
...
- 必须直接调用 `ember_create_image_generation_task` 创建真实图片任务。
- 调用 `ember_create_image_generation_task` 时，必须直接传扁平任务对象参数；不要包成 `{"image_task": ...}`，更不要把整个任务对象再序列化成字符串。
...
- 不要通过 `Bash` 拼接 `ember media image generate --json`、`ember task create image --json` 或任何 `/tmp/ember_task_image_*.json` 临时任务文件。
- `ember_create_image_generation_task` 返回后，应依赖同一份图片任务文件契约推进执行与结果回填；不要另起一套 markdown“提交成功”假产物，也不要输出独立的任务递交摘要。
- 调用 `ember_create_image_generation_task` 时不要传 `outputPath`，不要把任务写成 markdown 文稿。
...
工具调用成功后，不要再输出“任务类型 / 任务 ID / 任务文件 / 状态”这类递交摘要；Ember 会在同一条 assistant 消息里展示工具调用、任务进度和图片结果。若任务仍是 pending / queued / running，只保留一句自然承接，不重复 task_id、path 或排队模板；若已经拿到真实图片结果，可以用一两句自然收尾，引导用户继续调整，不要复述固定模板。
