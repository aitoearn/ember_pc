...
allowed-tools: Bash, ember_create_broadcast_generation_task
metadata:
  ember_argument_hint: 输入原文、目标听众、语气、预计时长、重点段落。
  ember_when_to_use: 用户希望把现有文稿转成播客内容，但不要求你直接写主持稿。
  ember_version: 1.1.0
...
- 优先调用 `Bash` 执行 `ember task create broadcast --json` 创建任务。
- 若当前环境暂时无法执行 `ember` CLI，再回退到 `ember_create_broadcast_generation_task`。
