...
use ember_knowledge::{KnowledgeBuilderRuntimeExecution, KnowledgeBuilderRuntimePlan};
use ember_skills::{ExecutionCallback, LoadedSkillDefinition};
use std::sync::Arc;
...
async fn execute_builder_skill(
    agent_state: &AsterAgentState,
    plan: &KnowledgeBuilderRuntimePlan,
) -> Result<ember_agent::SkillExecutionResult, String> {
...
fn load_executable_builder_skill(skill_name: &str) -> Result<LoadedSkillDefinition, String> {
    let skill = ember_skills::find_skill_by_name(skill_name)?;
