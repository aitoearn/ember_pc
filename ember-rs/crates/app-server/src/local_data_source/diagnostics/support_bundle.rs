...
fn config_path_guess() -> Option<PathBuf> {
    dirs::config_dir().map(|dir| dir.join("ember").join("config.yaml"))
}
...
fn export_support_bundle_to(
    ...
    let bundle_name = format!("Ember-Support-{timestamp}");
    ...
    let database_path = database::get_db_path()
        .ok()
        .or_else(|| legacy_data_dir.as_ref().map(|dir| dir.join("ember.db")));
