...
fn resolve_skill_scaffold_root(
    ...
    match target {
        ...
        SkillScaffoldTarget::Project => match app {
            SkillPackageApp::Ember => app_paths::resolve_project_skills_dir()
                .ok_or_else(|| "Failed to resolve project skills directory".to_string()),
            _ => Err("Project skill scaffold is only supported for ember".to_string()),
        },
    }
}
...
) -> Result<ember_core::models::skill_model::SkillPackageInspection, String> {
