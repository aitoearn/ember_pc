               ...
               AND NOT (
                    ...
                    OR CASE
                        WHEN json_valid(s.extension_data_json) THEN
                            COALESCE(json_extract(s.extension_data_json, '$.\"ember_harness.v0\".hiddenFromUserRecents') = 1, 0)
                            OR COALESCE(json_extract(s.extension_data_json, '$.\"ember_harness.v0\".hidden_from_user_recents') = 1, 0)
                        ELSE 0
                    END
                )
...
fn current_timeline_execution_runtime(session: &CurrentTimelineSessionRow) -> Value {
    ...
    let provider_selector = extension_data
        .pointer("/ember_provider_routing.v0/providerSelector")
        .and_then(Value::as_str)
        .or_else(|| {
            extension_data
                .pointer("/ember_provider_routing.v0/provider_selector")
                .and_then(Value::as_str)
        });
    let recent_access_mode = extension_data
        .get("ember_recent_access_mode.v0")
        .and_then(Value::as_str);
    json!({
        ...
        "recent_preferences": extension_data
            .get("ember_recent_preferences.v0")
            .cloned()
            .unwrap_or(Value::Null),
        "recent_team_selection": extension_data
            .get("ember_recent_team_selection.v0")
            .cloned()
            .unwrap_or(Value::Null),
    })
}
