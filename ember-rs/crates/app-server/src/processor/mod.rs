...
use app_server_protocol::METHOD_PERF_MONITOR_SESSION_LIST;
use app_server_protocol::METHOD_PERF_MONITOR_SESSION_READ;
use app_server_protocol::METHOD_PERF_MONITOR_SESSION_SAVE;
use app_server_protocol::METHOD_PERF_MONITOR_TRACE_ANALYSIS_LIST;
use app_server_protocol::METHOD_PERF_MONITOR_TRACE_ANALYSIS_SAVE;
use app_server_protocol::METHOD_PERF_MONITOR_TRACE_DELETE;
use app_server_protocol::METHOD_PERF_MONITOR_TRACE_LIST;
use app_server_protocol::METHOD_PERF_MONITOR_TRACE_READ;
use app_server_protocol::METHOD_PERF_MONITOR_TRACE_SAVE;
use app_server_protocol::METHOD_TEST_CASE_DELETE;
use app_server_protocol::METHOD_TEST_CASE_LIST;
use app_server_protocol::METHOD_TEST_CASE_MODULE_DELETE;
use app_server_protocol::METHOD_TEST_CASE_MODULE_LIST;
use app_server_protocol::METHOD_TEST_CASE_MODULE_SAVE;
use app_server_protocol::METHOD_TEST_CASE_READ;
use app_server_protocol::METHOD_TEST_CASE_RUN_LIST;
use app_server_protocol::METHOD_TEST_CASE_RUN_SAVE;
use app_server_protocol::METHOD_TEST_CASE_SAVE;
use app_server_protocol::METHOD_DEVICE_FLOW_DELETE;
use app_server_protocol::METHOD_DEVICE_FLOW_HEALING_LIST;
use app_server_protocol::METHOD_DEVICE_FLOW_HEALING_RESOLVE;
use app_server_protocol::METHOD_DEVICE_FLOW_HEALING_SAVE;
use app_server_protocol::METHOD_DEVICE_FLOW_LIST;
use app_server_protocol::METHOD_DEVICE_FLOW_READ;
use app_server_protocol::METHOD_DEVICE_FLOW_RUN_LIST;
use app_server_protocol::METHOD_DEVICE_FLOW_RUN_READ;
use app_server_protocol::METHOD_DEVICE_FLOW_RUN_SAVE;
use app_server_protocol::METHOD_DEVICE_FLOW_SAVE;
use app_server_protocol::METHOD_DEVICE_EXPLORE_READ;
use app_server_protocol::METHOD_DEVICE_EXPLORE_SAVE;
use app_server_protocol::METHOD_DEVICE_EXPLORE_RUN_LIST;
use app_server_protocol::METHOD_DEVICE_EXPLORE_RUN_READ;
use app_server_protocol::METHOD_DEVICE_EXPLORE_RUN_SAVE;
    ...
    async fn handle_request_inner(
        ...
        let result = match method.as_str() {
            ...
            METHOD_TEST_CASE_MODULE_DELETE => {
                self.handle_test_case_module_delete_impl(params).await
            }
            ...
            METHOD_PERF_MONITOR_SESSION_SAVE => {
                self.handle_perf_monitor_session_save_impl(params).await
            }
            METHOD_PERF_MONITOR_SESSION_LIST => {
                self.handle_perf_monitor_session_list_impl(params).await
            }
            METHOD_PERF_MONITOR_SESSION_READ => {
                self.handle_perf_monitor_session_read_impl(params).await
            }
            ...
            METHOD_DEVICE_FLOW_HEALING_LIST => {
                self.handle_device_flow_healing_list_impl(params).await
            }
            METHOD_DEVICE_FLOW_HEALING_SAVE => {
                self.handle_device_flow_healing_save_impl(params).await
            }
            METHOD_DEVICE_FLOW_HEALING_RESOLVE => {
                self.handle_device_flow_healing_resolve_impl(params).await
            }
            ...
            METHOD_PERF_MONITOR_TRACE_SAVE => {
                self.handle_perf_monitor_trace_save_impl(params).await
            }
            METHOD_PERF_MONITOR_TRACE_LIST => {
                self.handle_perf_monitor_trace_list_impl(params).await
            }
            METHOD_PERF_MONITOR_TRACE_READ => {
                self.handle_perf_monitor_trace_read_impl(params).await
            }
            METHOD_PERF_MONITOR_TRACE_DELETE => {
                self.handle_perf_monitor_trace_delete_impl(params).await
            }
            METHOD_PERF_MONITOR_TRACE_ANALYSIS_SAVE => {
                self.handle_perf_monitor_trace_analysis_save_impl(params)
                    .await
            }
            METHOD_PERF_MONITOR_TRACE_ANALYSIS_LIST => {
                self.handle_perf_monitor_trace_analysis_list_impl(params)
                    .await
            }
...
mod tests {
    ...
    async fn app_server_file_system_methods_require_initialized_and_return_current_results() {
        ...
        std::fs::write(&file_path, "# Ember").expect("write file");
        ...
        match &preview_messages[0] {
            JsonRpcMessage::Response(response) => {
                ...
                assert_eq!(response.result["content"], "# Ember");
