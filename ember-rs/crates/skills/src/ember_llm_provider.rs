//! Ember LLM Provider 实现
//!
//! 使用 API Key Provider 选择凭证并调用 LLM API。
//! trait 定义（LlmProvider, SkillError）已迁移到 ember-skills crate。
...
use ember_providers::providers::gemini::{GeminiApiKeyCredential, GeminiApiKeyProvider};
...
impl LimeLlmProvider {
    ...
    async fn call_claude_api(&self, call: ClaudeApiCall<'_>) -> Result<String, SkillError> {
        use ember_core::models::anthropic::AnthropicMessage;
