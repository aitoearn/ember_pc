...
use ember_agent::credential_bridge::CredentialBridge;
#[cfg(test)]
use ember_agent::request_tool_policy::REQUEST_TOOL_POLICY_MARKER;
use ember_agent::request_tool_policy::{
    merge_system_prompt_with_request_tool_policy, resolve_request_tool_policy,
    stream_message_reply_with_policy,
};
use ember_agent::{merge_system_prompt_with_runtime_agents, AsterAgentState, SessionConfigBuilder};
...
impl AgentExecutor {
    ...
    async fn execute_agent_chat(
        ...
        task: &ScheduledTask,
        db: &DbConnection,
        _aster_config: &ember_agent::credential_bridge::AsterProviderConfig,
    ) -> Result<serde_json::Value, String> {
        ...
        let execution = stream_message_reply_with_policy(
            ...
            &request_tool_policy,
            |event| match event {
                ember_agent::AgentEvent::ToolStart {
                    tool_name, tool_id, ..
                } => {
                    tracing::info!(
                ...
                ember_agent::AgentEvent::ToolEnd { tool_id, result } => {
                    tracing::info!(
