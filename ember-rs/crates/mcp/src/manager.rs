...
use ember_core::{tool_calling::ToolSurfaceMetadata, DynEmitter};
...
impl McpClientManager {
    ...
    pub async fn list_tools_for_context(
        ...
        let filtered = tools
            ...
            .filter(|tool| {
                ...
                if !ember_core::tool_calling::tool_visible_in_context(&metadata, include_deferred) {
                    return false;
                }
                ember_core::tool_calling::tool_matches_caller(&metadata, caller)
            })
            .collect();
    ...
    fn extract_tool_metadata(
        ...
        ember_core::tool_calling::extract_tool_surface_metadata(tool_name, input_schema)
    }
    ...
    fn score_tool_match(tool: &McpToolDefinition, query: &str) -> i32 {
        ...
        let inner_tool_name = extract_mcp_runtime_inner_tool_name(&tool.server_name, &tool.name)
            .unwrap_or(&tool.name);
        let mut score = [
            ember_core::tool_calling::score_tool_match(&tool.name, &tool.description, tags, query),
            ember_core::tool_calling::score_tool_match(
                inner_tool_name,
                &tool.description,
                tags,
                query,
