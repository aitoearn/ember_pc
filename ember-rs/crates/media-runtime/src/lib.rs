...
pub const DEFAULT_ARTIFACT_ROOT: &str = ".ember/tasks";
pub const IMAGE_TASK_RUNNER_WORKER_ID: &str = "ember-image-api-worker";
...
impl MediaRuntimeError {
    ...
    pub fn hint(&self) -> Option<&'static str> {
        match self {
            ...
            Self::TaskNotFound { .. } => {
                Some("可先运行 `ember task list` 或检查 `--artifact-dir`。")
            }
            Self::Conflict(_) => Some("请更换 `--output`，或使用稳定的 `--idempotency-key` 重试。"),
            Self::InvalidState(_) => Some("可先运行 `ember task status <task-id>` 查看当前状态。"),
...
fn build_attempt_record(
    ...
    TaskAttemptRecord {
        ...
        logs_ref: Some(format!(
            ".ember/task-logs/{task_id}/attempt_{attempt_index}.jsonl"
        )),
    }
}
    ...
    for (index, attempt) in record.attempts.iter_mut().enumerate() {
        ...
        if attempt.logs_ref.is_none() {
            attempt.logs_ref = Some(format!(
                ".ember/task-logs/{}/attempt_{}.jsonl",
...
fn apply_image_postprocess_prompt_hint(
    ...
    format!(
        "{prompt}\n\nLayered design alpha requirement: create the foreground subject on a flat chroma-key background ({}) so Ember can remove that key color after generation; avoid using that key color inside the subject.",
...
pub fn write_task_artifact(
    ...
    record.attempts[0].logs_ref = Some(format!(
        ".ember/task-logs/{}/attempt_1.jsonl",
        record.task_id
    ));
...
mod tests {
    ...
    fn write_media_task_artifact_uses_default_task_root() {
        ...
        assert!(output.path.starts_with(".ember/tasks/image_generate/"));
    ...
    fn write_task_artifact_supports_transcription_generate() {
        ...
        assert!(output
            .path
            .starts_with(".ember/tasks/transcription_generate/"));
    ...
    fn write_task_artifact_supports_audio_generate() {
        ...
        assert!(output.path.starts_with(".ember/tasks/audio_generate/"));
    ...
    fn load_task_output_upgrades_legacy_error_and_attempt_history() {
        ...
        let legacy_path = temp_dir
            .path()
            .join(".ember/tasks/image_generate/legacy-task.json");
    ...
    fn parse_media_task_output_accepts_serialized_success_payload() {
        let payload = MediaTaskOutput {
            ...
            progress: TaskProgress::default(),
            ui_hints: TaskUiHints::default(),
            path: ".ember/tasks/image_generate/demo.json".to_string(),
            absolute_path: "/tmp/demo.json".to_string(),
            artifact_path: ".ember/tasks/image_generate/demo.json".to_string(),
        ...
        assert_eq!(
            ...
            vec![".ember/tasks/image_generate/demo.json".to_string()]
        );
    }
