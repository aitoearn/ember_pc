...
fn ember_tenant_id_from_host(host: &str) -> Option<String> {
    let url = parse_config_url(host)?;

    url.query()
        .and_then(parse_ember_tenant_id_from_pairs)
        .or_else(|| url.fragment().and_then(parse_ember_tenant_id_from_pairs))
}
...
fn build_anthropic_api_client_with_auth(
    ...
    if let Some(tenant_id) = ember_tenant_id_from_host(host) {
        api_client = api_client.with_header(EMBER_TENANT_HEADER, &tenant_id)?;
    }
...
mod tests {
    use super::{
        ...
        AnthropicProvider, EMBER_TENANT_HEADER,
    };
    use crate::conversation::message::Message;
    use crate::model::ModelConfig;
    ...
    #[test]
    fn test_ember_tenant_fragment_adds_custom_header_to_api_client() {
        let api_client = build_anthropic_api_client(
