...
fn summarize_current_runtime_request_context() -> Option<Value> {
    ...
    let runtime_metadata = metadata
        .and_then(|metadata| metadata.get("ember_runtime"))
...
mod tests {
            ...
            "messages": [
                {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": "对比 Claude Code 与 Ember 的 task 调度差异",
    ...
    async fn test_summarize_request_log_input_includes_runtime_context() {
        ...
        metadata.insert("ember_runtime".to_string(), Value::Object(runtime_metadata));
