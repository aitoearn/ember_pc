...
impl Agent {
    ...
    ///
    /// Ember can refresh runtime tools per turn after the Agent has already been
...
mod tests {
            ...
            metadata: HashMap::from([(
                ...
                serde_json::json!({
                    "stop_after_tool_result": {
                        "metadata_equals": {
                            "skill_forwarded_tool_name": "ember_create_image_generation_task",
    ...
    fn test_should_stop_after_tool_result_ignores_unmatched_metadata() {
        ...
        messages.push(Message::user().with_tool_response(
            ...
            Ok(CallToolResult {
                ...
                structured_content: Some(serde_json::json!({
                    "skill_forwarded_tool_name": "ember_create_cover_generation_task",
    ...
    async fn test_inherit_frontend_tool_surface_from_copies_frontend_tools_without_extensions(
        ...
        assert!(
            target
                .is_frontend_tool("ember_create_image_generation_task")
                .await
        );
        assert_eq!(
            target
                .get_frontend_tool("ember_create_image_generation_task")
                .await
                .as_ref()
                .map(|tool| tool.name.as_str()),
            Some("ember_create_image_generation_task")
        );
