...
const EMBER_RUNTIME_TOOL_SURFACE_KEY: &str = "tool_surface";
const EMBER_RUNTIME_IMAGE_INPUT_POLICY_KEY: &str = "image_input_policy";
...
fn image_input_policy_disables_provider_images() -> bool {
    ...
    let Some(Value::Object(runtime_metadata)) =
        turn_context.metadata.get(EMBER_RUNTIME_METADATA_KEY)
    else {
        return false;
    };
    let Some(Value::Object(policy)) = runtime_metadata
        .get(EMBER_RUNTIME_IMAGE_INPUT_POLICY_KEY)
...
fn resolve_turn_tool_surface_mode() -> Option<String> {
    current_turn_context()?
        ...
        .and_then(|value| value.get(EMBER_RUNTIME_TOOL_SURFACE_KEY))
...
mod tests {
    ...
    fn build_turn_context_with_tool_surface(mode: &str) -> TurnContextOverride {
        ...
        runtime_metadata.insert(
            EMBER_RUNTIME_TOOL_SURFACE_KEY.to_string(),
            Value::String(mode.to_string()),
        );

        let mut metadata = HashMap::new();
        metadata.insert(
            EMBER_RUNTIME_METADATA_KEY.to_string(),
