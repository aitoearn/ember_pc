...
fn build_skill_agent_session(
    ...
    Session {
        ...
        // 动态注册工具（如 ember_create_image_generation_task）会在列举阶段被隐藏。
    ...
    fn test_extract_forwarded_skill_tool_call_prefers_allowed_tool_metadata() {
        let item = ItemRuntime {
            ...
            payload: ItemRuntimePayload::ToolCall {
                tool_name: "ember_create_image_generation_task".to_string(),
                arguments: None,
                output: Some(json!({
        ...
        let forwarded = extract_forwarded_skill_tool_call(
            &item,
            Some(&["ember_create_image_generation_task".to_string()]),
        )
        .expect("should capture forwarded tool metadata");

        assert_eq!(forwarded.tool_name, "ember_create_image_generation_task");
