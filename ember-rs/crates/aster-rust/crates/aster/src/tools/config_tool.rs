    ...
    fn description(self) -> &'static str {
        match self {
            ...
            Self::PermissionsDefaultMode => {
                "Default permission mode for tool execution. Current Ember runtime supports \"default\", \"acceptEdits\", \"auto\", \"approve\", and \"chat\". Upstream alias \"plan\" currently exists only as the explicit EnterPlanMode/ExitPlanMode flow, and \"dontAsk\" does not yet have a global provider/permission-inspector runtime."
            }
            Self::Language => {
    ...
    fn unsupported_message(self) -> Option<&'static str> {
        match self {
            Self::VoiceEnabled => Some(
                "Known upstream setting, but the current Ember runtime needs a host-backed callback to keep persisted voice config and global shortcut registration in sync.",
            ),
            Self::RemoteControlAtStartup => Some(
                "Known upstream setting, but Ember's current host surface only exposes OS auto-launch; it does not have an upstream-style remote-control-at-startup session default.",
            ),
            Self::TaskCompleteNotifEnabled => Some(
                "Known upstream setting, but Ember does not have a remote-control-backed mobile push control plane for task-complete notifications.",
            ),
            Self::InputNeededNotifEnabled => Some(
                "Known upstream setting, but Ember does not have a remote-control-backed mobile push control plane for input-needed notifications.",
            ),
            Self::AgentPushNotifEnabled => Some(
                "Known upstream setting, but Ember does not have a remote-control-backed mobile push control plane for agent-authored notifications.",
            ),
            _ => None,
        }
    }
    ...
    fn write_permission_mode_value(
        ...
        let resolved_value = match normalized.as_str() {
            ...
            "plan" => return Err(SettingWriteError::validation(format!(
                "Unsupported value for {}: \"{}\". Ember plan mode is currently an explicit tool-mediated flow via EnterPlanMode/ExitPlanMode, not a persisted global default permission mode. Supported values: {}.",
                self.key(),
                raw_mode,
                PERMISSION_MODE_SUPPORTED_VALUES.join(", ")
            ))),
            "dontask" => return Err(SettingWriteError::validation(format!(
                "Unsupported value for {}: \"{}\". Ember does not yet have a global dontAsk runtime across provider flags and PermissionInspector. Supported values: {}.",
                self.key(),
                raw_mode,
                PERMISSION_MODE_SUPPORTED_VALUES.join(", ")
            ))),
...
fn supported_settings_line(tool: &ConfigTool, setting: SupportedSetting) -> String {
    ...
    let availability = tool
        ...
        .map(|message| format!(" Current Ember runtime: {}.", message))
        .unwrap_or_default();
