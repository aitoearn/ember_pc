...
/// - 白/黑名单支持环境变量覆盖：
///   - `EMBER_WEB_SEARCH_REQUIRED_TOOLS`（兼容 `PROXYCAST_WEB_SEARCH_REQUIRED_TOOLS`）
///   - `EMBER_WEB_SEARCH_ALLOWED_TOOLS`（兼容 `PROXYCAST_WEB_SEARCH_ALLOWED_TOOLS`）
///   - `EMBER_WEB_SEARCH_DISALLOWED_TOOLS`（兼容 `PROXYCAST_WEB_SEARCH_DISALLOWED_TOOLS`）
...
mod tests {
    ...
    fn detects_incomplete_tool_batch_summary_text() {
        assert!(looks_like_incomplete_tool_batch_summary(
            "已确认 claudecode/src/tasks 下有 7 种 Task 类型。现在需要读取核心类型定义、调度框架和几个关键子 Task 的入口，才能和 Ember 的 task 系统做准确对比。"
        ));
        assert!(looks_like_incomplete_tool_batch_summary(
        ...
        assert!(!looks_like_incomplete_tool_batch_summary(
            "我已经完成对比。Claude Code 的任务面板更轻量，Ember 当前主要差异集中在任务展示位置、批次工具摘要和继续策略。"
        ));
        assert!(!looks_like_incomplete_tool_batch_summary(
            "已获得完整文件树，这是一个很大的 Claude Code CLI 项目。接下来需要看核心入口文件和关键模块来理解架构，才能对比 Ember 的优化点。\n\n## 一、Claude Code 项目概览\n这是 Claude Code CLI 的源码，主循环、工具注册、Task 系统与 compact 都已经识别清楚。\n\n## 二、Ember 当前还能继续对标优化的点\n优先补自动 compact、任务 runtime 和权限边界，然后再做长链路体验优化。"
        ));
    }
    ...
    fn resolves_retry_mode_for_incomplete_tool_batch_summary() {
        ...
        let mode = resolve_reply_retry_mode(
            ...
            "已确认 claudecode/src/tasks 下有 7 种 Task 类型。现在需要读取核心类型定义，才能和 Ember 的 task 系统做准确对比。",
    ...
    async fn stream_message_reply_with_policy_should_surface_disabled_auto_compaction_limit_from_aster(
        ...
        let (store, session) = create_test_session_store("ember-auto-compact-disabled");
    ...
    async fn stream_message_reply_with_policy_should_retry_empty_reply_without_tool_activity() {
        let (store, session) = create_test_session_store("ember-empty-reply-retry");
        let agent = Agent::new().with_session_store(store.clone());
    ...
    async fn stream_message_reply_with_policy_should_retry_retryable_provider_tail_failure() {
        let (store, session) = create_test_session_store("ember-provider-tail-retry");
        let agent = Agent::new().with_session_store(store.clone());
        let attempts = Arc::new(AtomicUsize::new(0));
        agent
            .update_provider(
    ...
    async fn stream_message_reply_with_policy_should_return_cancelled_without_waiting_next_chunk() {
        let (store, session) = create_test_session_store("ember-stream-cancel");
        let agent = Agent::new().with_session_store(store.clone());
        agent
    ...
    async fn stream_message_reply_with_policy_should_drain_inline_provider_error_and_mark_turn_failed(
        ...
        let (store, session) = create_test_session_store("ember-inline-provider-error");
        let agent = Agent::new().with_session_store(store.clone());
        agent
