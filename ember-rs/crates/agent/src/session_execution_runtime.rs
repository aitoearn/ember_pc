...
use ember_core::database::dao::agent_timeline::{AgentThreadItem, AgentThreadItemPayload};
use serde::{de::DeserializeOwned, Deserialize, Serialize};
use serde_json::Value;

const EMBER_RUNTIME_METADATA_KEY: &str = "ember_runtime";
const EMBER_RUNTIME_TASK_PROFILE_KEY: &str = "task_profile";
const EMBER_RUNTIME_ROUTING_DECISION_KEY: &str = "routing_decision";
const EMBER_RUNTIME_LIMIT_STATE_KEY: &str = "limit_state";
const EMBER_RUNTIME_COST_STATE_KEY: &str = "cost_state";
const EMBER_RUNTIME_PERMISSION_STATE_KEY: &str = "permission_state";
const EMBER_RUNTIME_LIMIT_EVENT_KEY: &str = "limit_event";
const EMBER_RUNTIME_OEM_POLICY_KEY: &str = "oem_policy";
const EMBER_RUNTIME_SUMMARY_KEY: &str = "runtime_summary";
...
impl ExtensionState for SessionExecutionRuntimeAccessMode {
    const EXTENSION_NAME: &'static str = "ember_recent_access_mode";
...
impl ExtensionState for SessionExecutionRuntimePreferences {
    const EXTENSION_NAME: &'static str = "ember_recent_preferences";
    const VERSION: &'static str = "v0";
}
...
impl ExtensionState for SessionExecutionRuntimeRecentTeamSelection {
    const EXTENSION_NAME: &'static str = "ember_recent_team_selection";
    const VERSION: &'static str = "v0";
}
...
fn extract_ember_runtime_object(
    metadata: &std::collections::HashMap<String, Value>,
) -> Option<&serde_json::Map<String, Value>> {
    metadata
        .get(EMBER_RUNTIME_METADATA_KEY)
        .and_then(Value::as_object)
}

fn extract_ember_runtime_payload<T: DeserializeOwned>(
    metadata: &std::collections::HashMap<String, Value>,
    key: &str,
) -> Option<T> {
    let runtime = extract_ember_runtime_object(metadata)?;
...
fn extract_task_profile_from_metadata(
    ...
    let mut profile: SessionExecutionRuntimeTaskProfile =
        extract_ember_runtime_payload(metadata, EMBER_RUNTIME_TASK_PROFILE_KEY)?;
...
fn extract_routing_decision_from_metadata(
    ...
    let mut decision: SessionExecutionRuntimeRoutingDecision =
        extract_ember_runtime_payload(metadata, EMBER_RUNTIME_ROUTING_DECISION_KEY)?;
...
fn extract_limit_state_from_metadata(
    ...
    let mut limit_state: SessionExecutionRuntimeLimitState =
        extract_ember_runtime_payload(metadata, EMBER_RUNTIME_LIMIT_STATE_KEY)?;
...
fn extract_cost_state_from_metadata(
    ...
    let mut cost_state: SessionExecutionRuntimeCostState =
        extract_ember_runtime_payload(metadata, EMBER_RUNTIME_COST_STATE_KEY)?;
...
fn extract_permission_state_from_metadata(
    ...
    let mut permission_state: SessionExecutionRuntimePermissionState =
        extract_ember_runtime_payload(metadata, EMBER_RUNTIME_PERMISSION_STATE_KEY)?;
...
fn extract_limit_event_from_metadata(
    ...
    let mut limit_event: SessionExecutionRuntimeLimitEvent =
        extract_ember_runtime_payload(metadata, EMBER_RUNTIME_LIMIT_EVENT_KEY)?;
...
fn extract_oem_policy_from_metadata(
    ...
    let mut oem_policy: SessionExecutionRuntimeOemPolicy =
        extract_ember_runtime_payload(metadata, EMBER_RUNTIME_OEM_POLICY_KEY)?;
...
mod tests {
    ...
    #[test]
    fn extracts_task_routing_and_limit_state_from_ember_runtime_metadata() {
        let now = Utc::now();
        let snapshot = SessionRuntimeSnapshot {
    ...
    #[test]
    fn extracts_agent_app_scope_from_ember_runtime_summary_fallback() {
        let now = Utc::now();
        let snapshot = SessionRuntimeSnapshot {
