...
//! 支持从 Ember API Key Provider 自动选择凭证
//!
//! ## 重要：SessionStore 注入
//!
//! 为了让 Aster Agent 的消息存储到 Ember 数据库，必须在创建 Agent 时
//! 注入 `EmberSessionStore`，并统一通过 `init_agent_with_db()` 初始化。
//!
//! ## Agent 身份配置
//!
//! 通过 Aster 框架的 `AgentIdentity` API 设置 Ember 专属的 Agent 身份，
//! 包括名称、语言偏好、产品描述等。这是架构层面的正确做法，
//! 而不是简单地追加提示词。
//!
//! ## Skills 集成
//!
//! Agent 初始化时会自动加载 Ember 当前应用数据目录中的 Skills 到
...
use ember_services::aster_session_store::EmberSessionStore;

async fn configure_ember_native_file_tools(agent: &Agent) {
...
impl AsterAgentState {
    ...
    /// 初始化 Agent（带数据库连接）
    ///
    /// 创建 Agent 并注入 EmberSessionStore，确保消息存储到 Ember 数据库。
    /// 同时设置 Ember 专属的 Agent 身份（名称、语言、描述）。
    /// 自动加载 Ember 当前应用数据目录中的 Skills 到 aster-rust 的 global_registry。
    ///
    /// 这是 Ember 当前唯一支持的 Agent 初始化入口。
    ///
    /// # 参数
    ...
    pub async fn init_agent_with_db(&self, db: &DbConnection) -> Result<(), String> {
        ...
        if agent_guard.is_none() {
            // 创建 SessionStore
            let session_store = Arc::new(EmberSessionStore::new(db.clone()));
            tracing::info!("[AsterAgent] 创建 EmberSessionStore 成功");

            // 创建 Agent（启用 Ask/LSP 回调）并注入 SessionStore
            let tool_config = crate::create_ember_tool_config();
            ...
            // 使用异步方法设置 Ember 专属身份
            let identity = crate::create_ember_identity();
            agent.set_identity(identity).await;
            configure_ember_native_file_tools(&agent).await;

            // 加载 Ember Skills 到 aster-rust 的 global_registry
            crate::reload_ember_skills();
            ...
            *agent_guard = Some(agent);
            ...
            tracing::info!(
                "[AsterAgent] Agent 初始化成功，已注入 EmberSessionStore、Ember 身份和 Skills"
            );
        } else {
            // 更新缓存
            self.initialized_cache.store(true, Ordering::Relaxed);
    ...
    ///
    /// 自动从 Ember API Key Provider 选择可用凭证并配置 Aster Provider
    ///
    /// # 参数
    /// - `db`: 数据库连接
    ...
    /// 注册 MCP 桥接客户端
    ///
    /// 将 Ember 托管的 MCP 客户端注册到 Aster Agent 的 ExtensionManager，
...
mod tests {
    ...
    async fn test_aster_state_init() {
            ...
            Arc::new(Mutex::new(rusqlite::Connection::open_in_memory().unwrap()));
        {
            ...
            ember_core::database::schema::create_tables(&conn).unwrap();
        }
...
"#,
    ...
    /// 测试：reload_ember_skills 不会 panic（即使目录不存在）
    #[test]
    fn test_reload_ember_skills_no_panic() {
        // 这个测试确保 reload_ember_skills 在各种情况下都不会 panic
        // 即使当前 Skills 目录不存在
