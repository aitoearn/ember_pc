...
//!
//! 将 Ember API Key Provider 主路径与 Aster Provider 系统连接。
//!
//! ## 功能
...
use ember_core::models::provider_type::is_custom_provider_id;
...
/// Provider 凭证桥接器
///
/// 负责从 Ember API Key Provider 选择凭证并转换为 Aster Provider 配置
...
impl CredentialBridge {
    ...
    /// 将 Ember 凭证转换为 Aster Provider 配置
...
fn parse_ember_tenant_id_from_pairs(value: &str) -> Option<String> {
    value.split('&').find_map(|pair| {
        ...
        (key == EMBER_TENANT_PARAM)
            .then(|| normalize_ember_tenant_id(value))
            .flatten()
    })
}
...
fn ember_tenant_id_from_api_host(api_host: &str) -> Option<String> {
    ...
    query
        .and_then(parse_ember_tenant_id_from_pairs)
        .or_else(|| fragment.and_then(parse_ember_tenant_id_from_pairs))
}
...
fn update_openai_ember_tenant_custom_header(tenant_id: Option<&str>) {
    ...
    let mut headers = parse_openai_custom_headers_env(&existing)
        .into_iter()
        .filter(|(key, _)| !key.eq_ignore_ascii_case(EMBER_TENANT_HEADER))
        .collect::<Vec<_>>();

    if let Some(tenant_id) = tenant_id.and_then(normalize_ember_tenant_id) {
...
fn set_provider_env_vars(config: &AsterProviderConfig) {
    ...
    if config.provider_name == "openai" {
        ...
        if config.base_url.is_none() {
            update_openai_ember_tenant_custom_header(None);
    ...
    if let Some(base_url) = &config.base_url {
        match config.provider_name.as_str() {
            "openai" => {
                let tenant_id = ember_tenant_id_from_api_host(base_url);
                update_openai_ember_tenant_custom_header(tenant_id.as_deref());
...
mod tests {
    ...
    fn test_split_url_host_and_path() {
        ...
        let (host, path) =
            split_url_host_and_path("https://llm.emberai.run/openai?ember_tenant_id=tenant-0001#x=1");
        assert_eq!(host, "https://llm.emberai.run");
        assert_eq!(path, "openai");
    }

    #[test]
    fn test_set_provider_env_vars_openai_ember_hub_adds_tenant_header_from_fragment() {
        let _env_guard = env_lock();
        std::env::remove_var("OPENAI_HOST");
        std::env::remove_var("OPENAI_BASE_PATH");
        std::env::set_var(
            OPENAI_CUSTOM_HEADERS_ENV,
            "X-Other=1,X-Ember-Tenant-ID=stale",
        );

        let config = AsterProviderConfig {
            provider_name: "openai".to_string(),
            provider_selector: Some("ember-hub".to_string()),
            model_name: "gpt-5.5".to_string(),
            api_key: Some("test-key".to_string()),
            base_url: Some("https://llm.emberai.run#ember_tenant_id=tenant-0001".to_string()),
            credential_uuid: "test-uuid".to_string(),
            reasoning_effort: None,
            force_responses_api: false,
            toolshim: false,
    ...
    #[test]
    fn test_set_provider_env_vars_openai_without_ember_tenant_clears_stale_header() {
        let _env_guard = env_lock();
        std::env::set_var(
            OPENAI_CUSTOM_HEADERS_ENV,
            "X-Ember-Tenant-ID=stale,X-Other=1",
        );
