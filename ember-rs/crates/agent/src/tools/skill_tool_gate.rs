...
const LIMECORE_POLICY_INPUT_VALUE_SOURCE_LIMECORE_PENDING: &str = "embercore_pending";
...
fn embercore_policy_snapshot(policy_refs: &[String]) -> Value {
    json!({
...
fn build_current_runtime_contract(contract_key: &str) -> Option<Value> {
    ...
    let policy_refs = read_string_array(contract.get("embercore_policy_refs"));
    let policy_snapshot = embercore_policy_snapshot(&policy_refs);
    ...
    Some(json!({
        ...
        "embercore_policy_snapshot": policy_snapshot,
...
mod tests {
    ...
    fn current_skill_should_seed_runtime_contract_metadata_from_governance_registry() {
        ...
        assert_eq!(
            metadata
                .runtime_contract
                .pointer("/embercore_policy_snapshot/status")
                .and_then(Value::as_str),
