...
use ember_core::agent::types::{FunctionCall, ImageUrl, ToolCall};
use ember_core::database::{schema, DbConnection};
    ...
    let snapshot = SessionRuntimeSnapshot {
        ...
        threads: vec![ThreadRuntimeSnapshot {
            thread: ThreadRuntime::new(
                thread_id,
                detail.id.clone(),
                std::path::PathBuf::from("/tmp/ember-runtime-overlay-test"),
            ),
...
fn convert_agent_message_should_preserve_tool_request_and_response() {
    let assistant = AgentMessage {
        ...
        tool_call_id: None,
        reasoning_content: None,
        usage: Some(ember_core::agent::types::TokenUsage::new(20_480, 10_240)),
    };
...
fn load_user_visible_message_flags_should_default_legacy_messages_to_visible() {
    ...
    insert_test_session_with_message(
        ...
        "/tmp/ember-workspace-visibility-flags",
        "用户消息",
    );
...
fn load_user_visible_message_flags_should_follow_history_tail_limit() {
    ...
    insert_test_session_with_message(
        ...
        "/tmp/ember-workspace-visibility-tail",
        "第一条",
    );
...
fn load_user_visible_message_flags_should_not_scan_huge_history_payloads() {
    ...
    insert_test_session_with_message(
        ...
        "/tmp/ember-workspace-visibility-huge",
        "第一条",
    );
...
fn convert_agent_messages_should_force_offload_old_large_tool_calls_under_context_pressure() {
    ...
    assert!(record.contains_key(crate::tool_io_offload::EMBER_TOOL_ARGUMENTS_OFFLOAD_KEY));
}
...
fn list_sessions_sync_should_resolve_workspace_id_from_working_dir() {
    let db = create_test_db();
    insert_test_workspace(&db, "workspace-1", "/tmp/ember-workspace-1");
    insert_test_session_with_message(&db, "session-1", "/tmp/ember-workspace-1", "你好，世界");
...
fn list_sessions_sync_should_include_archived_sessions_when_requested() {
    let db = create_test_db();
    insert_test_session_with_message(&db, "session-active", "/tmp/ember-workspace-6", "活跃");
    insert_test_session_with_message(&db, "session-archived", "/tmp/ember-workspace-7", "归档");
...
fn list_sessions_sync_should_support_workspace_filter_and_limit() {
    let db = create_test_db();
    insert_test_workspace(&db, "workspace-8", "/tmp/ember-workspace-8");
    insert_test_workspace(&db, "workspace-9", "/tmp/ember-workspace-9");
    insert_test_session_with_message(&db, "session-a", "/tmp/ember-workspace-8", "A");
    insert_test_session_with_message(&db, "session-b", "/tmp/ember-workspace-8", "B");
    insert_test_session_with_message(&db, "session-c", "/tmp/ember-workspace-9", "C");
...
fn get_session_sync_should_resolve_workspace_id_from_working_dir() {
    let db = create_test_db();
    insert_test_workspace(&db, "workspace-2", "/tmp/ember-workspace-2");
    insert_test_session_with_message(&db, "session-2", "/tmp/ember-workspace-2", "继续处理");

    let detail = get_session_sync(&db, "session-2").expect("get session");

    assert_eq!(detail.workspace_id.as_deref(), Some("workspace-2"));
    assert_eq!(detail.working_dir.as_deref(), Some("/tmp/ember-workspace-2"));
    assert_eq!(detail.messages.len(), 1);
}

#[test]
fn get_session_sync_with_full_timeline_without_messages_should_skip_messages() {
    use ember_core::database::dao::agent_timeline::{
        AgentThreadItem, AgentThreadItemPayload, AgentThreadItemStatus, AgentThreadTurn,
        AgentThreadTurnStatus, AgentTimelineDao,
    };

    let db = create_test_db();
    insert_test_workspace(&db, "workspace-light", "/tmp/ember-workspace-light");
    insert_test_session_with_message(
        &db,
        "session-light",
        "/tmp/ember-workspace-light",
        ...
        AgentTimelineDao::upsert_item(
            ...
            &AgentThreadItem {
                ...
                payload: AgentThreadItemPayload::FileArtifact {
                    path: ".ember/qc/code-runtime-fixture/src/greeting.ts".to_string(),
...
fn get_session_sync_with_history_limit_should_tail_persisted_history() {
    let db = create_test_db();
    insert_test_session_with_message(&db, "session-tail", "/tmp/ember-workspace-tail", "消息 1");
...
async fn get_runtime_session_detail_should_use_archived_fast_path() {
    ...
    insert_test_session_with_message(
        ...
        "/tmp/ember-workspace-archived-fast-path",
        "查看归档",
    );
...
async fn get_runtime_session_detail_should_use_empty_persisted_fast_path() {
    ...
    create_session_record_sync(
        ...
        CreateSessionRecordInput {
            ...
            model: Some("agent:test".to_string()),
            working_dir: Some("/tmp/ember-workspace-empty-fast-path".to_string()),
            execution_strategy: Some("react".to_string()),
            ..CreateSessionRecordInput::default()
        },
    )
...
fn update_session_working_dir_sync_should_refresh_workspace_binding() {
    let db = create_test_db();
    insert_test_workspace(&db, "workspace-3", "/tmp/ember-workspace-3");
    insert_test_workspace(&db, "workspace-4", "/tmp/ember-workspace-4");
    insert_test_session_with_message(&db, "session-3", "/tmp/ember-workspace-3", "切换目录");

    update_session_working_dir_sync(&db, "session-3", "/tmp/ember-workspace-4")
        .expect("update working_dir");

    let detail = get_session_sync(&db, "session-3").expect("get session");
    assert_eq!(detail.working_dir.as_deref(), Some("/tmp/ember-workspace-4"));
...
fn update_session_provider_config_sync_should_persist_provider_and_model_config() {
    ...
    insert_test_session_with_message(
        ...
        "/tmp/ember-workspace-provider-config",
        "切换模型",
    );
...
fn rename_session_sync_should_update_session_title() {
    let db = create_test_db();
    insert_test_session_with_message(&db, "session-rename", "/tmp/ember-workspace-5", "原始消息");
