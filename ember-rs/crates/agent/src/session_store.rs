...
//!
//! 提供会话创建、列表查询、详情查询能力。
//! 数据事实源收敛到 ember_core::database::agent_session_repository + Ember 数据库。
...
use chrono::DateTime;
use chrono::Utc;
use ember_core::agent::types::AgentSession;
#[cfg(test)]
use ember_core::agent::types::{AgentMessage, ContentPart, MessageContent};
use ember_core::database::agent_session_repository::{
    self, SessionRecordDetail, SessionRecordMetadata, SessionRecordPreviewMessage,
};
use ember_core::database::dao::agent::SessionArchiveFilter;
use ember_core::database::dao::agent_timeline::AgentTimelineDao;
#[cfg(test)]
use ember_core::database::dao::agent_timeline::{AgentThreadTurn, AgentThreadTurnStatus};
use ember_core::database::DbConnection;
use ember_core::workspace::WorkspaceManager;
...
use ember_core::database::dao::agent::AgentDao;
...
impl ExtensionState for SessionProviderRoutingState {
    const EXTENSION_NAME: &'static str = "ember_provider_routing";
...
pub async fn delete_session(db: &DbConnection, session_id: &str) -> Result<(), String> {
    aster::session::SessionStore::delete_session(&EmberSessionStore::new(db.clone()), session_id)
        .await
