//! Aster runtime 支持模块
//!
//! 收口 Ember 对 Aster thread runtime store 的访问边界，
...
use ember_core::database::agent_runtime_queue_repository::{self, LegacyRuntimeQueuedTurn};
    ...
    F: Future<Output = Result<(), String>> + Send + 'static,
{
    if tokio::runtime::Handle::try_current().is_ok() {
        let join_handle = std::thread::Builder::new()
            .name("ember-runtime-init".to_string())
...
/// 获取 Ember 当前统一使用的 Aster runtime store。
