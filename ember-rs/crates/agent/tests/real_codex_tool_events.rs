...
use ember_agent::event_converter::convert_agent_event;
use ember_agent::{AgentEvent, AsterAgentState, SessionConfigBuilder};
use ember_core::database::dao::api_key_provider::ApiProviderType;
use ember_core::database::init_database;
use ember_services::api_key_provider_service::ApiKeyProviderService;
use uuid::Uuid;

fn should_run_real_test() -> bool {
    ember_core::env_compat::var(&["EMBER_REAL_API_TEST", "PROXYCAST_REAL_API_TEST"]).as_deref()
        == Some("1")
}
...
fn resolve_model_name(
    ...
    Err(
        "未找到可用模型：请设置 EMBER_REAL_MODEL，或在 Provider custom_models 中配置模型（兼容旧的 PROXYCAST_REAL_MODEL）。"
            .to_string(),
    )
}
...
fn resolve_codex_provider_and_model(
    ...
    let explicit_model = ember_core::env_compat::var(&["EMBER_REAL_MODEL", "PROXYCAST_REAL_MODEL"]);
...
async fn test_real_codex_stream_emits_tool_events() {
    ...
    let prompt = if preferred_tool == "bash" {
        "请严格执行以下步骤：\
1) 必须调用工具 bash，执行命令 `echo EMBER_REAL_TOOL_EVENT`; \
