...
const MCP_EMBER_ENABLED_MIGRATED_KEY: &str = "migrated_mcp_ember_enabled";
const MCP_CREATED_AT_INTEGER_MIGRATED_KEY: &str = "migrated_mcp_created_at_to_integer";

/// 修复历史 MCP 导入数据：补齐 enabled_ember
///
/// 早期版本从 Claude/Codex/Gemini 导入 MCP 时，默认写入 enabled_ember=0，
/// 导致 Ember 本身不会使用这些服务器。
///
/// 迁移策略：
/// - 仅处理 enabled_ember=0 的记录
/// - 且至少在一个外部应用中启用（enabled_claude/codex/gemini 任一为 1）
/// - 将 enabled_ember 设为 1
pub fn migrate_mcp_ember_enabled(conn: &Connection) -> Result<usize, String> {
    if is_true_setting(conn, MCP_EMBER_ENABLED_MIGRATED_KEY) {
        tracing::debug!("[迁移] MCP ember 启用状态已迁移过，跳过");
        return Ok(0);
    }
    ...
    if !has_mcp_columns(
        ...
        tracing::info!("[迁移] mcp_servers 缺少 Ember 启用状态相关列，跳过 MCP ember 启用状态修复");
        return Ok(0);
    }
    ...
    let updated = conn
        .execute(
            "UPDATE mcp_servers
             SET enabled_ember = 1
             WHERE enabled_ember = 0
               AND (enabled_claude = 1 OR enabled_codex = 1 OR enabled_gemini = 1)",
            [],
        )
        .map_err(|e| format!("修复 MCP enabled_ember 失败: {e}"))?;

    mark_true_setting(conn, MCP_EMBER_ENABLED_MIGRATED_KEY)?;

    tracing::info!("[迁移] MCP ember 启用状态修复完成，更新 {} 条记录", updated);
...
mod tests {
    ...
    fn setup_mcp_migration_db() -> Connection {
        ...
        conn.execute_batch(
            ...
            CREATE TABLE mcp_servers (
                id TEXT PRIMARY KEY,
                enabled_ember INTEGER NOT NULL DEFAULT 0,
    ...
    #[test]
    fn migrate_mcp_ember_enabled_updates_only_imported_rows() {
        let conn = setup_mcp_migration_db();

        conn.execute(
            "INSERT INTO mcp_servers (id, enabled_ember, enabled_claude) VALUES (?1, 0, 1)",
            ["server-1"],
        )
        .unwrap();
        conn.execute(
            "INSERT INTO mcp_servers (id, enabled_ember, enabled_claude) VALUES (?1, 0, 0)",
            ["server-2"],
        )
        .unwrap();

        let updated = migrate_mcp_ember_enabled(&conn).unwrap();
        assert_eq!(updated, 1);

        let enabled_ember: i64 = conn
            .query_row(
                "SELECT enabled_ember FROM mcp_servers WHERE id = ?1",
                ["server-1"],
                |row| row.get(0),
            )
            .unwrap();
        assert_eq!(enabled_ember, 1);
        ...
        let updated_again = migrate_mcp_ember_enabled(&conn).unwrap();
    ...
    #[test]
    fn migrate_mcp_ember_enabled_skips_when_required_columns_are_missing() {
        let conn = Connection::open_in_memory().unwrap();
        conn.execute_batch(
            "
            CREATE TABLE settings (
        ...
        let updated = migrate_mcp_ember_enabled(&conn).unwrap();
        assert_eq!(updated, 0);
        assert!(!is_true_setting(&conn, MCP_EMBER_ENABLED_MIGRATED_KEY));
    }
}
