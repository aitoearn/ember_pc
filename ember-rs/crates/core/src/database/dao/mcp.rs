...
impl McpDao {
    pub fn get_all(conn: &Connection) -> Result<Vec<McpServer>, rusqlite::Error> {
        let mut stmt = conn.prepare(
            "SELECT id, name, server_config, description, enabled_ember,
        ...
        let servers = stmt.query_map([], |row| {
            ...
            Ok(McpServer {
                ...
                enabled_ember: row.get::<_, i32>(4)? == 1,
    ...
    pub fn insert(conn: &Connection, server: &McpServer) -> Result<(), rusqlite::Error> {
        conn.execute(
            "INSERT INTO mcp_servers (id, name, server_config, description,
                                     enabled_ember, enabled_claude, enabled_codex,
            ...
            params![
                ...
                server.description,
                if server.enabled_ember { 1 } else { 0 },
    ...
    pub fn update(conn: &Connection, server: &McpServer) -> Result<(), rusqlite::Error> {
        conn.execute(
            "UPDATE mcp_servers SET name = ?1, server_config = ?2, description = ?3,
             enabled_ember = ?4, enabled_claude = ?5, enabled_codex = ?6, enabled_gemini = ?7
             WHERE id = ?8",
            params![
                server.name,
                serde_json::to_string(&server.server_config).unwrap_or_default(),
    ...
    fn setup_conn() -> Connection {
        ...
        conn.execute(
            "CREATE TABLE mcp_servers (
                ...
                description TEXT,
                enabled_ember INTEGER DEFAULT 0,
    ...
    fn get_all_should_parse_integer_created_at() {
        ...
        conn.execute(
            "INSERT INTO mcp_servers (
                ...
                enabled_ember, enabled_claude, enabled_codex, enabled_gemini, created_at
