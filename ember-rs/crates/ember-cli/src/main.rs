...
fn run_doctor_command(args: DoctorArgs) -> Result<Value, MediaRuntimeError> {
    ...
    let docs_root = workspace_root.join("tools/ember-cli");
...
) -> Result<ember_media_runtime::MediaTaskOutput, MediaRuntimeError> {
    write_task_artifact(
        workspace_root,
...
fn resolve_cli_image_generation_runner_config() -> Result<ImageGenerationRunnerConfig, String> {
    let endpoint_override = read_non_empty_env("EMBER_MEDIA_IMAGE_ENDPOINT");
    let api_key_override = read_non_empty_env("EMBER_MEDIA_IMAGE_API_KEY")
        .or_else(|| read_non_empty_env("EMBER_SERVER_API_KEY"));
    let host_override = read_non_empty_env("EMBER_SERVER_HOST");
    let port_override = read_env_port("EMBER_SERVER_PORT")?;
    ...
    let api_key = api_key_override
        ...
        .ok_or_else(|| match config_load_error {
            Some(error) => format!("Ember 本地图片服务未配置 API Key，且加载本地配置失败: {error}"),
            None => "Ember 本地图片服务未配置 API Key".to_string(),
        })?;
...
mod tests {
    ...
    fn skill_show_image_generate_prefers_media_command() {
        ...
        assert_eq!(
            ...
            "ember media image generate --prompt \"...\""
        );
    }
    ...
    fn skill_show_video_generate_prefers_media_command() {
        ...
        assert_eq!(
            output["skill"]["recommended_command"],
            "ember media video generate --prompt \"...\" --aspect-ratio 9:16"
        );
    }
    ...
    fn skill_show_webpage_generate_returns_builtin_skill() {
        ...
        assert_eq!(
            ...
            "ember-rs/resources/default-skills/webpage_generate/SKILL.md"
        );
    }
    ...
    fn skill_show_presentation_generate_returns_builtin_skill() {
        ...
        assert_eq!(
            output["skill"]["skill_path"],
            "ember-rs/resources/default-skills/presentation_generate/SKILL.md"
        );
    }
    ...
    fn skill_show_form_generate_returns_builtin_skill() {
        ...
        assert_eq!(
            output["skill"]["skill_path"],
            "ember-rs/resources/default-skills/form_generate/SKILL.md"
        );
    }
