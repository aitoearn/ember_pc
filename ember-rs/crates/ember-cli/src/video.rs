...
fn create_video_task_artifact(
    ...
    ember_media_runtime::write_task_artifact(
        workspace_root,
...
fn resolve_cli_video_generation_runner_config() -> Result<VideoGenerationRunnerConfig, String> {
    let endpoint_override = read_non_empty_env("EMBER_MEDIA_VIDEO_ENDPOINT");
    let api_key_override = read_non_empty_env("EMBER_MEDIA_VIDEO_API_KEY")
    ...
    let api_key = api_key_override
        ...
        .ok_or_else(|| match config_load_error {
            Some(error) => format!("Ember 本地视频服务未配置 API Key，且加载本地配置失败: {error}"),
            None => "Ember 本地视频服务未配置 API Key".to_string(),
        })?;
...
fn mark_cli_video_task_failed(
    ...
    let output = ember_media_runtime::patch_task_artifact(
        workspace_root,
        task_id,
        None,
        TaskArtifactPatch {
