...
use chrono::Utc;
use ember_core::config::{Config, TelegramAccountConfig, TelegramBotConfig, TelegramGroupConfig};
use ember_core::database::DbConnection;
use ember_core::logger::LogStore;
use ember_websocket::handlers::{RpcHandler, RpcHandlerState};
use ember_websocket::protocol::{
...
fn read_bot_token_from_file(path: &str) -> Option<String> {
    let expanded = ember_core::config::expand_tilde(path);
...
fn extract_rpc_error(error: Option<ember_websocket::protocol::RpcError>, fallback: &str) -> String {
...
mod tests {
    ...
    fn authorize_group_allows_slash_command_without_mention() {
        ...
        let result = authorize_group(&account, &inbound, "ember_bot");
