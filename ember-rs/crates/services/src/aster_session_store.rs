...
/// Ember 的 SessionStore 实现
///
/// 将 aster 的会话数据存储到 Ember 的 SQLite 数据库
pub struct EmberSessionStore {
    db: DbConnection,
...
#[async_trait]
impl SessionStore for EmberSessionStore {
    ...
    async fn commit_session(&self, id: &str, _options: CommitOptions) -> Result<CommitReport> {
        Ok(CommitReport {
            ...
            source_end_ts: None,
            warnings: vec!["EmberSessionStore: memory commit skipped".to_string()],
        })
    }
    ...
    async fn memory_health(&self) -> Result<MemoryHealth> {
        Ok(MemoryHealth {
            healthy: true,
            message: "EmberSessionStore: memory subsystem disabled".to_string(),
        })
    }
}
...
mod tests {
    ...
    fn setup_test_store() -> EmberSessionStore {
        let conn = Connection::open_in_memory().expect("创建内存数据库失败");
        create_tables(&conn).expect("初始化表结构失败");
        EmberSessionStore::new(Arc::new(Mutex::new(conn)))
    }
    ...
    async fn get_session_should_prefer_default_workspace_root_when_missing_row() {
        let store = setup_test_store();
        let workspace_root = std::env::temp_dir().join("ember-aster-default-workspace");
        let conn = store.db.lock().expect("锁数据库");
        conn.execute(
    ...
    async fn get_session_should_fallback_to_app_paths_default_project_dir() {
        ...
        assert!(!session
            .working_dir
            .to_string_lossy()
            .contains(".ember/projects/default"));
    }
    ...
    async fn update_session_metadata_should_roundtrip() {
        ...
        store
            ...
            .expect("更新名称失败");
        store
            .update_working_dir(&session.id, PathBuf::from("/tmp/ember-worktree-child"))
            .await
        ...
        assert_eq!(
            loaded.working_dir,
            PathBuf::from("/tmp/ember-worktree-child")
        );
