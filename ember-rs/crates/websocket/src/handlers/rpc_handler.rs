...
use ember_core::database::dao::agent_run::{AgentRun, AgentRunDao, AgentRunStatus};
use ember_core::database::dao::chat::{ChatDao, ChatMessage, ChatMode, ChatSession};
use ember_scheduler::{
...
pub struct RpcHandlerState {
    /// 数据库连接
    pub db: Arc<RwLock<Option<ember_core::database::DbConnection>>>,
    /// Agent 调度器（可选）
    pub scheduler: Arc<RwLock<Option<ember_agent::LimeScheduler>>>,
    /// 日志存储
    pub logs: Arc<RwLock<ember_core::LogStore>>,
...
impl RpcHandlerState {
    ...
    pub fn new(
        ...
        logs: Arc<RwLock<ember_core::LogStore>>,
    ) -> Self {
        Self {
...
impl RpcHandler {
    ...
    async fn handle_cron_list(&self) -> Result<serde_json::Value, RpcError> {
        ...
        let tasks: Vec<CronTaskInfo> = raw_tasks
            ...
            .map(|item| {
                let enabled =
                    item.status != ember_scheduler::TaskStatus::Cancelled && !item.is_in_cooldown();
                let next_run = if item.status == ember_scheduler::TaskStatus::Pending {
    ...
    async fn handle_cron_health(
        ...
        for task in &tasks {
            match task.status {
                ember_scheduler::TaskStatus::Pending => pending_tasks += 1,
                ember_scheduler::TaskStatus::Running => running_tasks += 1,
                ember_scheduler::TaskStatus::Completed => completed_tasks += 1,
                ember_scheduler::TaskStatus::Failed => failed_tasks += 1,
                ember_scheduler::TaskStatus::Cancelled => cancelled_tasks += 1,
            }
            ...
            if task.status == ember_scheduler::TaskStatus::Running {
        ...
        let mut risky_tasks = tasks
            ...
            .filter(|task| {
                task.is_in_cooldown()
                    ...
                    || task.status == ember_scheduler::TaskStatus::Failed
            })
    ...
    async fn require_db(&self) -> Result<ember_core::database::DbConnection, RpcError> {
        self.state
            .db
            .read()
            .await
...
fn finalize_run(
    ...
    if let Ok(conn) = ember_core::database::lock_db(db) {
...
mod tests {
    use super::*;
    use ember_core::database::{self, schema};
    use ember_scheduler::SchedulerDao;
    ...
    fn create_test_handler() -> RpcHandler {
        ...
        let state = RpcHandlerState::new(
            Some(db),
            None,
            Arc::new(RwLock::new(ember_core::LogStore::new())),
        );
