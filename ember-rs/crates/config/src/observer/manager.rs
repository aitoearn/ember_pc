...
use ember_core::config::{Config, EndpointProvidersConfig, HotReloadManager, ReloadResult};
...
impl GlobalConfigManager {
    ...
    pub async fn update_config(&self, new_config: Config, source: ConfigChangeSource) {
        ember_core::tool_calling::apply_tool_calling_runtime_config(&new_config);
        {
    ...
    pub async fn save_config(&self, config: &Config) -> Result<(), String> {
        ember_core::config::save_config(config).map_err(|e| e.to_string())?;
