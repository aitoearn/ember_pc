...
export function isRustCodePath(relativePath) {
  return relativePath.startsWith("ember-rs/src/") && relativePath.endsWith(".rs");
}
...
export function buildRustModulePathFromFile(relativePath) {
  ...
  if (!normalizedPath.startsWith("ember-rs/src/") || !normalizedPath.endsWith(".rs")) {
    return null;
  }

  const withoutRoot = normalizedPath.slice("ember-rs/src/".length);
