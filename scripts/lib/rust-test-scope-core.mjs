...
const MANIFEST_PATH = "ember-rs/Cargo.toml";
...
function isWorkspaceWideRustPath(relPath) {
  return (
    relPath === "ember-rs" ||
    relPath === "ember-rs/" ||
    relPath === "ember-rs/Cargo.toml" ||
    relPath === "ember-rs/Cargo.lock" ||
    relPath === "ember-rs/rust-toolchain.toml" ||
    relPath === "ember-rs/rust-toolchain" ||
    relPath.startsWith("ember-rs/.cargo/")
  );
}

function isVendoredRustPath(relPath) {
  return relPath === "ember-rs/vendor" || relPath.startsWith("ember-rs/vendor/");
}
...
export function resolveRustPathSelection(
  ...
  for (const inputPath of inputPaths) {
    ...
    if (!relPath.startsWith("ember-rs")) {
    ...
    if (!packageRoot) {
      ...
      rustPaths.push(relPath);
      errors.push(
        `${relPath}: 无法映射到 ember-rs workspace crate；请扩大到 --workspace 或手动指定 -p <crate>`,
      );
      continue;
    }
    ...
    if (!packageInfo.workspaceMember) {
      ...
      rustPaths.push(relPath);
      errors.push(
        `${relPath}: ${packageInfo.packageName} 已被 ember-rs workspace exclude，不能通过根 manifest 的 -p 定向运行`,
      );
      continue;
    }
...
function collectChangedPaths(ref, repoRoot) {
  ...
  const untrackedResult = spawnSync(
    "git",
    ["ls-files", "--others", "--exclude-standard", "--", "ember-rs"],
    {
      cwd: repoRoot,
      encoding: "utf8",
    },
...
export function resolvePathScopedCargoArgs(
  ...
  if (selection.rustPaths.length === 0) {
    const mode = options.changed
      ...
      : "--related";
    console.log(`[rust-layer] ${mode} 未命中 ember-rs 路径，跳过 Rust 层测试。`);
