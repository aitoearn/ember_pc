...
const AGENT_RUNTIME_BYPASS_MARKERS = [
  {
    marker: "EMBER_GATEWAY_BASE",
    reason: "Agent App package must not call Ember Gateway directly; use ember.agent / AgentRuntime Host facade",
  },
  {
    marker: "EMBER_GATEWAY_PROTOCOL",
  ...
  },
  {
    ...
    reason: "Agent App package must not own provider credentials; credentials belong to Ember runtime policy",
  },
  {
    ...
    status === "ready"
      ...
      : status === "needs_handoff"
        ...
        : agentRuntimeBypass.totalMatches > 0
          ? "Do not claim Agent App runtime completion; remove direct provider/Gateway calls and route AI work through ember.agent / AgentRuntime Host facade."
