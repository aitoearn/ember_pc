...
describe("agent-app-package-handoff-core", () => {
  ...
  it("无私有 marker 但 dirty 时要求 handoff", () => {
    const report = createAgentAppPackageHandoffReport({
      ...
      files: {
        hostBridge: {
          exists: true,
          content: "createLimeHostBridgeCapabilityInvoker(); createEmberCoreCapabilityAdapters();",
        },
        uiTest: {
          exists: true,
  ...
  it("把直接调用模型或 Gateway 的 Agent App package 判为 blocked", () => {
    const report = createAgentAppPackageHandoffReport({
      files: {
        ...
        runtimeFiles: [
          {
            path: "src/integrations/embercore.mjs",
            content:
              "const base = process.env.EMBER_GATEWAY_BASE; await fetch(`${base}/v1/chat/completions`);",
          },
          {
  ...
  it("统计 src / dist 产物漂移并作为 handoff warning", () => {
    const distArtifacts = createDistArtifactReport([
      ...
      {
        status: "missing-dist",
        src: "src/ui/ember-app-sdk.js",
        dist: "dist/ui/ember-app-sdk.js",
        srcHash: "ddd",
        distHash: "",
      },
    ]);
