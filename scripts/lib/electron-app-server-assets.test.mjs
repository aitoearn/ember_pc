...
describe("electron app-server assets", () => {
  ...
  it("解析 Electron resources 输出路径", () => {
    const outputRoot = electronAppServerResourcesRoot("/repo/ember");
    expect(outputRoot).toBe(path.resolve("/repo/ember/dist-electron"));
    expect(electronAppServerManifestPath({ outputRoot })).toBe(
      path.resolve("/repo/ember/dist-electron/app-server.release.json"),
    );
    expect(
      electronAppServerBinaryDestination({
        outputRoot,
      ...
      path.resolve("/repo/ember/dist-electron/app-server/darwin-arm64/app-server"),
    );
  });

  it("生成 Electron packaged app-server manifest", async () => {
    const manifest = await buildElectronAppServerReleaseManifest({
      binaryPath: "/repo/ember/dist-electron/app-server/darwin-arm64/app-server",
      version: "1.59.0",
  ...
  it("准备 sidecar 二进制和 release manifest", async () => {
    const calls = [];
    const outputRoot = path.resolve("/repo/ember/dist-electron");
    const result = await prepareElectronAppServerAssets({
      repoRoot: "/repo/ember",
      outputRoot,
      platform: "darwin",
      arch: "arm64",
      sourceBinary: "/repo/ember/ember-rs/target/debug/app-server",
    ...
    expect(calls[0]).toEqual([
      "mkdir",
      path.resolve("/repo/ember/dist-electron/app-server/darwin-arm64"),
      { recursive: true },
    ]);
    expect(calls[1]).toEqual([
      "copy",
      path.resolve("/repo/ember/ember-rs/target/debug/app-server"),
      path.resolve("/repo/ember/dist-electron/app-server/darwin-arm64/app-server"),
    ]);
    ...
    expect(
      resolveElectronAppServerRuntimeEnv({
        ...
        resolveBinary: (options) => {
          expect(options.repoRoot).toBe("/repo/ember");
          expect(options.platform).toBe("darwin");
          return "/repo/ember/ember-rs/target/debug/app-server";
        },
      }),
    ).toEqual({ APP_SERVER_BIN: "/repo/ember/ember-rs/target/debug/app-server" });
  });
});
