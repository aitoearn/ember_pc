...
describe("agent app connector production webhook delivery", () => {
  it("只接受非本地 https production webhook URL", () => {
    expect(isProductionWebhookUrl("https://hooks.example.com/ember")).toBe(true);
    expect(isProductionWebhookUrl("http://hooks.example.com/ember")).toBe(false);
    expect(isProductionWebhookUrl("https://localhost/ember")).toBe(false);
    expect(isProductionWebhookUrl("http://127.0.0.1:3000/ember")).toBe(false);
  });
  ...
  it("dry-run 默认阻塞且不宣称 production delivered", () => {
    const dryRun = buildProductionWebhookDryRun({
      action: "deliver",
      connector: "webhook",
      targetUrl: "https://hooks.example.com/ember",
    });
  ...
  it("send 前要求 matching connector 的 ready preflight", () => {
    ...
    expect(
      productionPreflightReadyForConnector(
        {
          connector: "webhook",
          missingSecrets: [{ key: "EMBER_AGENT_APP_CONNECTOR_WEBHOOK_URL" }],
