...
`);
  ...
  it("为 macOS arm64 解析预置运行时归档和库文件", () => {
    ...
    expect(plan.releaseDir).toBe(
      "/repo/ember-rs/target/aarch64-apple-darwin/release",
    );
    expect(plan.debugDirs).toEqual([
      "/repo/ember-rs/target/debug",
      "/repo/ember-rs/target/aarch64-apple-darwin/debug",
    ]);
  });
  ...
  it("解析 Mach-O rpath load command", () => {
    const rpaths = readMachORpaths("/repo/ember-rs/target/debug/app-server", {
      platform: "darwin",
      runner(command, args) {
        expect(command).toBe("otool");
        expect(args).toEqual(["-l", "/repo/ember-rs/target/debug/app-server"]);
        return {
          status: 0,
          stdout: `
Load command 0
...
`,
  ...
  it("缺少 macOS rpath 时为 app-server 二进制补 @executable_path", () => {
    const calls = [];
    const result = ensureMacBinaryRpath("/repo/ember-rs/target/debug/app-server", {
      exists: () => true,
    ...
    expect(calls[1]).toEqual([
      ...
      [
        "-add_rpath",
        MACOS_EXECUTABLE_RPATH,
        "/repo/ember-rs/target/debug/app-server",
      ],
      {
        stdio: "inherit",
        shell: false,
