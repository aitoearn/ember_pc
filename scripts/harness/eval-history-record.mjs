...
import { deriveVerificationHistoryRecordFacts as deriveSharedVerificationHistoryRecordFacts } from "../lib/harness-verification-facts.mjs";
...
function runHistoryRecordCli() {
  ...
  if (!options.skipCleanup) {
    ...
    result.cleanup = {
      ...
      currentObservabilityGapCaseCount:
        cleanupReport.summary?.trend?.latestCurrentObservabilityGapCaseCount ??
        0,
      degradedObservabilityGapCaseCount:
        cleanupReport.summary?.trend?.latestDegradedObservabilityGapCaseCount ??
        0,
