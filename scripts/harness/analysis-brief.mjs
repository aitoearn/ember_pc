...
function sanitizeValue(value, workspaceRoot, placeholder) {
  ...
  if (Array.isArray(value)) {
    return value.map((entry) =>
      sanitizeValue(entry, workspaceRoot, placeholder),
    );
  }
...
function sanitizeAbsolutePathForExternalUse(
  absolutePath,
  workspaceRoot,
  placeholder,
) {
...
function listExistingArtifacts(
  rootPath,
  artifactNames,
  workspaceRoot,
  placeholder,
  ...
  return artifactNames.map((fileName) => {
    ...
    return {
      ...
      absolutePath: exists
        ...
        : "",
      relativePath: exists
        ? toPortablePath(path.relative(rootPath, absolutePath))
        : "",
    };
  });
}
...
function buildReadingOrder(handoffArtifacts, evidenceArtifacts) {
  const order = [
    "先读 replay/input.json 与 replay/expected.json，确认任务目标与判定标准。",
  ];

  if (
    handoffArtifacts.some(
      (entry) => entry.fileName === "handoff.md" && entry.exists,
    )
  ) {
    order.push(
  ...
  if (
    evidenceArtifacts.some(
      (entry) => entry.fileName === "summary.md" && entry.exists,
    )
  ) {
    order.push(
  ...
  if (
    evidenceArtifacts.some(
      (entry) => entry.fileName === "timeline.json" && entry.exists,
    )
  ) {
...
function buildHumanReviewChecklist(inputPayload, expectedPayload) {
  ...
  if (expectedPayload?.graderSuggestion?.requiresHumanReview === true) {
    checklist.unshift(
      "当前样本本来就要求人工复核，不应把外部 AI 结论当成最终裁决。",
    );
  }
...
function buildAnalysisContext({
  ...
  return {
    ...
    summary: {
      ...
      pendingRequestCount: Array.isArray(
        sanitizedInput.runtimeContext.pendingRequests,
      )
        ...
        : (handoffJson?.status?.pendingRequestCount ??
          ...
          0),
      queuedTurnCount: Array.isArray(sanitizedInput.runtimeContext.queuedTurns)
...
function main() {
  ...
  const workspaceRootFromInput =
    inputPayload?.session?.workspaceRoot &&
    typeof inputPayload.session.workspaceRoot === "string"
