...
function isCurrentObservabilityGapCase(entry) {
  return (
    entry.observabilityGapCount > 0 && !isDegradedObservabilityGapCase(entry)
  );
}
...
function pushUniqueObservabilityVerificationOutcome(
  outcomes,
  seen,
  signal,
  outcome,
...
function normalizeObservabilityVerificationSummary(
  inputPayload,
  evidencePayload,
) {
  const observabilitySummary = resolveObservabilitySummary(
  ...
  const verificationSummary = isObject(
...
function buildSummary(manifest, suites, options) {
  ...
  const currentRecoveredObservabilityVerificationOutcomes =
    aggregateCaseBreakdown(currentObservabilityDiagnosticCases, (entry) =>
  ...
  return {
    ...
    breakdowns: {
      ...
      degradedObservabilityVerificationOutcomes: aggregateCaseBreakdown(
        allCases.filter((entry) =>
          isDegradedObservabilityDiagnosticCase(entry),
        ),
        (entry) => entry.observabilityVerificationOutcomes,
      ),
    },
...
function renderText(summary) {
  ...
  const topReviewStatuses = summary.breakdowns.reviewDecisionStatuses.slice(
    0,
    5,
  );
...
function renderMarkdown(summary) {
  ...
  if (
    ...
    lines.push("| Outcome | case | ready | invalid |");
    lines.push("| --- | --- | --- | --- |");
    for (const entry of summary.breakdowns
      .currentRecoveredObservabilityVerificationOutcomes) {
      lines.push(
        `| ${entry.name} | ${entry.caseCount} | ${entry.readyCount} | ${entry.invalidCount} |`,
      );
    ...
    for (const entry of suite.cases) {
      ...
      const reviewText = entry.reviewDecisionStatus
        ? [
            ...
            entry.reviewHumanReviewer ? `by: ${entry.reviewHumanReviewer}` : "",
          ]
            .filter(Boolean)
            .join("<br>")
        : "无";
