...
function printUsage() {
  ...
  node scripts/app-server/release-manifest.mjs \\
    --binary ember-rs/target/debug/app-server \\
...
async function main() {
  ...
  if (result.outPath) {
    console.log(
      `[app-server:manifest] wrote ${path.relative(process.cwd(), result.outPath)}`,
    );
  }
}
...
if (isCli) {
  main().catch((error) => {
    console.error(
      `[app-server:manifest] failed: ${error instanceof Error ? error.message : String(error)}`,
    );
    process.exitCode = 1;
  });
}
