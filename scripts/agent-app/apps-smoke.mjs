...
const CONTENT_FACTORY_ACTIONS = {
  "build-store": {
    ...
    runningPattern:
      /正在整理资料|整理当前项目内容|当前进度|正在连接 Ember AI 同事/,
  },
  "run-scenarios": {
    ...
    expectedSkills: ["knowledge-builder", "content-reviewer"],
    runningPattern:
      /正在准备场景|整理当前项目内容|当前进度|正在连接 Ember AI 同事/,
  },
  "run-production": {
    ...
    runningPattern:
      /正在整理本轮内容|整理当前项目内容|当前进度|正在连接 Ember AI 同事/,
  },
  "only-copy": {
    action: "only-copy",
    page: "produce",
    ...
    expectedSkills: ["article-writer", "content-reviewer"],
    runningPattern:
      /正在重写草稿|整理当前项目内容|当前进度|正在连接 Ember AI 同事/,
  },
  "run-scripts": {
    action: "run-scripts",
    page: "produce",
    ...
    expectedSkills: ["article-writer", "content-reviewer"],
    runningPattern:
      /正在生成脚本|整理当前项目内容|当前进度|正在连接 Ember AI 同事/,
  },
  "run-strategy": {
    action: "run-strategy",
    page: "deliver",
    ...
    expectedSkills: ["article-writer", "content-reviewer"],
    runningPattern:
      /正在准备交付|整理当前项目内容|当前进度|正在连接 Ember AI 同事/,
  },
  "run-review": {
    action: "run-review",
    page: "review",
    ...
    runningPattern:
      /正在分析复盘|整理当前项目内容|当前进度|正在连接 Ember AI 同事/,
  },
};
...
function isBenignDevBridgeEventAbortText(value) {
  ...
  return (
    ...
    (text.includes("/events?") ||
      ...
      text.includes("ember-open-voice-model-settings") ||
    ...
    <script>
      ...
      function runAction(action) {
        [
          ...
          ["ember.models", "getRouting"],
          ["ember.usage", "getTokenUsage"],
          ["ember.usage", "getCostSummary"],
          ["ember.skills", "list"],
          ["ember.agent", "streamTask"],
        ...
        document.querySelector("#status").textContent =
          "正在连接 Ember AI 同事 · 正在整理当前项目内容 · 当前进度已记录";
      }
      ...
      function renderPage(page) {
        ...
        content.innerHTML =
          '<h2>' + pages[page] + '</h2>' +
          '<p id="status">Ember AI 运行现场 · 当前进度等待启动</p>' +
          campaignControls +
          projectControls +
  ...
  try {
    ...
    if (frame) {
      const [bodyText, sdkCallLog, hostTaskRecord] = await Promise.all([
        ...
        frame
          .evaluate(() => window.emberAgentAppBridge?.getSdkCallLog?.() ?? [])
          .catch((frameError) => ({
            error:
              frameError instanceof Error
                ? frameError.message
...
function summarizeContentFactoryRuntimeFacts(capabilityCalls) {
  ...
  return {
    modelsStarted: callSet.has("ember.models.getRouting"),
    usageStarted:
      callSet.has("ember.usage.getTokenUsage") ||
      callSet.has("ember.usage.getCostSummary"),
    skillsStarted: callSet.has("ember.skills.list"),
    streamOrGetTaskStarted:
      callSet.has("ember.agent.streamTask") || callSet.has("ember.agent.getTask"),
  };
}
...
async function runContentFactoryActionE2e(page, options) {
  ...
  await frame.waitForFunction(
    () =>
      window.emberAgentAppBridge
        ...
        ?.some(
          (call) =>
            call.capability === "ember.agent" && call.method === "startTask",
        ),
    undefined,
    { timeout: boundedTimeoutMs },
  );
  await frame.waitForFunction(
    () => {
      const record = window.emberAgentAppBridge?.getHostTaskRunRecord?.(
        "contentFactoryProduction",
      );
      const callLog = window.emberAgentAppBridge?.getSdkCallLog?.();
      const findTaskId = (value, depth = 0) => {
  ...
  const startTaskSeen = newCalls.includes("ember.agent.startTask");
  ...
  const processPanelVisible =
    bodyText.includes("Ember AI 运行现场") ||
    bodyText.includes("正在连接 Ember AI 同事") ||
  ...
  assert(
    startTaskSeen,
    `Content Factory ${actionConfig.action} E2E should invoke ember.agent.startTask`,
  );
  assert(
    taskAccepted,
    ...
    apps: [
      {
        ...
        enabled: true,
        packageUrl:
          "https://ember.local/agent-apps/content-factory-app/releases/0.3.0/package.zip",
        packageHash:
...
async function runFlagOffRegression(options) {
  ...
  const userDataDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "ember-agent-apps-flag-off-"),
  );
...
async function main() {
  ...
  const userDataDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "ember-agent-apps-smoke-"),
  );
