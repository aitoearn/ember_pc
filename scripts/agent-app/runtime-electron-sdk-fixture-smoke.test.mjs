...
describe("agent app runtime Electron SDK fixture smoke guard", () => {
  ...
  it("drives task lifecycle from iframe SDK instead of renderer direct invoke", () => {
    ...
    expect(content).toContain('capability: "ember.agent"');
  ...
  it("declares adapter capabilities in installed state and readiness", () => {
    ...
    expect(content).toContain('"ember.agent": "^0.3.0"');
    expect(content).toContain('"ember.storage": "^0.3.0"');
    expect(content).toContain('"ember.artifacts": "^0.3.0"');
    expect(content).toContain('"ember.evidence": "^0.3.0"');
    expect(content).toContain('"ember.knowledge": "^0.3.0"');
    expect(content).toContain("REQUIRED_HOST_ADAPTER_CAPABILITIES");
