...
const DEFAULTS = {
  ...
  evidenceDir: path.join(
    ...
    "qc",
    "gui-evidence",
    "agent-app-ui-runtime-electron-fixture",
  ),
  prefix: "agent-app-ui-runtime-electron-fixture",
  timeoutMs: 120_000,
  intervalMs: 250,
  keepTemp: false,
};
...
用法:
  node scripts/agent-app/ui-runtime-electron-fixture-smoke.mjs

选项:
  --app-url <url>        可选 renderer dev server，例如 http://127.0.0.1:1420/
  --fixture-dir <path>   fixture Agent App 目录
...
function logStage(stage) {
  console.log(`[smoke:agent-app-ui-runtime-electron-fixture] stage=${stage}`);
}
...
function createTempRuntimeEnv() {
  const tempRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "agent-app-ui-runtime-electron-"),
  );
...
function buildInstalledState(fixtureDir) {
  ...
  const manifest = {
    ...
    status: "draft",
    appType: "domain-app",
    description: "Agent App UI runtime Electron fixture",
...
function summarizeTraceEntries(traceEntries) {
  ...
  const legacyAgentAppCommandsSeen = LEGACY_AGENT_APP_COMMANDS.filter(
...
async function waitForRuntimeEvidence(page, options) {
  ...
  while (Date.now() - startedAt < options.timeoutMs) {
    ...
    if (
      ...
      traceSummary.legacyAgentAppCommandsSeen.length === 0
    ) {
      return lastSnapshot;
    }
    await sleep(options.intervalMs);
...
async function run() {
  ...
  try {
    ...
    assert(runtimeEvidence, "未收集到 Agent App runtime iframe 证据");
    Object.assign(summary, {
      ...
      legacyAgentAppCommandsSeen:
        runtimeEvidence.legacyAgentAppCommandsSeen ?? [],
    ...
    summary.legacyAgentAppCommandsSeen =
      finalTraceSummary.legacyAgentAppCommandsSeen;
    ...
    console.log(
      `[smoke:agent-app-ui-runtime-electron-fixture] summary=${summaryPath}`,
    );
    console.log(
      `[smoke:agent-app-ui-runtime-electron-fixture] frameSrc=${summary.frameSrc}`,
    );
  } catch (error) {
