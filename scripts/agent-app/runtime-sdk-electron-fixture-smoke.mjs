...
const DEFAULTS = {
  ...
  evidenceDir: path.join(
    ...
    "qc",
    "gui-evidence",
    "agent-app-runtime-sdk-electron-fixture",
  ),
  prefix: "agent-app-runtime-sdk-electron-fixture",
  timeoutMs: 120_000,
  intervalMs: 250,
  keepTemp: false,
};
...
const WORKSPACE_ID = "agent-app-sdk-electron-workspace";
const SESSION_ID = "agent-app-sdk-electron-session";
const REQUEST_ID = "agent-app-sdk-electron-action";
...
function printHelp() {
  console.log(`
Agent App Runtime SDK Electron Fixture Smoke

用途:
  启动真实 Electron Desktop Host，种子临时 Agent App installed state，
  通过正式侧栏打开 Agent App runtime iframe，然后在 iframe 内使用
  @embercloud/agent-app-runtime browser SDK 发起 ember.agent.startTask /
  getTask / submitHostResponse / cancelTask，验证链路进入 Electron
  Desktop Host facade 与 App Server JSON-RPC RuntimeCore/backend。

目标链路:
  Agent App iframe -> Host Bridge postMessage capability:invoke
    -> AgentRuntimeCapabilityHost -> src/lib/api/agentAppRuntime.ts
...
用法:
  node scripts/agent-app/runtime-sdk-electron-fixture-smoke.mjs
...
function parseArgs(argv) {
  ...
  if (!fs.existsSync(path.join(SDK_DIST_DIR, "index.js"))) {
    throw new Error(
      `Agent App runtime SDK dist 不存在，请先构建 packages/agent-app-runtime: ${SDK_DIST_DIR}`,
    );
  }
...
function logStage(stage) {
  console.log(`[smoke:agent-app-runtime-sdk-electron-fixture] stage=${stage}`);
}
...
function createTempRuntimeEnv() {
  const tempRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "agent-app-runtime-sdk-electron-"),
  );
...
function resolveTempPreferredDataDir({
  ...
  if (platform === "win32") {
    return path.join(localAppData, "ember");
  }
  if (platform === "darwin") {
    return path.join(home, "Library", "Application Support", "ember");
  }
  return path.join(xdgDataHome, "ember");
}
...
function stableStringifyAgentAppValue(value) {
  if (Array.isArray(value)) {
  ...
  if (isRecord(value)) {
    return `{${Object.entries(value)
      ...
      .map(
        ([key, entry]) =>
          `${JSON.stringify(key)}:${stableStringifyAgentAppValue(entry)}`,
      )
      .join(",")}}`;
  }
...
function buildAgentAppManifestHash(manifest) {
  return `manifest-fnv1a-${fnv1a(stableStringifyAgentAppValue(manifest))}`;
}

function buildAgentAppPackageHash({ manifest, sourceUri }) {
...
function buildFixtureManifest() {
  return {
    ...
    status: "draft",
    appType: "domain-app",
    description: "Agent App SDK Electron task fixture",
    runtimeTargets: ["local"],
    requires: {
      appRuntime: ">=0.3.0 <1.0.0",
      sdk: "@ember/app-sdk@^0.3.0",
      capabilities: {
        "ember.ui": "^0.3.0",
        "ember.agent": "^0.3.0",
      },
    ...
    entries: [
      {
        ...
        title: "SDK 任务验证",
        route: ENTRY_ROUTE,
        requiredCapabilities: ["ember.agent"],
        permissions: [],
...
function buildInstalledState({ fixtureDir, manifest, packageHash, manifestHash }) {
  ...
  const appLevelAgentRequirement = buildCapabilityRequirement("ember.agent", [
    "requires",
  ]);
  ...
  const uiRequirement = buildCapabilityRequirement("ember.ui", ["requires"]);
  const entry = {
    appId: APP_ID,
    key: ENTRY_KEY,
    kind: "page",
      ...
      readinessHints: [
        {
          ...
          message: "Fixture installed state for Agent App SDK Electron smoke.",
          severity: "info",
        },
      ],
      provenance,
...
function writeFixtureAgentApp({ fixtureDir, packageHash, manifestHash }) {
...
<html lang="zh-CN">
  ...
  <body>
    <main>
      <h1>内容工厂 SDK</h1>
      <p class="muted">Agent App iframe · Host Bridge · ember.agent.startTask</p>
      <section class="panel">
    ...
    <script type="module">
      ...
      const invoker = createLimeHostBridgeCapabilityInvoker({
        ...
        requestIdPrefix: "agent-app-runtime-sdk-electron-fixture",
      });
      ...
      async function run() {
        ...
        state.error = null;
        updateStatus("通过 Host Bridge 调用 ember.agent.startTask");
        try {
          const started = await ember.agent.startTask({
            title: "Agent App SDK Electron fixture task",
            prompt: "生成一段 Agent App SDK Host Bridge fixture 文案",
            taskKind: "content_factory.write",
            idempotencyKey: "agent-app-runtime-sdk-electron-fixture",
            sessionId,
            workspaceId,
            ...
            input: {
              topic: "Agent App iframe Host Bridge",
            ...
            turnConfig: {
              ...
              system_prompt: "Agent App SDK Electron fixture system prompt",
              ...
              metadata: {
                source: "agent-app-runtime-sdk-electron-fixture",
              },
            },
          });
          updateStatus("读取 task blocked 状态");
          const firstTask = await ember.agent.getTask({
            taskId: started.taskId,
            sessionId: started.sessionId,
            turnId: started.turnId,
          });
          updateStatus("提交 Host response");
          const hostResponse = await ember.agent.submitHostResponse({
            taskId: started.taskId,
            requestId,
            actionType: "ask_user",
            confirmed: true,
          ...
          const secondTask = await ember.agent.getTask({
            taskId: started.taskId,
            sessionId: started.sessionId,
            turnId: started.turnId,
          });
          updateStatus("取消 task");
          const cancelTask = await ember.agent.cancelTask({
            taskId: started.taskId,
            sessionId: started.sessionId,
            turnId: started.turnId,
          });
          const finalTask = await ember.agent.getTask({
            taskId: started.taskId,
            sessionId: started.sessionId,
            turnId: started.turnId,
          });
          ...
          state.result = {
            protocol: EMBER_AGENT_APP_BRIDGE_PROTOCOL,
            version: EMBER_AGENT_APP_BRIDGE_VERSION,
            started,
            firstTask,
            hostResponse,
            secondTask,
      ...
      window.__emberSdkSmoke = {
        appId,
        entryKey,
        bridgeProtocol: EMBER_AGENT_APP_BRIDGE_PROTOCOL,
        bridgeVersion: EMBER_AGENT_APP_BRIDGE_VERSION,
    ...
    events: [
      {
        type: "turn.started",
        payload: {
          backend: "external-agent-app-sdk-fixture",
          sessionId: readSessionId(),
          turnId: readTurnId(),
        },
      },
      {
        ...
        payload: {
          ...
          actionType: "ask_user",
          message: "确认 Agent App SDK Electron fixture 继续",
        },
      },
    ],
  }));
...
async function waitForAgentAppSidebarEntry(page, options) {
  ...
  throw new Error("未在正式侧栏观察到 fixture Agent App 入口");
}
...
async function clickAgentAppSidebarEntry(page) {
  ...
  throw new Error("fixture Agent App 侧栏按钮不可点击");
}
...
async function waitForRuntimeFrame(page, options) {
  ...
  while (Date.now() - startedAt < options.timeoutMs) {
    ...
    if (frame) {
      const ready = await frame
        .evaluate(() => {
          const smoke = window.__emberSdkSmoke;
          return {
            url: window.location.href,
            bodyText: document.body?.innerText || "",
      ...
      if (
        ready?.hasSmoke &&
        ready.ready &&
        ready.protocol === "ember.agentApp.bridge" &&
        ready.version === 1
      ) {
        return { frame, ready };
      }
  ...
  throw new Error("fixture iframe 未加载 Agent App SDK smoke");
}

async function runSdkSmokeFromFrame(frame) {
  return await frame.evaluate(async () => {
    if (!window.__emberSdkSmoke?.run) {
      throw new Error("Agent App SDK smoke bridge is unavailable");
    }
    return await window.__emberSdkSmoke.run();
  });
}

async function readFrameSmokeState(frame) {
  return await frame
    .evaluate(() => window.__emberSdkSmoke?.getState?.() ?? null)
    .catch(() => null);
}

function assertSdkSmokeResult(result, backendSummary) {
  assert(result?.protocol === "ember.agentApp.bridge", "Host Bridge protocol 不正确");
  ...
  for (const key of [
    "ember.agent.startTask",
    "ember.agent.getTask",
    "ember.agent.submitHostResponse",
    "ember.agent.cancelTask",
  ]) {
  ...
  assert(
    backendSummary.turnConfigSystemPrompt ===
      "Agent App SDK Electron fixture system prompt",
      ...
      return await window.electronAPI.invoke("app_server_handle_json_lines", {
        request: {
          lines: [
            JSON.stringify({
              jsonrpc: "2.0",
              id: "stop-agent-app-sdk-runtime",
              method: "agentAppUiRuntime/stop",
              params: { appId },
            }),
          ],
        },
...
async function run() {
  ...
  const fixtureDir = path.join(runtimeEnv.tempRoot, "agent-app-fixture");
  const backendPath = path.join(
    runtimeEnv.tempRoot,
    "agent-app-sdk-backend.mjs",
  );
  const backendLogPath = path.join(runtimeEnv.tempRoot, "backend-log.jsonl");
  const manifest = buildFixtureManifest();
  const manifestHash = buildAgentAppManifestHash(manifest);
  const packageHash = buildAgentAppPackageHash({
    manifest,
    sourceUri: fixtureDir,
  });
  writeFixtureAgentApp({ fixtureDir, packageHash, manifestHash });
  ...
  try {
    ...
    console.log(
      `[smoke:agent-app-runtime-sdk-electron-fixture] summary=${summaryPath}`,
    );
    console.log(
      `[smoke:agent-app-runtime-sdk-electron-fixture] backendLog=${backendLogEvidencePath}`,
    );
  } catch (error) {
