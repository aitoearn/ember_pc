...
function resolveDefaultContentFactoryDir() {
  ...
  return path.resolve(process.cwd(), "..", "..", "embercloud", "content-factory-app");
}
...
const DEFAULTS = {
  ...
  overallTimeoutMs: 0,
  evidenceDir: path.join(process.cwd(), ".ember", "qc", "gui-evidence", "agent-apps"),
...
const CONTENT_FACTORY_ACTIONS = {
  "build-store": {
    ...
    runningPattern:
      /AI 同事正在整理知识库|Ember AI 运行现场|正在连接 Ember AI 同事/,
  },
  "run-scenarios": {
    ...
    expectedSkills: ["knowledge-builder", "content-reviewer"],
    runningPattern:
      /AI 同事正在准备场景|Ember AI 运行现场|正在连接 Ember AI 同事/,
  },
  "run-production": {
    ...
    runningPattern:
      /正在整理本轮内容|AI 同事正在生成内容包|Ember AI 运行现场|正在连接 Ember AI 同事/,
  },
  ...
  "only-copy": {
    ...
    expectedSkills: ["article-writer", "content-reviewer"],
    runningPattern:
      /AI 同事正在重写文案|Ember AI 运行现场|正在连接 Ember AI 同事/,
  },
  "run-scripts": {
    action: "run-scripts",
    page: "produce",
    ...
    expectedSkills: ["article-writer", "content-reviewer"],
    runningPattern:
      /AI 同事正在生成脚本|Ember AI 运行现场|正在连接 Ember AI 同事/,
  },
  "run-strategy": {
    action: "run-strategy",
    page: "deliver",
    ...
    expectedSkills: ["article-writer", "content-reviewer"],
    runningPattern:
      /AI 同事正在准备交付包|Ember AI 运行现场|正在连接 Ember AI 同事/,
  },
  "run-review": {
    action: "run-review",
    page: "review",
    ...
    runningPattern:
      /AI 同事正在分析复盘|Ember AI 运行现场|正在连接 Ember AI 同事/,
  },
};
...
function printHelp() {
  console.log(`Usage:
  node scripts/agent-app/content-factory-flow.mjs [options]

Options:
  --app-url <url>                 Ember WebView URL, default http://127.0.0.1:1420/
  ...
  --content-factory-dir <dir>      Current content-factory-app directory, default ../../embercloud/content-factory-app
...
function textShowsRuntimeProcess(text, actionConfig) {
  return (
    text.includes("Ember AI 运行现场") ||
    text.includes("正在连接 Ember AI 同事") ||
    ...
    apps: [
      {
        ...
        enabled: true,
        packageUrl: `https://ember.local/agent-apps/${appInfo.appId}/releases/${appInfo.version}/package.zip`,
...
async function activateBootstrapCatalog(page, bootstrap) {
  await page.evaluate((payload) => {
    window.__EMBER_AGENT_APP_E2E_BOOTSTRAP_CATALOG__ = payload;
    if (!window.__EMBER_AGENT_APP_E2E_FETCH_PATCHED__ && typeof window.fetch === "function") {
      const originalFetch = window.fetch.bind(window);
      window.__EMBER_AGENT_APP_E2E_FETCH_PATCHED__ = true;
          ...
          if (
            ...
            return new Response(
              JSON.stringify({
                code: 200,
                message: "success",
                data: window.__EMBER_AGENT_APP_E2E_BOOTSTRAP_CATALOG__,
              }),
              {
                status: 200,
    ...
    window.__EMBER_SESSION_TOKEN__ = "smoke-agent-apps-token";
    window.__EMBER_BOOTSTRAP__ = { data: { agentAppCatalog: payload } };
  }, bootstrap);
...
async function launchStandaloneContentFactoryShell(page) {
  return page.evaluate(async () => {
    ...
    const runtimeProfile =
      runtimeProfileModule.buildEmberRuntimeProfileForInstalledState({
        state,
        hostProfile,
      });
    const entry =
  ...
  if (options.launchMode === "standalone-shell") {
    ...
    await runtimePage.addInitScript(({ appId, entryKey }) => {
      window.sessionStorage.setItem(
        "ember.appNavigation.restore.v1",
        JSON.stringify({
          page: "agent-app",
          params: {
            appId,
...
async function markFrameProbe(frame) {
  return frame.evaluate(() => {
    const globalKey = "__emberContentFactoryFlowProbe";
    if (!window[globalKey]) {
      window[globalKey] = {
        id:
...
async function readSdkCallLogLength(frame) {
  try {
    return await frame.evaluate(() => {
      const callLog = window.emberAgentAppBridge?.getSdkCallLog?.();
...
async function readHostTaskRecord(frame, actionTaskId = "") {
  try {
    return await evaluateWithTimeout(frame, (taskId) => {
      const bridge = window.emberAgentAppBridge;
...
async function runFlowAction(frame, page, options, actionName) {
  ...
  const hostFallbackVisible = bodyText.includes("Ember AI 同事连接失败");
...
async function main() {
  ...
  if (options.actions.length > 0) {
    assertLiveProviderSmokeAllowed({
      ...
      scriptName: "scripts/agent-app/content-factory-flow.mjs",
    });
  }
  ...
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "ember-content-factory-flow-"));
  ...
  await context.addInitScript((payload) => {
    window.localStorage.removeItem("ember.agentAppHost.flags");
    window.localStorage.removeItem("ember.agentAppHost.labEnabled");
    ...
    window.__EMBER_AGENT_APPS_SMOKE_BOOTSTRAP__ = payload;
  }, {
    ...bootstrap,
    activeProjectStorageKey: CONTENT_FACTORY_ACTIVE_PROJECT_STORAGE_KEY,
    hostTaskStorageKey: CONTENT_FACTORY_HOST_TASK_STORAGE_KEY,
  ...
  await page.route(
    "https://user.emberai.run/api/v1/public/tenants/*/client/agent-apps",
    async (route) => {
      await route.fulfill({
        status: 200,
  ...
  attachPageTelemetry(page, consoleErrors, failedRequests, "ember-desktop");
