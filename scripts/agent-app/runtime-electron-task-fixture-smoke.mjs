...
const DEFAULTS = {
  ...
  evidenceDir: path.join(
    ...
    "qc",
    "gui-evidence",
    "agent-app-runtime-electron-task-fixture",
  ),
  prefix: "agent-app-runtime-electron-task-fixture",
  keepTemp: false,
};

const APP_ID = "content-factory-app";
const ENTRY_KEY = "writer";
const WORKSPACE_ID = "workspace-agent-app-task-fixture";
const SESSION_ID = "agent-app-electron-task-session";
const TASK_ID = "agent-app-electron-task-1";
const TURN_ID = "agent-app-electron-turn-1";
const REQUEST_ID = "agent-app-electron-request-1";
...
function printHelp() {
  console.log(`
Agent App Runtime Electron Task Fixture Smoke

用途:
  启动真实 Electron Desktop Host，在 renderer 内直接调用
  agent_app_runtime_start_task / get_task / submit_host_response / cancel_task，
...
用法:
  node scripts/agent-app/runtime-electron-task-fixture-smoke.mjs
...
function logStage(stage) {
  console.log(`[smoke:agent-app-runtime-electron-task-fixture] stage=${stage}`);
}
...
function createTempRuntimeEnv() {
  const tempRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "agent-app-runtime-electron-task-"),
  );
...
async function waitForRendererReady(page, options, onSnapshot) {
  ...
  throw new Error(
    "Electron renderer invoke bridge / Agent App runtime commands 未就绪",
  );
}
...
async function runTaskLifecycleFromPage(page) {
  return await page.evaluate(
    async ({
      ...
      const start = await api.invoke("agent_app_runtime_start_task", {
        request: {
          appId,
          entryKey,
          workspaceId,
          ...
          taskId,
          taskKind,
          title: "Electron Agent App task fixture",
          ...
          turnConfig: {
            ...
            system_prompt: "Agent App task fixture system prompt",
      ...
      const firstSnapshot = await api.invoke("agent_app_runtime_get_task", {
      ...
      const secondSnapshot = await api.invoke("agent_app_runtime_get_task", {
        request: { appId, taskId, sessionId },
      });
    ...
    events: [
      {
        type: "turn.started",
        payload: {
          backend: "external-agent-app-task-fixture",
          sessionId: sessionId(),
          turnId: turnId(),
        },
      },
      {
        ...
        payload: {
          ...
          requestId: "${REQUEST_ID}",
          actionType: "ask_user",
          message: "确认 Electron Agent App task fixture 继续",
        },
      },
    ],
  }));
...
async function run() {
  ...
  try {
    ...
    console.log(
      `[smoke:agent-app-runtime-electron-task-fixture] summary=${summaryPath}`,
    );
    console.log(
      `[smoke:agent-app-runtime-electron-task-fixture] backendLog=${backendLogEvidencePath}`,
    );
  } catch (error) {
