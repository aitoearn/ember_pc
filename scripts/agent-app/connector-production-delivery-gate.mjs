...
import {
  ...
  writeJsonFile,
} from "../lib/agent-app-connector-production-delivery-gate-core.mjs";
...
function parseArgs(argv) {
  const options = {
    ...
    output: "",
    preflight:
      ".ember/qc/gui-evidence/agent-apps/p18-7-e-connector-production-preflight-check-20260519-codex.json",
  };
...
function printHelp() {
  console.log(`Usage:
  node scripts/agent-app/connector-production-delivery-gate.mjs [options]

Options:
  --preflight <path>  Production preflight JSON.
  --delivery <path>   Production delivery evidence JSON.
  --gui-evidence <path>
                      GUI evidence proving delivery is visible to Agent App UI/task events.
...
async function main() {
  ...
  console.log(
    `[agent-app-connector-production-delivery-gate] status=${result.status} preflight=${result.preflight.status} deliveryProof=${result.delivery.proofLevel || "missing"} missing=${result.missingRequirements.length}`,
  );
  if (result.missingRequirements.length > 0) {
    console.log(
      `[agent-app-connector-production-delivery-gate] missingCodes=${result.missingRequirements
        .map((item) => item.code)
        .join(",")}`,
    );
  }
  if (options.output) {
    console.log(`[agent-app-connector-production-delivery-gate] output=${options.output}`);
  }
...
main().catch((error) => {
  console.error(
    `[agent-app-connector-production-delivery-gate] failed: ${
