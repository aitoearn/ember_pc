...
Options:
  ...
  --content-factory-dir <dir>     Content Factory Agent App directory, default ../../embercloud/content-factory-app
...
async function runStandaloneLaunch(page, options) {
  return page.evaluate(
    async ({ appDir }) => {
      ...
      window.__EMBER_SESSION_TOKEN__ = "standalone-shell-smoke-token";
            ...
            apps: [
              {
                ...
                enabled: true,
                packageUrl:
                  "https://ember.local/agent-apps/content-factory-app/releases/0.8.0/package.zip",
                packageHash:
...
async function main() {
  ...
  const userDataDir = fs.mkdtempSync(
    path.join(process.cwd(), ".ember", "tmp-standalone-shell-"),
  );
