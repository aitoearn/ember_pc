...
const DEFAULTS = {
  ...
  timeoutMs: 120_000,
  intervalMs: 1_000,
  evidenceDir: path.join(process.cwd(), ".ember", "qc", "gui-evidence", "agent-app-lab"),
...
async function runFlagOffRegression(options) {
  ...
  const userDataDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "ember-agent-app-lab-flag-off-"),
  );
...
async function main() {
  ...
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "ember-agent-app-lab-smoke-"));
  let context;
