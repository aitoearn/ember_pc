...
const DEFAULTS = {
  ...
  evidenceDir: path.join(
    ...
    "qc",
    "gui-evidence",
    "agent-app-runtime-electron-sdk-fixture",
  ),
  prefix: "agent-app-runtime-electron-sdk-fixture",
  timeoutMs: 120_000,
  intervalMs: 250,
  keepTemp: false,
};
...
const ARTIFACT_ID = "agent-app-sdk-artifact-1";
const ARTIFACT_REF = ".ember/artifacts/agent-app-sdk-artifact-1.json";
const ARTIFACT_TITLE = "Agent App SDK artifact fixture";
const TOOL_CALL_ID = "agent-app-sdk-tool-call-1";
const TOOL_NAME = "WebFetch";
const TOOL_OUTPUT_PREVIEW = "fetched https://example.com/agent-app-sdk-fixture";
...
function printHelp() {
  console.log(`
Agent App Runtime Electron SDK Fixture Smoke
...
用途:
  ...
  createLimeHostBridgeCapabilityInvoker 调用 ember.agent start/get/respond/cancel。
  后端使用一次性 external backend fixture 记录 App Server RuntimeCore 请求。

目标链路:
  Agent App iframe SDK -> AgentAppRuntimePage Host Bridge
...
说明:
  本脚本不在 renderer 中直接调用 agent_app_runtime_*，不使用 renderer mock、
  default mock、DevBridge mock、legacy agent_runtime_* 或 mock backend。

用法:
  node scripts/agent-app/runtime-electron-sdk-fixture-smoke.mjs
...
function logStage(stage) {
  console.log(`[smoke:agent-app-runtime-electron-sdk-fixture] stage=${stage}`);
}
...
function createTempRuntimeEnv() {
  const tempRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "agent-app-runtime-electron-sdk-"),
  );
  ...
  const fixtureDir = ensureDir(path.join(tempRoot, "agent-app-sdk-fixture"));
  const backendPath = path.join(tempRoot, "agent-app-sdk-backend.mjs");
  const backendLogPath = path.join(tempRoot, "agent-app-sdk-backend.jsonl");
    ...
    events: [
      ...
      },
      {
        ...
        payload: {
          ...
          actionType: "ask_user",
          message: "确认 Agent App SDK fixture 继续"
        }
      }
    ]
  }));
...
function writeFixtureAgentApp(fixtureDir) {
  ...
  if (!fs.existsSync(path.join(sdkDistDir, "index.js"))) {
    throw new Error(`Agent App SDK dist 不存在: ${sdkDistDir}`);
  }
  ...
  fs.writeFileSync(
    ...
    `# Content Factory SDK Fixture\n\nP3.216 Agent App SDK Host Bridge fixture.\n`,
  );
  fs.writeFileSync(
...
<html lang="zh-CN">
  <head>
    <meta charset="utf-8" />
    <title>Agent App SDK Fixture</title>
    <style>
      body {
        margin: 0;
    ...
    <script type="module">
      ...
      async function run() {
        const bridge = createLimeHostBridgeCapabilityInvoker({
          ...
          requestIdPrefix: "agent-app-sdk-fixture"
        });
        ...
        const startTask = unwrap("startTask", await bridge.call({
          ...
          args: {
            ...
            input: {
              topic: "Agent App SDK Host Bridge current path",
            ...
            turnConfig: {
              ...
              system_prompt: "Agent App SDK fixture system prompt",
              reasoning_effort: "medium",
              approval_policy: "on-request",
              sandbox_policy: "workspace-write",
              web_search: false,
              execution_strategy: "agent_app_sdk_fixture",
              metadata: {
...
server.listen(port, "127.0.0.1", () => {
  ...
  console.error("[agent-app-sdk-fixture] listening " + JSON.stringify(address));
});
`,
  );
}
...
function buildInstalledState(fixtureDir) {
  ...
  const manifest = {
    ...
    status: "draft",
    appType: "domain-app",
    description: "Agent App SDK runtime Electron fixture",
  ...
  while (Date.now() - startedAt < options.timeoutMs) {
    ...
    if (frame) {
      const result = await frame
        .evaluate(() => window.__agentAppSdkFixtureResult ?? null)
        .catch((error) => {
          if (isTransientPageEvaluationError(error)) {
            return null;
          }
...
function assertSdkLifecycleResult(result, backendSummary) {
  ...
  const methodsSeen = callLog
    .filter((entry) => entry?.capability === "ember.agent")
    .map((entry) => entry.method);
  for (const method of SDK_METHODS) {
    assert(
      methodsSeen.includes(method),
      `iframe SDK callLog 未包含 ember.agent.${method}`,
    );
  }
  ...
  assert(
    ...
    "SDK ember.ui.openAgentRun 未返回 host_agent_run surface",
  );
  assert(
    result.taskLifecycle?.hostRun?.opened === true,
    "SDK ember.ui.openAgentRun 未打开 Host Agent Run 面板",
  );
  assert(
...
async function run() {
  ...
  writeFixtureAgentApp(runtimeEnv.fixtureDir);
  ...
  try {
    ...
    assert(
      ...
      "正式 Agent App runtime surface 未出现",
    );
    assert(summary.runtimeFrameVisible, "正式 Agent App runtime iframe 未出现");
    assert(
    ...
    writeJsonFile(summaryPath, summary);
    console.log(
      `[smoke:agent-app-runtime-electron-sdk-fixture] summary=${summaryPath}`,
    );
    console.log(
      `[smoke:agent-app-runtime-electron-sdk-fixture] backendLog=${backendLogEvidencePath}`,
    );
  } catch (error) {
