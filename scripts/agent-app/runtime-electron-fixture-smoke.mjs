...
const DEFAULTS = {
  ...
  evidenceDir: path.join(
    ...
    "qc",
    "gui-evidence",
    "agent-app-runtime-electron-fixture",
  ),
  prefix: "agent-app-runtime-electron-fixture",
  timeoutMs: 120_000,
  intervalMs: 250,
  keepTemp: false,
};
...
const WORKSPACE_ID = "agent-app-runtime-electron-workspace";
const SESSION_ID = "agent-app-runtime-electron-session";
const TASK_ID = "agent-app-runtime-electron-task";
const TURN_ID = "agent-app-runtime-electron-turn";
const REQUEST_ID = "agent-app-runtime-electron-action";
...
function printHelp() {
  console.log(`
Agent App Runtime Electron Fixture Smoke

Purpose:
  Launch a real Electron Desktop Host with an explicit App Server external
  backend fixture, then invoke the Agent App task facade commands through the
...
Usage:
  node scripts/agent-app/runtime-electron-fixture-smoke.mjs
...
function logStage(stage) {
  console.log(`[smoke:agent-app-runtime-electron-fixture] stage=${stage}`);
}
...
function createTempRuntimeEnv() {
  const tempRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "agent-app-runtime-electron-"),
  );
  ...
  const backendPath = path.join(tempRoot, "agent-app-runtime-backend.mjs");
  const ledgerPath = path.join(
    tempRoot,
    "agent-app-runtime-backend-ledger.jsonl",
  );
    ...
    events: [
      {
        type: "message.delta",
        payload: {
          backend: "agent-app-runtime-electron-fixture",
...
async function waitForRendererReady(page, options) {
  ...
  while (Date.now() - startedAt < options.timeoutMs) {
    const snapshot = await evaluatePageSnapshot(page, () => ({
      ...
      supportsStart:
        ...
        window.electronAPI.supportsCommand("agent_app_runtime_start_task"),
  ...
  throw new Error(
    "Electron renderer / Agent App runtime facade bridge not ready",
  );
}
...
async function invokeAgentAppRuntimeLifecycle(page) {
    ...
    async ({
      ...
      const start = await invoke(commands.startTask, {
        request: {
          ...
          input: { topic: "Agent App runtime Electron fixture" },
          ...
          metadata: {
            source: "agent-app-runtime-electron-fixture",
          },
          turnConfig: {
            provider_config: {
      ...
      const hostResponse = await invoke(commands.submitHostResponse, {
        request: {
          ...
          runtimeRequest: {
            ...
            metadata: { source: "agent-app-runtime-electron-fixture" },
...
async function run() {
  ...
  try {
    ...
    logStage("invoke-agent-app-runtime-facade");
    const lifecycle = await invokeAgentAppRuntimeLifecycle(page);
    ...
    console.log(
      `[smoke:agent-app-runtime-electron-fixture] summary=${summaryPath}`,
    );
    console.log(
      `[smoke:agent-app-runtime-electron-fixture] backendKinds=${summary.backendKinds.join(",")}`,
    );
  } catch (error) {
...
run().catch((error) => {
  console.error(
    `[smoke:agent-app-runtime-electron-fixture] failed: ${
