...
const defaultBatchSize = Number.parseInt(
  process.env.EMBER_VITEST_BATCH_SIZE || "16",
  10,
);
const batchSize =
...
function runVitest(args, label) {
  if (!includeLiveProviderTests) {
    ...
    if (liveTestArgs.length > 0) {
      throw new Error(
        `[vitest-smart] ${liveTestArgs.join(", ")} 会调用真实模型或多模态 Provider。为避免消耗额度，默认禁止执行；如确需运行，请设置 EMBER_ALLOW_LIVE_PROVIDER_SMOKE=1 或 EMBER_REAL_API_TEST=1。`,
      );
    }
  }
...
function main() {
  ...
  if (!includeLiveProviderTests && skippedLiveTestCount > 0) {
    console.log(
      `[vitest-smart] 默认跳过 ${skippedLiveTestCount} 个 live Provider 测试；如确需运行，请设置 EMBER_ALLOW_LIVE_PROVIDER_SMOKE=1 或 EMBER_REAL_API_TEST=1。`,
    );
  }
