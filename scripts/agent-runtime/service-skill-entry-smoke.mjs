...
const cargoTargetDir = path.resolve(
  process.env.EMBER_AGENT_SERVICE_SKILL_ENTRY_TARGET_DIR ||
    path.join(os.tmpdir(), "ember-agent-service-skill-entry-target"),
);
...
function runCargoTestGroup(label, { packageName, testFilter, expectedTests }) {
  ...
  const result = spawnSync(
    ...
    {
      cwd: path.join(rootDir, "ember-rs"),
      encoding: "utf8",
      env: process.env,
    },
  );
  ...
  const transcript = {
    ...
    evidenceRequired: {
      ...
      skillToolGateTranscript:
        "ember-agent SkillTool gate exact tests covered allowlisted session source metadata and disabled-session denial.",
    },
    failureModes: {
