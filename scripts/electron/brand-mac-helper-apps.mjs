...
export function brandMacHelperApps({
  ...
  const helperResults = readdirSync(frameworksDir, { withFileTypes: true })
  ...
  return [mainAppBranding, ...helperResults].filter(Boolean);
}
...
function brandMainAppInfoPlist(
  ...
  const desiredIconPath = path.join(resourcesDir, MAIN_APP_ICON_FILE);
  const currentIconPath = path.join(resourcesDir, currentIconFile);
  if (currentIconFile !== MAIN_APP_ICON_FILE && existsSync(currentIconPath)) {
