...
function printHelp() {
  console.log(`
Ember Agent QC Release Summary

用法:
  npm run agent-qc:release-summary -- --evidence ./.ember/qc/agent-qc-evidence.json --require-scenario-manifest internal/test/agent-qc-scenarios.manifest.json --require-risk P0 --tag v1.2.3
...
function loadRequiredScenarioIds(manifestPath, risks) {
  ...
  const requiredRisks = new Set(
    (risks.length > 0 ? risks : ["P0"]).map((risk) => risk.toUpperCase()),
  );
  return Array.isArray(manifest?.scenarios)
    ? manifest.scenarios
        .filter((scenario) =>
          requiredRisks.has(String(scenario?.risk || "").toUpperCase()),
        )
