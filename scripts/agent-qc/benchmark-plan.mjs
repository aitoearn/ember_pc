...
const DEFAULT_CONFIG = "benchmarks/ember-agent-runtime/benchmark.json";
...
function printHelp() {
  console.log(`
Ember Harbor benchmark plan

用法:
  npm run agent-qc:benchmark:plan
  npm run agent-qc:benchmark:check
  node scripts/agent-qc/benchmark-plan.mjs --format json --output .ember/qc/benchmark-plan.json
...
function validateTask(rootDir, task) {
  ...
  if (!taskToml.includes("/logs/agent/trajectory.json")) {
    taskTomlIssues.push(
      "task.toml artifacts 未声明 /logs/agent/trajectory.json",
    );
  }
  ...
  const evidenceIssues =
    requiredEvidence.length === 0 ? ["requiredEvidence 不能为空"] : [];
...
function createPlan(rootDir, configPath) {
  ...
  if (benchmark.schemaVersion !== "ember-harbor-benchmark-v1") {
    issues.push("benchmark.json schemaVersion 必须是 ember-harbor-benchmark-v1");
  }
