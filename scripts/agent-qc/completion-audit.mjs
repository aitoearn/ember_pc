...
function loadFacts() {
  ...
  scenarioReport.p0ScenarioIds = Array.isArray(scenarioManifest?.scenarios)
    ? scenarioManifest.scenarios
        .filter(
          (scenario) => String(scenario?.risk || "").toUpperCase() === "P0",
        )
        .map((scenario) => String(scenario?.id || "").trim())
        .filter(Boolean)
    : [];
  const guiFlowReport = createAgentQcGuiFlowReport({
    flowManifest: guiFlowManifest,
    scenarioManifest,
  });
  const qcloopPayload = buildQCLoopJobPayload(scenarioManifest, {
    risks: ["P0"],
  ...
  const evidenceCore = exists("scripts/lib/agent-qc-evidence-core.mjs")
    ...
    : "";
  const releaseSummaryCore = exists(
  ...
  const nightly = exists(".github/workflows/harness-nightly.yml")
    ...
    : "";
  const release = exists(".github/workflows/release.yml")
  ...
  return {
    ...
    qcloopPayload: {
      ...
      verifierHasAttemptStatus:
        qcloopPayload.verifier_prompt_template.includes("{{attempt_status}}"),
      verifierHasExitCode:
        qcloopPayload.verifier_prompt_template.includes("{{exit_code}}"),
      ...
      workerPromptHasStructuredEvidence:
        qcloopPayload.prompt_template.includes(
      ...
      verifierRequiresStructuredEvidence:
        qcloopPayload.verifier_prompt_template.includes(
    ...
    staleOwnerIntervention: {
      ...
      guiOwnerReportHasWatchHistory:
        ...
        readText("scripts/agent-qc/gui-owner-check.mjs").includes(
          "--watch-history-output",
        ),
      docMentionsDecisionPacket:
        staleOwnerInterventionDoc.includes("ownerIntervention") &&
        staleOwnerInterventionDoc.includes(
          "stale-owner-intervention-request.json",
        ),
    ...
    githubActions: {
      ...
      contractsDetached: !containsAgentQcAction(
