...
const DEFAULTS = {
  ...
  evidenceDir: path.join(
    ...
    "qc",
    "gui-evidence",
    "plugin-ui-runtime-stdio-fixture",
  ),
  prefix: "plugin-ui-runtime-stdio-fixture",
  timeoutMs: 120_000,
  keepTemp: false,
};
...
function printHelp() {
  console.log(`
Plugin UI Runtime Stdio Fixture Smoke

用途:
  用临时 app data 种子 fixture Plugin installed state，
  通过 App Server stdio JSON-RPC current method 验证真实 UI runtime 子进程生命周期：
  pluginUiRuntime/start -> /api/bootstrap ready -> entryUrl -> status running -> stop。

说明:
  本脚本不调用 legacy plugin_* 命令，不写真实用户 app data，不消耗 live provider。
  installed state 仅作为测试夹具写入临时 HOME / XDG_DATA_HOME / APPDATA。

用法:
  node scripts/plugin/ui-runtime-stdio-fixture-smoke.mjs

选项:
  --app-server-bin <path>  app-server 二进制，默认 ember-rs/target/debug/app-server 或 APP_SERVER_BIN
  --fixture-dir <path>     fixture Plugin 目录，默认 .lime/qc/plugins-runtime-fixtures/content-factory-app
  --evidence-dir <path>    证据目录，默认 .lime/qc/gui-evidence/plugin-ui-runtime-stdio-fixture
  --prefix <name>          证据文件前缀
...
function parseArgs(argv) {
  ...
  if (!options.appServerBin || !fs.existsSync(options.appServerBin)) {
    throw new Error(
      `app-server binary 不存在: ${options.appServerBin}。请先运行 cargo build --manifest-path "ember-rs/Cargo.toml" -p app-server --bin app-server`,
    );
  }
...
function createTempRuntimeEnv() {
  const tempRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "plugin-ui-runtime-stdio-"),
  );
...
function buildInstalledState(fixtureDir) {
  ...
  const entry = {
    ...
    provenance: {
      ...
      appId: APP_ID,
      appVersion: "0.3.0",
      packageHash: "sha256:fixture-plugin-ui-runtime",
      manifestHash: "sha256:fixture-plugin-ui-runtime-manifest",
      entryKey: ENTRY_KEY,
    },
  };
  return {
    ...
    projection: {
      app: {
        ...
        status: "ready",
        appType: "domain-app",
        description: "Plugin UI runtime stdio fixture",
      },
      package: {
        appId: APP_ID,
        appVersion: "0.3.0",
...
function logStage(stage) {
  console.log(`[smoke:plugin-ui-runtime-stdio-fixture] stage=${stage}`);
}
...
async function main() {
  ...
  try {
    ...
    const initialize = await rpc.request(
      ...
      {
        clientInfo: {
          name: "plugin-ui-runtime-stdio-fixture-smoke",
          version: "1.59.0",
        },
        capabilities: {
          experimental: true,
    ...
    console.log(
      `[smoke:plugin-ui-runtime-stdio-fixture] summary=${summaryPath}`,
    );
    console.log(
      `[smoke:plugin-ui-runtime-stdio-fixture] entryUrl=${startResult.entryUrl}`,
    );
  } catch (error) {
    ...
    console.error(
      `[smoke:plugin-ui-runtime-stdio-fixture] failureSummary=${failurePath}`,
    );
    throw error;
  } finally {
...
main().catch((error) => {
  console.error(
    `[smoke:plugin-ui-runtime-stdio-fixture] failed: ${
