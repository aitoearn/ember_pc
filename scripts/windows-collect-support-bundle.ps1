...
function Write-Step {
  ...
  Write-Host "[Ember Support] $Message"
}
...
$bundleName = "Ember-Support-$timestamp"
$bundleDir = Join-Path $OutputRoot $bundleName
$zipPath = "$bundleDir.zip"

$appDataDir = Join-Path $env:APPDATA "ember"
$legacyDir = Join-Path $env:USERPROFILE ".ember"
$configPath = Join-Path $appDataDir "config.yaml"
$dbPath = Join-Path $legacyDir "ember.db"
