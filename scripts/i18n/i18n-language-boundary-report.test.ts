...
describe("i18n language boundary report", () => {
  it("应把 language-like marker 分到不同产品语义边界", () => {
    ...
    writeFile(
      root,
      "ember-rs/src/services/browser_environment_service.rs",
    ...
    writeFile(
      root,
      "ember-rs/src/commands/voice_model_cmd.rs",
    ...
    expect(report.summary.fileSummaries).toEqual(
      expect.arrayContaining([
        {
          count: 1,
          file: "ember-rs/src/commands/media_task_cmd.rs",
        },
      ]),
    );
    expect(report.summary.markerSummaries).toEqual(
      expect.arrayContaining([{ marker: "accept_language", count: 1 }]),
    );
  ...
  it("应输出 JSON、text，并支持 CLI 写入文件", () => {
    ...
    const json = JSON.parse(
      formatI18nLanguageBoundaryReport(report, "json"),
    ) as {
  ...
  it("应支持按 language boundary category 输出聚焦 evidence", () => {
    ...
    expect(
      report.entries.every(
        (entry) => entry.category === "contentTargetLanguage",
      ),
    ).toBe(true);
    expect(report.summary.fileSummaries).toEqual([
