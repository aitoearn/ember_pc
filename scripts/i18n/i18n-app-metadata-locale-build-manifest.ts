...
function readScopeItems(
  ...
  return rawItems.filter(isRecord).map((item) => ({
    ...
    localizedValues: isRecord(item.localizedValues) ? item.localizedValues : {},
...
export function buildAppMetadataLocaleBuildManifest(
  ...
  const missingFieldCount = entries.filter(
    (entry) => !entry.fieldExists,
  ).length;
  ...
  const blockedEntryCount = missingFieldCount + requiredLocalizedMissingCount;
...
function printHelp(): void {
  console.log(`Usage: tsx scripts/i18n/i18n-app-metadata-locale-build-manifest.ts [options]
...
export function runCli(argv: string[] = process.argv.slice(2)): number {
  ...
  const output = formatAppMetadataLocaleBuildManifest(manifest, options.format);
