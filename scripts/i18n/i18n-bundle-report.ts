...
export function analyzeI18nBundleReport(
  ...
  const bundleGroups = [
    ...collectBundleGroupMaps(resourcesDir, sourceLocale).entries(),
  ]
  ...
  return {
    ...
    summary: {
      ...
      inlineGroupCount: bundleGroups.filter((group) => group.role === "inline")
        .length,
      lazyGroupCount: bundleGroups.filter((group) => group.role === "lazy")
        .length,
      ...
      totalRawBytes: bundleGroups.reduce(
        (total, group) => total + group.totalRawBytes,
        0,
      ),
    },
  };
