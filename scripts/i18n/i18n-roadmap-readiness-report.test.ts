...
function writeReadyRepo(root: string, sourceKeyCount = 120): void {
  writeJson(root, "package.json", {
    scripts: {
      "detect-translations": "tsx scripts/i18n/detect-missing-translations.ts",
      "i18n:bundle-report:json":
        "tsx scripts/i18n/i18n-bundle-report.ts --format json",
      "i18n:check": "npm run detect-translations",
      "i18n:source-export:json":
        "tsx scripts/i18n/i18n-source-locale-export.ts --format json",
      "i18n:translation-pr-pack:json":
        "tsx scripts/i18n/i18n-translation-pr-pack.ts --format json",
      "i18n:unused:json":
        "tsx scripts/i18n/i18n-unused-key-check.ts --format json",
    },
  });
  writeText(
    root,
...
`,
  ...
  writeText(
    root,
    "src/i18n/legacy-patch/I18nPatchProvider.tsx",
    "export {};\n",
  );
  writeText(
    root,
    "src/lib/api/appConfigTypes.ts",
    "export interface Config { language: string; }\n",
  );
  writeText(
    root,
  ...
  writeJson(root, "docs/roadmap/i18n/evidence/source-locale-export.json", {
    namespaces: [
      ...
      {
        namespace: "agentInputbar",
        values: { "agentChat.inputbar.send": "发送" },
      },
      {
        namespace: "agentRuntime",
        values: { "agentChat.runtime.title": "运行" },
      },
      {
...
describe("i18n roadmap readiness report", () => {
  ...
  it("应在 P3 evidence source key 计数不一致时标记整体未完成", () => {
    ...
    expect(report.phases.find((phase) => phase.id === "P3")?.status).toBe(
      "incomplete",
    );
    expect(
      report.phases
