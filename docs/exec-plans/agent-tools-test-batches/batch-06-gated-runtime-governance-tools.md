...
- 只参考架构和行为，禁止把具体 session、path、provider、model、tool name 特例硬编码进 Ember。
...
- `ember-rs/crates/aster-rust/crates/aster/src/tools/mod.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/tools/registry.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/tools/config_tool.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/tools/cron_tools.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/tools/remote_trigger_tool.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/tools/worktree_tools.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/tools/lsp.rs`
...
```bash
cargo test --manifest-path "ember-rs/Cargo.toml" tools:: -- --nocapture
cargo test --manifest-path "ember-rs/Cargo.toml" current_surface_tool_gates -- --nocapture
cargo test --manifest-path "ember-rs/Cargo.toml" tools::cron_tools -- --nocapture
cargo test --manifest-path "ember-rs/Cargo.toml" tools::remote_trigger_tool -- --nocapture
cargo test --manifest-path "ember-rs/Cargo.toml" tools::worktree_tools -- --nocapture
cargo test --manifest-path "ember-rs/Cargo.toml" tools::lsp -- --nocapture
```
...
- Rust 验证状态：
  - `cargo test --manifest-path "ember-rs/Cargo.toml" current_surface_tool_gates -- --nocapture`：退出码 0，但输出为 `running 0 tests`，筛选未命中有效测试，不能记为 Rust 通过证据。
  - `cargo test --manifest-path "ember-rs/Cargo.toml" register_all_tools_with_cron_gate_without_scheduler -- --nocapture`：退出码 0，但输出为 `running 0 tests`，筛选未命中有效测试，不能记为 Rust 通过证据。
