...
本批次覆盖已经由 Ember 前端兼容展示、但容易被遗漏的动态工具别名：`MCPTool`、`McpAuthTool`、`REPLTool`、`ListSkills`、`LoadSkill`、`WaitAgent`、`ResumeAgent`、`CloseAgent`。这些工具不是新的 runtime 能力，而是旧命名、provider alias 或动态调度层会继续产出的展示面。
...
- `docs/exec-plans/agent-tools-test-batches/batch-14-compat-dynamic-aliases.md`
