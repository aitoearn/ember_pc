...
```bash
cargo test --manifest-path "ember-rs/Cargo.toml" tools::web -- --nocapture
```
...
- 最佳实践查证：
  ...
  - 对 Ember 的结论：搜索来源渲染应消费稳定语义投影与结构化来源解析；工具族判断必须收敛到共享 helper，不能在 `ToolCallDisplay` / batch grouping 中散落 `includes("websearch")` 之类局部规则。
- 本轮修复：
...
- 仍需 GUI 证据：
  - 进入 Ember 页面验证历史“国际新闻简报”或新建联网任务，记录 DOM 顺序：前置正文 -> `已搜索网页` -> 最终正文。
