...
> 状态：待执行  
> 创建时间：2026-06-03  
> 适用范围：Ember 对齐 Codex 与 Claude Code 的 current 主线、GUI 工具渲染、runtime / host surface / extension 系统。  
> 参考优先级：不确定时优先参考 `/Users/coso/Documents/dev/rust/codex`，其次参考 `/Users/coso/Documents/dev/js/claudecode`。若两者冲突，以 Codex current 行为为准；若与 Ember GUI current 架构冲突，必须写明取舍。

## 使用方式

本文不是公众号文章素材，而是后续修复认领清单。其他进程可以复制本文件 path 后按单项认领：

`internal/exec-plans/devember-codex-claude-alignment-gap-list.md`
...
- `current gap`：Ember 已决定要进入 current 主链，但能力还缺或不稳定。
- `product choice`：上游有，但 Ember 是否需要还没定，不能默认当缺陷。
...
- 参考：
  - Ember：`ember-rs/crates/aster-rust/crates/aster/src/tools/mod.rs`
  - Ember：`ember-rs/crates/aster-rust/crates/aster/src/tools/registry.rs`
  - Ember：`src/components/agent/chat/hooks/agentStreamCompletionController.ts`
...
- 参考：
  ...
  - Ember：`internal/roadmap/markdown/` 既有方案
- 期望：
...
- 分类：`current gap` / `test gap`
- 现象：用户指出 `runtime_policy_test_request` 不能 hard code OpenAI，因为 Ember 是多模型。
- 参考：
  - provider routing / runtime policy 相关测试
  - `ember-rs/crates/aster-rust/crates/aster/src/providers/*`
  - `ember-rs/crates/aster-rust/crates/aster/src/agents/*`
- 期望：
...
- 参考：
  ...
  - Ember：`ember-rs/crates/aster-rust/crates/aster/src/skills/*`
  - Ember：`ember-rs/crates/aster-rust/crates/aster/src/hooks/*`
- 期望：
...
- 参考：
  ...
  - `ember-rs/crates/aster-rust/crates/aster/src/tools/config_tool.rs`
  - `ember-rs/src/commands/gateway_channel_cmd.rs`
- 期望：
  - 先做产品决策：Ember 是否要实现 remote control / mobile push。
...
- 参考：
  ...
  - Ember：`ember-rs/crates/aster-rust/crates/aster/src/tools/agent_control.rs`
  - Ember：`ember-rs/crates/aster-rust/crates/aster/src/tools/team_tools.rs`
- 期望：
...
- 分类：`current gap` / `product choice`
- 现状：
  - Ember 已有 Memory / Compaction 主链。
  - Claude Code 的长期 assistant、daily log、dream/consolidation、team memory 方向仍未完全产品化到 Ember。
- 参考：
  - Claude Code：`src/memdir/*`
  - Claude Code：assistant / KAIROS / TEAMMEM 相关代码
  - Ember：`internal/aiprompts/memory-compaction.md`
- 期望：
  - 决定 Ember 是否需要长期 assistant 模式。
...
- 分类：`current gap`
- 现状：
  - Ember 已有 Agent / Team / Task 相关工具和 taxonomy。
  - 但多 Agent 的拆解、并行、合并、冲突处理、证据归并还需要继续验证。
- 参考：
  - Claude Code：`COORDINATOR_MODE`
  - Ember：`internal/aiprompts/task-agent-taxonomy.md`
  - Ember：`internal/exec-plans/claude-code-agent-task-runtime-alignment-plan.md`
- 期望：
...
- 分类：`current gap` / `test gap`
- 现状：
  - Ember 有 contracts、governance、GUI smoke。
...
- 参考：
  ...
  - Ember：`npm run test:contracts`
  - Ember：`npm run governance:legacy-report`
- 期望：
