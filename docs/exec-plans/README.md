...
- 生产命令 current 迁移计划：`docs/exec-plans/production-command-current-migration-plan.md`
- Agent App uninstall current UI 进度：`docs/exec-plans/agent-app-uninstall-current-ui-progress.md`
- P16 Diagnostics current fail-closed 进度：`docs/exec-plans/p16-diagnostics-current-fail-closed-progress.md`
- 旧 Tauri wrapper 清理队列：`docs/exec-plans/tauri-wrapper-quick-cleanup-queue.md`
- 旧 Tauri wrapper 机械 inventory：`docs/exec-plans/tauri-wrapper-command-inventory.md`
- Provider 模型能力 taxonomy 进度日志：`docs/exec-plans/provider-model-taxonomy-progress.md`
- Ember 多模态运行合同实施计划：`docs/exec-plans/multimodal-runtime-contract-plan.md`
...
- Agent QC 运营级测试体系执行计划：`docs/exec-plans/agent-qc-ops-testing-plan.md`
- Agent App v2 独立安装与 Runtime 底座拆分执行计划：`docs/exec-plans/agentapp-v2-standalone-runtime.md`
...
- Skill Prompt 执行与历史保留计划：`docs/exec-plans/skill-prompt-execution-retention-plan.md`
- 端自动化统一（agent-device + scrcpy，去 AutoGLM sidecar）执行计划：`docs/exec-plans/device-automation-unify-agent-device-scrcpy-plan.md`
- 端自动化统一进度日志：`docs/exec-plans/device-automation-unify-agent-device-scrcpy-progress.md`
- 移动端性能监控 P1 执行计划：`docs/exec-plans/device-performance-monitor-plan.md`
- 确定性可复现测试流与自愈回放执行计划：`docs/exec-plans/deterministic-flow-self-healing-plan.md`
