...
主计划：`docs/exec-plans/production-command-current-migration-plan.md` 的 P12 Agent App install / package / shell。
...
- 本轮启动的 `npm run electron:dev`、Vite、Electron Ember 与 App Server sidecar 已在验证后停止，`127.0.0.1:1420` / `127.0.0.1:3030` 已释放。
...
- `npm run smoke:agent-apps -- --timeout-ms 120000` 通过；summary：`.ember/qc/gui-evidence/agent-apps/agent-apps-smoke-summary.json`。
