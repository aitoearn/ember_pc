...
>
> 本 inventory 跟踪的 `ember-rs/src/commands/**` 旧 Tauri wrapper、runner.rs 注册、DevBridge dispatcher、`*_cmd.rs` 文件全部随 `ember-rs/src/**` 整目录在 2026-06-10 物理删除。下面的命令对照表、low-reference 候选、devbridge-only 候选**全部 superseded**，不再作为可执行参考；仅保留作历史快照与 `current-rules-guard` 守卫所要求的字串证据（`ember-rs/src/commands/**` 旧 Tauri wrapper 清理区）。

生成时间：2026-06-08 CST  
状态：`snapshot_current_dirty_worktree`  
关联队列：`docs/exec-plans/tauri-wrapper-quick-cleanup-queue.md`
...
- `ember-rs/src/commands/**` 是旧 Tauri command wrapper 删除清理区，不再承接新的业务逻辑、API adapter、runtime 分支、领域服务实现、compat wrapper 或退场 stub；下表中的文件只代表待删除 surface，不是新增落点，也不是退场包装落点。
...
| Count | File                                                              | 推荐拆分                                     |
| ----: | ----------------------------------------------------------------- | -------------------------------------------- |
|    29 | `ember-rs/src/commands/webview_cmd.rs`                             | Desktop Host / Browser Runtime               |
|    28 | `ember-rs/src/commands/skill_cmd.rs`                               | Skills / marketplace / execution 分拆        |
|    21 | `ember-rs/src/commands/aster_agent_cmd/command_api/runtime_api.rs` | Agent Runtime 主链，最后处理                 |
|    19 | `ember-rs/src/commands/mcp_cmd.rs`                                 | 历史计数；旧 wrapper 已删除，MCP current real stdio fixture 已补齐 |
|    16 | `ember-rs/src/commands/config_cmd.rs`                              | Config / Desktop Host shell split            |
|    16 | `ember-rs/src/commands/machine_id_cmd.rs`                          | Machine identity / diagnostics split         |
|    15 | `ember-rs/src/commands/memory_management_cmd.rs`                   | Memory App Server current                    |
|    15 | `ember-rs/src/commands/workspace_cmd.rs`                           | Workspace App Server current                 |
|    14 | `ember-rs/src/commands/model_registry_cmd.rs`                      | Model Provider current                       |
|    14 | `ember-rs/src/commands/session_files_cmd.rs`                       | Session files App Server current             |
|    13 | `ember-rs/src/commands/memory_cmd.rs`                              | Memory App Server current                    |
|    12 | `ember-rs/src/commands/agent_app_cmd.rs`                           | 历史计数；旧 wrapper 已删除，禁止恢复                         |
|    12 | `ember-rs/src/commands/site_capability_cmd.rs`                     | Site capability current                      |
|    10 | `ember-rs/src/commands/browser_connector_cmd.rs`                   | Desktop Host / Browser Runtime               |
|    10 | `ember-rs/src/commands/telemetry_cmd.rs`                           | Telemetry / App Server split                 |
|     9 | `ember-rs/src/commands/gateway_tunnel_cmd.rs`                      | Gateway channel / tunnel split               |
|     9 | `ember-rs/src/commands/material_cmd.rs`                            | Materials / project resources                |
|     9 | `ember-rs/src/commands/models_cmd.rs`                              | Model Provider current                       |
|     9 | `ember-rs/src/commands/persona_cmd.rs`                             | Persona / project memory split               |
|     9 | `ember-rs/src/commands/prompt_cmd.rs`                              | Prompt service split                         |

## DevBridge 热点

| Count | File                                                     | 清理口径                         |
| ----: | -------------------------------------------------------- | -------------------------------- |
|    31 | `ember-rs/src/dev_bridge/dispatcher/agent_sessions.rs`    | Agent Runtime 高风险，最后处理   |
|    22 | `ember-rs/src/dev_bridge/dispatcher/skills.rs`            | Skills 高风险，单进程处理        |
|    22 | `ember-rs/src/dev_bridge/dispatcher/voice.rs`             | Voice split                      |
|    14 | `ember-rs/src/dev_bridge/dispatcher/browser/runtime.rs`   | Browser Runtime current gateway  |
|     8 | `ember-rs/src/dev_bridge/dispatcher/agent_apps.rs`        | Agent App current gateway        |
|     7 | `ember-rs/src/dev_bridge/dispatcher/browser/sessions.rs`  | Browser Runtime current gateway  |
|     7 | `ember-rs/src/dev_bridge/dispatcher/capability_drafts.rs` | Capability Draft current gateway |
|     7 | `ember-rs/src/dev_bridge/dispatcher/files.rs`             | File / layered design gateway    |
|     7 | `ember-rs/src/dev_bridge/dispatcher/knowledge.rs`         | Knowledge App Server current     |
|     4 | `ember-rs/src/dev_bridge/dispatcher/browser/bridge.rs`    | Browser Runtime current gateway  |
|     4 | `ember-rs/src/dev_bridge/dispatcher/providers.rs`         | Provider current gateway         |
|     3 | `ember-rs/src/dev_bridge/dispatcher/app_runtime.rs`       | Config / diagnostics split       |
|     3 | `ember-rs/src/dev_bridge/dispatcher/browser/cdp.rs`       | Browser Runtime current gateway  |
|     3 | `ember-rs/src/dev_bridge/dispatcher/models.rs`            | 已在 Q3A 处理中                  |
|     3 | `ember-rs/src/dev_bridge/dispatcher/project_resources.rs` | 已在 Q3B blocked                 |
|     1 | `ember-rs/src/dev_bridge/dispatcher/memory.rs`            | Memory current gateway           |
|     1 | `ember-rs/src/dev_bridge/dispatcher/runtime_queries.rs`   | Runtime query current gateway    |

## 低引用候选

当前复核口径：这些命令最初被归为低引用候选；截至 2026-06-08，除 `get_material` 外，当前磁盘事实显示它们已不再出现在 `runner.rs` 或 `ember-rs/src/commands/**` 生产代码中，只应剩 `scripts/check-command-contracts.mjs` retired guard。后续并行 Agent 不应再认领这些已退役项。
...
| `get_material`                   | `ember-rs/src/app/runner.rs`<br>`ember-rs/src/commands/material_cmd.rs` | 仍有真实 legacy wrapper；确认前端入口后拆独立删除任务          |
