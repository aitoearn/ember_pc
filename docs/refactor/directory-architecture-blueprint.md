...
Ember 的终态是一个**协议驱动的单后端、多客户端壳**产品：
...
│   │   └── generated 协议类型来自 @embercloud/app-server-client
...
```
pages → domains/*/ui → domains/*/viewmodel → platform/api → @embercloud/app-server-client
  │          │                  │                │
  └──────────┴──────────────────┴────────────────┴──→ shared/*（任何层可用）
```
...
| 新共享类型 | 优先 `app-server-protocol`（若属协议）；**不默认进 ember-core** |
...
- **Rust**：`services/` 按四类落物理分组；`ember-core` 拆"纯类型 crate"（参照 codex-protocol 模式），core 收缩为最小共享层；协议宏（`client_request_definitions!` 等价物）把 protocol crate 4 处注册收敛为 1 处。
- **packages**：完成 § 3 收缩。
- **docs/**：`test`/`testing`/`tests` 三目录归并、`bussniss` typo 修正、16 个一级目录收到 ~10 个。
...
- **插件生态**：开放第三方插件时新增 `packages/plugin-sdk/`（runtime-api / ui-components / types）；届时 Rust crate 统一 `ember-` 前缀并评估发布。
...
| 指标 | 当前 | T1 | T2 |
|---|---|---|---|
| 新增 1 个 JSON-RPC 方法的手写触点 | ~10 文件 | ≤5 | ≤4（协议宏后） |
| TS 协议手写行数 | ~3600 | ≈0 | ≈0 |
| lib→components/features 依赖违例 | 6+（无守卫） | 0 新增（baseline 冻结） | 0（含存量） |
| 业务直连 dev-bridge | 多处 | 0 | 0 |
| runtime.rs + processor.rs 合计 | ~13000 行 | 单调下降 | ≤3000 行（纯接线） |
| `src/` 一级目录 | 11 个 | 不变 | 4 个（domains/platform/shared/pages） |
| 超 1000 行文件 | 233 | 只减不增 | <60（与 codex 同量级） |
