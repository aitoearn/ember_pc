...
```json
{
  ...
  "policy": "Prevents code bloat. Existing oversized files may only shrink (±5% tolerance). New files must not exceed 800 lines. See AGENTS.md § 基础约束 3 and docs/refactor/README.md.",
  "frontend": {
  ...
  "rust": {
    "scanPaths": ["ember-rs/crates/**/*.rs"],
    "excludePatterns": ["**/*_test.rs", "**/tests/**"],
    "oversizedFrozen": [
      {
        "path": "ember-rs/crates/aster-rust/crates/aster/src/agents/agent.rs",
        "lines": 8206,
        "comment": "待拆分（R-05）"
      },
      {
        "path": "ember-rs/crates/app-server/src/runtime.rs",
        "lines": 8010,
        "comment": "待拆分（R-04）"
      }
...
Prevents code bloat. Existing oversized files may only shrink (±5% tolerance). New files must not exceed 800 lines. See AGENTS.md § 基础约束 3 and docs/refactor/README.md.
...
- [ ] 生成 `governance/file-size-baseline.json`：
  ...
  - [ ] 扫描 `ember-rs/crates/**/*.rs`，排除 `*_test.rs` 和 `tests/`。
