# Ember 结构重构方案中心（架构驱动版）

> 状态：proposed（v2，2026-06-11 重写）
> 适用范围：开发 Ember 源码仓库本身
...
| **E core/services 垃圾抽屉化** | `ember-core` 被 11+ crate 依赖且装着 config/models/database/plugin；`services` 32 个 service 平铺；模型注册类型在 core 与 services 重复定义 | 与 Codex 仓库"resist adding to core"问题同构 |
...
6. 长期残留回挂 `docs/exec-plans/tech-debt-tracker.md`（dev-bridge 相关统一挂 CCD-012）。
