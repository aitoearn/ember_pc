...
**执行清单**：
1. ✅ `AGENTS.md` 基础约束 21 已补："新增 Rust 逻辑禁止默认落 `ember-core` / `services` 平铺层"。
2. ✅ 模型注册类型调查：`ember_core::models::model_registry` 定义类型（`EnhancedModelMetadata` 等），`services/model_registry_service.rs` 消费这些类型。不是"双写"，是"类型定义 + 服务实现"的正常分层。无需归并。
...
1. **认领**：开工前改条目状态为 `in_progress` + `执行者`；多 Agent 并行时按 `docs/aiprompts/parallel-agent-collaboration.md` 声明写集。
