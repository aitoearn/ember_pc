...
> 阅读方式：按 Ember 的轴 A-F 逐轴对照，每轴给出 codex 的做法、证据路径、Ember 的借鉴决策
...
**codex 新增一个 JSON-RPC 方法的写集**：v2 类型文件 1 处 + 宏条目 1 行 + domain processor 实现 + 中心 match 1 个 arm + 跑生成命令。对比 Ember 当前 ~10 个手写触点（含 TS 手抄 2 处），这就是轴 A 的目标态。

### Ember 借鉴决策（R-10 的实现路线修正）

1. **优先评估 ts-rs 路线**：Ember 的 `app-server-protocol` 已有 schemars；ts-rs 与 schemars 可共存于同一结构体。比起"从 JSON Schema bundle 再生成 TS"的两跳方案，ts-rs 直接 derive 一跳到位、类型保真度更高（codex 已验证 v11 可用）。R-10 执行时先做 spike 对比两条路线。
2. **生成物进 repo + 命令固化**：照抄 codex 的 `schema/typescript/` 模式，Ember 落点 `packages/app-server-client/src/generated/`；生成命令进 npm scripts（Ember 无 just）。
3. **codex 的缺口 Ember 要补上**：codex **没有 CI 防漂移检查**（只靠 AGENTS.md 文字提醒"改了就跑生成"）。Ember 的 R-10 必须加 `生成后 git diff --exit-code` 进 CI——这是我们比 codex 多走的半步。
...
**结论性证据**：codex 用"中心 match 1 行接线 + domain processor 承载实现"把路由文件压在 1.4K 行——证明 Ember 的 R-20（按 domain 拆 `runtime/<domain>.rs` + 中心文件收缩为接线）是已被验证的形态，且**不需要发明 trait registry**，显式薄 match 就够。

### Ember 借鉴决策

1. R-20 的目标形态直接对标：`processor.rs` 收缩到 ~1500 行以内的纯 match 接线；实现全部下放 domain 模块。
2. codex 的教训也要吸收：`thread_processor.rs` 自己长到 4268 行——domain processor 本身也要受 Ember 棘轮（R-60）约束，domain 内继续按子职责拆。
3. codex 用 `inventory` crate 做实验性 API 的编译期自注册（`experimental_api.rs`）——Ember 暂不需要，登记为 R-20 后期可选项（若 domain 数量失控再启用）。

---

## 轴 C/F · 大文件与分层治理（codex 只有文档约束，Ember 要做得更硬）
...
**结论**：codex 证明了"只写规则不上守卫，热点文件照样失控"。这正是 Ember R-60 棘轮（baseline + CI 红线）的价值依据——Ember 233 个超线文件比 codex 的 40 个严重 6 倍，更不能只靠文档。
...
1. ✅ R-50 的 AGENTS.md 抗膨胀条款照抄精神："新增 Rust 逻辑禁止默认落 `ember-core` / `services` 平铺层"。
2. ✅ "契约小 crate"模式 Ember 已有雏形（`app-server-protocol`），方向正确；`ember-core` 里的 models/config 类型未来可参照 `codex-protocol` 拆纯类型 crate（进中长期 T2，见蓝图）。
3. ⏸️ crate 前缀：Ember 不发布 crates.io，暂不强制；若未来开放插件 SDK 再统一（蓝图 T3）。
4. ⚠️ 度量参照：codex 125 crate / 97 万行 ≈ 平均每 crate 8K 行；Ember 30 crate 承载约 40 万行，单 crate 明显过重——T2 拆 crate 的方向有据可依，但不急于近期。

---

## 顶层拓扑 · 一个后端多客户端壳（Ember 未来结构的直接参照）
...
所有客户端（CLI / TUI / IDE 扩展 / SDK）面对**同一个 app-server JSON-RPC 协议**，协议类型从 Rust 单向生成。这与 Ember 的 current 主线（Electron 只是 Desktop Host bridge，能力收敛 App Server）完全同构——Ember 的 `electron/` 对应 codex 的一个"客户端壳"，`src/`（React）是壳里的 renderer。未来若加第二个壳（Web/CLI），协议层不需要任何返工。这是蓝图 § 北极星的依据。
...
**codex 没解决、Ember 要补的两件事**：
1. 协议生成无 CI 防漂移 → Ember R-10 加 `git diff --exit-code`。
2. 文件体量规则无机械守卫（chat_composer 11K 行）→ Ember R-60 棘轮。
