...
当前唯一可安全推进的是继续维护标准、sidecar 和 runbook。必须等待 PID `69738` 自然释放，或由 owner 明确确认处置后，才能按 `docs/tests/ember-agent-qc-stale-owner-intervention.md` 执行 post-stale runbook。确认前不得直接处理进程或修改 qcloop DB。
...
3. `npm run agent-qc:release-summary -- --evidence ./.ember/qc/agent-qc-evidence.json --require-scenario-manifest docs/test/agent-qc-scenarios.manifest.json --require-risk P0 --check` 通过。
...
npm run agent-qc:release-summary -- --evidence ./.ember/qc/agent-qc-evidence.isolated-p0-full-v1-after-intervention.json --require-scenario-manifest docs/test/agent-qc-scenarios.manifest.json --require-risk P0 --tag sidecar-isolated-p0-full-v1-after-intervention --check
```
...
`docs/tests/ember-agent-autonomous-test-execution-matrix.md` 的执行前 Owner Gate 已补充 `npm run agent-qc:payload-coverage`。后续 Agent 在启动 full P0 前，不仅要确认 GUI / raw process / DB lease 清空，还必须确认 ready payload 与 P0 manifest 一致。
...
`docs/tests/ember-agent-qc-stale-owner-intervention.md` 已新增 8.6，明确确认前可以做只读进程树取证，但不能发送 signal。该节把以下 sidecar 纳入标准处置链：
