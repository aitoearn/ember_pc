...
> 本文件是 `docs/tests/ember-agent-autonomous-testing-plan.md` 的执行层补充：把“应该如何测试 Ember”落成 Agent 可以按步骤执行的矩阵。它不替代通用标准，也不替代 qcloop Evidence Pack；它只定义 Ember 作为样本产品时，测试 Agent 如何选择场景、采集证据、阻断发布和回写回归。
...
- Ember 分阶段计划：`docs/tests/ember-agent-autonomous-testing-plan.md`
- 当前阻断：`docs/tests/ember-agent-qc-current-blockers.md`
- qcloop 运维：`docs/tests/ember-agent-qc-qcloop-operations.md`
- Evidence 契约：`docs/tests/ember-agent-qc-evidence-contract.md`
