...
本文件定义 Ember 当前 `Remote / SDK / Server Mode` 的唯一 remote runtime 事实源，主要回答：
...
后续 Ember 的 remote runtime 只允许向下面这条主链收敛：
...
补充迁移边界：本页保留的 `ember-rs/src/commands/*_cmd.rs` 只用于定位旧 Tauri wrapper 或迁移期命令面，不表示 `ember-rs/src/commands/**` 仍是 remote runtime 的实现目录。新增 remote control plane、agent/browser runtime 或跨 App remote 能力应进入 App Server / RuntimeCore / services；桌面壳、浏览器窗口、CDP 和系统打开能力进入 Electron Desktop Host。旧 wrapper 只允许迁出、撤注册、删除，删不动时登记 blocker。
...
- `ember_gateway::{telegram, feishu, discord, wechat}`
...
- `ember-rs/src/services/browser_connector_service.rs`
...
- 不再恢复 `src/components/openclaw/*`、`src/lib/api/openclaw.ts`、`ember-rs/src/commands/openclaw_cmd.rs` 或 `ember-rs/src/services/openclaw_service/*`
...
1. Rust `ember-rs/src/dev_bridge.rs` 仅在 `debug_assertions` 下提供旧 `3030` HTTP 桥
2. Rust `ember-rs/src/dev_bridge/dispatcher/**` 只作为旧 Tauri command facade 的迁移来源和清理对象
...
- `ember-rs/src/commands/telegram_remote_cmd.rs`
...
- `ember-rs/src/commands/browser_connector_cmd.rs`
- `ember-rs/src/commands/webview_cmd.rs`
- `ember-rs/src/commands/browser_runtime_cmd.rs`
- `ember-rs/src/dev_bridge.rs`
- `ember-rs/src/dev_bridge/*`
...
- `ember-rs/src/commands/openclaw_cmd.rs`
- `ember-rs/src/services/openclaw_service/*`
- `ember-rs/src/dev_bridge/dispatcher/openclaw.rs`
- `ember-rs/crates/core/src/openclaw_install.rs`
