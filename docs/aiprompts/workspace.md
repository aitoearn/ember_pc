...
Workspace 是 Ember 应用层的概念，用于组织和管理 AI Agent 的工作上下文。它是对 Aster 框架 `Session.working_dir` 的命名和配置包装，不修改 Aster 框架本身。
...
对 Ember 的通用工作区，真实文件是第一事实源，`artifact` 只作为增强层存在。
...
- Ember 按 Workspace 分组显示 Session 列表
...
```rust
// ember/ember-rs/src/workspace/types.rs
...
```rust
// ember/ember-rs/src/workspace/manager.rs
...
~/Library/Application Support/ember/  # Ember 应用目录
├── ember.db                       # 应用数据库（包含 workspaces 表）
...
```typescript
// ember/src/components/workspace/WorkspaceSelector.tsx
...
| 决策点 | 选择 | 理由 |
|--------|------|------|
| Workspace 存储位置 | Ember 应用层 | 不修改 Aster 框架，保持框架通用性 |
