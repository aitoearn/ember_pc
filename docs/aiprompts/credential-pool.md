...
启动期迁移会清理 Ember 管理的旧凭证池数据：

- 清空 `provider_pool_credentials`
- 删除 Ember 应用数据目录下托管的 `credentials/` 副本

这只处理 Ember 自己复制和管理过的数据，不删除用户外部 CLI 原始目录，例如 `~/.codex`、`~/.gemini` 或其它第三方工具目录。
...
- 协议转换器，尤其是 `ember-rs/crates/providers/src/converter/openai_to_antigravity.rs`
- `ember_core::models::runtime_provider_model` 暴露的 `Runtime*` DTO；它只能承载 current API Key Provider 数据，`RuntimeProviderType` 也不得别名回含旧凭证池 Provider 的历史 `ProviderType`
