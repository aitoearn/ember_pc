...
**Current 主线（去 AutoGLM sidecar）：** 见 `docs/exec-plans/device-automation-unify-agent-device-scrcpy-plan.md`
...
3. **UI Genie 恢复** — 见 `docs/roadmap/device-ui-agent/README.md` 与 `docs/exec-plans/device-ui-agent-plan.md`（App Server `device_control` + 服务模型，参考 ClawMobile / lmuiagent；不恢复 AutoGLM sidecar）

## 相关文档

- `docs/roadmap/device-ui-agent/README.md` — Device UI Agent 方案（自然语言操控手机）
- `docs/exec-plans/device-ui-agent-plan.md` — UI Agent 分阶段实施计划
- `docs/exec-plans/device-automation-unify-agent-device-scrcpy-plan.md` — 统一 agent-device + scrcpy 执行计划
- `docs/exec-plans/device-automation-unify-agent-device-scrcpy-progress.md` — 进度日志
