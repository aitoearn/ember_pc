...
本文件约束多个 Agent / 终端进程同时开发 Ember 仓库时的协作方式。目标是减少猜测、避免夹写，并让每个进程都能明确知道自己能做什么。

## 何时启用

**默认不启用**。Codex / Claude / 其它 Agent 接到任务时按单 Agent 模式直接动手，不需要先盘点写集、声明认领、做避让汇报。只有在以下条件之一成立时，才进入下面的启动协议：

- 用户明确说明同时有其他 Agent / 进程在跑同一仓库
- 工作树（`git status --short`）已经有大量与本轮无关的脏改动，且不确定它们是否在被其他进程修改
- 任务本身要求登记到一份多 Agent 共享认领队列（如执行计划里显式声明）

如果不符合以上任一条件，跳过本文件，按 AGENTS.md 主规则正常工作。
