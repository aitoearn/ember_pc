...
本文件只定义 Ember 当前真正入模的基础 Prompt 主链，回答四个问题：
...
路径边界：`runtime_turn.rs`、`prompt_context.rs` 目前仍位于 `ember-rs/src/commands/aster_agent_cmd/**`，只能作为现有行为锚点和迁移来源。新增 Prompt 能力、augmentation stage、diagnostics 或 provider prompt 组装逻辑应进入 App Server / RuntimeCore / prompt services / `ember-rs/crates/agent`，不能继续在 `ember-rs/src/commands/**` 新增业务逻辑、compat wrapper 或退场 stub。
...
aster Agent::prepare_tools_and_prompt(...)
  ...
  -> Session Context（Ember 组装后的 session prompt）
...
- `runtime_turn.rs` 里记录的 `base_system_prompt_len / final_system_prompt_len` 只覆盖 Ember 侧的 `session prompt` 片段，不等于 provider 侧最终收到的完整 system prompt 长度。
...
### 2. Ember 侧 augmentation 主链

- `ember-rs/src/commands/aster_agent_cmd/runtime_turn.rs`
- `ember-rs/src/commands/aster_agent_cmd/prompt_context.rs`
- `ember-rs/src/services/memory_profile_prompt_service.rs`
- `ember-rs/src/services/artifact_prompt_service.rs`
- `ember-rs/src/services/web_search_prompt_service.rs`
- `ember-rs/crates/agent/src/request_tool_policy.rs`
- `ember-rs/crates/agent/src/prompt/runtime_agents.rs`
...
- `ember-rs/crates/aster-rust/crates/aster/src/agents/reply_parts.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/agents/prompt_manager.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/prompt_template.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/prompts/*.md`

当前 provider prompt 的最终结构不是“Ember 直接整段覆盖”，而是：

1. `identity.md` 或 `AgentIdentity.custom_prompt`
2. `Session Context`
   Ember 在 `runtime_turn.rs` 组好的 session prompt 会放在这里
3. `capabilities.md`
...
`ember-rs/crates/aster-rust/crates/aster/src/prompts` 是 Aster 的嵌入式模板目录，但不是所有文件都在 current 主链里。
...
- `ember-rs/src/commands/aster_agent_cmd/runtime_turn.rs` 现有行为锚点；新逻辑迁向 RuntimeCore / prompt services
- `ember-rs/src/commands/aster_agent_cmd/prompt_context.rs` 现有行为锚点；新逻辑迁向 RuntimeCore / prompt services
- `ember-rs/src/services/memory_profile_prompt_service.rs`
- `ember-rs/src/services/artifact_prompt_service.rs`
- `ember-rs/src/services/web_search_prompt_service.rs`
- `ember-rs/crates/agent/src/request_tool_policy.rs`
- `ember-rs/crates/agent/src/prompt/runtime_agents.rs`
- `ember-rs/crates/agent/src/turn_input_envelope.rs`
- `ember-rs/crates/agent/src/aster_state_support.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/agents/prompt_manager.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/agents/reply_parts.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/prompt_template.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/prompts/identity.md`
- `ember-rs/crates/aster-rust/crates/aster/src/prompts/capabilities.md`
- `ember-rs/crates/aster-rust/crates/aster/src/prompts/subagent_system.md`
- `ember-rs/crates/aster-rust/crates/aster/src/prompts/plan.md`
- `ember-rs/crates/aster-rust/crates/aster/src/prompts/recipe.md`
- `ember-rs/crates/aster-rust/crates/aster/src/prompts/summarize_oneshot.md`
- `ember-rs/crates/aster-rust/crates/aster/src/prompts/permission_judge.md`

### Compat

以下路径仍保留，但不属于当前基础 Prompt 主链，后续不要继续把新能力长进去：

- `ember-rs/crates/agent/src/prompt/builder.rs`
- `ember-rs/crates/agent/src/prompt/templates.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/prompt/builder.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/prompt/templates.rs`
...
- `ember-rs/crates/aster-rust/crates/aster/src/prompts/system.md`
- `ember-rs/crates/aster-rust/crates/aster/src/prompts/system_gpt_4.1.md`
- `ember-rs/crates/aster-rust/crates/aster/src/prompts/desktop_prompt.md`
- `ember-rs/crates/aster-rust/crates/aster/src/prompts/desktop_recipe_instruction.md`

### 特殊说明

- `ember-rs/crates/aster-rust/crates/aster/src/prompts/mock.md` 目前只看到 `prompt_template.rs` 内部测试覆盖，不属于基础 Prompt current 主链。
...
- Ember 的基础 Prompt 主链不是某一份前端 `systemPrompt`、某一份 PRD，或某个样板包
- Ember 的基础 Prompt 主链也不是单独某个 builder 文件
