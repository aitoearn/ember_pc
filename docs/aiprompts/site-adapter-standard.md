...
本文件定义 Ember 仓库中站点适配器能力的唯一工程标准，主要回答：

- Ember 自己认可的站点适配器标准长什么样
- 外部来源为什么不能直接成为 Ember 的事实源
- 站点适配器应该如何接入、执行、校验与治理
- 如何避免因为接入外部适配器库而把 Ember 做成“万国牌”
...
- 可以有多个 adapter 来源
  ...
  - 服务端下发
  - 外部项目导入
- 但 Ember 内部只能存在一个继续演进的适配器标准

从现在开始，站点适配器能力的唯一事实源应收敛到：

> `Ember Site Adapter Spec`

外部项目只能提供“原料”，不能提供 Ember 的运行时标准、协议标准或状态标准。

换句话说：

- 可以接更多来源
- 不能接更多“标准”
- 不能让来源格式反过来定义 Ember 的产品边界

如果某个外部来源和 Ember 标准冲突，优先保留 Ember 标准，而不是为了兼容把 Ember 改成第二套产品。
...
- 是外部来源进入 Ember 的唯一入口
- 是站点适配器治理边界
- 负责拒绝不符合 Ember 标准的来源 adapter

### 3. 执行层

作用：

- 执行已经被编译为 Ember 标准的适配器

特点：

- 只能使用 Ember 当前浏览器执行主链
- 不允许外部来源自带执行内核绕过 Ember runtime

## Ember Site Adapter Spec v1

Ember 内部适配器标准至少包含以下字段语义：
...
- `ember-rs/src/services/site_adapter_registry.rs` 中的 `SiteAdapterSpec`

如果未来字段扩展，仍然必须收敛到 Ember 自己的标准模型，而不是向外部项目的原始结构靠拢。
...
1. Ember 产品边界
2. Ember Site Adapter Spec
3. Ember 当前浏览器运行时主链
4. 外部来源可提供的原始 adapter 定义

这意味着：

- 外部来源只能被编译、裁剪、白名单化后进入 Ember
- 不能为了保留来源格式的完整性，引入第二套 runtime、协议或错误语义
- 不能因为“某来源支持某能力”就直接判定 Ember 也应该支持
...
- 让来源项目自己的内部 ID 成为 Ember 对外主标识
- 同时维护多套命名规则

## 参数标准

参数定义必须先归一化，再进入 Ember 主链。
...
- 直接调用外部项目自带 daemon 作为 Ember 主执行链
- 直接让外部项目控制 Chrome / Chromium 生命周期
- 在 Ember 内部并行保留第二套 site adapter runtime

### 2. 当前执行模型以 script 为唯一主格式

对 Ember 来说，站点适配器当前主格式是：
...
都必须先编译为 Ember 现有 script 模型。

不要直接把外部 pipeline engine 搬进 Ember。
...
错误信息可以引用来源 adapter 的上下文，但错误分类、前端提示和结果结构必须以 Ember 为准。
...
如果某个外部来源天然依赖“后台常驻自动化”，它就不能原样进入 Ember 主链。
...
> Ember 可以借 YAML 来源 的 adapter，但不能把 YAML 来源 变成 Ember。
...
1. 这次改动是否仍然收敛到 `Ember Site Adapter Spec`
2. 是否新增了第二套执行主链
...
如果下一步要把外部 adapter 引入 Ember，推荐按以下顺序推进：

1. 先冻结 `Ember Site Adapter Spec v1`
2. 建立来源导入服务
3. 建立步骤白名单
...
> Ember 可以吸收外部 adapter 能力，但所有来源都必须先编译成 Ember 标准，再交给 Ember 自己的 runtime 执行。
