...
本文件定义 Ember 当前 `Memory / Compaction` 的唯一主链，主要回答：
...
后续 Ember 的 `Memory / Compaction` 只允许向下面这条主链收敛：
...
补充迁移边界：`memory_runtime_*`、`unified_memory_*`、`agent_runtime_compact_session` 是 current 命令语义，但 `ember-rs/src/commands/**` 不是新记忆实现目录。本文列出的 `memory_management_cmd.rs`、`unified_memory_cmd.rs`、`memory_search_cmd.rs` 和 `aster_agent_cmd/**` 是现有迁移锚点；新增记忆来源、durable recall、compaction、Team Memory 或 prompt prefetch 能力应进入 App Server / RuntimeCore / services / `ember-rs/crates/agent`，旧 wrapper 迁出后撤注册并删除。
...
- `ember-rs/src/commands/memory_management_cmd.rs`
- `ember-rs/src/services/memory_source_resolver_service.rs`
- `ember-rs/src/services/auto_memory_service.rs`
- `ember-rs/src/services/runtime_agents_template_service.rs`

当前这里负责：

1. 解析 managed / user / project / local / rules / durable / additional 记忆来源。
2. 初始化 `memdir` 套件，并读取与写入 `MEMORY.md`、四类 `README.md` 与 topic note。
3. 生成 `.ember/AGENTS.md` / `.ember/AGENTS.local.md` 模板与 `.gitignore` 守卫。
...
- `ember-rs/crates/aster-rust/crates/aster/src/agents/agent.rs`
...
- `ember-rs/src/commands/unified_memory_cmd.rs`
- `ember-rs/src/commands/memory_search_cmd.rs`
...
- `ember-rs/src/dev_bridge/dispatcher/memory_runtime.rs`
...
- `src/lib/api/memoryRuntime.ts`
- `ember-rs/src/commands/memory_management_cmd.rs` 现有迁移锚点；新来源链逻辑迁向 services / RuntimeCore
- `ember-rs/src/services/memory_source_resolver_service.rs`
- `ember-rs/src/services/auto_memory_service.rs`
- `ember-rs/src/services/runtime_agents_template_service.rs`
- `ember-rs/src/commands/aster_agent_cmd/command_api/runtime_api.rs` 的 `agent_runtime_compact_session`
- `ember-rs/src/commands/aster_agent_cmd/runtime_turn.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/agents/agent.rs` 的 `compact_session`
- `src/lib/api/unifiedMemory.ts`
- `ember-rs/src/commands/unified_memory_cmd.rs` 现有迁移锚点；新 durable memory 逻辑迁向 services / App Server
- `ember-rs/src/commands/memory_search_cmd.rs` 现有迁移锚点；新搜索逻辑迁向 services / App Server
- `src/components/memory/MemoryPage.tsx`
- `src/components/settings-v2/general/memory/index.tsx`
- `src/components/agent/chat/components/AgentThreadMemoryPrefetchPreview.tsx`
- `src/components/agent/chat/components/AgentThreadReliabilityPanel.tsx`
...
- `ember-rs/src/commands/memory_cmd.rs`
...
- `ember-rs/src/commands/memory_feedback_cmd.rs`
...
- `ember-rs/src/commands/memory_search_cmd.rs.bak`
