...
本文件定义 Ember 当前 `Task / Agent / Coordinator` 的唯一 taxonomy，主要回答：
...
补充迁移边界：`agent_runtime_*`、`automation_*`、`execution_run_*` 这类命令名可以作为迁移期 surface，但 `ember-rs/src/commands/**` 不是新的 taxonomy、coordinator 或 runtime 实现目录。本文列出的 `aster_agent_cmd/**`、`automation_cmd.rs`、`execution_run_cmd.rs` 只作为现有行为锚点和清理参考；新增长时执行、子代理、自动化或执行摘要能力应进入 RuntimeCore / services / App Server protocol，旧 wrapper 迁出后撤注册并删除。

补充边界：

[Codex `/goal`](../research/codex-goal/README.md) 这类 persistent objective / continuation loop 只能作为“目标推进控制层”理解，不能成为第四类执行实体。若 Ember 后续实现 `Managed Objective`：
...
- `ember-rs/src/commands/aster_agent_cmd/subagent_runtime.rs`
- `ember-rs/src/commands/aster_agent_cmd/command_api/subagent_api.rs`
- `ember-rs/src/commands/aster_agent_cmd/tool_runtime/subagent_tools.rs`
...
- `ember-rs/src/services/automation_service/mod.rs`
...
- `ember-rs/src/services/execution_tracker_service.rs`
- `ember-rs/crates/core/src/database/dao/agent_run.rs`
...
- `ember-rs/src/app/scheduler_service.rs`
- `ember-rs/crates/scheduler/*`
...
- `docs/aiprompts/query-loop.md`
- `ember-rs/src/commands/aster_agent_cmd/subagent_runtime.rs` 现有行为锚点；新协作 runtime 逻辑迁向 RuntimeCore / services
- `ember-rs/src/services/automation_service/*`
- `ember-rs/src/services/execution_tracker_service.rs`
- `agent_runs`
- `automation_jobs`
