...
本文件定义 Ember 当前与 Claude Code 持久化对齐相关的单一事实源，重点回答：
...
路径边界：`agent_runtime_*` 文件快照命令名可以作为迁移期读取 surface，但 `ember-rs/src/commands/**` 不是新增持久化实现目录。本文提到的 `aster_agent_cmd/dto.rs` 只作为现有读模型锚点；新增 file checkpoint、artifact sidecar、export / replay 持久化逻辑应进入 services、App Server / RuntimeCore 或 `ember-rs/crates/agent`，旧 command wrapper 迁出后撤注册并删除。
...
- `ember-rs/src/services/runtime_file_checkpoint_service.rs`
