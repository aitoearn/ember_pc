...
| 方案                               | 判断                             | 原因                                                                                |
| ---------------------------------- | -------------------------------- | ----------------------------------------------------------------------------------- |
| 直接 copy Ember Rust 代码到独立 App | `dead`                           | 无法治理版本、修复、安全和 runtime facts，会快速分叉。                              |
| Git submodule 引入 Ember 源码       | `deprecated for app consumption` | 适合 pin 外部源码，不适合高频演进 runtime SDK；App 会被迫理解 Ember 内部 workspace。 |
| Git subtree / vendor               | `dead`                           | 更像低频 vendoring，会制造双向同步成本。                                            |
| 独立 App 直接依赖 Ember Rust crate  | `not recommended`                | Electron App 不应链接内部 Rust workspace；跨平台构建和发布成本过高。                |
| 版本化 TS client + sidecar binary  | `current`                        | App 只依赖稳定协议和可校验二进制；RuntimeCore 仍由 Ember 维护。                      |

执行规则：

1. Ember 内部使用 Cargo workspace 管理 `app-server*` Rust crate，保持单一 `Cargo.lock` 和统一构建产物目录。
...
4. content-studio 本地开发可以通过 `APP_SERVER_BIN` 指向 Ember debug binary，但生产包必须 pin 到 manifest 版本和 sha256。
5. 独立 App 不应该把 Ember Rust workspace 拉进自己的构建图；否则依赖、锁文件、平台产物和安全修复都会分叉。
6. 独立 App 不得调用、复制或依赖 `ember-rs/src/commands/**`；该目录只作为 Ember 内部旧 Tauri wrapper 清理区。需要 Agent 后端能力时消费 App Server JSON-RPC / client；需要窗口、shell、picker、托盘等桌面壳能力时由各 App 的 Desktop Host bridge 承接。

## 3. 分发单元

Ember 应只向独立 App 暴露三个稳定产物。
...
12. content-studio 生产集成不得依赖 Ember Desktop in-process legacy desktop facade；真实 runtime 必须等待 host-agnostic backend sidecar mode。
...
1. Rust 源码。
2. Aster 私有类型。
3. Ember Desktop legacy desktop command facade。
4. App 业务 UI。
5. `ember-rs/src/commands/**` 中的旧 Tauri wrapper、compat stub 或退场 tombstone。
...
1. `package.json`：新增 `app-server-client` npm 依赖，不拉 Ember Rust workspace。
...
6. `src/shared/types.ts`：放 content-studio 自有薄 DTO，避免 renderer 依赖 Ember 内部类型。
...
2. `file:../ember/packages/app-server-client`
3. workspace overlay
...
```mermaid
flowchart LR
    Ember["Ember CI"] --> Build["Build app-server binaries"]
...
Ember 当前已提供本地 smoke 入口：

```bash
cargo build --manifest-path "ember-rs/Cargo.toml" -p app-server
npm --prefix "packages/app-server-client" test
npm run smoke:app-server-stdio
npm run app-server:manifest:test
npm run app-server:manifest -- --binary "ember-rs/target/debug/app-server" --url "https://example/app-server-darwin-arm64.tar.gz" --platform "darwin-arm64" --out "/tmp/app-server-manifest.json"
```
...
| 代码                            | 位置                    | 分发                     |
| ------------------------------- | ----------------------- | ------------------------ |
| RuntimeCore                     | Ember Rust workspace     | 不直接给 App import。    |
| ExecutionBackend / AsterBackend | Ember Rust workspace     | 不直接给 App import。    |
| app-server-protocol Rust DTO    | Ember Rust workspace     | 生成 schema / TS types。 |
| `app-server-client`             | Ember 发布的 npm package | 独立 App 依赖。          |
| `app-server` binary             | Ember release artifact   | 独立 App 打包。          |
| App UI projection               | 各 App repo             | 自己实现。               |

## 10. 何时考虑 monorepo

只有满足以下条件，才考虑把 Ember 和 content-studio 放进同一个更大的 monorepo：
...
1. Ember 维护 runtime 和 sidecar。
2. content-studio 维护产品 UI 和业务对象。
3. 两者通过 npm package + binary manifest 协作。

## 11. 参考依据

1. Cargo workspace 官方文档：适合在 Ember 内部统一管理 Rust packages。
...
content-studio 集成完成不是“能找到 Ember 源码”，而是：
...
4. renderer 只消费业务投影。
5. App 不 import Ember Rust crate。
