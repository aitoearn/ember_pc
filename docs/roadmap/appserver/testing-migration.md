...
4. 为了让旧测试通过，在 `ember-rs/src/commands/**` 新增 stub、compat wrapper、mock fallback 或业务逻辑。
...
| `current`                     | `scripts/electron/current-rules-guard.test.mjs`                 | 验证根规则、`docs/aiprompts` 与核心 skills 已锁定 Electron / App Server current、禁止新增品牌前缀命名，并要求新 Agent 逻辑走 App Server。 |
...
| `compat cleanup guard`        | `src/lib/governance/rustCommandsCurrentBoundary.test.ts`         | 锁定 `ember-rs/src/commands/**` 只能作为旧 wrapper 清理区，防止 in-process bridge / runtime 新实现回流到 Tauri command。                      |
...
`ember-rs/src/commands/**` 相关测试只允许作为 cleanup guard：证明旧 wrapper 已删除、已迁出、已撤注册或没有回流。新增 Agent / runtime / workspace / artifact / evidence / Knowledge / MCP 能力的正向测试必须覆盖 App Server protocol / RuntimeCore / client / Electron Host current 链路；新增桌面壳能力的正向测试必须覆盖 Electron main / preload / desktop-host API。不能把 legacy Rust command 注册测试当作 GUI current 可交付证据，也不能用 stub / compat wrapper 测试替代 current 交付证据。
...
4. `ember-rs/src/commands/**` 的剩余 wrapper 清理按命令族分批推进；等待 App Server / Electron Host current 覆盖后删除，不再为它们补新正向用例。
...
git diff --check -- "docs/testing/skills-e2e-testing.md" "docs/tests/ember-agent-qc-rollout-plan.md" "scripts/electron/current-docs-guard.test.mjs"
```
...
git diff --check -- "docs/testing/skills-e2e-testing.md" "docs/tests/ember-agent-qc-rollout-plan.md" "docs/tests/ember-agent-qc-qcloop-operations.md" "scripts/lib/agent-qc-report-core.test.ts"
```
...
rg -n "Tauri 的测试框架|npm run tauri(?::dev)?\\b|\\btauri dev\\b|headless Tauri|验证 Tauri 壳" "docs/testing/skills-e2e-testing.md" "docs/tests/ember-agent-qc-rollout-plan.md" "docs/tests/ember-agent-qc-qcloop-operations.md" "scripts/electron/current-docs-guard.test.mjs"
git diff --check -- "docs/testing/skills-e2e-testing.md" "scripts/electron/current-docs-guard.test.mjs" "scripts/lib/agent-qc-process-owner-current.test.mjs" "scripts/lib/gui-smoke-run-lock.test.mjs" "scripts/lib/agent-qc-report-core.test.ts"
```
...
10. `scripts/update-version.sh` 只更新 `package.json`、`packages/ember-cli-npm/package.json` 与 `ember-rs/Cargo.toml`，不再写 `tauri.conf`。
...
npm run test:contracts
git diff --check -- "src/lib/api/hintRoutes.ts" "src/lib/api/hintRoutes.test.ts" "src/lib/api/agentAppRuntime.ts" "src/lib/api/agentAppRuntime.test.ts" "src/lib/api/companion.ts" "src/lib/api/companion.test.ts" "src/lib/api/tray.ts" "src/lib/api/tray.test.ts" "docs/roadmap/appserver/testing-migration.md"
```
...
rg -n "tauri dev|tauri\.conf|src-tauri|headless Tauri|node_modules/.bin/tauri" "scripts/verify-gui-smoke.mjs" "scripts/electron/current-entrypoints.test.mjs" "docs/roadmap/appserver/testing-migration.md"
```
...
git diff --check -- "scripts/standalone-deprecated-artifact-adapter-guard.test.mjs" "scripts/agent-app/standalone-native-shell-config-writer.mjs" "scripts/agent-app/standalone-native-build-runner.mjs" "scripts/lib/agent-app-standalone-native-shell-config-writer-core.mjs" "scripts/lib/agent-app-standalone-native-build-runner-core.mjs" "docs/roadmap/appserver/testing-migration.md"
```
...
1. 根 `AGENTS.md`、`docs/aiprompts/README.md`、命令边界、治理和质量工作流文档已经把 Electron Desktop Host + App Server 作为 current 事实源。
2. 三个核心 skills：`ember-command-boundary`、`ember-governance`、`ember-quality-workflow` 都已声明新增命名不得添加 `Ember` / `ember_` / `ember-` 品牌前缀。
...
1. `docs/aiprompts/performance-profiling.md` 中的 `npm run tauri:dev:profile:*` 只允许保留为 legacy profiling 口径。
...
1. `docs/test/README.md` 已把测试体系事实源固定为 Electron Desktop Host、App Server JSON-RPC、`packages/app-server-client`、`src/lib/desktop-host/` 与 `smoke:electron` / `verify:gui-smoke`。
2. `docs/tests/agent-qc-p0-scenarios.md` 的 P0 场景最低入口不再引用 Tauri GUI 启动，`release-package-startup-smoke` 只要求 release / GUI startup smoke 与版本一致性证据。
3. `docs/tests/ember-agent-autonomous-test-execution-matrix.md` 只把 legacy Tauri runtime 当作 owner gate 旁路上下文，不再作为 current desktop runtime 证据。
...
1. `docs/test/e2e-tests.md` 的 `current` 小节只允许 Electron dev、Electron smoke、Bridge health、App Server / runtime surface smoke 等 current 入口。
2. `docs/test/testing-strategy-2026.md` 的 `current` 小节已固定 Electron Desktop Host、App Server JSON-RPC、`packages/app-server-client`、`src/lib/desktop-host/`、`smoke:electron` 与 `verify:gui-smoke`。
...
3. 新临时目录命名不再使用 `ember-` 品牌前缀，符合新增命名禁止品牌前缀规则。
...
npx vitest run "scripts/electron/current-entrypoints.test.mjs"
rg -n "Tauri 打包|tauri build|/tmp/tauri-build|src-tauri/target|ember-rs/target/release/bundle" "scripts/monitor-build.sh" "scripts/electron/current-entrypoints.test.mjs"
...
1. sherpa-onnx runtime 准备脚本默认读取 `ember-rs/Cargo.lock`，不再以 `src-tauri` 作为运行时目录事实源。
2. 公开函数参数已从旧 host 命名收敛为 `rustWorkspaceDir`。
3. CLI 只暴露 `--ember-rs-dir`，不再暴露 `--src-tauri-dir`。
...
git diff --check -- "scripts/lib/vitest-layer-classifier.mjs" "scripts/lib/vitest-layer-classifier.unit.test.mjs" "scripts/electron/current-rules-guard.test.mjs" "docs/roadmap/appserver/testing-migration.md"
```
...
npm run test:contracts
npm run verify:app-version
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-protocol -p app-server-client -p app-server-transport
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server --lib
npm run smoke:electron
git diff --check -- "ember-rs/crates/app-server-protocol/src/protocol/v0.rs" "scripts/electron/current-rules-guard.test.mjs" "docs/roadmap/appserver/testing-migration.md"
```
...
npm run test:contracts
git diff --check -- "scripts/electron/current-entrypoints.test.mjs" "docs/roadmap/appserver/testing-migration.md"
```
...
1. `ember-rs/build.rs`、`ember-rs/src/main.rs`、`ember-rs/src/lib.rs`、`ember-rs/tauri.conf.json`、`ember-rs/tauri.conf.headless.json` 已按 `dead` root GUI app entry 删除。
...
3. Rust layer classifier 现在按 virtual workspace 事实源识别真实 `crates/*` package；无 root `[package]` 时，`ember-rs/src` 残留旧模块只计为 `unmanaged-root` 治理统计，不再进入默认 Rust test 执行口径。
...
node --check "scripts/electron/current-entrypoints.test.mjs"
npx vitest run "scripts/electron/current-entrypoints.test.mjs" --silent=passed-only --disableConsoleIntercept
rg -n "ember-rs/tauri.windows.conf|tauri.windows.conf" "package.json" "forge.config.mjs" ".github" "electron" "scripts/check-app-version-consistency.mjs"
git diff --check -- "scripts/electron/current-entrypoints.test.mjs" "docs/roadmap/appserver/testing-migration.md"
```

这组证据证明：

1. `ember-rs/tauri.windows.conf.json` 已按 `dead` Windows 旧宿主配置删除，不能回流 package scripts、Electron Forge、CI、Electron host、版本一致性检查或 i18n app metadata evidence。
