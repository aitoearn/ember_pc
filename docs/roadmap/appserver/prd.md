...
> 更新时间：2026-06-08
> Owner：Ember Runtime / 独立 App 集成

## 1. 背景

Ember 当前已经具备较完整的本地 AI Agent runtime：会话、线程、回合、工具、技能、MCP、workspace、memory、artifact、evidence、review、policy、task 和 subagent 都已在 Rust runtime 主链中形成基础能力。

问题在于，这些能力仍主要服务于 Ember Desktop 的当前壳层。当 content-studio 或未来更多独立 App 需要复用同一套 Agent 能力时，如果每个 App 都各自接入、复制或裁剪 runtime，就会出现：

1. runtime 事实源分裂。
2. tool / skill / evidence / permission 行为不一致。
3. 事件、审批、取消、恢复和 artifact 写回协议重复发明。
4. Ember Desktop 和独立 App 的能力演进无法共享。
5. 后续替换壳层、替换前端框架或扩展 App 形态时，会被 runtime glue 绑定。

因此需要把 Ember Agent runtime 抽象为 App Server：壳层和独立 App 不直接拥有 runtime，而是通过稳定协议调用同一个本地服务。

## 2. 一句话目标

把 Ember AI Agent Runtime 服务化为本地 App Server，让 Ember Desktop、content-studio 和未来更多独立 App 通过统一 JSON-RPC 协议复用同一套 Agent 执行能力。

## 3. 业务目标

1. 让 content-studio 能复用 Ember Agent runtime，而不是自建完整执行层。
2. 让 Ember Desktop 继续可用，同时逐步把 legacy desktop command glue 迁出或下线；确需临时兼容时只能在 App Server / RuntimeCore / Electron Desktop Host current 边界内投影，不能继续落在 `ember-rs/src/commands/**`。
...
迁移边界：`ember-rs/src/commands/**` 只是旧 Tauri command wrapper 清理区，不再承接本 PRD 的新业务逻辑、API adapter、runtime 分支、领域服务实现、compat wrapper 或退场 stub。PRD 中提到的 legacy desktop command glue 只表示待迁出 / 待删除对象；新增 Rust 后端能力必须进入 App Server crates / RuntimeCore / services，桌面壳能力进入 Electron Desktop Host。
...
### 5.2 Ember Desktop 用户：我不应该感知 runtime 被服务化
...
2. legacy desktop 命令不再作为完成态；迁移期若确需兼容命令名，只能投影到 App Server / RuntimeCore / Electron Desktop Host current 边界，并写清退出条件。
3. `ThreadReadModel`、artifact、evidence 行为不分裂。

### 5.3 App 开发者：我只想接入协议

作为独立 App 开发者，
当我要接入 Ember Agent 能力时，
我希望只需要使用 App Server Client 和稳定协议类型，
这样不需要理解 Ember Desktop 内部目录、legacy desktop command 和 runtime_turn 细节。
...
这些接口只暴露 App 可消费的 capability，不暴露 Ember 内部模块路径。
...
7. 不在 `ember-rs/src/commands/**` 新增或恢复 Tauri command wrapper、stub、compat facade 或业务实现；旧 wrapper 迁完后必须撤注册并删除，删不动只登记 blocker 和退出条件。
...
9. `ember-rs/src/commands/**` 的旧 wrapper 清理状态已登记到执行计划；任何暂留对象都有 blocker、阻塞文件和退出条件，不能作为完成态。
...
4. Tool / action / artifact / evidence 事件在 Ember Desktop 和 content-studio 中语义一致。
