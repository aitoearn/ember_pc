...
`ember-desktop-platform` 正在承接 `zhongcao`、`content-studio` 和后续 OEM 桌面 App 的公共桌面能力。与此同时，Ember App Server JSON-RPC 已经成为 Agent Runtime、Provider store、session、turn、tool、artifact 和 evidence 的服务化边界。

这带来一个必须先固定的架构问题：

> `ember-desktop-platform` 是否多余？如果不多余，它和 App Server、Agent App 标准、Product App 的边界是什么？

结论：`ember-desktop-platform` 不多余，但它不能成为第二个 App Server，也不能成为业务 App 宿主大壳。它的正确位置是桌面宿主平台套件：公共设置 UI、应用中心、Host Bridge、Capability Gateway、sidecar lifecycle、Electron / Tauri adapter 和 Product App 接入层。

## 2. 架构决策

### 2.1 选定主线

保留 `ember-desktop-platform`，但把它收敛为 Host Kit / 公共平台 UX。
...
| 把 Product App 重新塞回 Ember Desktop 大壳 | `zhongcao` 不再是真独立 Product App，后续 OEM / standalone 方向被锁死。 |
...
flowchart TB
  ...
  subgraph Platform[ember-desktop-platform Host Kit]
  ...
  subgraph Cloud[embercore / Cloud control plane]
...
| Ember App Server | Agent runtime、Provider metadata/key、session/turn、tool、artifact、evidence、runtime DB | Electron/Tauri 壳、设置页、应用中心 UI、业务页面 |
| `ember-desktop-platform` | 公共平台 UX、Host Snapshot、Capability Gateway、sidecar owner、settings orchestration、Product App settings/storage boundary | Agent 执行循环、Provider key 双存、业务 workflow、业务 DB schema |
...
sequenceDiagram
  ...
  App->>HostCore: invokeCapability(ember.agent)
...
flowchart TD
  ...
  Business -->|是| ProductStore[Product App workspace / ember.storage]
...
- 新 Product App 接入时只需要 JSON-RPC client / Host Kit，不需要 import Ember 内部 runtime 模块。
...
- `ember-desktop-platform` 使用自己的 `userData/app-server/ember.db`，不污染 Existing Ember 默认 DB。
...
- `current`：Product App 通过 Host Bridge / Capability SDK 调用 `ember.agent`。
