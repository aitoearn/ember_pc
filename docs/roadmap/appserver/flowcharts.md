...
说明：服务抽取完成后，`ember-rs/src/commands/**` 不保留新的 thin facade、fail-closed stub、compat wrapper 或 tombstone；该目录只作为旧 Tauri wrapper 删除清理区。需要保留兼容命令名时，兼容入口也必须在 Electron Desktop Host / App Server current 边界内说明退出条件。
...
flowchart TD
    ...
    B -.禁止.-> I["新增 / 复活 ember-rs/src/commands wrapper"]
    C --> H["Runtime facts"]
```
