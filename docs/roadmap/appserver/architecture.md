...
4. legacy desktop facade 逐步迁出或下线；确需临时兼容时只做 current 边界投影，不继续拥有业务执行逻辑，也不在 `ember-rs/src/commands/**` 保留 thin facade。
5. 独立 App 通过 JSON-RPC client 复用 Agent 能力。
6. Tool / action / artifact / evidence / workspace / skill capability 不在 App 侧复制。

参考 `/Users/coso/Documents/dev/rust/codex` 时，只参考 Codex CLI 版本代码中 `codex-rs` 的 App Server / protocol / client / daemon / Rust runtime 分层；该仓库不包含 Codex App 前端实现，Ember 不参考 Codex App UI 或桌面壳实现。
...
flowchart TB
    subgraph Apps["App Surfaces"]
        Desktop["Ember Desktop"]
        ContentStudio["content-studio"]
        OtherApps["更多独立 App"]
    end

    subgraph HostBridge["Desktop Host Bridge / Clients"]
        LimeElectronBridge["Ember Electron Desktop Host Bridge<br/>current"]
    ...
    subgraph Runtime["Ember RuntimeCore"]
...
Rust 实现落点必须跟随这张分层表。`ember-rs/src/commands/**` 是旧 Tauri command wrapper 清理区，不属于 RuntimeCore、ExecutionBackend 或 Runtime Services 的新实现目录；它只允许迁出核心逻辑、撤 runner / dispatcher / catalog / mock 注册和删除旧 wrapper。删不动时登记 blocker，不在该目录保留 stub、compat wrapper 或退场 tombstone。新增后端能力进入 App Server crates / RuntimeCore / services；桌面壳能力进入 Electron Desktop Host。
...
2. 独立 App 直接 import Ember 内部 runtime 模块。
...
2. 真实 Aster backend 仍依赖 host state，只能由 Ember Desktop in-process host 注入 `AsterBackendHost`，不能通过 standalone CLI 直接开启；in-process 只是去掉进程边界，仍必须保持 App Server JSON-RPC result envelope，不引入第二响应合同。
...
服务化不是一次性替换 Ember Desktop，而是把 runtime owner 从 legacy desktop command glue 下沉到 service。Electron Desktop Host 是 GUI current bridge，legacy desktop facade 只保留兼容委托语境。
...
7. 新实现不得落到 `ember-rs/src/commands/**`；发现旧 wrapper 阻碍 current 主链时，优先迁出或删除，而不是新增 compat 包装。
...
3. 独立 App 不需要链接 Ember Rust crate。
