...
> 补充执行记录：`docs/roadmap/test/README.md` 已回填最新汇总；本文件继续保留更细的验证证据和临时执行上下文。

## 目标

- 将 Ember 前后端测试拆成纯单元、集成和 E2E / live-gated 层。
...
- 测试分层规则已写入 `AGENTS.md` 和 `docs/aiprompts/quality-workflow.md`。
...
- 新增 Rust E2E 默认运行预算检查：
  ...
  - 该检查不把 unit 文件内“普通单测 + ignored live 测试”的混合结构当成失败；此类文件仍可运行普通单测，但 live 测试必须继续由 `#[ignore]` / `EMBER_REAL_API_TEST=1` 门禁控制。
...
- `HarnessStatusPanel` 已完成第一轮拆分：
  ...
  - 新增 `harnessEvidenceViewModel.ts`，承载 evidence / replay / review / EmberCore policy 纯逻辑。
...
- `npm run test:unit -- scripts/run-vitest-layer.unit.test.mjs`
  ...
  - 覆盖 `--single-fork` 参数解析、unit 默认不强制 single fork、unit 默认 Node 环境、unit 默认 threads pool、`EMBER_VITEST_UNIT_POOL` 覆盖、显式 `--pool` / `--environment` 覆盖、非 unit 默认保留 single fork、以及 `EMBER_VITEST_SINGLE_FORK=1` 回退。
...
- `npm run test:unit -- src/test/fastCheckRuns.unit.test.ts src/lib/artifact/streaming.test.ts src/lib/artifact/store.test.ts src/lib/artifact/registry.test.ts src/lib/artifact/parser.test.ts src/lib/api/importExport.test.ts src/lib/utils/apiKeyMask.test.ts`
  ...
  - 新增 `src/test/fastCheckRuns.ts`；本地 / AI TDD 默认 `25` runs，CI 保持原始 `50/100` runs，`EMBER_FAST_CHECK_RUNS=100` 可本地满量复现。
...
- `npm run test:rust:e2e`
  - 默认 gate 生效；未设置 `EMBER_REAL_API_TEST=1` 时不运行 ignored/live Rust E2E。
...
- `make tdd-rust-filter FILTER=workspace_support::tests::sanitize_project_dir_name_should_replace_invalid_chars`
  - 先执行 Rust E2E 默认运行预算检查，随后只运行 root `ember` lib 中 1 个 Rust unit 测试。
...
- 当前工作区仍有非本轮写集：`docs/roadmap/soul/rollout-plan.md`、`src/components/agent/chat/utils/toolNameFamily.ts`、`src/components/agent/chat/utils/toolNameFamily.unit.test.ts` 等，本轮不接管。
...
1. 前端 unit 速度主线已完成当前阶段：
   - 已完成第一刀：`scripts/run-vitest-layer.mjs` 的 `test:unit` 默认不再全层 `singleFork`；`--single-fork` 和 `EMBER_VITEST_SINGLE_FORK=1` 保留为排查回退。
   - 已完成第二刀：4 个 browser-dependent unit 已移到 component，`test:unit` 默认 `--environment node`。
   - 已完成第三刀：`test:unit` 默认 `--pool threads`，并保留 `--pool=forks`、`EMBER_VITEST_UNIT_POOL=...`、`--single-fork`、`EMBER_VITEST_SINGLE_FORK=1` 回退 / 覆盖；高负载复测 `293.89s` 和临时取消 pool 后 `366.78s` 不作为稳定基准，下一轮先等机器安静或做慢文件画像。
