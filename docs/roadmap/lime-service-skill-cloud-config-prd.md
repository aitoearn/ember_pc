...
> 更新时间：2026-04-21  
> 当前主规划：`docs/roadmap/embernextv2/README.md`
> 目标：吸收 Ribbi 式服务型技能入口的方法，但保持 `embercore` 只做目录控制、`ember` 客户端负责全部执行
> 作用域：本文只保留 `ServiceSkill` 在 `ember` / `embercore` 之间的产品对象、目录边界与本地执行原则，不替代 `docs/aiprompts/command-runtime.md`、`docs/aiprompts/embercore-collaboration-entry.md`
...
服务端文档纠偏由 `embercore/docs/exec-plans/at-command-control-plane-only.md` 统一承接。
