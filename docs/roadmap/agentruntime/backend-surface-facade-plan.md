...
当前 `agent_runtime_submit_turn` 已经能进入 Aster / ember_agent 主链，但它的调用形态和事件消费更贴近 Chat / Claw。Agent App 需要完整 Agent 能力时，不应直接调用 Chat 专用命令，也不应新建平行 runtime；应新增后端 facade，把 App task 映射到同一 AgentRuntime control plane。
...
| 路径 | 责任 |
| --- | --- |
| `ember-rs/src/commands/agent_app_runtime_cmd.rs` | Agent App runtime Tauri command 外壳，只做参数反序列化和 state 注入。 |
| `ember-rs/src/services/agent_app_runtime_service.rs` | App task facade：校验 appId / entryKey / manifest capability / provenance，组装 surface request。 |
| `ember-rs/src/services/agent_runtime_surface_service.rs` | 多 surface 共享入口：把 Chat、Claw、Agent App、Automation 输入转换成 runtime turn。 |
| `ember-rs/src/services/agent_app_runtime_capability_catalog_service.rs` | Agent App -> Claw capability catalog 第一刀：声明 capabilityId / alias / launch metadata contract，并由 runtime facade 复用。 |
...
ember_runtime = {
  ...
  capability_hints: ["ember.capability.research.search", "ember.capability.image.generate"],
  artifact_policy_ref: "...",
  evidence_policy_ref: "..."
}
```
