# Ember AgentRuntime Profile PRD

> 状态：implementation-audited
> 更新时间：2026-05-12
> Owner：Ember runtime / evidence / GUI 主链
> 关联标准：`/Users/coso/Documents/dev/ai/embercloud/agentruntime` 的 `Agent Runtime v0.4.0` 与 `Ember AgentRuntime Profile`；相邻 owner 为 `/Users/coso/Documents/dev/ai/embercloud/agentcontext`、`/Users/coso/Documents/dev/ai/embercloud/agentevidence`、`/Users/coso/Documents/dev/ai/embercloud/agentpolicy`、`/Users/coso/Documents/dev/ai/embercloud/agentui`
...
Codex 与 Claude Code 给 Ember 的启发是：
...
3. Ember 需要的是两者结合：**公开标准 + Ember 严格 profile + current runtime 实现映射**。
...
当前没有统一 AgentRuntime Profile 时，Ember 会持续遇到这些问题：
...
| 标准协议与实现脱节 | 文档越来越多，但 Ember current 主链没有被强制收敛。 |

## 3. 产品目标

### 3.1 一句话目标

把 Ember 的 agent 执行统一到 `Ember AgentRuntime Profile`：所有 current runtime、GUI、evidence、review、replay、analysis 都消费同一组 `RuntimeEvent + Snapshot + ThreadReadModel`。
...
4. 让 Ember 的标准协议真正服务产品实现，而不是成为平行文档。
...
5. 不承诺跨产品兼容，只先让 Ember current 主链可落地。
...
验收：新增 `agentruntime` fixture 校验与 Ember command mapping 测试。
...
Ember current runtime 必须稳定产出：

| Identity | 作用 |
| --- | --- |
| `runtimeId` | Ember runtime 实例或安装归因。 |
...
- 新标准协议能回挂到 Ember current 实现，减少空转。
...
| 风险 | 缓解 |
| --- | --- |
| Profile 变成新协议空转 | 每个 profile 字段都必须映射 Ember current 主链或标记为未来扩展。 |
| 一次性重构过大 | 分阶段先收 submit turn/evidence，再收 task/subagent/job。 |
| GUI 迁移不彻底 | 增加反漂移扫描，禁止 UI-only runtime truth。 |
| Schema 过严卡住演进 | 公开 schema 保持宽松，Ember profile schema 只约束 current core。 |

## 11. 验收摘要

1. 文档层：本 roadmap 与 PRD 已落地。
2. 标准层：`agentruntime v0.4.0` 已有 Ember Profile 与 fixtures。
3. 实现层：后续 PR 必须把 Ember current 主链映射到 profile。
