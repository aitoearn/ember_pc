...
> 更新时间：2026-05-17
> 对比来源：`/Users/coso/Documents/dev/ai/embercloud/agentruntime` (`agentruntime-standard` v0.4.0)、Ember AgentRuntime Profile、Agent App Runtime Surface、Claw capability sharing、AgentUI adoption gap
> 目标：判断给 Claw 设计的 AgentRuntime 标准对 Ember Agent App 是否有用，并固定可采用子集、适配边界和禁止方向。

## 1. 结论

有用，而且比 AgentUI 更关键。

AgentUI 解决的是 **运行事实如何投影成可复用 UI**；AgentRuntime 解决的是 **这些运行事实、控制命令、恢复状态、证据和成本到底由谁拥有**。如果只采用 AgentUI，内容工厂能长得像 Claw，但仍可能在 App 内自建第二套任务状态、模型选择、Skill 调用、Token 统计和证据链。采用 AgentRuntime 后，Agent App 只能通过 Ember Host / AgentRuntime 主链使用 AI 能力。

因此本轮判断固定为：

```text
外部 agentruntime 标准 = Ember / Claw / Agent App 的 runtime truth 参考标准
Ember 当前 AgentRuntime = implementation/current 主路径
AgentUI = runtime facts 的 projection / UI 标准
业务 Agent App = 业务流程、业务对象和产物写回 surface
```

不能把 `/Users/coso/Documents/dev/ai/embercloud/agentruntime` 当成一个要拷进 Ember 的新运行时实现；它是标准与 profile 参考，用来约束 Ember current runtime、Agent App facade 和 Claw capability catalog。
...
| Agent App | 业务路径、业务对象、表单、工作区、产物确认和写回 | AI 执行事实、模型路由、Token 统计、Skill runtime、Evidence owner | 通过 `ember.agent / ember.workflow / ember.*` SDK 调 Host runtime。 |

一句话：**AgentRuntime 决定“发生了什么”，AgentUI 决定“怎么展示发生过程”，Agent App 决定“业务上下一步做什么”。**

## 4. 可直接采用的子集

这些能力与 Ember current 路线图一致，应进入 Agent App / Host Run UI 的 P0-P1 验收口径。

| 标准能力 | 采用口径 | Ember 当前落点 |
| --- | --- | --- |
...
| Control plane semantic | Ember 命令可保留产品名，但必须映射到 `submit_turn / start_task / get_task / respond_action / cancel_task / export_evidence` | `agent_app_runtime_start_task / get_task / submit_host_response / cancel_task`；evidence 走 `agent_runtime_export_*`。 |
...
| 标准能力 | 当前差距 | 适配要求 | 优先级 |
| --- | --- | --- | --- |
| Ember Profile schema 强校验 | 外部 schema 要求 `schemaVersion=ember-profile-0.4.0`、`runtimeId`、`sequence`、scope ids；Ember current 还有部分投影事件来自历史字段 | 增加 fixture / contract 测试，缺失 correlation 时标记 degraded，不从正文猜 ids | P0 |
...
| Tool inventory snapshot | 标准要求当前 scope 下可用 tools / policy / capabilities 可解释 | `ember.skills / ember.tools / ember.models` 能力目录需要从 Host 投影给 App，App 只显示可用/不可用 | P1 |
...
| 方向 | 分类 | 原因 |
| --- | --- | --- |
| 直接把外部 `agentruntime` 项目当实现依赖搬进 Ember | dead | 它是标准/文档/profile，不是 Ember current runtime 实现。 |
| Agent App 自建 `agent_app_agent_runtime` facts | dead | 会和 AgentRuntime Profile 形成双事实源。 |
| 内容工厂直连 OpenAI-compatible API 或 `EMBER_GATEWAY_*` 完成主流程 | dead | 只能得到模型补全，拿不到 Skills、Tools、routing、cost、evidence、HITL。 |
...
1. Ember current AgentRuntime：`RuntimeEvent + ThreadReadModel + TaskSnapshot + EvidencePack`。
...
4. `AgentRuntimeProfileEvent`、Ember Profile fixtures、Evidence Pack、Replay / Review / Analysis exports。
...
4. `EMBER_GATEWAY_*` 仅作为低阶模型 executor 或开发期 fallback，不能被 UI/文档称为完整 Agent 能力。
...
AgentRuntime 标准不是额外范围，而是把“Agent App 内完整使用 Ember AI 能力”这件事从 UI 诉求变成可验证工程合同：

```text
业务 App 负责业务递进
Host SDK 负责运行 UI projection
Ember AgentRuntime 负责 execution facts
...
| --- | --- | --- | --- |
| 检查 `/Users/coso/Documents/dev/ai/embercloud/agentruntime` 是否有用 | 已读取 `package.json`，确认是 `agentruntime-standard` v0.4.0；已读取 `README.md`、`docs/zh/specification.md`、`docs/zh/profiles/ember.md`、核心 contracts、schemas 和 fixtures | covered | 无；结论已沉淀到本文。 |
| 不能把 AgentRuntime 当成 AgentUI | 本文第 1 / 3 节固定 AgentRuntime 是 runtime truth，AgentUI 是 projection；`docs/roadmap/agentruntime/agentui-adoption-gap.md` 已记录 AgentUI 侧差距 | covered | 后续实现需要 Host drawer 真正消费 projection state。 |
...
| 要明确 Ember Agent / App 边界 | 本文第 3 / 7 节与 `app-surface-runtime.md` 对齐：AgentRuntime 管 execution facts，App 管业务流程和产物写回 | covered | Host SDK UI seam 还需继续抽包。 |
...
2026-05-17 本轮继续只读核查 `.ember/qc/gui-evidence/agent-apps`，不把路线图文字本身当完成证据。

已存在且显示 key action ready 的 evidence：

| Evidence file | 复核到的关键事实 |
| --- | --- |
| `.ember/qc/gui-evidence/agent-apps/agent-app-required-skills-runtime-enforced-run-production-summary.json` | `completionE2e.ready=true`，且 `modelReady / usageReady / costReady / skillInvocationReady / artifactReady / evidenceReady / workspacePatchReady / terminalReady` 全为 `true`。 |
| `.ember/qc/gui-evidence/agent-apps/agent-app-required-skills-runtime-enforced-only-copy-summary.json` | `only-copy` 同样全 readiness 为 `true`，required Skills 不再只是 prompt 提示。 |
| `.ember/qc/gui-evidence/agent-apps/agent-app-output-contract-materialized-run-strategy-summary.json` | `run-strategy` 交付包物化完成，全 readiness 为 `true`。 |
| `.ember/qc/gui-evidence/agent-apps/agent-app-output-contract-materialized-run-review-summary.json` | `run-review` 复盘物化完成，全 readiness 为 `true`。 |
| `.ember/qc/gui-evidence/agent-apps/agent-app-run-production-direct-audit-summary.json` | `run-production` direct audit 全 readiness 为 `true`。 |
| `.ember/qc/gui-evidence/agent-apps/agent-app-only-copy-direct-audit-summary.json` | `only-copy` direct audit 全 readiness 为 `true`。 |
| `.ember/qc/gui-evidence/agent-apps/agent-app-direct-runtime-postcheck.json` | 两个 direct get 复核均显示 `threadArtifactCount=1` 且 `hasWorkspacePatchByDirectSnapshot=true`。 |
...
| `node --test tests/ui.test.mjs` in `/Users/coso/Documents/dev/ai/embercloud/content-factory-app` | 通过；当前并行工作树中内容工厂 UI 单测为 `30` 个全部通过，新增覆盖项目流程导航、统一抽屉、Host 运行明细、Host profile 缺 AI 能力时阻止本地模型兜底等页面行为。 |
...
| `node scripts/agent-app/package-handoff-check.mjs --package-dir "/Users/coso/Documents/dev/ai/embercloud/content-factory-app" --format summary` | 修复后返回 `status=needs_handoff`；`agentRuntimeBypass=none`、`blockers=none`、`distArtifacts total=0`。仍有 dirty / high-risk build script warning，所以外部 package 需要持有者 handoff，但“直连 provider/Gateway”这一 blocker 已清零。 |
...
本节是只读诊断；`/Users/coso/Documents/dev/ai/embercloud/content-factory-app` 当前存在大量已脏文件，本轮不接管、不覆盖，只给持有该写集的进程提供最小修复线索。
...
本节用于判断“让 Agent App / 内容工厂在 App 内完整复用 Ember AgentRuntime / Claw / Skills / ToolRuntime / Evidence，而不是跳回 Chat、直连模型 API、或每个 App 自建 AI 同事”这一整体目标是否可以标记完成。
