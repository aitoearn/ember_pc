# Ember AgentRuntime Profile 分阶段落地计划

> 状态：implementation-audited
> 更新时间：2026-05-12
> 作用：把 AgentRuntime Profile 从标准文档推进到 Ember current runtime、证据导出和 GUI 展示。
...
4. 先保守收紧 Ember Profile schema，不收紧公开 schema。
...
| 阶段 | 目标 | 关键产物 | 退出条件 |
| --- | --- | --- | --- |
| P0 | 文档与 profile 冻结 | 本 roadmap、PRD、架构图、AgentRuntime v0.4.0 Ember Profile | 团队对主链、owner、非目标无歧义 |
...
1. `agentruntime` 标准项目新增 Ember AgentRuntime Profile。
2. Ember 主仓库新增 `docs/roadmap/agentruntime/`。
3. 固定 current / compat / deprecated / dead 分类。

后续如果改 Profile，必须同步：

- `docs/roadmap/agentruntime/prd.md`
- `docs/roadmap/agentruntime/architecture.md`
- `docs/aiprompts/harness-engine-governance.md`，仅当事实源规则改变时
...
4. AgentUI 对 `schemaVersion = ember-profile-0.4.0` 的 dotted profile event 保持 stream 活跃且不告警，不把它转换成 UI-owned truth。
...
- 前端未知事件控制器能识别 Ember Profile 事件为已知旁路事实，不污染用户可见 projection。
...
- 让 Profile 覆盖 Ember 当前最关键的复杂运行态。
...
| Profile schema 过早收紧 | 中 | public schema 宽松，Ember profile 只约束 current core。 |
...
- `cargo check -p ember --lib` 不出现 presentation copy unused warning。
- 未新增 legacy desktop facade、前端 gateway、mock 或 request shape；不得在 `ember-rs/src/commands/**` 新增业务逻辑。
