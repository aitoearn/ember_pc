...
> 更新时间：2026-04-29  
> 目标：先盘清 Ember 多模态运行链中的底层事实源，避免从 `@` 命令、viewer 或 CLI 旁路倒推架构。
...
| `execution_profile` | current | runtime policy merge | 用户设置、EmberCore policy、本地安全策略 | executor、router、evidence | thread read / evidence | 权限和模型必须合并解释 |
...
| `embercore_catalog_policy` | current | EmberCore + Ember bootstrap cache | `client/skills`、`client/scenes`、model catalog、policy | contract registry、profile | bootstrap cache / policy snapshot | 云端控制面，不默认执行 |
...
| `embercore_default_executor` | dead | 无 | 无 | 无 | 无 | 不允许作为 current 规划出现 |
