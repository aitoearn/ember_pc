...
> 目标：把 Ember 从“能显示文件/画布的聊天工作台”升级为“交付物优先的 Artifact Workbench”，让回复不再只是普通 Markdown，而是可扫描、可编辑、可版本化、可复用的正式产物
>
> 配套文档：
...
1. Ember 已经是 `Artifact First` 方向，而不是纯聊天产品。
...
这说明 Ember 不需要再造第二套“文档产品”，而是要把现有能力从“散件”收敛成“Artifact Workbench”。
...
3. Ember 的 Artifact 不只是富文本，还包括来源、评分块、表格块、执行绑定、浏览器会话引用、导出记录等领域对象。
...
为 Ember 建立一套统一的 Artifact Workbench，让高价值回复默认沉淀为：
...
这与当前 Ember 的工作台分栏方向一致，不需要推翻现有 UI 模式。
...
**结构化产物协议属于 Ember 产品层，不等于 runtime 协议。**
...
Ember 是本地优先产品，Artifact 应支持落盘，但不能写死平台路径。
...
`ember-rs/src/services/agent_timeline_service.rs` 当前已能投影 `ArtifactSnapshot`。  
高配版建议扩展为：
...
- 让 Artifact Workbench 成为 Ember 的统一交付层
...
```text
ember-rs/crates/aster-rust/crates/aster/src/runtime/
```

这部分是框架层远期方向，不覆盖 Ember 当前仓库已确定的运行时收口主计划。
...
**把 Artifact 从“聊天的附件”升级为“Ember 的正式交付层”。**
