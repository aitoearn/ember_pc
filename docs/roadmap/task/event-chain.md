# Ember 事件链与 Claude Code 对照

> 状态：提案
> 更新时间：2026-04-23
> 作用：把 Claude Code 的事件分层、Ember 当前已有事件骨架、以及后续如何把任务 / 模型 / 成本 / 限额收进同一条事件链讲清楚。
> 依赖文档：
...
这和 Ember 后续要落的 `LimitState` 非常接近，说明“限额事件”在上游也是结构化状态，不是 toast 文案。
...
Ember 其实不是没有事件，而是已经有一条相当强的事件骨架。

### 4.1 运行时事件总线已经存在

`ember-rs/crates/agent/src/protocol.rs` 里的 `AgentEvent` 已经覆盖：
...
这说明 Ember 已经有“流式 runtime 事件层”，不是从零开始。

### 4.2 时间线投影也已经存在

`ember-rs/src/services/agent_timeline_service.rs` 已经会把一部分 runtime event 投影进 timeline：
...
这相当于 Ember 已经有“原始事件 -> 稳定读模型”的第三层。

### 4.4 Request telemetry 也已经存在

`ember-rs/crates/infra/src/telemetry/types.rs` 中的 `RequestLog` 已经记录：
...
这说明 Ember 也已经有“请求级经济与遥测事实层”。

### 4.5 但 Ember 还没有做成 Claude Code 那种“事件层 != 展示层”
...
这件事对 Ember 很重要，因为它说明：

- 不是所有 runtime event 都应该直接变成聊天气泡
- 但即便不展示，它们也必须是稳定事件事实

## 5. Ember 当前真正缺的，不是事件数量，而是事件语义缺口
...
| 层 | Claude Code 当前做法 | Ember 当前状态 | 缺口 |
| --- | --- | --- | --- |
...
后续应优先扩展 `ember_agent::AgentEvent`，而不是另造一个平行 `RoutingEventBus`。
...
**把“任务 / 模型 / 成本 / 限额是环环相扣的”这件事，从一句原则，落成 Ember 当前可以直接接线的事件主链方案。**
