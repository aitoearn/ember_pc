...
> 状态：路线图与架构设计
> 更新时间：2026-05-27
> 范围：Ember 对话工作区下一阶段 AgentUI，包括 UI 架构、代码层级、事件流程、时序图、后端协作与落地顺序。

## 目标

AgentUI 不是再做一个聊天页面，而是把 Ember 已有的 runtime、timeline、artifact、task、team、harness、evidence 能力收束成一个可观察、可控制、可交付的工作台。

本目录回答四类问题：

1. **产品结构**：Ember 的 AgentUI 应该由哪些层组成，哪些信息应该出现在首屏，哪些应该进入展开详情。
...
| 文档                                                                                             | 作用                                                                                                        |
| ------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------- |
| [agent-ui-research-and-ember-direction.md](agent-ui-research-and-ember-direction.md)               | 竞品与本地参考调研，说明为什么 Ember 要走“对话 + 过程 + 任务 + 产物 + 证据”路线。                            |
| [ember-agentui-target-architecture.md](ember-agentui-target-architecture.md)                       | 目标 UI 架构图与五层模型，是后续 UI 改造的总图。                                                            |
| [ember-agentui-code-map.md](ember-agentui-code-map.md)                                             | Ember 当前代码层级地图，标出前端、协议、后端、服务和测试入口。                                               |
| [ember-agentui-event-flow.md](ember-agentui-event-flow.md)                                         | 关键流程图，包括发送消息、旧会话恢复、queue/steer、权限、artifact、evidence。                               |
| [ember-agentui-sequence-diagrams.md](ember-agentui-sequence-diagrams.md)                           | 端到端时序图，适合实现和排查首字慢、恢复慢、流式错乱。                                                      |
| [ember-agentui-backend-coordination.md](ember-agentui-backend-coordination.md)                     | 后端配合代码架构，定义 UI 需要后端继续补齐的投影、分页、指标与批量接口。                                    |
| [ember-agentui-implementation-roadmap.md](ember-agentui-implementation-roadmap.md)                 | P0/P1/P2/P3 落地顺序、验收标准和验证命令。                                                                  |
...
| [ember-agentui-standard-alignment.md](ember-agentui-standard-alignment.md)                         | AgentUI 标准全流程 taxonomy 与 Ember 当前实现差距，固定 current / compat / deprecated / dead 分类和下一刀。  |
...
Ember AgentUI 的主线应保持一个事实源，不新增第二套事件系统：
...
- **Rust 边界复验**：2026-05-27 继续复跑 `cargo test --manifest-path "ember-rs/Cargo.toml" -p ember thread_read_should_project_runtime_permission -- --test-threads=1`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p ember commands::aster_agent_cmd::runtime_turn::tests::request_permission -- --test-threads=1` 与 `cargo test --manifest-path "ember-rs/Cargo.toml" -p ember commands::aster_agent_cmd::runtime_turn::tests::routing::should_prewarm_mcp_runtime_should_skip_pending_runtime_permission_confirmation -- --test-threads=1` 并通过，确认 runtime permission confirmation 的 pending / denied DTO 投影、turn gating 与 prewarm skip 仍由 Rust 主链约束；随后补跑 `create_runtime_session_internal_without_strategy_should_default_to_auto`、`create_session_record_sync_without_strategy_should_default_to_auto` 与 `tool_end_preserves_embedded_checkpoint_metadata`，确认无策略会话默认进入 `auto`，且 Write 工具嵌入的 checkpoint diff metadata 不会在 tool end 快照丢失；该复验不替代真实 Provider 长编程任务证据。
...
- **OpenVibeCoding 编程工作台对齐**：2026-05-27 用户复盘指出当前编程界面仍是诊断卡、文件正文、输出与对话混排，尚未解决“编程工作台应该先看可视预览、再从右侧继续对话和修复”的核心问题。后续 UI 主线以 `docs/roadmap/agentui/openvibecoding-code-workbench-alignment.md` 为准：先冻结继续叠卡方向，再把编程模式收敛为中央预览 / 文件 / 变更 / 输出标签 + 右侧对话与可折叠任务进度；Harness 诊断、evidence、inventory、replay 默认进入显式诊断抽屉，不再作为编程首屏主体验。
