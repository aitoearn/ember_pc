...
当前物理包名以 `@embercloud/agent-runtime-projection` 和 `@embercloud/agent-runtime-ui` 为准。`@embercloud/agent-ui-projection`、`@embercloud/agent-ui-react` 只允许作为历史规划名或未来 alias 讨论，不能写成 current owner。

## Install

```bash
npm install @embercloud/agent-ui-contracts
```
...
```ts
import type { AgentRuntimeExecutionEvent } from "@embercloud/agent-ui-contracts";
...
```ts
import { collectRuntimeEventValidationIssues } from "@embercloud/agent-ui-contracts";
...
Product App runtime service
  ...
  -> @embercloud/agent-runtime-projection
  -> @embercloud/agent-runtime-ui 或产品自己的 presentation adapter
```
