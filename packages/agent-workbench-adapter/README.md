# @embercloud/agent-workbench-adapter

`@embercloud/agent-workbench-adapter` 是 Agent 工作台的 headless adapter。它不渲染 UI，只把产品页面的 intent、composer 状态、runtime facts 和 turn payload 拼成可复用的标准结构。
