...
Options:
  ...
  --version <version>     Release version, defaults to packages/ember-cli-npm/package.json
  --out-dir <path>        Output directory, defaults to packages/ember-cli-npm/dist
...
function resolveBinaryCandidates(meta, targetTriple, explicitBinary) {
  ...
  if (targetTriple) {
    push(path.join(REPO_ROOT, "ember-rs", "target", targetTriple, "release", meta.binaryName));
    push(path.join(REPO_ROOT, "target", targetTriple, "release", meta.binaryName));
  }

  push(path.join(REPO_ROOT, "ember-rs", "target", "release", meta.binaryName));
