...
`@embercloud/agent-runtime-client` 是 Lime Agent Runtime 的标准 TypeScript client facade。它不定义新协议，也不重建运行时；当前实现只复用 `@embercloud/app-server-client` 中已经存在的 App Server JSON-RPC current 主链。
