...
`@embercloud/agent-runtime-client` 是 Ember Agent Runtime 的标准 TypeScript client facade。它不定义新协议，也不重建运行时；当前实现只复用 `@embercloud/app-server-client` 中已经存在的 App Server JSON-RPC current 主链。
...
- 暴露 browser-safe 子路径 `@embercloud/agent-runtime-client/sessionGateway`，用于把已有 App Server session gateway 适配为标准 runtime client。
...
```bash
npm install @embercloud/agent-runtime-client
```
...
`@embercloud/agent-runtime-client` 只拥有 transport facade。它不导出 `AgentUiProjectionState`，也不依赖 `@embercloud/agent-runtime-projection` 或 `@embercloud/agent-runtime-ui`。

## Root Usage

根入口适合 App Server connection owner 使用。它直接复用 `@embercloud/app-server-client` 的 `AppServerConnection`，不新增 runtime protocol：

```ts
import {
  AppServerConnection,
  createAgentRuntimeClient,
} from "@embercloud/agent-runtime-client";
...
```ts
import type { AgentRuntimeLifecycleClient } from "@embercloud/agent-runtime-client/sessionGateway";
...
| --- | --- |
| Root entry | 适合 Node / host owner，复用 `@embercloud/app-server-client` 的 `AppServerConnection`。 |
...
- bundle：`@embercloud/agent-runtime-client/sessionGateway` dist 不包含 Node builtin 或 `@embercloud/app-server-client` 依赖。
...
| --- | --- |
| Runtime | Node `>=20`，ESM。 |
| Root dependency | `@embercloud/app-server-client`。 |
| Browser-safe export | `@embercloud/agent-runtime-client/sessionGateway`。 |
| Side effects | `false`。 |
