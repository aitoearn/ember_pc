...
```bash
npm install @embercloud/agent-ui-contracts @embercloud/agent-runtime-ui @embercloud/agent-runtime-projection
```
...
`RuntimeFactsPanel` 只渲染宿主传入的 read model，不会订阅 JSON-RPC，也不会判断某个 artifact 是否已经被业务层物化。宿主如果已经把 App Server `artifact.snapshot` 写成业务产物，应在进入 `@embercloud/agent-runtime-projection` 前去掉上游快照事件，只保留本地业务产物的 `artifact.changed`。这样右侧事实栏展示的是可继续编辑、可应用的交付物，而不是后端中间事件。
...
```tsx
import { SubagentsView } from "@embercloud/agent-runtime-ui";
...
| Dependencies | `@embercloud/agent-ui-contracts`。 |
| Dev dependency | `@embercloud/agent-runtime-projection` 仅用于 fixture replay 测试。 |
...
发布前必须先发布同版本的 `@embercloud/agent-runtime-projection`，并确认 `npm pack --dry-run` 输出只包含 `dist` 和 `README.md`。
