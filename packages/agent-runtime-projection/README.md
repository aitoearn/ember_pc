...
`@embercloud/agent-runtime-projection` 是 Lime Cloud 系列应用共享的 Agent Runtime 事件投影层。它把 App Server / JSON-RPC / 本地运行时产生的 execution events 转成前端稳定可渲染的 read model，并复用 `@embercloud/agent-ui-contracts` 中的标准类型契约。

这个包只包含纯 TypeScript 类型和函数，不依赖 React，不调用 JSON-RPC，不访问 Electron IPC，也不绑定任何具体业务模块。

当前物理包名是 `@embercloud/agent-runtime-projection`。历史规划中出现过的 `@embercloud/agent-ui-projection` 只能作为未来 alias 或文档历史引用，不能写成 current owner。
...
- `@embercloud/agent-runtime-client` 只封装 App Server current runtime facade，不新增协议。
- `@embercloud/agent-ui-contracts` 只定义跨宿主类型契约，不承接投影逻辑。
- `@embercloud/agent-runtime-projection` 只做 headless projection 和 selectors，不接触宿主、网络、React 或业务对象。
- `@embercloud/agent-runtime-ui` 只消费 projection state 渲染标准 React primitives，不订阅运行时。
...
src/lifecycle.ts   -> session.opened / run lifecycle / runtime status / model routing / task profile projection helpers
...
```bash
npm install @embercloud/agent-ui-contracts @embercloud/agent-runtime-projection
```
...
App Server JSON-RPC
  ...
  -> executionEvents
  -> @embercloud/agent-runtime-projection
  -> @embercloud/agent-runtime-ui 或宿主 UI
```
...
```ts
import { replayAppServerFacts } from "@embercloud/agent-runtime-projection";
...
```ts
import { getAgentUiFixture } from "@embercloud/agent-ui-contracts";
import { replayAgentUiFixture } from "@embercloud/agent-runtime-projection";
...
```ts
import { buildAgentUiToolStartEvents } from "@embercloud/agent-runtime-projection";
