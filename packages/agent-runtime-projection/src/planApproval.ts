...
export type AgentUiPlanApprovalProjectionBase = Pick<
  AgentUiProjectionEvent,
  | "sourceType"
  | "timestamp"
  | "sessionId"
...
export interface AgentUiPlanApprovalProjection {
  requestId: string;
  from?: string;
  planFilePath?: string;
  planContent?: string;
...
export interface AgentUiPlanApprovalResponseProjection {
  requestId: string;
  approved?: boolean;
  feedback?: string;
...
export function extractAgentUiPlanApprovalProjection(
  metadata: unknown,
): AgentUiPlanApprovalProjection | null {
...
export function extractAgentUiPlanApprovalResponseProjection(
  metadata: unknown,
): AgentUiPlanApprovalResponseProjection | null {
  const record = readRecord(metadata);
...
export function buildAgentUiPlanApprovalRequiredEvent(params: {
  base: AgentUiPlanApprovalProjectionBase;
  projection: AgentUiPlanApprovalProjection;
...
export function buildAgentUiPlanApprovalResolvedEvent(params: {
  base: AgentUiPlanApprovalProjectionBase;
  projection: AgentUiPlanApprovalResponseProjection;
  persistence: AgentUiPersistence;
  toolCallId?: string;
}): AgentUiProjectionEvent {
  const { base, projection, persistence, toolCallId } = params;
