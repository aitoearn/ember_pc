...
const OPTIONS_TRANSLATIONS = {
  en: {
    title: 'Ember Browser Bridge',
    page_title_text: 'Ember Browser Bridge',
    page_subtitle: 'Connect your browser to Ember.',
    language_label: 'Language',
    section_attached_tabs: 'Attached Tabs',
    tabs_empty: 'No tabs attached',
    section_quick_start: 'Quick Start',
    quick_start_1: 'Make sure <strong>Ember</strong> is running with browser control enabled.',
    quick_start_2: 'The relay connects automatically — a green dot on the icon means ready.',
    quick_start_3: 'Ask Ember a browser-related question — it can now see and control your tabs.',
    section_how_it_works: 'How It Works',
    how_always_on_title: 'Always-on relay',
    how_always_on_body: 'The relay is enabled by default and connects automatically when Ember is running. Use the toggle above to pause if needed. Agent-opened tabs are grouped under an <strong>"Ember Agent"</strong> tab group.',
    ...
    indicator_setup_required: 'Setup required — sync config from Ember or paste config in the toolbar popup',
    ...
    troubleshooting_setup_body: 'This extension does not have <code>serverUrl</code> / <code>bridgeKey</code> yet. Load the exported <code>Ember Browser Connector</code> folder from Ember, or open the toolbar popup and paste the config copied from Ember.',
    troubleshooting_connecting_title: 'Stays on yellow dot (connecting)',
    troubleshooting_connecting_body: 'Relay server not reachable. Ensure Ember is running with browser control enabled.',
    ...
    settings_tab_group_label: 'Close "Ember Agent" tab group when relay is disabled',
    settings_control_port_title: 'Control Port',
    settings_control_port_body: 'Only change if you use a custom port in Ember config.',
    ...
    state_unconfigured_detail: 'Ember relay config is missing. Export the connector from Ember, or paste config in the toolbar popup.',
    ...
    unconfigured_chip: 'Load the exported Ember Browser Connector folder or paste config in the toolbar popup.',
  ...
  zh: {
    title: 'Ember 浏览器桥接',
    page_title_text: 'Ember 浏览器桥接',
    page_subtitle: '把你的浏览器连接到 Ember。',
    language_label: '语言',
    section_attached_tabs: '已附着标签页',
    tabs_empty: '当前没有附着标签页',
    section_quick_start: '快速开始',
    quick_start_1: '确认 <strong>Ember</strong> 已启动，并且已经开启浏览器控制。',
    quick_start_2: 'Relay 会自动连接，图标变成绿色表示就绪。',
    quick_start_3: '直接向 Ember 提一个浏览器相关问题，它现在就能看到并控制你的标签页。',
    section_how_it_works: '工作方式',
    how_always_on_title: '常驻 Relay',
    how_always_on_body: 'Relay 默认启用，只要 Ember 正在运行就会自动连接。需要暂停时再用上方开关关闭。代理新开的标签页会被归到 <strong>“Ember Agent”</strong> 标签组里。',
    ...
    indicator_setup_required: '需要先完成配置，请从 Ember 同步配置或在工具栏弹窗里粘贴配置',
    ...
    troubleshooting_setup_body: '当前扩展还没有拿到 <code>serverUrl</code> / <code>bridgeKey</code>。请从 Ember 加载导出的 <code>Ember Browser Connector</code> 目录，或者打开工具栏弹窗，粘贴从 Ember 复制的配置。',
    troubleshooting_connecting_title: '一直停在黄色状态点',
    troubleshooting_connecting_body: '说明 Relay 服务不可达。请确认 Ember 已启动，并开启了浏览器控制。',
    ...
    settings_tab_group_label: '关闭 Relay 时一并关闭 “Ember Agent” 标签组',
    settings_control_port_title: '控制端口',
    settings_control_port_body: '只有在你给 Ember 配了自定义端口时才需要修改。',
    ...
    state_unconfigured_detail: '当前还没有 Ember Relay 配置。请从 Ember 导出扩展，或在工具栏弹窗里粘贴配置。',
    ...
    unconfigured_chip: '请加载 Ember 导出的 Ember Browser Connector 目录，或在工具栏弹窗里粘贴配置。',
...
relayToggle.addEventListener('change', async () => {
  ...
  try {
    ...
    console.warn('[ember-options] toggle failed:', err)
  } finally {
    toggleBusy = false
  }
})
