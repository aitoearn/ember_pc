...
const DELETED_AGENT_PATHS = [
  "ember-rs/crates/agent/src/agent_runtime_projection.rs",
  "ember-rs/crates/agent/src/agent_session_store.rs",
  "ember-rs/crates/agent/src/agent_session_store",
  "ember-rs/crates/agent/src/event_converter.rs",
  "ember-rs/crates/agent/src/identity_adapter.rs",
  "ember-rs/crates/agent/src/message_content_adapter.rs",
  "ember-rs/crates/agent/src/native_tools/runtime_tool_bridge.rs",
  "ember-rs/crates/agent/src/runtime_conversation_agent_adapter.rs",
  "ember-rs/crates/agent/src/runtime_snapshot_adapter.rs",
  "ember-rs/crates/agent/src/runtime_state/gateway_registration_tests.rs",
  "ember-rs/crates/agent/src/runtime_store_agent_adapter.rs",
  "ember-rs/crates/agent/src/runtime_timeline_adapter.rs",
  "ember-rs/crates/agent/src/session_config_adapter.rs",
  "ember-rs/crates/agent/src/execution_strategy_compat.rs",
  "ember-rs/crates/agent/src/session_execution_runtime_adapter.rs",
  "ember-rs/crates/agent/src/session_execution_runtime/runtime_payload.rs",
  "ember-rs/crates/agent/src/session_execution_runtime_query.rs",
  "ember-rs/crates/agent/src/session_runtime_conversation_query.rs",
  "ember-rs/crates/agent/src/session_state_snapshot.rs",
  "ember-rs/crates/agent/src/session_store.rs",
  "ember-rs/crates/agent/src/session_store_history_visibility.rs",
  "ember-rs/crates/agent/src/session_store_message_projection.rs",
  "ember-rs/crates/agent/src/session_store_provider_routing.rs",
  "ember-rs/crates/agent/src/session_store_runtime_detail.rs",
  "ember-rs/crates/agent/src/session_store_runtime_projection.rs",
  "ember-rs/crates/agent/src/session_store_subagent_context.rs",
  "ember-rs/crates/agent/src/session_store_subagent_projection.rs",
  "ember-rs/crates/agent/src/session_store_subagent_query.rs",
  "ember-rs/crates/agent/src/session_store_todo_agent_adapter.rs",
  "ember-rs/crates/agent/src/session_store_todo_projection.rs",
  "ember-rs/crates/agent/src/session_store_tests.rs",
  "ember-rs/crates/agent/src/session_store_types.rs",
  "ember-rs/crates/agent/src/subagent_control.rs",
  "ember-rs/crates/agent/src/subagent_profiles.rs",
  "ember-rs/crates/agent/src/subagent_runtime_adapter.rs",
  "ember-rs/crates/agent/src/tools/skill_tool_gate.rs",
  "ember-rs/crates/agent/src/turn_context_configuration/agent_adapter.rs",
];

const DELETED_THREAD_STORE_PATHS = [
  "ember-rs/crates/thread-store/src/conversation_transcript.rs",
  "ember-rs/crates/thread-store/src/history_search.rs",
  "ember-rs/crates/thread-store/src/in_memory_runtime_store.rs",
  "ember-rs/crates/thread-store/src/legacy_conversation.rs",
  "ember-rs/crates/thread-store/src/runtime_status_item.rs",
  "ember-rs/crates/thread-store/src/runtime_store.rs",
  "ember-rs/crates/thread-store/src/session_insights.rs",
  "ember-rs/crates/thread-store/src/sqlite_runtime_store.rs",
];

const DELETED_DEAD_SURFACE_PATHS = [
  "ember-rs/crates/thread-store/src/subagent_tree.rs",
  "ember-rs/crates/agent/src/team_runtime_governor.rs",
];
...
describe("Agent migration boundary", () => {
  it("agent-compat 必须保持物理删除", () => {
    expect(existsSync(repoPath("ember-rs/crates/agent-compat"))).toBe(false);
  });
  ...
  it("workspace 与 lime-agent 不得恢复 Agent 依赖", () => {
    const manifests = [
      ...
      ...CURRENT_RUNTIME_CRATES.map(
        (crateName) => `ember-rs/crates/${crateName}/Cargo.toml`,
      ),
    ];
  ...
  it("current runtime crates 不得导入 Agent", () => {
    const files = CURRENT_RUNTIME_CRATES.flatMap((crateName) =>
      filesUnder(`ember-rs/crates/${crateName}/src`).filter((path) =>
        path.endsWith(".rs"),
      ),
    );
  ...
  it("lime-agent 生产源码不得导入 Agent", () => {
    const files = filesUnder("ember-rs/crates/agent/src").filter((path) =>
      path.endsWith(".rs"),
    );
    const leaks = files.filter((path) =>
      /\baster::|\buse\s+agent\b|extern\s+crate\s+agent/u.test(read(path)),
  ...
  it("已删除的 lime-agent Agent adapter 不得恢复", () => {
    ...
    expect(
      filesUnder("ember-rs/crates/agent/src/request_tool_policy").filter(
  ...
  it("已删除的 dead roster/runtime surface 不得恢复", () => {
    ...
    const threadStoreSource = read("ember-rs/crates/thread-store/src/lib.rs");
    expect(threadStoreSource).not.toContain("pub mod subagent_tree");

    const agentSource = read("ember-rs/crates/agent/src/lib.rs");
  ...
  it("Runtime session 不得恢复 app-data fallback 或 hydration helper", () => {
    expect(
      existsSync(
        repoPath(
          "ember-rs/crates/app-server/src/runtime/session_hydration.rs",
        ),
      ),
    ).toBe(false);
  ...
  it("provider current owner 必须保留 OpenCode 风格中立 part 与集中 lowering", () => {
    const source = read("ember-rs/crates/model-provider/src/current_client.rs");
  ...
  it("provider turn 必须消费 current provider 与 typed tool executor", () => {
    const source = read("ember-rs/crates/agent-runtime/src/provider_turn.rs");
  ...
  it("MCP current owner 必须由 tool-runtime registry 和每回合 snapshot 分工", () => {
    const registrySource = read(
      "ember-rs/crates/tool-runtime/src/mcp_connection/registry.rs",
    );
    const snapshotSource = read(
      "ember-rs/crates/tool-runtime/src/mcp_connection/step_snapshot.rs",
    );
  ...
  it("request_user_input pending state 必须归属 session/turn current state", () => {
    const source = read("ember-rs/crates/agent-runtime/src/action_required.rs");
    const turnSnapshotSource = read(
      "ember-rs/crates/agent/src/current_provider_turn/mcp_step_snapshot.rs",
    );
    const turnExecutorSource = read(
      "ember-rs/crates/agent/src/current_provider_turn/tool_executor.rs",
    );
    const bridgeSource = read(
      "ember-rs/crates/agent/src/request_user_input_bridge.rs",
    );
  ...
  it("ProjectionStore 必须是唯一 Thread/Turn/Item truth，queue payload 单独持久化", () => {
    const support = read("ember-rs/crates/agent/src/runtime_support.rs");
    const projectionStore = read(
      "ember-rs/crates/app-server/src/runtime/projection_store.rs",
    );
    const queueStore = read(
      "ember-rs/crates/agent-runtime/src/runtime_queue/sqlite.rs",
    );
