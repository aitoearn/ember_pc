...
const VENDOR_CONTEXT_MGMT_PATH =
  "ember-rs/vendor/agent-rust/crates/agent/src/context_mgmt/mod.rs";
const VENDOR_AGENT_PATH =
  "ember-rs/vendor/agent-rust/crates/agent/src/agents/agent.rs";
const CURRENT_CONTEXT_PROJECTION_PATH =
  "ember-rs/crates/agent/src/protocol_context_projection.rs";
const CURRENT_MODEL_POLICY_PATH =
  "ember-rs/crates/agent/src/model_request_policy.rs";
...
const APP_SERVER_AUTO_COMPACTION_PATH =
  "ember-rs/crates/app-server/src/runtime/context_auto_compaction.rs";
...
describe("Agent context policy migration boundary", () => {
  ...
  it("vendor Agent 不得重新承接 selected model context policy / token-limit 规则", () => {
    ...
    expect(
      vendorContextLeaks,
      "context / auto-compact token-limit 新规则必须归属 Lime current owner；vendor Agent context_mgmt 只能保留迁移期旧 threshold 行为，不得解析 selected model context_policy",
    ).toEqual([]);
    expect(
      vendorAgentLeaks,
      "Agent Agent 只能作为兼容执行器；不得在 provider loop 中重新实现 selected model context / auto-compact policy",
    ).toEqual([]);
  });
});
