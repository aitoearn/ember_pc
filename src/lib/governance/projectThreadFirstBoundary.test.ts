...
const PROJECT_THREAD_IDENTITY_BOUNDARY_FILES = [
  ...
  "ember-rs/crates/core/src/database/agent_session_repository.rs",
  "ember-rs/crates/thread-store/src/session_record.rs",
  "ember-rs/crates/thread-store/src/session_repository.rs",
  ...
  "ember-rs/crates/app-server/src/runtime/memory_prompt.rs",
  "ember-rs/crates/app-server/src/memory_store.rs",
  "ember-rs/crates/app-server/src/processor/agent_session.rs",
  "ember-rs/crates/app-server/src/processor/memory_store.rs",
  "ember-rs/crates/app-server/src/local_data_source/impls/sessions.rs",
  ...
  "ember-rs/crates/app-server-protocol/src/protocol/v0/model.rs",
...
const AUTOMATION_THREAD_FIRST_SURFACE_FILES = [
  "ember-rs/crates/app-server/src/automation_execution.rs",
...
const SUBAGENT_THREAD_FIRST_SURFACE_FILES = [
  ...
  "ember-rs/crates/app-server/src/runtime/tests/evidence_exports/team_facts.rs",
...
function projectThreadSchemaFiles(): string[] {
  return collectFiles(
    "ember-rs/crates/app-server-protocol/schema/json/v0",
    (file) =>
      file.endsWith(".json") &&
...
describe("Project / Thread-first boundary", () => {
  ...
  it("AgentSessionStartParams 只通过 businessObjectRef.metadata 承载能力来源", () => {
    const schema = readJsonFile(
      "ember-rs/crates/app-server-protocol/schema/json/v0/AgentSessionStartParams.json",
    ) as {
      $defs?: {
        BusinessObjectRef?: {
  ...
  it("MemoryStoreScope 只能按 global / workspace 组织长期记忆", () => {
    const schema = readJsonFile(
      "ember-rs/crates/app-server-protocol/schema/json/v0/MemoryStoreScope.json",
    ) as { enum?: string[] };
  ...
  it("子代理和 Team facts 必须挂在 parent thread，不得恢复独立子代理历史入口", () => {
    ...
    const evidenceProviderSource = readFileSync(
      repoPath("ember-rs/crates/app-server/src/runtime/evidence_provider.rs"),
      "utf8",
    );
