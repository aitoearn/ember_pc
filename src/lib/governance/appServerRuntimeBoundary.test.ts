...
describe("app-server runtime boundary", () => {
  ...
  it("App Server 只能通过 agent runtime registry 初始化 Agent runtime", () => {
    const offenders = collectRustFiles(APP_SERVER_SRC_DIR)
      ...
      .filter(
        (path) =>
          path !== "ember-rs/crates/app-server/src/agent_runtime_registry.rs",
      );
  ...
  it("App Server 不应新增 runtime_backend 之外的 Agent 回合执行链", () => {
    const unregistered = collectRustFiles(APP_SERVER_SRC_DIR)
      ...
      .filter(
        ({ path }) =>
          path !== "ember-rs/crates/app-server/src/runtime_backend.rs" &&
          !path.startsWith("ember-rs/crates/app-server/src/runtime_backend/") &&
  ...
  it("Knowledge Builder App Server adapter 不应直接持有 Agent 状态", () => {
    const adapter = readFileSync(
      join(
        REPO_ROOT,
        "ember-rs/crates/app-server/src/runtime_backend/knowledge_builder_runtime.rs",
      ),
      "utf8",
    );
  ...
  it("runtime_backend 主文件不应重新承接 action response 或事件 mapper 细节", () => {
    const runtimeBackend = readFileSync(
      join(REPO_ROOT, "ember-rs/crates/app-server/src/runtime_backend.rs"),
      "utf8",
    );
  ...
  it("App Server 主 turn 执行不应直接调用 Agent streaming loop", () => {
    ...
    const agentTurnExecution = readFileSync(
      join(REPO_ROOT, "ember-rs/crates/agent/src/turn_execution.rs"),
      "utf8",
    );
  ...
  it("App Server 不应直接构造 Agent SessionConfig", () => {
    ...
    expect(
      existsSync(
        join(REPO_ROOT, "ember-rs/crates/agent/src/session_config_adapter.rs"),
      ),
  ...
  it("workspace patch host tool plan 和 evidence 语义应下沉到 lime-agent", () => {
    const appServerAdapter = productionSource(
      join(
        REPO_ROOT,
        "ember-rs/crates/app-server/src/runtime_backend/workspace_patch_host_tools.rs",
      ),
    );
    const agentBoundary = readFileSync(
      join(
  ...
  it("App Server 受控文本生成 adapter 不应重新复制 Agent streaming loop", () => {
    const pluginWorkerAdapter = {
      path: "ember-rs/crates/app-server/src/runtime_backend/plugin_worker_generation.rs",
      source: readFileSync(
        join(
          REPO_ROOT,
    ...
    const imagePresentationAdapter = {
      path: "ember-rs/crates/app-server/src/runtime_backend/image_command/presentation.rs",
      source: readFileSync(
        join(
          REPO_ROOT,
          "ember-rs/crates/app-server/src/runtime_backend/image_command/presentation.rs",
    ...
    const agentHostManagedGeneration = readFileSync(
      join(REPO_ROOT, "ember-rs/crates/agent/src/host_managed_generation.rs"),
      "utf8",
    );
    const agentDirectTextGeneration = readFileSync(
      join(REPO_ROOT, "ember-rs/crates/agent/src/direct_text_generation.rs"),
      "utf8",
    );
  ...
  it("App Server native tool adapter 不应重新实现 Agent Tool surface", () => {
    ...
    const adapters = [
      {
        path: "ember-rs/crates/app-server/src/runtime_backend/image_tools.rs",
        requiredSnippets: ["image_task_gateway", "ImageTaskGateway"],
      },
      {
        path: "ember-rs/crates/app-server/src/runtime_backend/memory_tools.rs",
  ...
  it("App Server execution process 不应直接注册 Agent shell tools", () => {
    const appServerExecutionProcess = productionSource(
      join(REPO_ROOT, "ember-rs/crates/app-server/src/execution_process.rs"),
    );
    const toolRuntimeShellPermission = readFileSync(
      join(REPO_ROOT, "ember-rs/crates/tool-runtime/src/shell_permission.rs"),
      "utf8",
    );
    const forbiddenSnippets = [
      "BashTool",
