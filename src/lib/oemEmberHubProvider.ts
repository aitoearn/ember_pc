...
export const OEM_EMBER_HUB_PROVIDER_ID = "ember-hub";
export const DEFAULT_OEM_EMBER_HUB_PROVIDER_NAME = "Ember Hub";
...
function withLimeTenantFragment(baseUrl: string, tenantId: string): string {
  ...
  params.set("ember_tenant_id", normalizedTenantId);
  url.hash = params.toString();
  return url.toString().replace(/\/#/, "#");
}

export function buildOemEmberHubApiHost(
  runtime:
...
export function resolveOemEmberHubProviderName(
...
export function isLegacyProxyCastHubProvider(
  ...
  if (providerId === OEM_EMBER_HUB_PROVIDER_ID) {
    return false;
  }
...
export function isOemManagedHubProvider(
  ...
  const providerId = normalizeText(provider.id).toLowerCase();
  return (
    providerId === OEM_EMBER_HUB_PROVIDER_ID ||
