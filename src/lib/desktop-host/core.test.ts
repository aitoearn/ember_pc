...
describe("desktop-host/core invoke", () => {
  ...
  it("知识库 legacy 显式 mock 已退场", async () => {
    for (const command of [
      ...
      await expect(
        invokeMockOnly(command, {
          request: {
            workingDir: "/tmp/ember-knowledge-e2e",
    ...
    expect(result).toEqual(
      expect.objectContaining({
        ...
        catalog_tools: expect.arrayContaining([
          ...
          expect.objectContaining({ name: "ember_site_list" }),
          expect.objectContaining({ name: "ember_site_run" }),
          expect.objectContaining({
            name: "mcp__ember-browser__",
        ...
        registry_tools: expect.arrayContaining([
          ...
          expect.objectContaining({ name: "ember_site_search" }),
        ]),
        extension_tools: expect.arrayContaining([
          ...
          expect.objectContaining({
            name: "mcp__ember-browser__click",
            status: "deferred",
  ...
  it("工具库存显式 mock 应按 workbench + browser surface 补齐当前工具面", async () => {
    ...
    expect(result.default_allowed_tools).not.toContain("mcp__ember-browser__");
    expect(result.catalog_tools).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ name: "ember_run_service_skill" }),
        expect.objectContaining({ name: "ember_site_recommend" }),
        expect.objectContaining({ name: "mcp__ember-browser__" }),
      ]),
    );
    expect(result.registry_tools).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ name: "social_generate_cover_image" }),
        expect.objectContaining({ name: "ember_search_web_images" }),
        expect.objectContaining({ name: "ember_create_typesetting_task" }),
        expect.objectContaining({ name: "ember_site_info" }),
      ]),
    );
    expect(result.counts.mcp_tool_total).toBe(20);
    expect(result.mcp_tools).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ name: "mcp__ember-browser__navigate" }),
        expect.objectContaining({ name: "mcp__ember-browser__read_page" }),
        expect.objectContaining({ name: "mcp__ember-browser__click" }),
      ]),
    );
    expect(mocks.invokeViaHttp).not.toHaveBeenCalled();
  });
