...
import {
  ...
  isOfficialSkillMarketplaceRemoteEnabled,
...
describe("officialSkillMarketplace", () => {
  beforeEach(() => {
    ...
    vi.stubEnv("VITE_EMBER_SKILL_MARKETPLACE_REMOTE_ENABLED", "true");
  });
  ...
  it("默认不请求远程官方技能市场", async () => {
    vi.unstubAllEnvs();
    const fetchMock = vi.mocked(fetch);

    await expect(
      listOfficialSkillMarketplace({ sort: "default" }),
    ).resolves.toEqual([]);
    expect(fetchMock).not.toHaveBeenCalled();
    expect(isOfficialSkillMarketplaceRemoteEnabled()).toBe(false);
  });

  it("远程市场关闭时不应拉取安装包", async () => {
    vi.unstubAllEnvs();

    await expect(getOfficialSkillMarketplaceBundle("analysis")).rejects.toThrow(
      "The official skill marketplace remote API is disabled",
    );
  ...
  it("应按官网 marketplace envelope 解析官方技能列表", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "https://ember-api.emberai.run/api/v1/public/service-skills/marketplace?sort=default",
  ...
  it("应解析官方技能安装包并保留文件校验信息", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "https://ember-api.emberai.run/api/v1/public/service-skills/marketplace/analysis/bundle",
      expect.any(Object),
    );
  });
