...
export const AGENT_APPS_CHANGED_EVENT = "ember:agent-apps-changed";
...
function readBootstrapAgentAppCatalog(): CloudBootstrapPayload | null {
  ...
  try {
    return normalizeCloudCatalogPayload(window.__EMBER_BOOTSTRAP__);
  } catch {
    return null;
  }
}
...
export async function submitAgentAppRegistrationCode(
  ...
  const runtime = resolveOemCloudRuntimeContext();
  if (!runtime) {
    throw new Error("Agent App 注册需要先登录 Ember Cloud账号。");
  }
  return {
