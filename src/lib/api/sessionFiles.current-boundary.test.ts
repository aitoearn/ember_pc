...
describe("Session files frontend boundary", () => {
  ...
  it("Session files 旧 Rust Tauri wrapper / runner / DevBridge command surface 不得回流", () => {
    expect(
      existsSync(resolve(cwd(), "ember-rs/src/commands/session_files_cmd.rs")),
    ).toBe(false);

    const commandsModSource = readOptionalRepoFile("ember-rs/src/commands/mod.rs");
    ...
    const dispatcherSource = readOptionalRepoFile(
      "ember-rs/src/dev_bridge/dispatcher/files.rs",
    );
    expect(existsSync(resolve(cwd(), "ember-rs/src/app/runner.rs"))).toBe(
      false,
    );
    expect(
      existsSync(resolve(cwd(), "ember-rs/src/dev_bridge/dispatcher/files.rs")),
    ).toBe(false);
