...
import type {
  ...
  AgentRuntimeEvidenceEmberCorePolicyEvaluation,
  AgentRuntimeEvidenceEmberCorePolicyIndex,
  AgentRuntimeEvidenceEmberCorePolicyInput,
  AgentRuntimeEvidenceEmberCorePolicyItem,
  AgentRuntimeEvidenceEmberCorePolicyValueHit,
...
function normalizeEmberCorePolicyItem(
  value: unknown,
): AgentRuntimeEvidenceEmberCorePolicyItem | null {
  if (!isRecord(value)) {
    return null;
  }
  ...
  const policyInputs = readArrayField(value, "policyInputs", "policy_inputs")
    .map((entry: unknown) => normalizeEmberCorePolicyInput(entry))
    .filter(Boolean) as AgentRuntimeEvidenceEmberCorePolicyInput[];
  ...
  const policyValueHits = readArrayField(
    ...
    .map((entry: unknown) => normalizeEmberCorePolicyValueHit(entry))
    .filter(Boolean) as AgentRuntimeEvidenceEmberCorePolicyValueHit[];
  ...
  const policyEvaluation = normalizeEmberCorePolicyEvaluation(
    value.policyEvaluation ?? value.policy_evaluation,
  );
  const item: AgentRuntimeEvidenceEmberCorePolicyItem = {
    artifact_path: readOptionalStringField(
      value,
      "artifactPath",
      "artifact_path",
...
function normalizeEmberCorePolicyEvaluation(
  value: unknown,
): AgentRuntimeEvidenceEmberCorePolicyEvaluation | undefined {
  if (!isRecord(value)) {
    return undefined;
  }

  const evaluation: AgentRuntimeEvidenceEmberCorePolicyEvaluation = {
    status: readOptionalStringField(value, "status"),
    decision: readOptionalStringField(value, "decision"),
    decision_source: readOptionalStringField(
      value,
...
function normalizeEmberCorePolicyInput(
  value: unknown,
): AgentRuntimeEvidenceEmberCorePolicyInput | null {
  if (!isRecord(value)) {
    return null;
  }
...
function normalizeEmberCorePolicyValueHit(
  value: unknown,
): AgentRuntimeEvidenceEmberCorePolicyValueHit | null {
  if (!isRecord(value)) {
    return null;
  }
...
function normalizeEmberCorePolicyIndex(
  value: unknown,
): AgentRuntimeEvidenceEmberCorePolicyIndex | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
  ...
  const index: AgentRuntimeEvidenceEmberCorePolicyIndex = {
    ...
    items: rawItems
      .map((entry: unknown) => normalizeEmberCorePolicyItem(entry))
      .filter(Boolean) as AgentRuntimeEvidenceEmberCorePolicyItem[],
  };
...
function normalizeEvidenceModalityRuntimeContracts(value: unknown) {
  ...
  const emberCorePolicyIndex = normalizeEmberCorePolicyIndex(
    snapshotIndexRecord
      ? readRecordField(
          snapshotIndexRecord,
          "embercorePolicyIndex",
  ...
  const snapshotIndex =
    taskIndex || browserActionIndex || emberCorePolicyIndex
      ? {
          ...
          ...(browserActionIndex
            ...
            : {}),
          ...(emberCorePolicyIndex
            ? { embercore_policy_index: emberCorePolicyIndex }
            : {}),
        }
      : undefined;
