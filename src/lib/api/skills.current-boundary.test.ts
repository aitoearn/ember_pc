...
describe("skillsApi current App Server boundary", () => {
  ...
  it("App Server protocol 和治理 catalog 应记录 Skill 管理 current 方法", () => {
    ...
    const rustProtocolSource = [
      ...
      readRepoFile(
        "ember-rs/crates/app-server-protocol/src/protocol/v0/skills.rs",
      ),
    ].join("\n");
  ...
  it("旧 Skill 管理 facade 不应回到 Electron Host、DevBridge、mock 或 legacy Rust", () => {
    const productionSources = [
      ...
      readOptionalRepoFile("ember-rs/src/dev_bridge/dispatcher/skills.rs"),
    ].join("\n");
    const runnerSource = readOptionalRepoFile("ember-rs/src/app/runner.rs");

    expect(repoFileExists("ember-rs/src/commands/skill_cmd.rs")).toBe(false);
    expect(repoFileExists("ember-rs/src/app/runner.rs")).toBe(false);
    expect(repoFileExists("ember-rs/src/dev_bridge/dispatcher/skills.rs")).toBe(
      false,
    );
