...
function readAppServerAsrCredentialSources(): string {
  return [
    ...
    readRepoFile("ember-rs/crates/app-server-protocol/src/protocol/v0/voice.rs"),
    readRepoFile(
      "ember-rs/crates/app-server/src/local_data_source/voice_asr_credentials.rs",
    ),
    readRepoFile(
...
describe("ASR / Voice current boundary", () => {
  ...
  it("Voice Model catalog / install-state 应固定为 Electron Host current 读链", () => {
    ...
    const runnerSource = readRepoFileIfExists("ember-rs/src/app/runner.rs");
    ...
    expectLegacyRustFileDeleted("ember-rs/src/app/runner.rs");
    expectLegacyRustFileDeleted("ember-rs/src/dev_bridge/dispatcher.rs");
    expect(repoFileExists("ember-rs/src/dev_bridge/dispatcher/voice.rs")).toBe(
      false,
    );
    expect(repoFileExists("ember-rs/src/commands/voice_model_cmd.rs")).toBe(
      false,
    );
  });
  ...
  it("Voice Model test-transcribe 应固定走 App Server current 且不回流旧命令面", () => {
    ...
    const restrictedSources = [
      ...
      readRepoFileIfExists("ember-rs/src/dev_bridge/dispatcher.rs"),
      readRepoFileIfExists("ember-rs/src/dev_bridge/dispatcher/voice.rs"),
      readRepoFileIfExists("ember-rs/src/commands/mod.rs"),
      readRepoFile("src/lib/governance/agentCommandCatalog.json"),
    ].join("\n");
    ...
    expectLegacyRustFileDeleted("ember-rs/src/app/runner.rs");
    expectLegacyRustFileDeleted("ember-rs/src/dev_bridge/dispatcher.rs");
    expectLegacyRustFileDeleted("ember-rs/src/commands/mod.rs");
    expect(repoFileExists("ember-rs/src/commands/voice_model_cmd.rs")).toBe(
      false,
    );
  });
  ...
  it("旧 ASR 凭证 facade 不应回到 Electron、DevBridge、mock 或 legacy Rust", () => {
    ...
    expectLegacyRustFileDeleted("ember-rs/src/dev_bridge/dispatcher.rs");
    expectLegacyRustFileDeleted("ember-rs/src/commands/mod.rs");
    expectLegacyRustFileDeleted("ember-rs/src/commands/asr_cmd.rs");
