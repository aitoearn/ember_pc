...
describe("logs diagnostics current App Server boundary", () => {
  ...
  it("App Server protocol 和治理 catalog 应记录日志诊断 current 方法", () => {
    ...
    const rustProtocolSource = [
      ...
      readRepoFile(
        "ember-rs/crates/app-server-protocol/src/protocol/v0/observability.rs",
      ),
    ].join("\n");
  ...
  it("旧日志诊断 facade 不应回到 Electron Host、DevBridge、mock 或 legacy Rust", () => {
    const productionSources = [
      ...
      readOptionalRepoFile("ember-rs/src/app/runner.rs"),
      readOptionalRepoFile("ember-rs/src/dev_bridge/dispatcher.rs"),
      readOptionalRepoFile("ember-rs/src/dev_bridge/dispatcher/app_runtime.rs"),
      readOptionalRepoFile("ember-rs/src/dev_bridge/dispatcher/logs.rs"),
    ].join("\n");
    const runnerSource = readOptionalRepoFile("ember-rs/src/app/runner.rs");
