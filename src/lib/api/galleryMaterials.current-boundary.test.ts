...
describe("galleryMaterials current App Server boundary", () => {
  ...
  it("App Server protocol / client 应记录 Gallery material current 方法", () => {
    ...
    const rustProtocolSource = [
      ...
      readRepoFile(
        "ember-rs/crates/app-server-protocol/src/protocol/v0/gallery.rs",
      ),
    ].join("\n");
  ...
  it("旧 Gallery material Rust wrapper / runner / dispatcher 不应回流", () => {
    const legacyRustSources = [
      ...
      readOptionalRepoFile("ember-rs/src/dev_bridge/dispatcher/files.rs"),
      readOptionalRepoFile("ember-rs/src/dev_bridge/dispatcher/materials.rs"),
    ].join("\n");
    ...
    expect(
      existsSync(
        resolve(cwd(), "ember-rs/src/commands/gallery_material_cmd.rs"),
      ),
    ).toBe(false);
  });
