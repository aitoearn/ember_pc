...
function normalizeBrowserAssistBackend(
  value: string | null | undefined,
): "ember_extension_bridge" | "cdp_direct" | undefined {
  if (value === "ember_extension_bridge" || value === "cdp_direct") {
    return value;
  }
  return undefined;
}
...
function getWorkspaceText(
  ...
  const instance = i18n.isInitialized ? i18n : initEmberI18n();
      ...
      try {
        const attempts =
          transportKind === "existing_session"
            ? [
                {
                  backend: "ember_extension_bridge" as const,
                  args: {
                    url,
                    wait_for_page_info: true,
                  },
