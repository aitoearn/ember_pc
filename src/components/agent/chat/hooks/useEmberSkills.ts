...
interface UseEmberSkillsOptions {
...
export function useEmberSkills(options: UseEmberSkillsOptions = {}) {
  const {
    autoLoad = "immediate",
    deferredDelayMs = SKILLS_IDLE_TIMEOUT_MS,
    logScope = "useEmberSkills",
    onError,
  } = options;
    ...
    async (includeRemote = false): Promise<Skill[]> => {
      ...
      try {
        const loadedSkills = includeRemote
          ? await skillsApi.getAll("ember")
          : await skillsApi.getLocal("ember");
