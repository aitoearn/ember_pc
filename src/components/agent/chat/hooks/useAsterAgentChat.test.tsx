...
describe("useAsterAgentChat 首页新会话", () => {
  ...
  it("Agent 初始化返回 current provider 但无模型时不应回流旧 Provider 缓存", async () => {
    ...
    try {
      ...
      expect(harness.getValue().providerType).toBe("ember-hub");
    ...
    try {
      ...
      act(() => {
        stream.emit({
          ...
          arguments: JSON.stringify({
            command:
              "ember task create resource-search --json --resource-type image --query 'cozy coffee table'",
          }),
        });
        stream.emit({
          type: "tool_end",
      ...
      expect(assistantMessage?.taskPreview).toMatchObject({
        ...
        status: "running",
        artifactPath: ".ember/tasks/modal_resource_search/task-resource-1.json",
  ...
  it("收到联网搜图 tool_end 后应挂载素材预览并生成可打开的 artifact", async () => {
    ...
    try {
      ...
      act(() => {
        stream.emit({
          type: "tool_start",
          tool_name: "ember_search_web_images",
      ...
      expect(assistantMessage?.taskPreview).toMatchObject({
        ...
        taskType: "modal_resource_search",
        status: "complete",
        artifactPath: ".ember/runtime/resource-search/tool-web-search-1.md",
  ...
  it("命中 /review 时应转换为预置 prompt 后走 chat_stream", async () => {
    ...
    try {
      ...
      await act(async () => {
        await harness
          .getValue()
          .sendMessage("/review ember-rs", [], false, false, false, "react");
      });
      ...
      expect(mockSubmitAgentRuntimeTurn.mock.calls[0]?.[0]).toEqual(
        expect.objectContaining({
          message: expect.stringContaining("ember-rs"),
        }),
      );
    } finally {
      harness.unmount();
...
describe("useAsterAgentChat action_required 渲染链路", () => {
  ...
  it("收到带 Ember 元数据块的 tool_end 后应清洗输出并恢复失败态 metadata", async () => {
    ...
    try {
      ...
      expect(toolCall?.result?.output).not.toContain("Ember 工具元数据");
  ...
  it("收到带 Ember 元数据块的 tool_end error 后应清洗错误文本并恢复失败态 metadata", async () => {
    ...
    try {
      ...
      expect(toolCall?.result?.error).not.toContain("Ember 工具元数据");
      expect(toolCall?.result?.metadata).toMatchObject({
        reported_success: false,
        exit_code: 1,
        stderr_length: 128,
      ...
      act(() => {
        stream.emit({
          ...
          artifact: {
            ...
            filePath: "/tmp/ember/output_image.jpg",
            content: "",
            metadata: {
              complete: true,
            },
      ...
      act(() => {
        stream.emit({
          ...
          artifact: {
            ...
            filePath: ".ember/artifacts/thread-1/source-report.artifact.json",
      ...
      act(() => {
        stream.emit({
          ...
          artifact: {
            ...
            filePath: ".ember/artifacts/thread-1/source-report-2.artifact.json",
            content: JSON.stringify({
              schemaVersion: "artifact_document.v1",
              artifactId: "artifact-doc-search-2",
              kind: "analysis",
...
describe("useAsterAgentChat 偏好持久化", () => {
      ...
      messages: [
        {
          ...
          content: [
            { type: "text", text: "正在生成珠江夜景封面" },
            {
              ...
              arguments: {
                command:
                  'ember media image generate --prompt "珠江夜景封面" --size 1024x1024 --count 1',
              },
            },
          ],
        },
  ...
  it("切换话题时应清洗 tool_response error 中的 Ember 元数据块", async () => {
