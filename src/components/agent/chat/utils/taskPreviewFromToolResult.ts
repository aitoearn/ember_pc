...
const WEB_IMAGE_SEARCH_TOOL_NAMES = new Set(["ember_search_web_images"]);
const AUDIO_TASK_PREVIEW_ARTIFACT_ROOT = ".ember/runtime/audio-generate";
const TRANSCRIPTION_TASK_PREVIEW_ARTIFACT_ROOT =
  ".ember/runtime/transcription-generate";
...
function extractImageTaskPromptFromToolArguments(
  ...
  try {
    ...
    if (
      ...
      (command.includes("ember media image generate") ||
        command.includes("ember task create image"))
    ) {
      return {
...
function extractVideoTaskPromptFromToolArguments(
  ...
  try {
    ...
    if (
      toolName.trim().toLowerCase() === "bash" &&
      (command.includes("ember media video generate") ||
        command.includes("ember task create video"))
    ) {
      return {
        prompt: readCommandArgumentValue(command, "--prompt"),
...
function buildWebImageSearchArtifactPath(
  ...
  return `.ember/runtime/resource-search/${identifier}.md`;
}
  ...
  const transcriptBody =
    preview.status === "failed"
      ...
      : transcriptPath
        ...
        : "当前步骤只创建标准 transcription_generate 任务产物；ember-transcription-worker 会回写 transcript.completed 或 transcript.failed，不回退 frontend ASR。";
