...
describe("buildImageTaskPreviewFromToolResult", () => {
  it("应在图片任务完成后输出更友好的完成态摘要", () => {
    const preview = buildImageTaskPreviewFromToolResult({
      ...
      toolArguments: JSON.stringify({
        command: 'ember media image generate --prompt "未来感青柠实验室"',
      }),
      toolResult: {
        metadata: {
          task_id: "task-1",
          ...
          artifact_path: ".ember/tasks/image_generate/task-1.json",
          requested_count: 2,
          received_count: 2,
        },
      },
    ...
    expect(preview).toMatchObject({
      ...
      artifactPath: ".ember/tasks/image_generate/task-1.json",
      phase: "succeeded",
  ...
  it("图片任务完成但尚未带回数量时，应输出面向用户的完成态文案", () => {
    const preview = buildImageTaskPreviewFromToolResult({
      ...
      toolName: "Bash",
      toolArguments: JSON.stringify({
        command: 'ember media image generate --prompt "清晨广州塔"',
      }),
      toolResult: {
        metadata: {
          task_id: "task-3",
  ...
  it("图片任务刚提交时，应输出同会话生成态文案", () => {
    const preview = buildImageTaskPreviewFromToolResult({
      ...
      toolName: "Bash",
      toolArguments: JSON.stringify({
        command: 'ember media image generate --prompt "广州塔夜景"',
      }),
      toolResult: {
        metadata: {
          task_id: "task-4",
  ...
  it("应从纯 JSON 工具输出恢复图片轻卡，而不是依赖额外 metadata", () => {
    const preview = buildImageTaskPreviewFromToolResult({
      ...
      toolResult: {
        ...
        output: JSON.stringify({
          ...
          path: ".ember/tasks/image_generate/task-json-image-1.json",
          absolute_path:
            "/workspace/.ember/tasks/image_generate/task-json-image-1.json",
          artifact_path: ".ember/tasks/image_generate/task-json-image-1.json",
          progress: {
    ...
    expect(preview).toMatchObject({
      ...
      taskFilePath:
        "/workspace/.ember/tasks/image_generate/task-json-image-1.json",
      artifactPath: ".ember/tasks/image_generate/task-json-image-1.json",
  ...
  it("3x3 分镜完成后应输出更贴近布局语义的摘要", () => {
    const preview = buildImageTaskPreviewFromToolResult({
      ...
      toolArguments: JSON.stringify({
        command:
          'ember media image generate --prompt "三国主要人物" --layout-hint storyboard_3x3',
      }),
      toolResult: {
        metadata: {
          task_id: "task-5",
...
describe("buildTaskPreviewFromToolResult web image search", () => {
  it("联网搜图工具应输出图片候选预览与带来源的 artifact document", () => {
    ...
    const preview = buildTaskPreviewFromToolResult({
      ...
      toolName: "ember_search_web_images",
      toolArguments: JSON.stringify({
        query: "cozy coffee table",
        count: 2,
        aspect: "landscape",
    ...
    expect(preview).toMatchObject({
      ...
      title: "Pexels 图片候选",
      status: "complete",
      artifactPath: ".ember/runtime/resource-search/tool-web-image-1.md",
      providerId: "pexels",
      phase: "completed",
      statusMessage:
    ...
    expect(artifact).toMatchObject({
      filePath: ".ember/runtime/resource-search/tool-web-image-1.md",
      metadata: {
...
describe("buildTaskPreviewFromToolResult video", () => {
  it("视频任务完成但尚未带回结果地址时，应输出完成态同步文案", () => {
    const preview = buildTaskPreviewFromToolResult({
      ...
      toolArguments: JSON.stringify({
        command:
          'ember media video generate --prompt "广州塔城市宣传片" --duration 15 --aspect-ratio 16:9 --resolution 720p',
      }),
      toolResult: {
        metadata: {
  ...
  it("视频任务排队中时，应输出用户可理解的排队态文案", () => {
    const preview = buildTaskPreviewFromToolResult({
      ...
      toolArguments: JSON.stringify({
        command:
          'ember media video generate --prompt "新品发布短视频" --duration 15 --aspect-ratio 16:9 --resolution 720p',
      }),
      toolResult: {
        metadata: {
...
describe("buildTaskPreviewFromToolResult audio", () => {
  it("配音任务应输出 audio_generate 预览并指向可打开的运行时文档", () => {
    const preview = buildTaskPreviewFromToolResult({
      toolId: "tool-audio-1",
      toolName: "ember_create_audio_generation_task",
      toolArguments: JSON.stringify({
        sourceText: "欢迎来到 Ember 多模态工作台。",
        voice: "warm_female",
        voiceStyle: "温暖克制",
  ...
  describe("buildTaskPreviewFromToolResult transcription", () => {
    it("转写任务应输出 transcription_generate 预览并指向运行时文档", () => {
      const preview = buildTaskPreviewFromToolResult({
        ...
        toolName: "ember_create_transcription_task",
        toolArguments: JSON.stringify({
          prompt: "请转写访谈音频",
    ...
    it("转写任务工具结果应生成 transcript viewer 文档，避免打开隐藏 task json", () => {
      const artifact = buildToolResultArtifactFromToolResult({
        ...
        toolResult: {
          metadata: {
            ...
            status: "succeeded",
            artifact_path:
              ".ember/tasks/transcription_generate/task-transcription-2.json",
            transcript_path:
              ".ember/runtime/transcripts/task-transcription-2.txt",
            transcript_text: "欢迎来到 Ember 访谈节目。",
            transcript_segments: [
              {
                start: 1,
                end: 3.2,
      ...
      expect(artifact).toMatchObject({
        filePath:
          ".ember/runtime/transcription-generate/task-transcription-2.md",
        metadata: {
          artifact_type: "document",
          ...
          taskFilePath:
            ".ember/tasks/transcription_generate/task-transcription-2.json",
          transcriptPath: ".ember/runtime/transcripts/task-transcription-2.txt",
  ...
  it("配音任务工具结果应生成轻量 artifact document，避免打开隐藏 task json", () => {
    const artifact = buildToolResultArtifactFromToolResult({
      ...
      toolResult: {
        metadata: {
          ...
          task_type: "audio_generate",
          status: "succeeded",
          artifact_path: ".ember/tasks/audio_generate/task-audio-2.json",
    ...
    expect(artifact).toMatchObject({
      filePath: ".ember/runtime/audio-generate/task-audio-2.md",
      metadata: {
        artifact_type: "document",
        taskId: "task-audio-2",
        taskType: "audio_generate",
        taskFilePath: ".ember/tasks/audio_generate/task-audio-2.json",
