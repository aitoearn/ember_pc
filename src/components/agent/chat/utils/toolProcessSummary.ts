...
const EMBER_TASK_SUMMARY_LABELS: Partial<
  Record<string, { key: string; defaultValue: string }>
> = {
  embercreatevideogenerationtask: {
    key: "label.videoGeneration",
    defaultValue: "视频生成",
  },
  embercreateaudiogenerationtask: {
    key: "label.audioGeneration",
    defaultValue: "配音生成",
  },
  embercreatetranscriptiontask: {
    key: "label.transcription",
    defaultValue: "转写",
  },
  embercreatebroadcastgenerationtask: {
    key: "label.broadcastGeneration",
    defaultValue: "口播生成",
  },
  embercreatecovergenerationtask: {
    key: "label.coverGeneration",
    defaultValue: "封面生成",
  },
  embercreateresourcesearchtask: {
    key: "label.resourceSearch",
    defaultValue: "素材检索",
  },
  embercreatemodalresourcesearchtask: {
    key: "label.resourceSearch",
    defaultValue: "素材检索",
  },
  embercreateimagegenerationtask: {
    key: "label.imageGeneration",
    defaultValue: "图片生成",
  },
  embercreateurlparsetask: {
    key: "label.urlParse",
    defaultValue: "链接解析",
  },
  embercreatetypesettingtask: {
...
function buildEmberTaskSummary(
  phase: "pre" | "post",
  ...
  const taskLabel = EMBER_TASK_SUMMARY_LABELS[normalizedName];
  if (!taskLabel) {
    return null;
  }
...
function buildSiteToolSummary(
  ...
  if (normalizedName === "embersitelist") {
    return phase === "pre" ? "先查看可用站点能力" : "已查看可用站点能力";
  }

  if (normalizedName === "embersiterecommend") {
    return normalizedSubject
  ...
  if (normalizedName === "embersitesearch") {
    return normalizedSubject
  ...
  if (normalizedName === "embersiteinfo") {
    return normalizedSubject
  ...
  if (normalizedName === "embersiterun") {
    return normalizedSubject
...
function buildGenericPostSummary(params: {
  ...
  const emberTaskSummary = buildEmberTaskSummary(
    "post",
    normalizedName,
    normalizedSubject,
  );
  if (emberTaskSummary) {
    return emberTaskSummary;
  }
  ...
  if (normalizedName === "emberrunserviceskill") {
    return normalizedSubject
...
function buildNarrative(input: ToolProcessInput): ToolProcessNarrative {
  ...
  if (input.status === "failed") {
    if (emberTaskFailureSummary) {
      postSummary = emberTaskFailureSummary;
      postSource = "error";
    ...
    if (postSummary && !emberTaskFailureSummary) {
