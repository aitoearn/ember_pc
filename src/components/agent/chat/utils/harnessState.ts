...
const EMBER_TOOL_METADATA_BEGIN = "[Ember 工具元数据开始]";
const EMBER_TOOL_METADATA_END = "[Ember 工具元数据结束]";
...
function stripAuxiliaryOutput(raw?: string): string {
  ...
  let normalized = raw;
  const beginIndex = normalized.lastIndexOf(EMBER_TOOL_METADATA_BEGIN);
  const endIndex = normalized.lastIndexOf(EMBER_TOOL_METADATA_END);

  if (beginIndex >= 0 && endIndex >= beginIndex) {
    normalized =
      normalized.slice(0, beginIndex) +
      normalized.slice(endIndex + EMBER_TOOL_METADATA_END.length);
  }

  normalized = normalized.replace(
    /^\[Ember Offload\]\s*完整输出已转存到文件：.+$/gm,
    "",
  );
...
function extractMetadata(
  ...
  const beginIndex = output.lastIndexOf(EMBER_TOOL_METADATA_BEGIN);
  const endIndex = output.lastIndexOf(EMBER_TOOL_METADATA_END);
  if (beginIndex < 0 || endIndex < beginIndex) {
    return null;
  }

  const raw = output
    .slice(beginIndex + EMBER_TOOL_METADATA_BEGIN.length, endIndex)
    .trim();
  if (!raw) return null;
...
function extractOutputSignal(
  ...
  const offloaded =
    normalizeBoolean(metadata?.ember_offloaded) === true ||
    !!offloadFile ||
    output.includes("[Ember Offload]");
