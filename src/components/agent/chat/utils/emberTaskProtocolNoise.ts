...
const EMBER_CREATE_TASK_RE = /\bember_create_([a-z0-9_]+?)_task\b/i;
const INTERNAL_RPC_ERROR_PAIR_RE =
  /(?:^|[^\d])-?\d{4,6}\s*:\s*-?\d{4,6}(?:\b|:)/;
const INTERNAL_TASK_FILE_RE =
  /(?:^|[\s"'`])\.ember[\\/]+tasks[\\/]+[a-z0-9_-]+[\\/]+/i;
...
const EMBER_CREATE_TASK_FAILURE_LABELS: Record<
  string,
...
const NORMALIZED_EMBER_CREATE_TASK_KINDS: Record<string, string> = {
  embercreatevideogenerationtask: "video_generation",
  embercreateaudiogenerationtask: "audio_generation",
  embercreatetranscriptiontask: "transcription",
  embercreatebroadcastgenerationtask: "broadcast_generation",
  embercreatecovergenerationtask: "cover_generation",
  embercreateresourcesearchtask: "resource_search",
  embercreatemodalresourcesearchtask: "modal_resource_search",
  embercreateimagegenerationtask: "image_generation",
  embercreateurlparsetask: "url_parse",
  embercreatetypesettingtask: "typesetting",
};
...
export function extractLimeCreateTaskKind(
  ...
  const match = (value || "").match(EMBER_CREATE_TASK_RE);
  if (match?.[1]) {
    return match[1].toLowerCase();
  }

  const normalized = normalizeLimeProtocolName(value);
  return NORMALIZED_EMBER_CREATE_TASK_KINDS[normalized] || null;
}
...
function hasImageGenerationContext(value?: string | null): boolean {
  ...
  return Boolean(
    ...
    normalized.includes("embercreateimagegenerationtask") ||
...
export function containsInternalLimeTaskProtocolNoise(
  ...
  return (
    ...
    EMBER_CREATE_TASK_RE.test(text) ||
...
export function resolveImageGenerationFailureDisplayText(): string {
  return getEmberI18n().t("agentChat.imageWorkbenchPreview.placeholder.failed", {
    ns: "agent",
    defaultValue: "生成失败",
  });
}
...
export function resolveLimeTaskProtocolFailureDisplayText(params: {
  ...
  const copy = taskKind ? EMBER_CREATE_TASK_FAILURE_LABELS[taskKind] : null;
  return copy
