...
function createFileArtifactItem(
  ...
  return {
    ...
    type: "file_artifact",
    path: ".ember/artifacts/thread-1/analysis-20260328.artifact.json",
  ...
  it("普通 JSON 文件也不应把原始结构直接摊在聊天区", () => {
    const container = renderCard(
      createFileArtifactItem({
        path: ".ember/artifacts/thread-1/runtime-state.json",
    ...
    const container = renderCard(
      createFileArtifactItem({
        ...
        metadata: {
          ...
          snapshotPath: ".ember/checkpoints/thread-1/artifact-demo-v8.json",
          artifactVersionDiff: {
            changedBlocks: [
              {
                blockId: "body-1",
