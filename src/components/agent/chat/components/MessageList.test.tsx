...
const VIDEO_WORKBENCH_TASK_ACTION_EVENT = "ember:video-workbench-task-action";
...
继续验证
    ...
    const messages: Message[] = [
      ...
      },
      {
        ...
        imageWorkbenchPreview: {
          ...
          mode: "generate",
          status: "complete",
          imageUrl: "https://example.com/ember-minimal.png",
          imageCount: 1,
          modelName: "gpt-images-2",
  ...
  it("图片任务消息卡不应在聊天区展示 EmberCore 策略输入标签", () => {
    const now = new Date();
    const messages: Message[] = [
      {
    ...
    const messages: Message[] = [
      {
        ...
        taskPreview: {
          ...
          status: "running",
          artifactPath:
            ".ember/tasks/modal_resource_search/task-resource-1.json",
  ...
  it("配音任务消息卡应展示 audio_generate 预览并支持打开运行时文档", () => {
    ...
    expect(previewCard?.textContent).toContain("欢迎来到 Ember 多模态工作台");
    ...
    const messages: Message[] = [
      {
        ...
        taskPreview: {
          ...
          title: "配音生成任务",
          status: "failed",
          artifactPath: ".ember/runtime/audio-generate/task-audio-failed-1.md",
          taskFilePath: ".ember/tasks/audio_generate/task-audio-failed-1.json",
  ...
  it("转写任务消息卡应展示 transcript 路径与 provider 错误", () => {
    ...
    expect(previewCard?.textContent).toContain("主持人：欢迎来到 Ember 访谈。");
    ...
    const messages: Message[] = [
      {
        ...
        taskPreview: {
          ...
          status: "failed",
          artifactPath:
            ".ember/runtime/transcription-generate/task-transcription-failed-1.md",
          taskFilePath:
            ".ember/tasks/transcription_generate/task-transcription-failed-1.json",
    ...
    const messages: Message[] = [
      {
        ...
        taskPreview: {
          ...
          title: "Pexels 图片候选",
          status: "complete",
          artifactPath: ".ember/runtime/resource-search/tool-1.md",
  ...
  it("不应把 .ember/artifacts 下的内部 artifact 文稿 JSON 渲染成尾部时间线", () => {
    const now = new Date();
    const messages: Message[] = [
      {
      ...
      threadItems: [
        {
          ...
          type: "file_artifact",
          path: ".ember/artifacts/thread-1/report.artifact.json",
          source: "artifact_snapshot",
        ...
        artifacts: [
          {
            ...
            meta: {
              filePath:
                "C:\\Users\\Administrator\\AppData\\Local\\ember\\projects\\default\\山冶工造_PRD_V2_完整版.md",
  ...
  it("聊天主列与助手消息气泡应保持更宽的桌面阅读宽度", () => {
    ...
    const assistantBubble = container.querySelector('[aria-label="Ember"]');
  ...
  it("助手消息不应再渲染旧的继续处理标签或品牌头像", () => {
    ...
    expect(container.querySelector('img[alt="Ember"]')).toBeNull();
  });
        ...
        artifacts: [
          {
            ...
            title: "greeting.ts",
            content:
              "export function greeting() { return 'Hello Ember Runtime'; }",
            status: "complete",
            meta: {
              filePath:
                "/Users/coso/Library/Application Support/ember/projects/demo/src/greeting.ts",
    ...
    const container = render(messages, {
      ...
      threadItems: [
        ...
        },
        {
          ...
          sequence: 4,
          type: "file_artifact",
          path: "/Users/coso/Library/Application Support/ember/projects/code-runtime-fixture/src/greeting.ts",
          source: "tool_result",
  ...
  it("不应把 .ember/tasks 下的内部任务快照 JSON 渲染成用户可见产物卡片", () => {
    const now = new Date();
    const messages: Message[] = [
      {
        ...
        artifacts: [
          {
            ...
            status: "complete",
            meta: {
              filePath: ".ember/tasks/image_generate/task-image-1.json",
  ...
  it("不应把 .ember/artifacts 下的内部 artifact 文稿 JSON 渲染成用户可见产物卡片", () => {
    const now = new Date();
    const messages: Message[] = [
      {
