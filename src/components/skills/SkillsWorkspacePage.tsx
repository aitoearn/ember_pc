...
const skillsPrimaryActionButtonClassName =
  "border border-[color:var(--theme-border)] bg-[linear-gradient(135deg,var(--theme-default)_0%,var(--theme-dark)_100%)] text-white shadow-sm shadow-slate-950/10 hover:opacity-95 disabled:opacity-50";

const skillsSelectedSurfaceClassName =
  "border-[color:var(--theme-border)] bg-[color:var(--theme-subtle)] ring-1 ring-[color:var(--theme-border)]";
  ...
  return (
    <img
      ...
      className={cn(
        ...
        variant === "cover"
          ? "h-full w-full"
          : "h-10 w-10 rounded-xl border border-[color:var(--ember-surface-border)] bg-[color:var(--ember-surface)]",
      )}
    />
  );
}
...
const SKILL_AUTO_LOAD_PREFERENCES_STORAGE_KEY =
  "ember.skills.autoLoadPreferences.v1";
...
export function SkillsWorkspacePage({
  ...
  } = useSkills("ember", { includeRepos: false });
  ...
  const handleRevealLocalSkill = useCallback(
    async (skill: Skill) => {
      ...
      try {
        await skillsApi.revealLocalSkill(skill.directory, "ember");
      } catch (error) {
        toast.error(
  ...
  const handleCreateScaffold = useCallback(
    async (request: CreateSkillScaffoldRequest) => {
      ...
      try {
        const inspection = await skillsApi.createSkillScaffold(request, "ember");
  ...
  useEffect(() => {
    ...
    void skillsApi
      .inspectLocalSkillDetail(directory, "ember")
      .then((result) => {
        if (cancelled) return;
  ...
  const renderInstalledSkillActionMenu = (skill: Skill) => {
    ...
    return (
      <DropdownMenu>
        ...
        <DropdownMenuContent
          align="end"
          className="z-[80] min-w-[210px] rounded-xl border border-[color:var(--ember-surface-border)] bg-[color:var(--ember-surface)] p-1 text-[color:var(--ember-text)] shadow-lg"
        >
          <DropdownMenuItem
            className="rounded-lg px-3 py-2 text-[13px] font-medium hover:bg-[color:var(--ember-surface-hover)]"
          ...
          <DropdownMenuItem
            className={cn(
              "rounded-lg px-3 py-2 text-[13px] font-medium hover:bg-[color:var(--ember-surface-hover)]",
          ...
          {!isProtected ? (
            <>
              <DropdownMenuSeparator className="bg-[color:var(--ember-surface-border)]" />
              <DropdownMenuItem
  ...
  const renderMarketplaceSkillCard = (item: SkillStoreItem, index: number) => {
    ...
    return (
      <article
        ...
        className={cn(
          "group flex min-h-[132px] flex-col rounded-[10px] border bg-[color:var(--ember-surface)] p-4 text-left shadow-sm shadow-[color:var(--ember-shadow-color)] transition hover:border-[color:var(--ember-surface-border-strong)] hover:bg-[color:var(--ember-surface-hover)] hover:shadow-md",
          isSelected
            ? skillsSelectedSurfaceClassName
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="line-clamp-1 text-[15px] font-semibold leading-5 text-[color:var(--ember-text-strong)]">
              {skill.title}
            </h3>
            <p className="mt-0.5 line-clamp-1 text-[12px] leading-4 text-[color:var(--ember-text-muted)]">
              {skill.name}
            </p>
          </div>
        </div>
        <p className="mt-3 line-clamp-2 flex-1 text-[13px] leading-5 text-[color:var(--ember-text)]">
          {summary}
        </p>
        <div className="mt-3 flex items-center justify-between gap-3 text-[12px] leading-4 text-[color:var(--ember-text-muted)]">
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            {canUninstall ? (
              <Button
                ...
                variant="ghost"
                size="sm"
                className="h-7 rounded-full px-2.5 text-[12px] font-semibold text-[color:var(--ember-warning)] hover:bg-[color:var(--ember-warning-soft)]"
            ...
            <Button
              ...
              variant="ghost"
              size="sm"
              className="h-7 rounded-full px-2.5 text-[12px] font-semibold text-[color:var(--ember-text)] hover:bg-[color:var(--ember-surface-hover)]"
              onClick={openDetail}
            >
              {t("skills.workspace.marketplace.action.detail")}
            </Button>
            <Button
              ...
              className={cn(
                ...
                skillsPrimaryActionButtonClassName,
              )}
              disabled={actionDisabled}
    ...
    return (
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-[17px] font-semibold tracking-[-0.01em] text-[color:var(--ember-text-strong)]">
            {title}
          </h2>
          {meta ? (
            <span className="text-[12px] leading-5 text-[color:var(--ember-text-muted)]">
              {meta}
            </span>
          ) : null}
        </div>
  ...
  return (
    <>
      <div className="ember-workbench-theme-scope flex h-full min-h-0 flex-col overflow-hidden bg-[color:var(--ember-app-bg)] text-[color:var(--ember-text)]">
        <header className="flex h-16 shrink-0 items-center justify-end gap-3 border-b border-[color:var(--ember-surface-border)] bg-[color:var(--ember-app-bg)] px-5 lg:px-8">
          <Button
            type="button"
            variant="ghost"
            size="sm"
            className="h-8 w-8 rounded-full p-0 text-[color:var(--ember-text-muted)] hover:bg-[color:var(--ember-surface-hover)]"
          ...
          <label className="relative hidden w-[280px] sm:block">
            <Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[color:var(--ember-text-muted)]" />
            <Input
              value={searchQuery}
              onChange={(event) => setSearchQuery(event.target.value)}
              placeholder={t("skills.workspace.search.placeholder")}
              className="h-9 rounded-full border-[color:var(--ember-surface-border)] bg-[color:var(--ember-surface)] pl-10 pr-4 text-sm font-semibold text-[color:var(--ember-text-strong)] shadow-none placeholder:text-[color:var(--ember-text-muted)]"
            />
          </label>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                variant="outline"
                size="sm"
                className="h-9 rounded-full border-[color:var(--ember-surface-border)] bg-[color:var(--ember-surface)] px-3 text-sm font-semibold text-[color:var(--ember-text-strong)] shadow-none hover:bg-[color:var(--ember-surface-hover)]"
            ...
            <DropdownMenuContent
              align="end"
              className="z-[80] min-w-[240px] rounded-xl border border-[color:var(--ember-surface-border)] bg-[color:var(--ember-surface)] p-1 text-[color:var(--ember-text)] shadow-lg"
            >
              <DropdownMenuItem
                className="rounded-lg px-3 py-2 text-[13px] font-medium hover:bg-[color:var(--ember-surface-hover)]"
              ...
              <DropdownMenuSeparator className="bg-[color:var(--ember-surface-border)]" />
              <div className="px-3 pb-1 pt-2 text-[11px] font-semibold text-[color:var(--ember-text-muted)]">
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  {t("skills.workspace.manageMenu.createWithEmber")}
                </span>
        ...
        <main className="min-h-0 flex-1 overflow-auto bg-[color:var(--ember-surface)] px-5 pb-10 pt-10">
          <div className="mx-auto w-full max-w-[900px] space-y-8">
            <section className="space-y-4">
              <div>
                <h1 className="text-[28px] font-semibold tracking-[-0.03em] text-[color:var(--ember-text-strong)]">
                  {t("skills.workspace.header.title")}
                </h1>
                <p className="mt-2 text-sm leading-6 text-[color:var(--ember-text-muted)]">
                  {t("skills.workspace.header.subtitle")}
                </p>
              </div>
              <div className="relative h-[128px] overflow-hidden rounded-lg border border-[color:var(--ember-info-border)] bg-[color:var(--ember-info-soft)]">
                <div className="absolute left-6 top-1/2 -translate-y-1/2">
                  <div className="text-base font-semibold leading-6 text-[color:var(--ember-text-strong)]">
                    {t("skills.workspace.hero.title")}
                  </div>
                  <p className="mt-2 text-sm leading-5 text-[color:var(--ember-text)]">
              ...
              {activeView === "store" ? (
                <div className="hidden items-center gap-2 sm:flex">
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className="h-8 rounded-lg border-[color:var(--ember-surface-border)] bg-[color:var(--ember-surface)] px-3 text-xs font-semibold text-[color:var(--ember-text)] shadow-none hover:bg-[color:var(--ember-surface-hover)]"
                  >
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            ...
                            isHighlighted && skillsSelectedSurfaceClassName,
                          )}
                        >
                          <SkillTileSvg
      ...
      >
        ...
        >
          {detailInstalledSkill ? (
            <div
              className="flex max-h-[calc(100vh-3rem)] min-h-[560px] flex-col bg-[color:var(--ember-surface)]"
              data-testid="skills-installed-detail"
            >
              <div className="shrink-0 border-b border-[color:var(--ember-surface-border)] px-6 py-5 pr-14">
                <DialogHeader className="space-y-0 text-left">
                  <div className="flex items-center gap-3">
                    <SkillTileSvg tone="slate" />
                    <div className="min-w-0">
                      <DialogTitle className="line-clamp-1 text-[22px] font-semibold leading-7 tracking-[-0.02em] text-[color:var(--ember-text-strong)]">
                        {detailInstalledSkill.name}
                      </DialogTitle>
                      <div className="mt-1 line-clamp-1 text-[13px] leading-5 text-[color:var(--ember-text-muted)]">
                ...
                if (!contentState || contentState.status === "loading") {
                  return (
                    <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
                      <div className="rounded-lg border border-[color:var(--ember-surface-border)] bg-[color:var(--ember-surface-soft)] px-4 py-8 text-center text-sm text-[color:var(--ember-text-muted)]">
                        {t(
              ...
              <div className="flex shrink-0 justify-end border-t border-[color:var(--ember-surface-border)] bg-[color:var(--ember-surface-soft)] px-6 py-4">
                <Button
                  type="button"
                  size="sm"
                  className={cn(
        ...
        >
          {detailStoreItem ? (
            ...
            >
              ...
              <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
                <div className="mb-5 rounded-lg border border-[color:var(--ember-info-border)] bg-[color:var(--ember-info-soft)] px-4 py-3 text-[13px] leading-5 text-[color:var(--ember-info)]">
                    ...
                    <div className="flex items-center gap-2">
                      {canUninstall ? (
                        <Button
                          ...
                          variant="outline"
                          size="sm"
                          className="h-9 rounded-full border-[color:var(--ember-surface-border)] bg-[color:var(--ember-surface)] px-5 text-sm font-semibold text-[color:var(--ember-text)] shadow-none hover:bg-[color:var(--ember-surface-hover)]"
