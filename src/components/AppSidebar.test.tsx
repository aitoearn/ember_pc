...
describe("AppSidebar navigation", () => {
  ...
  it("文件管理器临时折叠导航栏后应恢复用户原始状态", async () => {
    ...
    await act(async () => {
      window.dispatchEvent(
        new CustomEvent("ember:app-sidebar-collapse", {
  ...
  it("Ember 首页入口应保持在左侧栏顶部，并在 macOS 预留系统按钮安全区", async () => {
    ...
    const homeButton = container.querySelector<HTMLButtonElement>(
      'button[aria-label="返回 Ember 首页"]',
    );
    const homeLogo =
      homeButton?.querySelector<HTMLImageElement>('img[alt="Ember"]');
