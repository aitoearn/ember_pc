...
beforeEach(async () => {
  ...
  mockSiteImportAdapterYamlBundle.mockResolvedValue({
    directory: "/tmp/ember/site-adapters/imported",
    adapter_count: 1,
...
describe("DeveloperSettings", () => {
  ...
  it("导入外部来源 YAML 后应调用 Ember 标准导入命令并刷新摘要", async () => {
    const container = renderComponent();
    await waitForLazyPanels();
    ...
    await clickButton(findButton(container, "Import to Ember Standard"));
    await flushEffects();
    ...
    expect(container.textContent).toContain(
      "Imported 1 external adapters to the Ember standard; 1 are currently effective",
    );
