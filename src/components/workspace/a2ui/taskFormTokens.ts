...
export const A2UI_FORM_TOKENS = {
  ...
  fieldLabel:
    "a2ui-field-label text-[13px] font-semibold leading-5 text-[color:var(--ember-text-strong)]",
  helperText:
    "a2ui-helper-text text-[12px] leading-4 text-[color:var(--ember-text-muted)]",
  ...
  optionIdle:
    "border-slate-200 bg-white text-[color:var(--ember-text-muted)] hover:border-slate-300 hover:bg-slate-50",
  optionTitle:
    "a2ui-choice-option-title flex min-w-0 items-center gap-1.5 font-normal",
  optionTitleSelected: "text-neutral-950",
  optionTitleIdle: "text-[color:var(--ember-text-muted)]",
  optionDescription:
    "a2ui-option-description mt-0.5 pl-5 text-[11px] leading-4 text-[color:var(--ember-text-muted)]",
  radioIndicatorBase:
  ...
  textInput:
    "a2ui-text-input h-8 w-full rounded-[10px] border border-slate-200 bg-white px-2.5 text-[12px] text-[color:var(--ember-text)] shadow-none outline-none transition placeholder:text-[color:var(--ember-text-muted)] focus:border-slate-400 focus:ring-2 focus:ring-slate-100",
  textarea:
    "a2ui-textarea min-h-[76px] w-full resize-y rounded-[10px] border border-slate-200 bg-white px-2.5 py-1.5 text-[12px] leading-[18px] text-[color:var(--ember-text)] shadow-none outline-none transition placeholder:text-[color:var(--ember-text-muted)] focus:border-slate-400 focus:ring-2 focus:ring-slate-100",
  checkboxRow:
    "inline-flex min-h-7 cursor-pointer items-center gap-1.5 rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[12px] leading-4 text-[color:var(--ember-text-muted)] transition-colors hover:border-slate-300 hover:bg-slate-50",
  checkboxInput:
    "h-3.5 w-3.5 rounded border-slate-300 text-neutral-900 accent-neutral-900 focus:ring-2 focus:ring-slate-100",
  checkboxText: "text-[12px] text-[color:var(--ember-text-muted)]",
  sliderRow: "flex items-center justify-between",
  sliderValue: "text-[12px] text-[color:var(--ember-text-muted)]",
  sliderInput: "h-5 w-full accent-neutral-900",
  sliderMarks:
    "flex justify-between text-[11px] leading-4 text-[color:var(--ember-text-muted)]",
} as const;
