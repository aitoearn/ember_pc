...
export const Container = styled.aside<{
  ...
  --sidebar-surface-top: ${({ $themeMode }) =>
    $themeMode === "dark"
      ? "#15202b"
      : "var(--ember-sidebar-surface-top, #f6fbf4)"};
  --sidebar-surface-middle: ${({ $themeMode }) =>
    $themeMode === "dark"
      ? "#17232d"
      : "var(--ember-sidebar-surface-middle, #f9fcf6)"};
  --sidebar-surface-bottom: ${({ $themeMode }) =>
    $themeMode === "dark"
      ? "#1a2530"
      : "var(--ember-sidebar-surface-bottom, #fbfff5)"};
  --sidebar-surface: ${({ $themeMode }) =>
    $themeMode === "dark"
      ? "linear-gradient(180deg, #15202b 0%, #17232d 48%, #1a2530 100%)"
      : "var(--ember-sidebar-surface, linear-gradient(180deg, var(--sidebar-surface-top) 0%, var(--sidebar-surface-middle) 46%, var(--sidebar-surface-bottom) 100%))"};
  --sidebar-foreground: ${({ $themeMode }) =>
    $themeMode === "dark" ? "#eef4f7" : "var(--ember-text, #1a3b2b)"};
  --sidebar-muted: ${({ $themeMode }) =>
    $themeMode === "dark" ? "#a1afbd" : "var(--ember-text-muted, #6b826b)"};
  --sidebar-border: ${({ $themeMode }) =>
    $themeMode === "dark" ? "#2d3a46" : "var(--ember-sidebar-border, #e2f0e2)"};
  --sidebar-card-border: ${({ $themeMode }) =>
    $themeMode === "dark"
      ? "rgba(55, 68, 81, 0.76)"
      : "var(--ember-sidebar-card-border, var(--ember-sidebar-border, #e2f0e2))"};
  --sidebar-divider: ${({ $themeMode }) =>
    $themeMode === "dark"
      ? "rgba(148, 163, 184, 0.14)"
      : "var(--ember-sidebar-divider, rgba(132, 204, 22, 0.15))"};
  --sidebar-hover: ${({ $themeMode }) =>
    $themeMode === "dark" ? "#22303c" : "var(--ember-sidebar-hover, #eef7ee)"};
  --sidebar-active: ${({ $themeMode }) =>
    $themeMode === "dark"
      ? "#2a3e3b"
      : "var(--ember-sidebar-active, #e6f8ea)"};
  --sidebar-active-foreground: ${({ $themeMode }) =>
    $themeMode === "dark"
      ? "#dff4ea"
      : "var(--ember-sidebar-active-text, #166534)"};
  --sidebar-search-bg: ${({ $themeMode }) =>
    $themeMode === "dark"
      ? "#1f2b36"
      : "var(--ember-sidebar-search-bg, #fcfff9)"};
  --sidebar-search-hover: ${({ $themeMode }) =>
    $themeMode === "dark"
      ? "#24313d"
      : "var(--ember-sidebar-search-hover, #f4fdf4)"};
  --sidebar-search-border-hover: ${({ $themeMode }) =>
    $themeMode === "dark"
      ? "#3a4a57"
      : "var(--ember-sidebar-search-border-hover, #bbf7d0)"};
  --sidebar-card-surface: ${({ $themeMode }) =>
    $themeMode === "dark"
      ? "linear-gradient(180deg, rgba(24, 34, 44, 0.96) 0%, rgba(19, 29, 38, 0.98) 100%)"
      : "var(--ember-sidebar-card-surface, linear-gradient(180deg, rgba(255, 255, 255, 0.76) 0%, rgba(249, 252, 246, 0.94) 100%))"};
  --sidebar-card-shadow: ${({ $themeMode }) =>
    $themeMode === "dark"
      ? "0 18px 34px -28px rgba(2, 8, 23, 0.7)"
      : "var(--ember-sidebar-card-shadow, 0 14px 28px -26px rgba(15, 23, 42, 0.32))"};
  --sidebar-card-highlight: ${({ $themeMode }) =>
    $themeMode === "dark"
      ? "rgba(255, 255, 255, 0.08)"
      : "var(--ember-sidebar-card-highlight, rgba(255, 255, 255, 0.72))"};
  display: flex;
  flex-direction: column;
  width: ${({ $collapsed }) => ($collapsed ? "72px" : "220px")};
  min-width: ${({ $collapsed }) => ($collapsed ? "72px" : "220px")};
  height: 100vh;
  padding: ${({ $collapsed }) =>
    $collapsed
      ? "calc(12px + var(--sidebar-window-control-safe-top)) 6px 12px"
      : "calc(14px + var(--sidebar-window-control-safe-top)) 10px 12px"};
  position: relative;
  isolation: isolate;
  z-index: 30;
  background: var(--sidebar-surface);
  border-right: 1px solid var(--sidebar-border);
  box-shadow: 12px 0 28px -30px rgba(15, 23, 42, 0.32);
  transition:
    width 180ms ease,
  ...
  &::before {
    ...
    background:
      radial-gradient(
        circle at top left,
        var(--ember-sidebar-glow-primary, rgba(132, 204, 22, 0.14)) 0%,
        transparent 54%
      ),
      radial-gradient(
        circle at 18% 18%,
        var(--ember-sidebar-glow-secondary, rgba(16, 185, 129, 0.12)) 0%,
        transparent 42%
      ),
      radial-gradient(
        circle at bottom left,
        var(--ember-sidebar-glow-tertiary, rgba(186, 230, 253, 0.12)) 0%,
        transparent 46%
      );
    opacity: ${({ $themeMode }) => ($themeMode === "dark" ? 0.82 : 0)};
    pointer-events: none;
    z-index: 0;
  }
...
export const SidebarSearchSurface = styled.div`
  ...
  overflow: hidden;
  border-radius: 28px;
  border: 1px solid var(--ember-card-subtle-border, rgba(226, 240, 226, 0.9));
...
export const SidebarSearchInput = styled.input`
  ...
  &::placeholder {
    color: var(--ember-text-soft, #9aa89a);
    font-weight: 600;
  }
`;
...
export const SidebarSearchKey = styled.kbd`
  ...
  height: 28px;
  border-radius: 9px;
  border: 1px solid var(--ember-card-subtle-border, rgba(226, 240, 226, 0.86));
  background: var(--ember-muted-surface, #f5f8f3);
  color: var(--ember-text-muted, #6b826b);
  display: inline-flex;
  align-items: center;
...
export const SidebarSearchDivider = styled.div`
  height: 1px;
  margin: 0 26px;
  background: var(--ember-divider-subtle, rgba(226, 240, 226, 0.86));
`;
...
export const SidebarSearchBody = styled.div`
  ...
  &::-webkit-scrollbar-thumb {
    border-radius: 999px;
    background: var(--ember-divider-strong, rgba(180, 196, 180, 0.7));
  }
`;
...
export const SidebarSearchCreateButton = styled.button`
  ...
  border-radius: 18px;
  background: var(--ember-surface-hover, #f4f7f2);
  color: var(--ember-text-strong, #0f172a);
  display: flex;
  align-items: center;
  gap: 12px;
  ...
  &:hover {
    border-color: var(--ember-card-subtle-border, #bbf7d0);
    background: var(--ember-surface, #ffffff);
...
export const SidebarSearchResultButton = styled.button<{ $active?: boolean }>`
  ...
  border: 1px solid
    ${({ $active }) =>
      $active ? "var(--ember-card-subtle-border, #bbf7d0)" : "transparent"};
  border-radius: 16px;
  background: ${({ $active }) =>
    $active ? "var(--ember-surface-hover, #f4fdf4)" : "transparent"};
  color: var(--ember-text, #1a3b2b);
  display: flex;
  align-items: center;
  gap: 12px;
  ...
  &:hover {
    border-color: var(--ember-card-subtle-border, rgba(187, 247, 208, 0.92));
    background: var(--ember-surface-hover, #f4fdf4);
    color: var(--ember-text-strong, #0f172a);
  }
...
export const SidebarSearchEmptyState = styled.div`
  ...
  justify-content: center;
  border-radius: 18px;
  border: 1px dashed var(--ember-card-subtle-border, rgba(226, 240, 226, 0.9));
  color: var(--ember-text-muted, #6b826b);
  background: var(--ember-muted-surface, #f8faf7);
  font-size: 14px;
  font-weight: 680;
`;
...
export const NavButton = styled.button<{ $active?: boolean; $collapsed?: boolean }>`
  ...
  width: 100%;
  height: ${({ $collapsed }) => ($collapsed ? "40px" : "40px")};
  border: 1px solid
    ${({ $active }) =>
      $active ? "var(--theme-border, var(--sidebar-border))" : "transparent"};
  border-radius: 10px;
  ...
  justify-content: ${({ $collapsed }) =>
    $collapsed ? "center" : "flex-start"};
  box-shadow: ${({ $active }) =>
    $active ? "inset 0 1px 0 rgba(255, 255, 255, 0.9)" : "none"};
...
export const AppearancePopover = styled.div`
  ...
  overflow-y: auto;
  border-radius: 18px;
  border: 1px solid var(--ember-card-subtle-border, rgba(226, 240, 226, 0.92));
  background: var(--ember-card-subtle, var(--ember-surface, #ffffff));
  box-shadow:
  ...
  &::after {
    ...
    left: -5px;
    bottom: 15px;
    border-left: 1px solid var(--ember-surface-border, rgba(226, 240, 226, 0.92));
    border-bottom: 1px solid
      var(--ember-surface-border, rgba(226, 240, 226, 0.92));
    width: 10px;
    height: 10px;
...
export const AppearanceGroup = styled.div`
  ...
  gap: 6px;
  padding: 7px 0;
  border-top: 1px solid var(--ember-divider-subtle, rgba(226, 240, 226, 0.82));
`;
...
export const ThemeModeButton = styled.button<{ $active?: boolean }>`
  ...
  border: 1px solid
    ${({ $active }) =>
      $active
        ? "var(--ember-card-subtle-border, #bbf7d0)"
        : "var(--ember-card-subtle-border, rgba(226, 240, 226, 0.82))"};
  background: ${({ $active }) =>
    $active
      ? "var(--ember-chrome-tab-active-surface, var(--ember-surface, #ffffff))"
  ...
  font-weight: 700;
  box-shadow: ${({ $active }) =>
    $active ? "0 10px 22px -20px var(--ember-shadow-color)" : "none"};
  transition:
    border-color 0.16s ease,
...
export const RandomColorSchemeButton = styled.button`
  ...
  min-height: 40px;
  border-radius: 13px;
  border: 1px solid var(--ember-card-subtle-border, rgba(226, 240, 226, 0.82));
  background: var(--ember-surface, #ffffff);
  color: var(--ember-text, #1a3b2b);
  cursor: pointer;
  display: inline-flex;
...
export const ColorSchemeSwatches = styled.span`
  ...
  box-shadow: 0 0 0 1px var(--ember-surface-border, rgba(226, 240, 226, 0.82));
...
export const AccountAvatar = styled.span`
  ...
  align-items: center;
  justify-content: center;
  background: var(--ember-brand, #10b981);
  color: white;
  font-size: 13px;
  font-weight: 800;
  ...
  border: 1px solid
    ${({ $connected }) =>
      $connected
        ...
        : "var(--ember-card-subtle-border, #d9eadf)"};
  background: ${({ $connected }) =>
    $connected
      ? "var(--ember-brand-soft, #ecfdf5)"
      : "var(--ember-surface-soft, #f8fcf9)"};
  color: ${({ $connected }) =>
    $connected
...
export const AccountPlanButton = styled.button`
  ...
  &:hover {
    ...
    background: var(--ember-surface-soft, #f8fcf9);
    transform: translateY(-1px);
  }
`;
  ...
  border: 1px solid
    ${({ $primary }) =>
      $primary
        ...
        : "var(--ember-card-subtle-border, #d9eadf)"};
  background: ${({ $primary }) =>
    $primary ? "var(--ember-brand-strong, #166534)" : "var(--ember-surface)"};
  color: ${({ $primary }) =>
    $primary ? "#ffffff" : "var(--ember-text, #1a3b2b)"};
  cursor: pointer;
  display: inline-flex;
  align-items: center;
  justify-content: center;
...
export const AccountMenuNotice = styled.div<{ $tone?: "error" | "info" }>`
  ...
  border: 1px solid
    ${({ $tone }) =>
      $tone === "error"
        ? "var(--ember-danger-border, #fecdd3)"
        : "var(--ember-card-subtle-border, #d9eadf)"};
  background: ${({ $tone }) =>
    $tone === "error" ? "var(--ember-danger-soft, #fff1f2)" : "#f8fcf9"};
  color: ${({ $tone }) =>
    $tone === "error" ? "var(--ember-danger, #be123c)" : "#526455"};
  padding: 8px 10px;
  font-size: 12px;
  font-weight: 700;
  line-height: 1.45;
...
export const AccountInfoIconButton = styled.button`
  ...
  height: 24px;
  border-radius: 999px;
  border: 1px solid var(--ember-card-subtle-border, #d9eadf);
  background: var(--ember-surface, #ffffff);
  color: var(--ember-text-muted, #6b826b);
  cursor: help;
  display: inline-flex;
  ...
  border: 1px solid
    ${({ $connected }) =>
      $connected
        ...
        : "var(--ember-card-subtle-border, #d9eadf)"};
  background: ${({ $connected }) =>
    $connected ? "var(--ember-brand-soft, #ecfdf5)" : "var(--ember-surface)"};
  color: ${({ $connected }) =>
    $connected
      ? "var(--ember-brand-strong, #166534)"
      : "var(--ember-text-muted, #6b826b)"};
...
export const AccountPlanProgressTrack = styled.span`
  ...
  height: 3px;
  border-radius: 999px;
  background: var(--ember-card-subtle-border, #e5e7eb);
  overflow: hidden;
`;
...
export const AccountPlanProgressFill = styled.span<{ $percent: number | null }>`
  ...
  background: var(--ember-brand-strong, #166534);
`;
...
export const AccountMenuItem = styled.button<{ $danger?: boolean; $active?: boolean }>`
  ...
  background: ${({ $active }) =>
    $active ? "var(--sidebar-active)" : "transparent"};
  color: ${({ $danger }) =>
    $danger ? "var(--ember-danger, #ef4444)" : "var(--ember-text, #1a3b2b)"};
  cursor: pointer;
  display: flex;
  align-items: center;
  justify-content: space-between;
...
export const AccountSubmenuItem = styled.button<{ $active?: boolean }>`
  ...
  background: ${({ $active }) =>
    $active ? "var(--sidebar-active)" : "transparent"};
  color: ${({ $active }) =>
    $active ? "var(--sidebar-active-foreground)" : "var(--ember-text, #1a3b2b)"};
  cursor: pointer;
  display: flex;
  align-items: center;
  justify-content: space-between;
...
export const AccountMenuDivider = styled.div`
  height: 1px;
  margin: 5px 0;
  background: var(--ember-divider-subtle, rgba(226, 240, 226, 0.82));
`;
