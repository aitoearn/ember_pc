...
describe("AppSidebar account menu", () => {
  ...
  it("首页侧边栏底部应展示设置入口与账号菜单", async () => {
    ...
    expect(accountMenu?.textContent).not.toContain("连接 熠测云端");
  ...
  it("已登录账号弹框应展示真实套餐摘要并跳出到用户中心详情", async () => {
    ...
    setStoredOemCloudSessionState({
      token: "session-token",
      tenant: { id: "tenant-0001", name: "熠测云端" },
      user: {
        id: "user-001",
        displayName: "晚风",
    ...
    expect(accountMenu?.textContent).toContain("熠测云端");
    ...
    expect(mockBuildOemCloudUserCenterUrl).toHaveBeenCalledWith(
      "https://user.emberai.run",
      "/billing?tab=usage",
    );
    expect(mockOpenExternalUrl).toHaveBeenCalledWith(
      "https://user.emberai.run/billing?tab=usage",
      { browserTarget: null },
    );
    expect(mockToastSuccess).toHaveBeenCalledWith("已打开 熠测云端 用户中心");
  });

  it("未连接熠测云端时设置菜单不应展示开源使用卡片", async () => {
    const container = mountSidebarContainer({
      currentPage: "agent",
      currentPageParams: {
        agentEntry: "new-task",
    ...
    expect(accountMenu?.textContent).not.toContain("开源使用");
    expect(accountMenu?.textContent).not.toContain("免费版");
    expect(accountMenu?.textContent).not.toContain("本地模型可配置");
    expect(
      accountMenu?.querySelector('[data-testid="app-sidebar-open-source-card"]'),
    ).toBeNull();
    expect(accountMenu?.textContent).toContain("模型设置");
    expect(accountMenu?.textContent).not.toContain("连接 熠测云端");
    expect(accountMenu?.textContent).not.toContain("退出登录");
    expect(
      accountMenu?.querySelector('button[aria-label="熠测云端"]'),
    ).toBeNull();
  });

  it("英文界面下未连接账号菜单不应展示连接云端入口", async () => {
    ...
    expect(container.textContent).not.toContain("Open Source Use");
    ...
    expect(accountMenu?.textContent).not.toContain("Open Source Use");
    expect(accountMenu?.textContent).not.toContain("Free plan");
    expect(accountMenu?.textContent).toContain("Model Settings");
    expect(accountMenu?.textContent).toContain("About");
    expect(accountMenu?.textContent).not.toContain("Connect Ember Cloud");
  });

  it("熠测云端登录完成后侧边栏设置菜单应展示账号套餐卡片", async () => {
    const container = mountSidebarContainer({
      currentPage: "agent",
      currentPageParams: {
        agentEntry: "new-task",
  ...
  it("用户弹框中的收缩入口应导航到真实页面", async () => {
    ...
    expect(mockOpenExternalUrl).toHaveBeenLastCalledWith(
      "https://user.emberai.run/welcome",
      { browserTarget: null },
    );
    expect(onNavigate).toHaveBeenLastCalledWith("settings", {
      tab: SettingsTabs.Providers,
    ...
    await clickAccountMenuItem(container, "熠测云端");
    expect(mockBuildOemCloudUserCenterUrl).toHaveBeenLastCalledWith(
      "https://user.emberai.run",
      "/welcome",
    );
