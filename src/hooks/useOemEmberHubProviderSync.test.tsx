...
import { useOemEmberHubProviderSync } from "./useOemEmberHubProviderSync";
...
function HookHarness() {
  ...
  return <div data-testid="oem-ember-hub-provider-sync" />;
}
...
describe("useOemEmberHubProviderSync", () => {
  ...
  it("应把默认云端来源的模型目录同步到内部 ember-hub provider", async () => {
    ...
    expect(apiKeyProviderMocks.updateProvider).toHaveBeenCalledWith(
      "ember-hub",
      {
        name: "Acme Hub",
        api_host:
          "https://gateway-api.emberai.run/root#ember_tenant_id=tenant-0001",
        type: "openai",
        enabled: true,
        sort_order: 0,
        custom_models: [
    ...
    expect(controlPlaneMocks.createClientAccessToken).toHaveBeenCalledWith(
      "tenant-0001",
      {
        name: "Ember Desktop Cloud Model Key",
        scopes: ["llm:invoke"],
        allowedModels: [
          "gpt-5.2-pro",
          "gpt-5.2-fast",
    ...
    expect(apiKeyProviderMocks.addApiKey).toHaveBeenCalledWith({
      provider_id: "ember-hub",
      api_key: "sk-ember-desktop",
      alias: "Ember cloud models",
    });
  });
    ...
    apiKeyProviderMocks.getProviders.mockResolvedValue([
      {
        id: "ember-hub",
        name: "Acme Hub",
        api_host: "https://gateway-api.emberai.run/root",
        type: "openai",
        enabled: true,
        sort_order: 0,
