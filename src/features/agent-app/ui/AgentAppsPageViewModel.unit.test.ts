...
const LOCAL_APP_DIR = "/tmp/ember/content-factory-app";
...
describe("AgentAppsPageViewModel", () => {
  ...
  it("应解析本地图标、Cloud 直链、内联 SVG 和兜底名称图标", () => {
    ...
    expect(
      resolveAppIconSrc({
        title: "Cloud",
        cloudApp: buildCloudApp({ iconUrl: "https://ember.local/icon.png" }),
      }),
    ).toBe("https://ember.local/icon.png");
    expect(
      resolveAppIconSrc({
        title: "Cloud",
        cloudApp: buildCloudApp({
          logo: "https://ember.local/logo.png",
        }),
      }),
    ).toBe("https://ember.local/logo.png");
    expect(
      resolveAppIconSrc({
        title: "Cloud",
        cloudApp: buildCloudApp({
          presentation: {
            logo: "https://ember.local/presentation-logo.png",
          },
        }),
      }),
    ).toBe("https://ember.local/presentation-logo.png");
    expect(
      decodeURIComponent(
        resolveAppIconSrc({
          title: "Inline",
    ...
    expect(
      resolveAppIconSrc({
        ...
        installedState: buildReadyState({
          manifest: buildManifest({
            presentation: {
              logo: "https://ember.local/installed-logo.png",
            },
          }),
        }),
      }),
    ).toBe("https://ember.local/installed-logo.png");
    expect(
      decodeURIComponent(
        resolveAppIconSrc({
          title: "兜底应用",
