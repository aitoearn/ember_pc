...
import { EMBER_THEME_CHANGED_EVENT } from "@/lib/appearance/themeMode";
import {
...
describe("AgentAppRuntimePage", () => {
  ...
  it("Agent App 卸载事件后应刷新 installed list 并释放旧 runtime 引用", async () => {
    ...
    await act(async () => {
      window.dispatchEvent(new Event("ember:agent-apps-changed"));
  ...
  it("Host iframe chrome 默认只显示静默 App 信息图标，点击后展开版本信息", async () => {
    ...
    expect(container.textContent).toContain("测试工作台");
        ...
        apps: [
          {
            ...
            enabled: true,
            packageUrl: "https://ember.local/content-factory-app-0.4.0.zip",
            packageHash:
  ...
  it("iframe 加载后通过 Host Bridge 发送 Ember 主题和运行快照", async () => {
    document.documentElement.style.setProperty("--ember-brand", "#166534");
    const container = await renderPage();
    await flush();
  ...
  it("只有声明 ember.cloudSession 的 App 才能从 Host snapshot 读取用户态上下文", async () => {
    window.__EMBER_OEM_CLOUD__ = {
      enabled: true,
      baseUrl: "https://user.emberai.run",
      tenantId: "tenant-0001",
    };
    window.__EMBER_SESSION_TOKEN__ = "secret-session-token";
    const state = buildReadyState({
      manifestPatch: {
        requires: {
    ...
    expect(postMessage).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        payload: expect.objectContaining({
          ...
          capabilities: expect.objectContaining({
            available: expect.arrayContaining(["ember.cloudSession"]),
          }),
        }),
      }),
      "http://127.0.0.1:4199",
  ...
  it("runtime page 会用当前 adapter profile 重算能力，避免沿用安装期旧 readiness 阻断 storage", async () => {
    ...
    expect(postMessage).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        payload: expect.objectContaining({
          capabilities: expect.objectContaining({
            available: expect.arrayContaining(["ember.storage"]),
            blocked: expect.not.arrayContaining(["ember.storage"]),
          }),
        }),
      }),
      "http://127.0.0.1:4199",
  ...
  it("App ready、主题切换和 capability 请求都走标准 Host Bridge 与 Ember Agent 能力", async () => {
    const container = await renderPage();
    await flush();
    const frame = getRuntimeFrame(container);
    const postMessage = vi.spyOn(frame.contentWindow!, "postMessage");
    ...
    await act(async () => {
      window.dispatchEvent(new CustomEvent(EMBER_THEME_CHANGED_EVENT));
      await Promise.resolve();
    });
