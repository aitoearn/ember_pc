...
describe("AgentAppRuntimePage Host Bridge", () => {
  ...
  it("App 可通过 ember.capabilities.getProfile 发现 Host capability profile", async () => {
  ...
  it("App 可通过 Ember 客户端 profile 读取 v0.6 AgentRuntime 合同", async () => {
    const agentRuntime = {
      agentTask: {
        eventSchema: "ember.agent-task-event.v1",
        resultSchema: "ember.agent-task-result.v1",
        structuredOutput: {
          type: "json_schema",
    ...
    const container = await renderPage(
      buildReadyState({
        manifestPatch: {
          ...
          version: "0.6.0",
          requires: {
            sdk: "@ember/app-sdk@^0.6.0",
            capabilities: ["ember.agent", "ember.skills", "ember.usage"],
          },
          agentRuntime,
        },
      }),
  ...
  it("App 可通过 Ember 客户端 profile 读取 v0.7 需求边界与能力交接", async () => {
    const requirements = {
      requirements: [
        {
          id: "CF-R001",
    ...
    const container = await renderPage(
      buildReadyState({
        manifestPatch: {
          ...
          version: "0.7.0",
          requires: {
            sdk: "@ember/app-sdk@^0.7.0",
            capabilities: ["ember.agent", "ember.connectors", "ember.evidence"],
          },
          requirements,
          boundary,
          integrations,
