...
describe("AgentAppLabPage", () => {
  ...
  it("开启 UI runtime 后点击 page entry 也必须先经过 P14 guard", async () => {
    ...
    expect(container.textContent).toContain("ember.ui");
  });
  ...
  it("开启 real adapter 后测试工作台 P4 demo 也必须先通过 P14 guard", async () => {
    const container = await renderPage({ realAdapterEnabled: true });
    const button = container.querySelector(
  ...
  it("开启 workflow runtime 后测试工作台 demo 仍不得绕过 P14 guard", async () => {
    const container = await renderPage({
      realAdapterEnabled: true,
