...
describe("AgentAppsPage", () => {
  ...
  it("正式入口页面壳和主按钮应接入 Ember 主题变量", async () => {
    const container = await renderPage();
    await flush();
    ...
    expect(page?.className).toContain("ember-workbench-theme-scope");
    expect(page?.className).toContain("bg-[color:var(--ember-app-bg)]");
    expect(installLocal?.className).toContain(
      "bg-[color:var(--ember-text-strong)]",
    );
  ...
  it("应用卡片应兼容 manifest presentation.logo 字段", async () => {
    installedStates.push(
      buildReadyState({
        manifest: {
          ...(contentFactoryFixture as AppManifest),
          presentation: {
            logo: "https://ember.local/content-factory-logo.png",
          },
        },
      }),
    );
    ...
    expect(icon?.getAttribute("src")).toBe(
      "https://ember.local/content-factory-logo.png",
    );
