...
function shellKindForMode(mode: AgentAppInstallMode): string {
  if (mode === "standalone") {
    return "Ember App Shell";
  }
  if (mode === "runtime_backed") {
    return "system Ember Runtime";
  }
  if (mode === "web_host") {
    return "Web Host";
  }
  return "Ember Desktop";
}
...
function unsupportedWebHostIssue(mode: AgentAppInstallMode): ReadinessIssue {
  return {
    ...
    severity: "blocker",
    message: "web_host is reserved in Agent App v2 and is not launchable in the Ember desktop client yet.",
    kind: "install-mode",
    key: mode,
    required: true,
...
function runtimeVersionIssue(params: {
  ...
  return {
    ...
    severity: "blocker",
    message: `${shellKindForMode(params.mode)} requires Ember Runtime ${params.minVersion}, but host runtime is ${params.profile.appRuntimeVersion}.`,
    kind: "install-mode",
    key: params.mode,
    required: true,
    remediation: `Upgrade Ember Runtime to ${params.minVersion} or choose another install mode.`,
  };
}
