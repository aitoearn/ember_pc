...
describe("Agent App manifest P0", () => {
  ...
  it("应把 v0.6 capability list 归一化为稳定 capability range", () => {
    ...
    expect(normalized.requires.sdk).toBe("@ember/app-sdk@^0.6.0");
  ...
  it("应消费 v0.8 install contract 并投影 install modes", () => {
    const normalized = normalizeManifest(
      parseManifest({
        ...
        version: "0.8.0",
        requires: {
          sdk: "@ember/app-sdk@^0.8.0",
          capabilities: ["ember.agent", "ember.storage"],
        },
  ...
  it("应接受 v0.10 manifest 并保留当前安装契约", () => {
    ...
    expect(normalized.requires.sdk).toBe("@ember/app-sdk@^0.10.0");
    expect(normalized.requires.capabilities).toMatchObject({
      "ember.agent": "*",
      "ember.connectors": "*",
      "ember.terminal": "*",
