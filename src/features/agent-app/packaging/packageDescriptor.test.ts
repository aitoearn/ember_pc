...
import { buildEmberRuntimeProfileFromHostProfile } from "../runtime-profile";
...
function buildStandaloneShell() {
  const manifest = parseManifest({
    ...
    install: {
      ...
      standalone: {
        shell: "ember-app-shell",
        bundleId: "ai.embercloud.contentfactory",
      },
  ...
  const runtimeProfile = buildEmberRuntimeProfileFromHostProfile({
...
function buildMacOsStandaloneTarget() {
  return {
    ...
    macosIdentity: buildMacOsStandaloneIdentity({
      teamId: "TEAMID1234",
      bundleId: "com.embercloud.agentapp.contentfactory",
      appGroups: ["group.com.embercloud.agentapps"],
      keychainAccessGroups: ["TEAMID1234.com.embercloud.agentapps"],
    }),
...
describe("Agent App v2 package descriptor", () => {
  ...
  it("macOS standalone target 不能复用 Ember Desktop Bundle ID", () => {
    const shell = buildStandaloneShell();
    const descriptor = buildPackageDescriptor({
      target: {
        kind: "standalone",
  ...
  it("native shell registration plan 应输出独立 Bundle ID 与 per-app deep link 配置", () => {
    ...
    expect(plan.bundleIdentifier).toBe("com.embercloud.agentapp.contentfactory");
    expect(plan.deepLinkSchemes).toEqual(["ember-agent-content-factory-app"]);
    expect(plan.nativeShellConfigPatch).toMatchObject({
      productName: "Content Factory",
      identifier: "com.embercloud.agentapp.contentfactory",
      plugins: {
        "deep-link": {
          desktop: {
            schemes: ["ember-agent-content-factory-app"],
          },
        },
      },
    });
    expect(plan.runtimeEnv).toEqual({
      EMBER_AGENT_APP_STANDALONE_APP_ID: "content-factory-app",
      EMBER_AGENT_APP_STANDALONE_ENTRY_KEY: "dashboard",
      EMBER_AGENT_APP_STANDALONE_DEEP_LINK_SCHEME:
        "ember-agent-content-factory-app",
    });
  ...
  it("native shell registration plan 缺失 macOS identity 时必须 blocked", () => {
    ...
    expect(plan.deepLinkSchemes).toEqual(["ember-agent-app"]);
  });
  ...
  it("standalone native shell config materializer 应把 per-app identity / deep link 写入独立配置", () => {
    ...
    const result = materializeStandaloneNativeShellConfig({
      ...
      baseConfig: {
        productName: "Ember",
        identifier: "com.embercloud.ember",
        app: {
          windows: [{ title: "Ember", visible: true, width: 1200 }],
        },
        plugins: {
          "deep-link": {
            desktop: { schemes: ["ember"] },
          },
        },
      },
    });
    ...
    expect(result.config).toMatchObject({
      ...
      plugins: {
        "deep-link": {
          desktop: { schemes: ["ember-agent-content-factory-app"] },
        },
      },
    });
  });
  ...
  it("standalone native shell config write plan 应生成可落盘的 config/env 文件计划", () => {
    ...
    const materializedConfig = materializeStandaloneNativeShellConfig({
      ...
      baseConfig: {
        productName: "Ember",
        identifier: "com.embercloud.ember",
        app: { windows: [{ title: "Ember", visible: true }] },
        plugins: { "deep-link": { desktop: { schemes: ["ember"] } } },
      },
    });
    ...
    const first = buildStandaloneNativeShellConfigWritePlan({
      ...
      configOutputPath:
        "<dist>/agent-apps/content-factory/ember-rs/native-shell.config.json",
    ...
    expect(first.files[0]).toMatchObject({
      path: "<dist>/agent-apps/content-factory/ember-rs/native-shell.config.json",
      encoding: "utf8",
      sensitive: false,
    });
    ...
    expect(first.files[1].content).toContain(
      "EMBER_AGENT_APP_STANDALONE_DEEP_LINK_SCHEME=ember-agent-content-factory-app",
    );
  });
  ...
  it("standalone release plan 应阻止 non-production descriptor 伪装发布", () => {
    ...
    const plan = buildStandaloneReleasePlan({
      ...
      signing: {
        ...
        notarizationProfileRef: "notarytool:ember-prod",
      },
      updater: {
        enabled: true,
        endpoint: "https://updates.embercloud.example/content-factory",
      },
      rollback: {
