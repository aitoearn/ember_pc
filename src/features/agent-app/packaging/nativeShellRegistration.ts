...
export interface AgentAppNativeShellRegistrationPlan {
  ...
  runtimeEnv: {
    EMBER_AGENT_APP_STANDALONE_APP_ID: string;
    EMBER_AGENT_APP_STANDALONE_ENTRY_KEY: string;
    EMBER_AGENT_APP_STANDALONE_DEEP_LINK_SCHEME: string;
  };
...
function isDesktopDeepLinkScheme(scheme: string): boolean {
  return scheme.trim().toLowerCase() === "ember";
}
...
export function buildNativeShellRegistrationPlan(params: {
  ...
  if (isDesktopDeepLinkScheme(chrome.deepLink.scheme)) {
    blockers.push({
      ...
      message: "Standalone Agent App deep link scheme must not reuse Ember Desktop's ember:// scheme.",
    });
  }
  ...
  return {
    ...
    nativeShellConfigPatch,
    runtimeEnv: {
      EMBER_AGENT_APP_STANDALONE_APP_ID: descriptor.shell.appId,
      EMBER_AGENT_APP_STANDALONE_ENTRY_KEY: chrome.deepLink.openEntryKey,
      EMBER_AGENT_APP_STANDALONE_DEEP_LINK_SCHEME: chrome.deepLink.scheme,
    },
