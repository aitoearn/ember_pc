  ...
  it("缺少 build artifact refs 时必须阻断发布", () => {
    ...
    const plan = buildStandaloneReleasePipelinePlan({
      ...
      buildEvidence: {
        status: "completed",
        artifactRefs: [],
        evidenceRef: ".ember/agent-apps/build.json",
      },
    });
    ...
    expect(plan).toMatchObject({
      ...
      buildEvidenceRef: ".ember/agent-apps/build.json",
  ...
  it("产物级 evidence 齐全时仍不会绕过 non-production descriptor 门禁", () => {
    ...
    const plan = buildStandaloneReleasePipelinePlan({
      ...
      signingEvidence: {
        ...
        evidenceRef: ".ember/agent-apps/signing.json",
      },
      notarizationEvidence: {
        acceptedArtifactRefs: ["sha256:dmg"],
        stapledArtifactRefs: ["sha256:dmg"],
        logRef: ".ember/agent-apps/notarization.json",
      },
      updaterEvidence: {
        manifestRef: ".ember/agent-apps/latest.json",
        artifactRefs: ["sha256:dmg"],
      },
      rollbackEvidence: {
        manifestRef: ".ember/agent-apps/rollback.json",
    ...
    expect(plan).toMatchObject({
      status: "blocked",
      readyToPublish: false,
      signingEvidenceRef: ".ember/agent-apps/signing.json",
      notarizationEvidenceRef: ".ember/agent-apps/notarization.json",
      updaterManifestRef: ".ember/agent-apps/latest.json",
      rollbackManifestRef: ".ember/agent-apps/rollback.json",
    });
  });
});
