...
export const DEFAULT_EMBER_DESKTOP_BUNDLE_IDS = [
  "com.embercloud.ember",
  "com.embercloud.ember.headless",
];
...
export function validateMacOsStandaloneIdentity(
  ...
  const desktopBundleIds =
    options.desktopBundleIds ?? DEFAULT_EMBER_DESKTOP_BUNDLE_IDS;
  const expectedAppId =
  ...
  if (desktopBundleIds.includes(identity.bundleId)) {
    issues.push({
      ...
      message:
        "Standalone Agent App must not reuse the Ember Desktop bundle id.",
    });
  }
