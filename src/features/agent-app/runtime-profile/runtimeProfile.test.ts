...
describe("Agent App v2 EmberRuntimeProfile", () => {
  ...
  it("应从 HostCapabilityProfile 生成统一 EmberRuntimeProfile", () => {
    const profile = buildEmberRuntimeProfileFromHostProfile({
    ...
    expect(profile).toMatchObject({
      ...
      storage: {
        namespaceRoot: "<LimeAppData>/agent-apps/storage/content-factory-app",
    ...
    expect(Object.keys(profile.capabilities)).toContain("ember.agent");
  });
  ...
  it("应从 preview 构建 RuntimeProfile，复用 projection storage namespace", () => {
    ...
    const profile = buildEmberRuntimeProfileForPreview({
      preview,
      hostProfile: p0HostCapabilityProfile,
    });
