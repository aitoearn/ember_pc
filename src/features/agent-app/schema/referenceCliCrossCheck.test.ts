...
const STANDARD_ROOT = resolve(
  process.env.EMBER_AGENTAPP_STANDARD_ROOT ??
    resolve(process.cwd(), "../../embercloud/agentapp"),
);
...
describeIfReferenceAvailable(
  ...
  () => {
    ...
    it("应把上游 projection schema 字段映射到 Ember projection，缺口必须显式登记", () => {
      const projectionSchema = readPublicSchema("app-projection.schema.json");
      ...
      const acceptedDivergences: Record<string, string> = {
        ok: "reference CLI envelope field; Ember projection is not a CLI response.",
        command:
          "reference CLI envelope field; Ember projection is not a CLI response.",
        permissions:
          "P17.2 exposes permissions through install review permissionCount; full permission projection is deferred to P17.3/P18.",
        triggers:
          "Agent App v0.5/v0.6 discovery metadata; Ember projection keeps runtime readiness first and will project discovery metadata in P18.7-C/F.",
        quickstart:
          "Agent App v0.5/v0.6 onboarding metadata; Ember setup guidance is still derived from readiness until P18.7-C/F.",
        skills:
        ...
        links:
          "Agent App v0.5/v0.6 external links; current Ember projection avoids exposing external metadata before policy review.",
        license:
        ...
        errors:
          "reference CLI diagnostics field; Ember projection reports errors through validation/readiness surfaces.",
        i18n: "Agent App v0.5/v0.6 localization metadata; Ember projection does not yet expose localized marketplace copy.",
        signature:
    ...
    it("应把 reference readiness checks 映射到 Ember readiness 和 install review descriptor", () => {
      const referenceProjection =
        runReferenceCli<ReferenceProjection>("project");
