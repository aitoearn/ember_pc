...
describe("P18.3 / P18.4 core capability adapters", () => {
  it("应按 catalog 生成所有 ember.* typed adapter，不再只覆盖局部能力", () => {
    const adapters = createEmberCoreCapabilityAdapters({
  ...
  it("P18.4 应把 ember.agent task facade 固定为 App-scoped typed adapter", async () => {
    const requests: LimeCapabilityInvokeRequest[] = [];
    const invoker = createLimeCapabilityInvoker(
      createMockLimeCapabilityTransport({
        "ember.agent": {
  ...
  it("应把 mock / host 的 stable error 原样暴露给 adapter 调用方", async () => {
    ...
    await expect(
      ...
      error: {
        code: "capability_unavailable",
        message: "ember.tools.getProgress is not available in the mock host.",
        capability: "ember.tools",
        method: "getProgress",
        requestId: "req-missing",
        causeCode: "UNSUPPORTED_CAPABILITY_METHOD",
