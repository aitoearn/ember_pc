...
export const EMBER_CAPABILITY_DEFINITIONS = [
  {
    name: "ember.ui",
    version: "0.3.0",
    group: "app_surface",
    ...
    summary: "Ember 桌面壳的提示、导航、下载、主题、快照和统一 Agent Run UI 能力。",
  ...
  {
    name: "ember.cloudSession",
    version: "0.1.0",
    group: "governance",
    stage: "preview",
    owner: "cloud_overlay",
    methods: ["getSnapshot", "getAccessToken", "requestLogin"],
    summary: "宿主 Ember 当前云端会话快照、登录触发与 just-in-time 访问令牌能力，快照不暴露 bearer token。",
...
export type LimeCapabilityDefinitionRecord =
  (typeof EMBER_CAPABILITY_DEFINITIONS)[number];
...
> = Name extends `ember.${infer Key}` ? Key : never;

export const EMBER_CAPABILITY_NAMES = EMBER_CAPABILITY_DEFINITIONS.map(
  (definition) => definition.name,
) as readonly LimeCapabilityName[];

export const EMBER_CAPABILITY_GROUPS = Array.from(
  new Set(EMBER_CAPABILITY_DEFINITIONS.map((definition) => definition.group)),
...
export function getLimeCapabilityDefinition(
  ...
  if (!definition) {
    throw new Error(`Unknown Ember capability: ${name}`);
  }
  return definition;
}
...
export function getLimeCapabilityAdapterKey<Name extends LimeCapabilityName>(
  ...
  return name.slice("ember.".length) as LimeCapabilityAdapterKey<Name>;
}
...
export function listEnabledLimeCapabilityNamesForMode(
  ...
  return EMBER_CAPABILITY_DEFINITIONS.filter((definition) => {
    const profile: LimeCapabilityDefinition["profile"] = definition.profile;
