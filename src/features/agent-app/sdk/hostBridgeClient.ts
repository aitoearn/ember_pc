...
export interface LimeAgentAppBridgeClientMessage {
  protocol: typeof EMBER_AGENT_APP_BRIDGE_PROTOCOL;
  version: typeof EMBER_AGENT_APP_BRIDGE_VERSION;
  type: string;
  requestId?: string;
  appId: string;
  entryKey?: string;
...
export interface EmberHostThemeSnapshot {
  themeMode?: string;
...
export interface EmberHostThemeDocumentLike {
  documentElement: EmberHostThemeElementLike;
}

export interface EmberHostThemeElementLike {
...
export interface SyncEmberHostThemeOptions {
  documentRef?: EmberHostThemeDocumentLike;
...
function readThemePayload(value: unknown): EmberHostThemeSnapshot | null {
  if (!isRecord(value)) {
    return null;
  }
  ...
  const tokens = isRecord(theme.tokens)
    ...
    : undefined;
  const snapshot: EmberHostThemeSnapshot = {};
...
export function applyEmberHostTheme(
  payload: unknown,
  options: SyncEmberHostThemeOptions = {},
): EmberHostThemeSnapshot | null {
  ...
  const documentRef =
    ...
    (typeof document === "undefined"
      ? undefined
      : (document as unknown as EmberHostThemeDocumentLike));
  ...
  if (theme.themeMode) {
    root.dataset.emberTheme = theme.themeMode;
  }
  if (theme.effectiveThemeMode) {
    root.dataset.emberThemeEffective = theme.effectiveThemeMode;
    root.style.colorScheme = theme.effectiveThemeMode === "dark" ? "dark" : "light";
  }
  if (theme.colorSchemeId) {
    root.dataset.emberColorScheme = theme.colorSchemeId;
  }
  return theme;
}

export function syncEmberHostTheme(
  invoker: Pick<
  ...
  const apply = (payload: unknown) => {
    applyEmberHostTheme(payload, options);
  };
...
function isBridgeMessage(
  ...
  return (
    isRecord(value) &&
    value.protocol === EMBER_AGENT_APP_BRIDGE_PROTOCOL &&
    value.version === EMBER_AGENT_APP_BRIDGE_VERSION &&
  ...
  implements LimeHostBridgeCapabilityInvoker
{
  ...
  constructor(options: CreateLimeHostBridgeCapabilityInvokerOptions) {
    ...
    this.requestIdPrefix = options.requestIdPrefix ?? "ember-capability";
  ...
  getHostSnapshot(): Promise<LimeCapabilityInvokeResponse<unknown>> {
    const request = this.buildHostActionContext("ember.ui", "getSnapshot");
  ...
  notifyHost(
    ...
    const payload =
      typeof payloadOrMessage === "string"
        ...
        : payloadOrMessage;
    const request = this.buildHostActionContext("ember.ui", "toast");
    return this.requestBridgeAction(
      "host:toast",
      payload,
      request,
  ...
  navigateHost(
    ...
    const request = this.buildHostActionContext("ember.ui", "navigate");
    return this.requestBridgeAction(
      "host:navigate",
      payload,
      request,
  ...
  openExternalHost(
    ...
    const request = this.buildHostActionContext("ember.ui", "openExternal");
    return this.requestBridgeAction(
      "host:openExternal",
      payload,
      request,
  ...
  downloadHost(
    ...
    const request = this.buildHostActionContext("ember.ui", "download");
    return this.requestBridgeAction(
      "host:download",
      payload,
      request,
    ...
    if (this.disposed || !this.windowRef || this.windowRef.parent === this.windowRef.self) {
      return Promise.resolve({
        ...
        error: this.buildError(
          "CAPABILITY_BLOCKED",
          "Ember host bridge is not connected.",
          request,
        ),
      });
    }
    ...
    return new Promise<LimeCapabilityInvokeResponse>((resolve) => {
      const timerId = this.windowRef!.setTimeout(() => {
        ...
        resolve({
          ...
          error: this.buildError(
            "TIMEOUT",
            "Ember host bridge request timed out.",
            request,
          ),
        });
