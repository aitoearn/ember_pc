...
describe("P18.5 Host Bridge SDK client", () => {
  it("应把标准 SDK facade 调用转成 Host Bridge v1 capability invoke envelope", async () => {
    ...
    const invoker = createLimeHostBridgeCapabilityInvoker({
      ...
      trustedHostOrigin: "https://ember.host",
    ...
    const taskPromise = ember.agent.startTask({
      title: "生成内容策略",
    ...
    expect(posted.targetOrigin).toBe("https://ember.host");
  ...
  it("应忽略非信任来源并把 host:error 映射成 stable SDK error", async () => {
    ...
    const ember = createEmberCoreCapabilityAdapters({ invoker, provenance });

    const writePromise = ember.storage.set({
  ...
  it("应提供统一主题同步 helper，App 不需要各自解析 host theme payload", async () => {
    ...
    const disposeThemeSync = syncEmberHostTheme(invoker, {
    ...
    fake.emit({
      ...
      payload: {
        theme: {
          ...
          colorSchemeId: "ember-forest",
          tokens: {
            "--ember-chrome-surface": "#101820",
            "--app-primary": "#7dd3a5",
            "--unsafe-token": "red",
          },
        },
    ...
    expect(themeDocument.properties.get("--ember-chrome-surface")).toBe("#101820");
    expect(themeDocument.properties.get("--app-primary")).toBe("#7dd3a5");
    expect(themeDocument.properties.has("--unsafe-token")).toBe(false);
    expect(themeDocument.documentRef.documentElement.dataset.emberTheme).toBe("system");
    expect(themeDocument.documentRef.documentElement.dataset.emberThemeEffective).toBe("dark");
    expect(themeDocument.documentRef.documentElement.dataset.emberColorScheme).toBe("ember-forest");
    ...
    fake.emit({
      ...
      payload: {
        ...
        tokens: { "--ember-chrome-text": "#123456" },
      },
    });
    expect(themeDocument.properties.get("--ember-chrome-text")).toBe("#123456");
    expect(themeDocument.documentRef.documentElement.dataset.emberThemeEffective).toBe("light");
  ...
  it("主题 helper 也支持直接应用单次 payload", () => {
    const themeDocument = createThemeDocument();
    const applied = applyEmberHostTheme(
      {
        effectiveThemeMode: "dark",
        tokens: {
          "--ember-sidebar-active-text": "#7dd3a5",
          "--external-token": "#fff",
        },
      },
      { documentRef: themeDocument.documentRef },
    ...
    expect(themeDocument.properties.get("--ember-sidebar-active-text")).toBe("#7dd3a5");
  ...
  it("应兼容测试工作台现有 Host Bridge facade 并保留回调与调用日志", async () => {
    const fake = new FakeBridgeWindow();
    const snapshots: unknown[] = [];
  ...
  it("测试工作台主链应能通过标准 Host Bridge SDK client 完成 task 与写回", async () => {
    const fake = new FakeBridgeWindow();
    const invoker = createLimeHostBridgeCapabilityInvoker({
      appId: "content-factory-app",
      entryKey: "dashboard",
    ...
    const startPromise = ember.agent.startTask({
      title: task.title,
    ...
    const streamPromise = ember.agent.streamTask({ taskId: task.taskId });
    fake.emit(
    ...
    const getPromise = ember.agent.getTask({ taskId: task.taskId });
    fake.emit(
      hostResponse(latestBridgeRequest(fake), {
        ok: true,
    ...
    const hostResponsePromise = ember.agent.submitHostResponse({
      taskId: task.taskId,
    ...
    const storagePromise = ember.storage.set({
      key: "projects/project-1/content-batches/batch-1",
    ...
    const artifactPromise = ember.artifacts.create({
      kind: "content_batch",
      title: "项目内容批次",
    ...
    const evidencePromise = ember.evidence.record({
      kind: "fact_grounding",
    ...
    expect(bridgeCallNames(fake)).toEqual([
      ...
      "ember.storage.set",
      "ember.artifacts.create",
      "ember.evidence.record",
    ]);
    for (const post of fake.posts) {
      expect(post.targetOrigin).toBe("https://ember.host");
