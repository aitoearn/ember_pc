...
export class MockCapabilityHost implements CapabilityHost {
  ...
  async runEntry(entryKey: string): Promise<AgentAppRunResult> {
    ...
    const runProvenance = buildAgentAppProvenance({
      preview: this.preview,
      entryKey,
      runId,
    });
  ...
  async uninstall(params: {
    ...
    const dataTargets = [
      ...
      ...this.artifacts.map((artifact) =>
        refTarget(`artifact:${artifact.id}`, "Mock Agent App artifact."),
      ),
      ...params.cleanupPlan.evidenceRefs,
      ...this.evidence.map((item) =>
        refTarget(`evidence:${item.id}`, "Mock Agent App evidence."),
      ),
      ...params.cleanupPlan.taskRefs,
      ...this.tasks.map((task) =>
        refTarget(`task:${task.taskId}`, "Mock Agent App task."),
      ),
  ...
  private createStorageCapability(
    ...
    this.assertCapabilityEnabled("ember.storage", provenance.entryKey);
    const namespace =
    ...
    return {
      ...
      set: async (key, value) => {
        const entry: AgentAppStorageEntry = {
          appId: this.preview.identity.appId,
          key,
          value,
          updatedAt: this.now(),
  ...
  private createArtifactsCapability(
    ...
    this.assertCapabilityEnabled("ember.artifacts", provenance.entryKey);
  ...
  private createEvidenceCapability(
    ...
    this.assertCapabilityEnabled("ember.evidence", provenance.entryKey);
  ...
  private createKnowledgeCapability(
    ...
    this.assertCapabilityEnabled("ember.knowledge", provenance.entryKey);

    return {
      search: async (input): Promise<AgentAppKnowledgeSearchResult> => {
        const records: AgentAppKnowledgeRecord[] =
  ...
  private createAgentCapability(
    ...
    this.assertCapabilityEnabled("ember.agent", provenance.entryKey);

    return {
      startTask: async (input) => {
        this.taskCounter += 1;
        const task = buildAgentAppTaskRecord({
      ...
      cancelTask: async (taskLookup) => {
        ...
        const task = {
          ...appendAgentAppTaskEvent(this.tasks[index], {
            type: "task:cancelled",
            status: "cancelled",
            at: timestamp,
      ...
      retryTask: async (task) => {
        ...
        this.taskCounter += 1;
        const retryTask = buildRetryAgentAppTaskRecord({
          taskId: `mock-task-${this.taskCounter}`,
          traceId: `mock-trace-${this.taskCounter}`,
          sourceTask,
          provenance,
  ...
  private submitHostResponse(
    ...
    this.tasks[index] = appendAgentAppTaskEvent(this.tasks[index], {
      type: "task:progress",
  ...
  private assertCapabilityEnabled(capability: string, entryKey?: string): void {
    ...
    throw new AgentAppCapabilityError({
      ...
      message: `${capability} is not enabled for this Agent App preview.`,
      appId: this.preview.identity.appId,
      entryKey,
      capability,
    });
