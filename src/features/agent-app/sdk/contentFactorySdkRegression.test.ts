...
function buildTaskRecord(
  ...
  return {
    ...
    tools: overrides.tools ?? ["ember.capability.research.search"],
...
describe("P18.5 content factory SDK regression", () => {
  it("应通过通用 SDK facade 完成测试工作台 task、写回和证据闭环", async () => {
    ...
    const task = await ember.agent.startTask(
      {
        title: "生成内容批次",
        ...
        tools: ["ember.capability.research.search"],
        humanReview: true,
      },
      {
    ...
    const stream = await ember.agent.streamTask(
      { taskId: task.taskId },
    ...
    const snapshot = await ember.agent.getTask(
      { taskId: task.taskId },
      { requestId: "req-content-get" },
    );
    const storageEntry = await ember.storage.set(
      {
    ...
    const artifact = await ember.artifacts.create(
      {
        kind: "content_batch",
        title: "内容批次 batch-1",
    ...
    const evidence = await ember.evidence.record(
      {
        kind: "fact_grounding",
    ...
    expect(ember.storage.namespace).toBe("content-factory-app");
