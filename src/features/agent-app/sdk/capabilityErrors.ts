...
export const EMBER_CAPABILITY_ERROR_CODES = [
...
export type LimeCapabilityErrorCode =
  (typeof EMBER_CAPABILITY_ERROR_CODES)[number];
...
export function isLimeCapabilityErrorCode(
  ...
  return (
    typeof value === "string" &&
    EMBER_CAPABILITY_ERROR_CODES.includes(value as LimeCapabilityErrorCode)
  );
}
...
export function toLimeCapabilityError(
  ...
  const message =
    error instanceof Error
      ? error.message
      : readStringField(error, "message") ?? "Ember capability call failed.";
