  ...
  it("应接受 v0.7 manifest 作为当前 Host 标准，不再按运行时版本阻断", () => {
    const rawManifest = parseManifest({
      ...
      requires: {
        ...
        capabilities: ["ember.capabilities"],
      },
        ...
        boundaries: [
          {
            ...
            planes: { host: { requires: ["ember.agent"] } },
          },
        ],
      },
    });
  ...
  it("应接受 v0.8 install modes，并把 web_host 保留为 blocked", () => {
    const rawManifest = parseManifest({
      ...
      install: {
        ...
        standalone: { shell: "ember-app-shell" },
  ...
  it("应接受 v0.10 manifest 和 install runtime 作为当前 Host 标准", () => {
    const rawManifest = parseManifest({
      ...
      requires: {
        ...
        capabilities: ["ember.agent", "ember.connectors", "ember.terminal"],
      },
      entries: [{ key: "dashboard", kind: "page" }],
      install: {
