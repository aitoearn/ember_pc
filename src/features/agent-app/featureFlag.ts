...
export const AGENT_APP_LAB_STORAGE_KEY = "ember.agentAppHost.labEnabled";
export const AGENT_APP_HOST_FLAGS_STORAGE_KEY = "ember.agentAppHost.flags";
...
export function resolveAgentAppHostFlags(
  ...
  const cloudBootstrapEnabled =
    ...
    readOptionalBooleanFlag(
      import.meta.env.VITE_EMBER_AGENT_APP_CLOUD_BOOTSTRAP,
    ) ??
    readLocalStorageHostFlag("cloudBootstrapEnabled") ??
    false;
  const explicitLabEnabled =
    overrides.labEnabled ??
    readOptionalBooleanFlag(import.meta.env.VITE_EMBER_AGENT_APP_LAB) ??
