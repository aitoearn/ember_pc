...
describe("UiExtensionHost", () => {
  it("关闭 uiRuntimeEnabled 时应拒绝挂载 UI entry", () => {
    ...
    expect(() => host.mountEntry("dashboard")).toThrow(AgentAppCapabilityError);
    try {
  ...
  it("开启 UI runtime 后应生成受控 sandbox 和 injected SDK bridge", () => {
    ...
    expect(mounted.sdkBridge.allowedCapabilities).toEqual(
      expect.arrayContaining(["ember.ui", "ember.storage", "ember.agent"]),
    );
    expect(mounted.sdkBridge.allowedCapabilities).not.toContain("ember.workflow");
    expect(mounted.sdkBridge.blockedCapabilities).toContainEqual({
      capability: "ember.workflow",
  ...
  it("非 UI entry 不应被 UI host 挂载", () => {
    ...
    expect(() => host.mountEntry("content_scenario_planning")).toThrow(AgentAppCapabilityError);
    try {
