...
const CLAW_CAPABILITY_ALIASES: Array<{
  ...
  {
    capabilityId: "ember.capability.image.generate",
    aliases: [
      "ember.capability.image.generate",
      "image.generate",
      "image_generation",
      "image",
      "asset.generate",
  ...
  {
    capabilityId: "ember.capability.cover.generate",
    aliases: [
      "ember.capability.cover.generate",
      "cover.generate",
      "cover_generation",
      "cover",
    ],
  ...
  {
    capabilityId: "ember.capability.research.search",
    aliases: [
      "ember.capability.research.search",
      "research.search",
      "research",
      "web_search",
      "search",
  ...
  {
    capabilityId: "ember.capability.report.generate",
    aliases: [
      "ember.capability.report.generate",
      "report.generate",
      "report",
      "competitor_report",
    ],
  ...
  {
    capabilityId: "ember.capability.pdf.read",
    aliases: ["ember.capability.pdf.read", "pdf.read", "pdf_extract", "pdf"],
  },
  {
    capabilityId: "ember.capability.summary.generate",
    aliases: [
      "ember.capability.summary.generate",
      "summary.generate",
      "summary",
      "text_summary",
    ],
...
function assertStorageWriteDeclared(projection: AgentAppProjection): void {
  ...
  throw new AgentAppCapabilityDispatcherError(
    ...
    "ember.storage write-back requires a declared storage namespace.",
  );
}
...
async function dispatchStorage(
  ...
  if (request.method === "set") {
    ...
    if (!hasOwn(input, "value")) {
      throw new AgentAppCapabilityDispatcherError(
        "INVALID_CAPABILITY_INPUT",
        "ember.storage.set requires value.",
      );
    }
...
async function dispatchArtifacts(
  ...
  if (request.method === "create") {
    ...
    if (!kind) {
      throw new AgentAppCapabilityDispatcherError(
        "INVALID_CAPABILITY_INPUT",
        "ember.artifacts.create requires kind.",
      );
    }
...
async function dispatchEvidence(
  ...
  if (request.method === "record") {
    ...
    if (!kind) {
      throw new AgentAppCapabilityDispatcherError(
        "INVALID_CAPABILITY_INPUT",
        "ember.evidence.record requires kind.",
      );
    }
...
function resolveCapabilityDefinition(
  ...
  const definition = EMBER_CAPABILITY_DEFINITIONS.find(
    (item) => item.name === name,
  );
  if (!definition) {
    throw new AgentAppCapabilityDispatcherError(
      "CAPABILITY_NOT_FOUND",
      `${name} is not a known Ember capability.`,
    );
  }
  return definition;
}
...
function resolveCapabilityProfileSupport(
  ...
  if (definition.name !== "ember.capabilities") {
    return support;
  }
  return {
...
function dispatchCapabilities(
  request: AgentAppHostBridgeCapabilityRequest,
  profile: HostCapabilityProfile,
  runtimeProfile: EmberRuntimeProfile | undefined,
  standardProfile: {
    manifestVersion?: string;
    agentRuntime?: unknown;
    requirements?: unknown;
  ...
  if (request.method === "list") {
    return EMBER_CAPABILITY_DEFINITIONS.map((definition) =>
  ...
  if (request.method === "getProfile") {
    ...
    const payload: Record<string, unknown> = {
      ...
      capabilities: Object.fromEntries(
        EMBER_CAPABILITY_DEFINITIONS.map((definition) => [
          definition.name,
          buildCapabilityDiscoveryEntry(definition, profile, runtimeProfile),
        ]),
      ),
...
> = {
  "ember.search": {
    ...
    toolHints: ["ember.capability.research.search", "web_search"],
    reason: "search_execution_requires_ember_agent_task",
  },
  "ember.browser": {
    keywords: [
      "browser",
    ...
    reason: "browser_runtime_execution_requires_ember_tool_runtime_policy",
  },
  "ember.documents": {
    keywords: ["document", "pdf", "docx", "word", "markdown", "ppt", "pptx"],
    toolHints: ["document_parser", "pdf.read"],
    reason: "document_runtime_execution_requires_ember_tool_runtime_policy",
  },
  "ember.media": {
    keywords: [
      "media",
    ...
    reason: "media_runtime_execution_requires_ember_tool_runtime_policy",
  },
  "ember.mcp": {
    keywords: ["mcp", "mcpserver", "mcp__"],
    toolHints: ["mcp__server__tool"],
    reason: "mcp_execution_requires_ember_tool_runtime_policy",
  },
  "ember.terminal": {
    keywords: ["terminal", "shell", "command", "bash", "powershell", "cmd"],
    toolHints: ["terminal.run"],
    reason: "terminal_execution_requires_ember_sandbox_policy",
  },
  "ember.connectors": {
    keywords: ["connector", "connectors", "integration", "notion", "slack"],
    toolHints: ["connector.invoke"],
    reason: "connector_execution_requires_ember_policy_and_secret_binding",
  },
};
...
function classifyToolIntegrationCapability(
  ...
  if (
    ...
    return "ember.terminal";
  }
  if (token.includes("connector")) {
    return "ember.connectors";
  }
  const capability = (
...
function buildDeclaredToolRun(
  ...
  return {
    ...
    statusText: "已声明",
    message:
      "Agent App task 声明了该 ToolRuntime intent，实际执行仍由 Ember AgentRuntime 管理。",
    source: "agent_runtime_process",
  };
}
...
function buildToolExecutionPolicy(
  capability: ToolIntegrationCapability | "ember.tools",
  reason: string,
): ToolExecutionPolicyProjection {
  const requiresApproval = new Set<ToolIntegrationCapability | "ember.tools">([
    "ember.browser",
    "ember.mcp",
    "ember.terminal",
    "ember.connectors",
  ...
  const policy: ToolExecutionPolicyProjection = {
    ...
    sandboxRequired: capability === "ember.terminal",
    mutationExposed: false,
    tokenExposed: false,
    reason,
  };
  if (capability === "ember.connectors") {
...
function buildToolExecutionHandoffTaskRequest(
  ...
  return {
    ...
    prompt: [
      ...
      "",
      "请由 Ember AgentRuntime / ToolRuntime policy owner 审核并执行该请求。",
...
function buildConnectorAuthorizationHandoffTaskRequest(
  ...
  return {
    ...
    idempotencyKey:
      ...
      }:ember.connectors:requestAuth:${authorizationRequest.connectorId}`,
    prompt: [
      "【Agent App Connector Authorization Request】",
      `Connector: ${authorizationRequest.connectorId}`,
      "",
      "请由 Ember Host / Connector policy owner 创建或恢复 host-managed 授权绑定。",
    ...
    expectedOutput: {
      ...
      secretBinding: "host_managed",
      tokenExposed: false,
      source: "ember_connector_policy",
    },
    requiredCapabilities: ["ember.connectors"],
    capabilityHints: [
      "ember.connectors",
      `connector:${authorizationRequest.connectorId}`,
    ],
...
function readConnectorAuthorizationRequestFromResponse(
  ...
  return isRecord(request) &&
    request.capability === "ember.connectors" &&
...
function buildToolIntentResponse(
  ...
  const response: ToolIntentResponse = {
    ...
    next: {
      ...
      method: "startTask",
      reason: "actual_tool_execution_is_owned_by_ember_agent_runtime",
    },
  };
...
function buildGenericToolIntentResponse(
  ...
  const reason = "tool_execution_requires_ember_tool_runtime_policy";
...
function dispatchTasks(
  ...
  if (request.method === "cancel") {
    ...
    return {
      status: "not_available",
      reason: "task_cancellation_must_use_ember_agent_cancel_task",
      source: "agent_runtime_projection",
      next: {
        capability: "ember.agent",
        method: "cancelTask",
  ...
  if (request.method === "subscribe") {
    ...
    return {
      status: "not_available",
      reason: "task_subscription_must_use_ember_agent_stream_task",
      source: "agent_runtime_projection",
      next: {
        capability: "ember.agent",
        method: "streamTask",
...
async function dispatchSearch(
  ...
  const input = readOptionalInputRecord(request);
  const tasks = filterRuntimeProjectionTasks(host, request);
  const runs = buildRuntimeToolRuns(tasks, "ember.search");
...
async function dispatchBrowser(
  ...
  const input = readOptionalInputRecord(request);
  const tasks = filterRuntimeProjectionTasks(host, request);
  const runs = buildRuntimeToolRuns(tasks, "ember.browser");
...
async function dispatchDocuments(
  ...
  const input = readOptionalInputRecord(request);
  const tasks = filterRuntimeProjectionTasks(host, request);
  const runs = buildRuntimeToolRuns(tasks, "ember.documents");
...
async function dispatchMedia(
  ...
  const input = readOptionalInputRecord(request);
  const tasks = filterRuntimeProjectionTasks(host, request);
  const runs = buildRuntimeToolRuns(tasks, "ember.media");
  ...
  const response = buildToolIntentResponse(request, "ember.media", input, runs);
  return attachToolExecutionHandoff(
    response,
    normalizeStringList(response.toolHints),
    resolveSdk,
...
async function dispatchMcp(
  ...
  const input = readOptionalInputRecord(request);
  const tasks = filterRuntimeProjectionTasks(host, request);
  const runs = buildRuntimeToolRuns(tasks, "ember.mcp");
  ...
  if (request.method === "invoke") {
    ...
    const response = buildToolIntentResponse(request, "ember.mcp", input, runs);
    return attachToolExecutionHandoff(
      response,
      normalizeStringList(response.toolHints),
      resolveSdk,
...
async function dispatchTerminal(
  ...
  const input = readOptionalInputRecord(request);
  const tasks = filterRuntimeProjectionTasks(host, request);
  const runs = buildRuntimeToolRuns(tasks, "ember.terminal");
  if (request.method === "getRun") {
    return readToolRun(request, runs);
  }
...
async function dispatchConnectors(
  ...
  const input = readOptionalInputRecord(request);
  const tasks = filterRuntimeProjectionTasks(host, request);
  const runs = buildRuntimeToolRuns(tasks, "ember.connectors");
  ...
  if (request.method === "requestAuth") {
    const connectorId = readStringParam(request, "connectorId", 0);
    const reason = "connector_auth_requires_ember_policy_and_secret_binding";
...
export function createAgentAppCapabilityDispatcher({
  ...
  return async (request) => {
    if (request.capability === "ember.capabilities") {
