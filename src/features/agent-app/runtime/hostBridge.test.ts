...
describe("AgentAppHostBridge", () => {
  ...
  it("构建 host snapshot 时应携带 Ember 主题 token 和 Host 上下文", () => {
  ...
  it("App ready 后应补发 snapshot，主题事件应广播 theme:update", () => {
    ...
    window.dispatchEvent(new CustomEvent("ember-theme-changed"));
  ...
  it("host:openExternal 只允许 http/https 外链", async () => {
    ...
    expect(openExternal).toHaveBeenCalledWith("https://ember.ai/");
    expect(postMessage).toHaveBeenCalledWith(
      expect.objectContaining({
        type: "host:response",
        payload: { opened: true },
  ...
  it("host:openExternal 默认应委托系统浏览器 current 网关", async () => {
    ...
    expect(
      externalUrlApiMocks.openExternalUrlWithSystemBrowser,
    ).toHaveBeenCalledWith("https://ember.ai/docs");
  ...
  it("ember.ui openExternal 默认网关失败时应返回错误而不是 window.open 兜底", async () => {
  ...
  it("ember.ui selectDirectory 应调用宿主封装目录选择命令", async () => {
  ...
  it("支持订阅 ember.agent task 并把 Host 侧更新推送回 App iframe", async () => {
    vi.useFakeTimers();
    const { frame, postMessage } = buildFrame();
    const dispatchCapability = vi.fn().mockResolvedValue({
      taskId: "task-1",
  ...
  it("订阅 ember.agent task 时应把 sessionId 一起用于 replay 轮询", async () => {
    vi.useFakeTimers();
    const { frame, postMessage } = buildFrame();
    const dispatchCapability = vi.fn().mockResolvedValue({
      taskId: "task-1",
    ...
    const dispatchCapability = vi
      ...
      .mockResolvedValueOnce({
        ...
        result: {
          artifacts: [
            {
              path: ".ember/artifacts/scene-table.json",
              title: "场景表创建中",
              metadata: {},
            },
          ],
    ...
    const dispatchCapability = vi
      ...
      .mockResolvedValueOnce({
        ...
        result: {
          artifacts: [
            {
              path: ".ember/tasks/image_generate/task-logo-1.json",
              title: "应用 Logo",
              metadata: {
                image: {
                  absolute_artifact_path:
                    "/workspace/app/.ember/tasks/image_generate/task-logo-1.json",
                },
              },
            },
          ],
