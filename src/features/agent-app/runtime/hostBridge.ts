...
export const AGENT_APP_BRIDGE_PROTOCOL = "ember.agentApp.bridge";
...
function collectCssVariableTokens(root: HTMLElement): Record<string, string> {
  ...
  const appendToken = (name: string, value: string) => {
    if (!name.startsWith("--ember-") && !name.startsWith("--app-")) {
      return;
    }
...
function normalizeCapabilities(
  ...
  return {
    available: Array.from(
      new Set(["ember.ui", ...capabilities.available]),
    ).sort(),
    blocked: Array.from(
      new Set(capabilities.blocked.filter((item) => item !== "ember.ui")),
    ).sort(),
  };
}
...
export function buildAgentAppThemePayload(
  ...
  const effectiveThemeMode = getEffectiveEmberThemeMode(themeMode);
  return {
    themeMode,
    effectiveThemeMode,
    colorSchemeId: loadEmberColorSchemeId(),
...
export class AgentAppHostBridge {
  ...
  start(): () => void {
    ...
    window.addEventListener(EMBER_THEME_CHANGED_EVENT, this.handleThemeChanged);
  ...
  private handleUiCapabilityInvoke(
    ...
    if (request.capability !== "ember.ui") {
      return null;
    }
  ...
  private enrichAgentCapabilityResult(
    ...
    if (request.capability !== "ember.agent" || !isRecord(result)) {
      return result;
    }
  ...
  private async handleCapabilitySubscribe(
    ...
    if (
      ...
      throw new AgentAppHostBridgeActionError(
        "INVALID_PAYLOAD",
        "capability:subscribe currently requires ember.agent task payload.taskId.",
      );
    }
