...
import { buildEmberRuntimeProfileForPreview } from "../runtime-profile";
...
import { createAgentAppCapabilityDispatcher } from "./capabilityDispatcher";
...
export function buildDispatcher(
  ...
  const dispatch = createAgentAppCapabilityDispatcher({
    ...
    runId: "bridge-run-1",
    profile,
    runtimeProfile: buildEmberRuntimeProfileForPreview({
      preview,
      hostProfile: profile,
    }),
    ...standardProfile,
...
export function buildRuntimeProjectionDispatcher(
  ...
  const connectorAuthorizationTask: AgentAppTaskRecord = {
    ...
    prompt: "请由 Ember Host 创建 host-managed Slack 授权绑定。",
