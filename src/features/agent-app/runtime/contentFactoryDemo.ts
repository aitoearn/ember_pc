...
export const defaultContentFactoryProjectInput: ContentFactoryProjectInput = {
  projectName: "测试工作台样板项目",
  industry: "软件测试",
  targetPlatforms: ["Web", "移动端"],
  audience: "测试团队 / QA",
  requiresIpKnowledge: true,
  contentGoal: "基于测试上下文生成场景表、用例资产和回归清单。",
};
...
function buildScenarios(params: {
  ...
  return records.slice(0, 3).map((record, index) => ({
    ...
    dimension: record.bindingKey === "ip_knowledge" ? "项目背景" : "核心功能",
    painPoint:
      index === 0
        ? "每次回归都要重新解释项目背景与测试边界。"
        : "测试设计缺少可复用的场景和用例结构。",
    solution:
      index === 0
        ? "把项目背景、测试规范和环境约束拆成可检索知识块。"
        : "用测试场景规划表把路径、风险点和验证切口结构化。",
...
function buildContentAssets(params: {
  ...
  return params.scenarios.flatMap((scenario, scenarioIndex) =>
    params.project.targetPlatforms.slice(0, 2).map((platform, platformIndex) => ({
      ...
      title: `${platform}｜${scenario.dimension}验证切口 ${scenarioIndex + 1}`,
