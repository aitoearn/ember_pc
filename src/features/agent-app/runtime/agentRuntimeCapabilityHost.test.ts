...
describe("AgentRuntimeCapabilityHost", () => {
  ...
  it("可以直接注入标准 AgentRuntimeClient 驱动 ember.agent task", async () => {
              ...
              artifacts: [
                {
                  ...
                  path: ".ember/artifacts/standard.json",
                  title: "标准任务产物",
                  status: "completed",
                },
              ],
    ...
    expect(snapshot).toMatchObject({
      ...
      events: expect.arrayContaining([
        expect.objectContaining({
          ...
          refs: [".ember/artifacts/standard.json"],
        }),
      ]),
    });
  });

  it("把 ember.agent start/get/cancel/retry 适配到 Agent App Runtime facade", async () => {
    let startCounter = 0;
    const api = {
      ...
      getTask: vi.fn(async (request) => ({
        ...
        taskEvents: [
          ...
          },
          {
            ...
            status: "created",
            message: "内容批次已创建",
            artifactRef: ".ember/artifacts/content-batch.json",
    ...
    expect(snapshot).toMatchObject({
      ...
      events: expect.arrayContaining([
        ...
        expect.objectContaining({
          type: "artifact:created",
          refs: [".ember/artifacts/content-batch.json"],
  ...
  it("从 threadRead artifacts 补投 artifact:created payload，保证 Host Bridge 可 replay 最终产物", async () => {
    ...
    expect(snapshot?.events).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          type: "evidence:recorded",
          refs: ["evidence:.ember/artifacts/content-batch.json"],
          payload: expect.objectContaining({
