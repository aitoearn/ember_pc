...
describe("Agent App projection P0", () => {
  ...
  it("应把 v0.8 app.install.yaml contract 投影为 install projection", () => {
    const manifest = parseManifest({
      ...
      install: {
        ...
        runtime: {
          minVersion: "0.8.0",
          distribution: {
            standalone: { embedRuntime: true, shell: "ember-app-shell" },
            runtimeBacked: { requires: "ember-runtime", minVersion: "0.8.0" },
          },
        },
        standalone: {
    ...
    expect(projection.install).toMatchObject({
      ...
      runtimeRequirements: [
        ...
        { mode: "runtime_backed", minVersion: "0.8.0", requires: "ember-runtime" },
      ],
      shellRequirements: [
        {
          mode: "standalone",
  ...
  it("应从 projection 生成 cleanup dry-run", () => {
    ...
    expect(plan.installedStatePaths[0].value).toBe(
      "/tmp/ember/agent-apps/installed/content-factory-app.json",
    );
    expect(plan.packageCachePaths[0].value).toContain(identity.packageHash);
    expect(plan.packageCacheIndexPaths[0].value).toBe(
      "/tmp/ember/agent-apps/package-index/content-factory-app.json",
    );
    expect(plan.packageStagingPaths[0].value).toBe(
      "/tmp/ember/agent-apps/staging/content-factory-app",
    );
    expect(plan.projectionPaths[0].value).toBe(
      "/tmp/ember/agent-apps/projections/content-factory-app.json",
    );
    expect(plan.setupStatePaths[0].value).toBe(
      "/tmp/ember/agent-apps/setup/content-factory-app.json",
    );
