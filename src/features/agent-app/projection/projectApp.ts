...
function collectGlobalRequirements(
  ...
  if (manifest.storage) {
    upsertRequirement(requirements, {
      ...
      requestedRange: manifest.requires.capabilities["ember.storage"] ?? "*",
      declaredBy: "storage",
    });
  }
  ...
  if (manifest.runtimePackage.ui) {
    upsertRequirement(requirements, {
      capability: "ember.ui",
      requestedRange: manifest.requires.capabilities["ember.ui"] ?? "*",
  ...
  if (manifest.runtimePackage.worker) {
    upsertRequirement(requirements, {
      ...
      requestedRange: manifest.requires.capabilities["ember.workflow"] ?? "*",
      declaredBy: "runtimePackage",
      required: false,
    });
  }
