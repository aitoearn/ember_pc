...
function buildMacOsStandaloneTarget() {
  return {
    ...
    macosIdentity: buildMacOsStandaloneIdentity({
      ...
      appGroups: ["group.com.limecloud.plugins"],
      keychainAccessGroups: ["TEAMID1234.com.limecloud.plugins"],
    }),
    productionReady: false as const,
  };
}

describe("Plugin v2 package descriptor", () => {
  ...
  it("native shell registration plan 应输出独立 Bundle ID 与 per-app deep link 配置", () => {
    ...
    expect(plan.bundleIdentifier).toBe("com.limecloud.plugin.contentfactory");
    ...
    expect(plan.runtimeEnv).toEqual({
      ...
      LIME_PLUGIN_STANDALONE_ENTRY_KEY: "dashboard",
    ...
    expect(plan.menu.items.map((item) => item.labelKey)).toEqual([
      "plugin.shell.menu.about",
      "plugin.shell.menu.open",
      "plugin.shell.menu.checkUpdates",
      "plugin.shell.menu.quit",
    ]);
  ...
  it("standalone native shell config write plan 应生成可落盘的 config/env 文件计划", () => {
    ...
    const first = buildStandaloneNativeShellConfigWritePlan({
      ...
      configOutputPath:
        "<dist>/plugins/content-factory/ember-rs/native-shell.config.json",
      envOutputPath: "<dist>/plugins/content-factory/.env.standalone",
    });
    ...
    expect(first.files[0]).toMatchObject({
      path: "<dist>/plugins/content-factory/ember-rs/native-shell.config.json",
      encoding: "utf8",
      sensitive: false,
    });
    ...
    expect(first.files[1]).toMatchObject({
      path: "<dist>/plugins/content-factory/.env.standalone",
      encoding: "utf8",
      sensitive: false,
    });
    ...
    expect(first.files[1].content).toContain(
      "LIME_PLUGIN_STANDALONE_DEEP_LINK_SCHEME=lime-agent-content-factory-app",
    );
  });
  ...
  it("production artifact build plan 应暴露真实 builder / signer / updater adapter 需求", () => {
    ...
    const buildPlan = buildStandaloneArtifactBuildPlan({
      releasePlan,
      outputDirectory: "<dist>/plugins/content-factory",
      nativeShellConfig: {
        materializerResult: materializedConfig,
        configOutputPath:
          "<dist>/plugins/content-factory/ember-rs/native-shell.config.json",
