...
export function buildAgentRunProjectionViewModelFromState(
  ...
  return buildPluginRunProjectionViewModelFromStandardState(
...
export function buildAgentRunStandardProjectionStateFromState(
  ...
  const executionEvents = buildPluginStandardRuntimeEvents({
    ...context,
    events,
  });
...
function buildProjectionBridgeContext(
  state: unknown,
  options: AgentRunProjectionStateOptions,
): Omit<PluginAgentUiProjectionBridgeOptions, "events"> {
  const root = isRecord(state) ? state : {};
  const task = recordValue(root, "task");
  const snapshot = recordValue(root, "snapshot");
  return {
