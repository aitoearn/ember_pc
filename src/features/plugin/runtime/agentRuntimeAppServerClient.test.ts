...
import { withEvent } from "@embercloud/agent-runtime-client/sessionGateway";
import {
...
describe("agentRuntimeAppServerClient", () => {
  ...
  it("拒绝非 canonical Plugin synthetic event，同时保留 pipeline 配置", async () => {
    ...
    const runtimeClient = createPluginRuntimeClientFromAppServer(
      appServerClient,
      {
        ...
        adapters: [
          ({ notification, event }) => {
            if (event.type !== "plugin.synthetic") {
              return;
            }
            return [
              withEvent(notification, {
                ...(event as AppServerAgentEvent),
                eventId: "evt-plugin-a",
                type: "message.delta",
                payload: { text: "A" },
              }),
              withEvent(notification, {
                ...(event as AppServerAgentEvent),
                eventId: "evt-plugin-b",
    ...
    await expect(
      runtimeClient.dispatchEvent({
        ...
        params: {
          event: {
            eventId: "evt-plugin-synthetic",
            sequence: 1,
            sessionId: "session-app-server",
            type: "plugin.synthetic",
  ...
  it("通过 agentSession/start 为 Plugin 默认宿主创建 current session", async () => {
    const appServerClient = buildAppServerClient();
    ...
    expect(appServerClient.startSession).toHaveBeenCalledWith({
      ...
      businessObjectRef: expect.objectContaining({
        kind: "plugin.task",
        id: "content-factory-app:plugin-task-1",
        title: "生成内容场景",
