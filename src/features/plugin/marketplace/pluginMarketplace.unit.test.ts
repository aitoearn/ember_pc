...
describe("Plugin marketplace projection", () => {
  it("应从 LimeCore marketplace item 建立 plugin contract provenance", () => {
    ...
    expect(contract).toMatchObject({
      ...
      provenance: {
        ...
        sourceId: "research-kit@embercloud",
        sourceVersion: "1.2.3",
        packageHash:
          "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        manifestHash:
  ...
  it("应把 available marketplace item 投影为可安装，安装后才可激活", () => {
    ...
    const installedRegistry = projectPluginMarketplaceRegistry(
      marketplace([item]),
      {
        installedPluginKeys: ["research-kit@embercloud"],
      },
    );
  ...
  it("应从 installed Plugin state 合并 installed / enabled key，hash 不一致仅标记可刷新", () => {
    ...
    const mismatchItem = marketplaceItem({
      pluginKey: "broken-kit@embercloud",
    ...
    expect(projection.installedPluginKeys).toEqual([
      ...
      "broken-kit@embercloud",
    ]);
    expect(projection.enabledPluginKeys).toEqual([
      "research-kit@embercloud",
      "broken-kit@embercloud",
    ]);
    expect(projection.disabledPluginKeys).toEqual(["notes-kit@embercloud"]);
    expect(projection.blockerCodesByPluginKey).toEqual({
      "broken-kit@embercloud": ["PLUGIN_INSTALLED_PACKAGE_MISMATCH"],
    });
    ...
    expect(
      registryItemByPluginId(registry, "research-kit@embercloud"),
    ).toMatchObject({
      installed: true,
      enabled: true,
      capabilityStates: ["activatable"],
    ...
    expect(
      registryItemByPluginId(registry, "notes-kit@embercloud"),
    ).toMatchObject({
      installed: true,
      enabled: false,
    ...
    expect(
      registryItemByPluginId(registry, "broken-kit@embercloud"),
    ).toMatchObject({
      installed: true,
      enabled: true,
      activationState: "activatable",
