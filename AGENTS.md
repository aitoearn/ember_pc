...
本文件只用于 **开发 Ember 源码仓库本身**。根 `AGENTS.md` 只保留仓库级约束、导航和统一入口；模块细节与长流程统一下沉到 `internal/`。
...
21. **Rust crate 抗膨胀** - 新增 Rust 逻辑禁止默认落 `ember-core` / `services` 平铺层，必须先回答"为什么不是独立模块 / 既有 domain"；参照 Codex "resist adding to core"（见 `internal/refactor/codex-engineering-patterns.md` § 2）。`processor.rs` / `runtime.rs` 等中心文件新增方法时，优先放到对应 domain 子模块（如 `processor/project_git.rs`），中心文件只做 dispatch 接线
...
6. **Playwright 续测优先稳定桌面 Chrome 会话** - 真实交互验证优先复用已有 Ember 页签；需要新启浏览器时走持久化 Chrome 上下文，避免本地桌面出现自动化横幅或 `--no-sandbox` 安全横幅，细则见 `internal/aiprompts/playwright-e2e.md`
