...
| `ember-rs/src/commands/agent_cmd/command_api/runtime_api.rs` | legacy `agent_runtime_*` cleanup reference | 迁移期兼容入口定位；迁出后删除，不作为新增能力入口 |
| `ember-rs/src/commands/agent_cmd/runtime_turn.rs` | turn orchestration cleanup reference | 现有行为锚点；新执行编排应进入 RuntimeCore / ExecutionBackend |
| `ember-rs/src/commands/agent_cmd/session_runtime.rs` | session runtime cleanup reference | sidebar/session summary 和 tab 管理的迁移来源；新读写进入 App Server / RuntimeCore |
| `ember-rs/src/commands/agent_cmd/subagent_runtime.rs` | subagent cleanup reference | team/capsule 子代理状态迁移来源；新实现进入 runtime service / `ember-rs/crates/agent` |
| `ember-rs/src/commands/agent_cmd/action_runtime.rs` | action response cleanup reference | HITL、权限确认、用户输入请求迁移来源；新 action 合同进入 App Server protocol |
| `ember-rs/src/commands/agent_cmd/tool_runtime/*` | tool cleanup reference | browser、workspace、service skill、media、mcp、subagent、site 的历史工具锚点；新工具事实进入 RuntimeCore/tool services |
| `ember-rs/src/commands/agent_cmd/dto.rs` | DTO cleanup reference | `AgentRuntimeSessionDetail`、history cursor、thread_read 投影迁移来源；公共 DTO 进入 protocol / `ember-rs/crates/agent` |
...
| `ember-rs/crates/agent/src/event_converter.rs` | Agent event -> Lime runtime event | text/thinking/tool/artifact/status 的协议生成点 |
...
| `ember-rs/src/services/artifact_document_service.rs` | ArtifactDocument 持久化、版本、`.lime/artifacts` | Artifact layer 主事实源 |
