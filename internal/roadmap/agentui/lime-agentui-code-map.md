...
flowchart TB
  ...
  AgentCrate[ember-rs/crates/agent runtime crate]
  Services[ember-rs/src/services]
  Store[(SQLite + .ember files)]
...
本层只用 `ember-rs/src/commands/**` 定位历史迁移锚点。该目录是旧 Tauri command wrapper 删除清理区，不是 AgentUI 的 current 实现目录；新增 runtime、tool、session、thread read、evidence 或 DTO 能力必须进入 App Server JSON-RPC、RuntimeCore、`ember-rs/crates/agent` 或 services。旧文件完成迁出后应撤 runner / DevBridge / catalog / mock 注册并删除，删不动时登记 blocker。
...
| `ember-rs/src/commands/aster_agent_cmd/command_api/runtime_api.rs` | legacy `agent_runtime_*` cleanup reference | 迁移期兼容入口定位；迁出后删除，不作为新增能力入口 |
| `ember-rs/src/commands/aster_agent_cmd/runtime_turn.rs` | turn orchestration cleanup reference | 现有行为锚点；新执行编排应进入 RuntimeCore / ExecutionBackend |
| `ember-rs/src/commands/aster_agent_cmd/session_runtime.rs` | session runtime cleanup reference | sidebar/session summary 和 tab 管理的迁移来源；新读写进入 App Server / RuntimeCore |
| `ember-rs/src/commands/aster_agent_cmd/subagent_runtime.rs` | subagent cleanup reference | team/capsule 子代理状态迁移来源；新实现进入 runtime service / `ember-rs/crates/agent` |
| `ember-rs/src/commands/aster_agent_cmd/action_runtime.rs` | action response cleanup reference | HITL、权限确认、用户输入请求迁移来源；新 action 合同进入 App Server protocol |
| `ember-rs/src/commands/aster_agent_cmd/tool_runtime/*` | tool cleanup reference | browser、workspace、service skill、media、mcp、subagent、site 的历史工具锚点；新工具事实进入 RuntimeCore/tool services |
| `ember-rs/src/commands/aster_agent_cmd/dto.rs` | DTO cleanup reference | `AgentRuntimeSessionDetail`、history cursor、thread_read 投影迁移来源；公共 DTO 进入 protocol / `ember-rs/crates/agent` |
...
| 文件 | 当前职责 | AgentUI 意义 |
| --- | --- | --- |
| `ember-rs/crates/agent/src/runtime_queue.rs` | turn queue、resume、queue event | queue/capsule/task layer 的事实源 |
| `ember-rs/crates/agent/src/session_store.rs` | session detail、messages、turns、items、todo、runtime overlay、subagent context | old session restore、history pagination、thread_read 的底层事实源 |
| `ember-rs/crates/agent/src/session_query.rs` | parent/child/cascade 查询 | team/subagent 视图 |
| `ember-rs/crates/agent/src/event_converter.rs` | Aster event -> Ember runtime event | text/thinking/tool/artifact/status 的协议生成点 |
| `ember-rs/crates/agent/src/protocol_projection.rs` | current projection 入口 | 新 UI 应优先跟 current projection 对齐 |
| `ember-rs/crates/agent/src/runtime_projection_snapshot.rs` | runtime summary snapshot | tab/capsule/sidebar summary 可复用 |
| `ember-rs/crates/agent/src/session_execution_runtime.rs` | execution runtime/cost/limit/routing | provider/model/cost/context usage UI |
| `ember-rs/crates/agent/src/queued_turn.rs` | queue snapshot 数据结构 | queue panel、task capsule |
| `ember-rs/crates/agent/src/tool_io_offload.rs` | tool 大输出 offload | tool UI 大输出不卡顿的后端基础 |
| `ember-rs/crates/agent/src/write_artifact_events.rs` | artifact write event | artifact snapshot / workbench 联动 |

## 7. Service 层

| 文件 | 当前职责 | AgentUI 使用方式 |
| --- | --- | --- |
| `ember-rs/src/services/agent_timeline_service.rs` | `AgentTimelineRecorder` 持久化 runtime event -> turn/item | Process layer 与 Evidence layer 的过程事实 |
| `ember-rs/src/services/artifact_document_service.rs` | ArtifactDocument 持久化、版本、`.ember/artifacts` | Artifact layer 主事实源 |
| `ember-rs/src/services/artifact_ops_service.rs` | incremental artifact envelope | artifact.begin/meta/source/block/complete/fail |
| `ember-rs/src/services/runtime_evidence_pack_service.rs` | evidence pack 导出 | Evidence layer 输出 `summary.md/runtime.json/timeline.json/artifacts.json` |
| `ember-rs/src/services/runtime_review_decision_service.rs` | review decision 模板与保存 | Human review 闭环 |
| `ember-rs/src/services/runtime_replay_case_service.rs` | replay case | 失败复现与验证 |
| `ember-rs/src/services/runtime_handoff_artifact_service.rs` | handoff bundle | 跨 agent/人工交接 |
| `ember-rs/src/services/runtime_file_checkpoint_service.rs` | file checkpoint / diff | 代码任务可审查变更 |
| `ember-rs/src/services/thread_reliability_projection_service.rs` | reliability projection | thread_read、incident、pending request |
...
| `.ember/artifacts` | artifact documents、版本、snapshot | Artifact layer |
| `.ember/harness/sessions/<session>/evidence` | evidence pack | Evidence panel |
| `.ember/harness/sessions/<session>/review` | review decision | Review dialog/panel |
...
| Backend history/session | `ember-rs/crates/agent/src/session_store.rs` unit tests |
