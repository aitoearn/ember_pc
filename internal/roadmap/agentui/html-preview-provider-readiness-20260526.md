...
- 未点击真实 Mimo 试跑按钮，原因是该动作会调用真实 Provider 并可能消耗用户余额。仓库 live Provider 门禁也要求显式 `--allow-live-provider` 或 `EMBER_ALLOW_LIVE_PROVIDER_SMOKE=1 / EMBER_REAL_API_TEST=1` 才允许真实 Provider smoke。
...
- 为拆解上述超时，已单跑 Rust 定向测试：
  ...
  - `ember-agent tools::skill_tool_gate::tests::`：编译 `aster-core` 阶段失败于 `No space left on device (os error 28)`；当前磁盘可用约 `797MiB`，`ember-rs/target` 约 `116G`。
- 再次尝试单跑 `ember-agent tools::skill_tool_gate::tests::`：仍在构建 `libaster-*.rlib` 时失败于 `No space left on device (os error 28)`；失败后磁盘可用约 `809MiB`。
