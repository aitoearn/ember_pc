...
- 为拆解上述超时，已单跑 Rust 定向测试：
  ...
  - `commands::agent_cmd::workspace_skill_binding_prompt::tests::should_project_workspace_skill_runtime_enable_as_callable_scope`：通过。
  - `lime-agent tools::skill_tool_gate::tests::`：编译 `agent-core` 阶段失败于 `No space left on device (os error 28)`；当前磁盘可用约 `797MiB`，`ember-rs/target` 约 `116G`。
