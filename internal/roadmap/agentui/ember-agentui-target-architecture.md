...
> 状态：目标架构
> 更新时间：2026-04-30
> 适用范围：`src/components/agent/chat` 主工作区、`src/lib/api/agentRuntime` 前端协议层、`ember-rs` agent runtime 主链。

## 1. 架构原则

Ember 的下一代 AgentUI 应是一个桌面工作台，而不是单列聊天页。它应当同时回答：
...
| 产物层 Artifact | 最终交付物在哪里继续编辑 | canvas、artifact preview、workbench | `artifact_snapshot`、ArtifactDocument、`.ember/artifacts` |
...
flowchart TB
  ...
  TimelineRecorder --> Db
  ArtifactServices --> ArtifactFiles[.ember/artifacts]
  EvidenceServices --> HarnessFiles[.ember/harness]
...
- 不为了视觉新鲜感引入和 Ember 现有设计语言冲突的大面积渐变、半透明主表面或嵌套卡片。
