...
| --- | --- | --- | --- |
| `/internal/roadmap/agentui/images/agentui-stream-latency-map-20260509.svg 要更新一下` | `internal/roadmap/agentui/images/agentui-stream-latency-map-20260509.svg` | SVG v6 已更新为“矩阵 gate 已通过，provider/model 波动已分类，剩余为 fallback 解释优化” | 覆盖 |
...
- Playwright 页面：`http://127.0.0.1:1420/`，标题 `Ember`。
...
| 缺口 | 当前状态 | 是否阻塞完成 |
| --- | --- | --- |
| fallback-only group 的错误分类枚举 | Mimo / Ember Hub / OpenRouter 已有 error-only 聚合证据，但文档不记录 `error_message` | 不阻塞；后续可补脱敏 reason code |
