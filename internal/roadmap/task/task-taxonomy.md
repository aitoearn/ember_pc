...
`workspace_preferences.service_models` 已经是现有 Ember 的任务级模型设置入口，因此后续必须明确映射：
...
**把 Ember 已经零散存在的服务模型设置、内部辅助任务和主对话任务，收敛成一份统一 `TaskProfile` 语言。**
