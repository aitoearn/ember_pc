# Ember 任务层 / 模型层 / 经济调度路线图

> 状态：方案已落盘，current runtime 基础链路已接入
> 更新时间：2026-04-23
> 作用：把 Ember 的“任务层与模型层分离、成本/限额沉底、OEM 与本地多模型协同、自动与设置平衡”收敛成一套可直接指导实现的专题路线图，而不是继续散落在聊天选择器、`service_models` 设置、OEM offer 与 runtime 局部逻辑里。
> 依赖文档：
> - `internal/aiprompts/query-loop.md`
> - `internal/aiprompts/task-agent-taxonomy.md`
> - `internal/aiprompts/state-history-telemetry.md`
> - `internal/aiprompts/providers.md`
...
1. Ember 为什么要把“任务层”和“模型层”拆开，即使当前只配了一个模型也要拆。
2. Ember 当前已有的设置面，例如聊天模型选择、`workspace_preferences.service_models`、媒体默认模型、OEM `providerPreference/defaultModel`，将来在调度里各算什么角色。
...
3. **设置不是障碍物，也不是唯一事实源**
   Ember 现有设置面必须进入调度链，但不能继续各自偷偷决定运行时真实模型。
...
1. `runtime_turn.rs` 已把 `task_profile / routing_decision / limit_state` 写入 `turn_context.metadata.ember_runtime`
...
- [event-chain.md](./event-chain.md)：对照 Claude Code 事件分层，固定 Ember 任务 / 路由 / 经济事件如何接到 current 主链
- [runtime-integration.md](./runtime-integration.md)：说明如何挂回 Ember current 主链
...
**它把 Ember 当前分散在会话模型选择、服务模型设置、OEM offer、configured providers / API Key Provider / 模型注册表和 runtime 恢复链里的模型决策，收敛成一条可解释、可落盘、可验收的 current 设计主线。**
