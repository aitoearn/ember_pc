...
```bash
rg -n "make-next-generation-more-like-me|active memory|external memory provider|ember-rs/src" "internal/roadmap/memory"
...
2. 旧 `ember-rs/crates/memory/**` SQLite memory crate、App Server 旧 `local_data_source/unified_memory.rs` 和旧 `processor/unified.rs` 已删除。
...
11. Rust memory store current 后端验证：2026-06-19，`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server memory_store --lib` 通过，覆盖 16 个 memory store 后端测试。
