...
> 更新时间：2026-05-01  
> 目标：把 Ember 的底层记忆能力收敛成面向创作者的 `灵感库` 产品，而不是把 Claude Code 式记忆工作台直接暴露给普通用户。

## 1. 本路线图回答什么

本目录统一回答下面几类问题：

1. Ember 为什么继续参考 Claude Code 的记忆架构，但不照搬它的前台形态。
...
4. [../embernextv2/README.md](../embernextv2/README.md)
5. [../embernextv2/product-principles.md](../embernextv2/product-principles.md)

## 3. 固定结论

### 3.1 Claude Code 是架构参考，不是前台模板

Ember 继续学习 Claude Code 的：
...
**底层按 Claude Code 分层，前台按 Ember 创作者心智重写。**
...
**Ribbi 是 Ember 的产品形态参考；Claude Code、OpenClaw、Hermes 是底层记忆架构参考。**
...
**Ember 的顶层能力叫 `Personalization Context Orchestration / 个性化上下文编排`；`Promptlet Router` 只是选择细粒度 promptlet 的子模块。**
...
9. Codex 虽然有 `use_memories / generate_memories` 开关，但它面向开发者配置；对 Ember 的借鉴是拆 read / write / enhancement，而不是给普通用户一个总失忆开关。
...
**把 Ember 已经存在的 memory runtime、unified memory、结果沉淀和推荐信号收成一个面向创作者的灵感库产品闭环，同时保留 Claude Code 式底层治理能力。**
