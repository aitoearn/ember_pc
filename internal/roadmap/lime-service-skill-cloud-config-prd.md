# Ember 服务型技能：目录云同步、本地执行 PRD

> 状态：current supporting cross-repo plan  
> 更新时间：2026-04-21  
> 当前主规划：`internal/roadmap/embernextv2/README.md`
> 目标：吸收 Ribbi 式服务型技能入口的方法，但保持 `embercore` 只做目录控制、`ember` 客户端负责全部执行
> 作用域：本文只保留 `ServiceSkill` 在 `ember` / `embercore` 之间的产品对象、目录边界与本地执行原则，不替代 `internal/aiprompts/command-runtime.md`、`internal/aiprompts/embercore-collaboration-entry.md`
...
1. 云端不会执行任何东西
2. `embercore` 只负责目录控制、发布治理与配置同步
3. `ember` 客户端负责全部 `@` 命令、`/scene`、`service skill` 执行
...
**`embercore` 不执行 `@` 命令，不执行 `/scene`，不执行 `service skill`。**
...
1. `embercore` 是在线目录事实源，而不是运行时执行面
2. `ember` 客户端负责：
   - 参数采集
   - 本地运行时路由
...
**EmberCore 决定前台能看到什么，Ember 决定真正如何执行。**
...
```mermaid
flowchart TB
    subgraph UX["用户交互层（Ember 客户端）"]
        Home["首页服务型技能入口"]
        Slots["参数采集 / Slot Filling"]
        Workspace["Workspace / 任务中心"]
    end

    subgraph Client["客户端执行层（ember）"]
    ...
    subgraph Control["目录控制层（embercore）"]
...
- 把 `ember_run_service_skill` 解释成云端调用入口
...
服务端文档纠偏由 `embercore/internal/exec-plans/at-command-control-plane-only.md` 统一承接。
