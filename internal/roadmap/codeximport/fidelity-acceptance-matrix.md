...
`current` 只允许继续向 App Server JSON-RPC、RuntimeCore、Agent UI projection 和 Preview Artifact Contract 收敛。Renderer 不直接扫描 `.codex`；Electron Desktop Host 只负责桌面桥接；旧 Tauri / `ember-rs/src/**` / 旧 `agent_runtime_*` 不作为新增能力落点。
...
| high-volume rollout | 上千 tool / command event | 所有 normalized events 全量进入 canonical EventLog / ThreadStore；`thread/items/list` 稳定游标分页 | current | `commit_preserves_high_volume_codex_tool_events_in_canonical_projection`、真实大样本 visual audit |
