# Ember Harness Engine 对照路线图与长期检查表

> 状态：进行中，P0 已完成首刀收口
> 更新时间：2026-04-13
> 对照基线：LangChain 博文《The Anatomy of an Agent Harness》
> 目标：把 Ember 当前已经具备的 Harness 能力、真实缺口、后续建设优先级和长期复查口径收敛到一份可执行文档，而不是继续停留在抽象口号层。

配套图纸：

- `internal/roadmap/harness-engine/diagrams.md`

## 1. 先给结论

按 LangChain 这篇文章的标准看，Ember **已经不是“只有模型壳”的产品**，而是已经具备较完整 Harness 底座的 Agent 工作台。
...
**Ember 当前最大的短板，不是“没有工具”或“没有运行时”，而是“证据闭环、长期执行闭环、动态装配闭环还不够强”。**
...
因此本文不只看 prompt，也不只看 tool 数量，而是按下面这些维度对 Ember 做判断：
...
这说明 Ember 的主问题已经不是“缺零件”，而是“怎样把这些零件收敛成更强的闭环”。

### 3.2 仍然偏弱的部分

Ember 当前仍然缺少下面三类关键闭环：
...
| LangChain Harness 能力                | Ember 当前状态 | 当前事实源                                                                                                                                                                                                                                                     | 结论                                                                                                                                                                                                          |
| ------------------------------------- | ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| System Prompts / 规则注入             | 已落地        | `ember-rs/src/services/memory_profile_prompt_service.rs`、`ember-rs/src/services/memory_source_resolver_service.rs`                                                                                                                                          | Ember 已把 profile、memory source、project rule 注入到 system prompt 主链，不是裸 prompt 模式                                                                                                                  |
| Filesystem / Durable Storage          | 已落地        | `internal/aiprompts/overview.md`、`ember-rs/src/commands/aster_agent_cmd/tool_runtime/workspace_tools.rs`                                                                                                                                                         | Workspace、artifact、项目目录、文件工具都已经进入主链                                                                                                                                                         |
| Bash / Code Execution                 | 已落地        | `ember-rs/src/agent_tools/catalog.rs`、`ember-rs/src/agent_tools/execution.rs`                                                                                                                                                                               | Ember 已具备通用执行能力，不依赖“预先定义完所有工具”                                                                                                                                                           |
| Sandbox / Approval / Policy           | 已落地        | `ember-rs/src/agent_tools/execution.rs`、`internal/aiprompts/commands.md`                                                                                                                                                                                         | restriction profile、sandbox profile、warning policy 都已进入 runtime 主链                                                                                                                                    |
| Tools / Skills / MCP / Browser        | 已落地        | `internal/aiprompts/skill-standard.md`、`internal/aiprompts/command-runtime.md`、`ember-rs/src/agent_tools/catalog.rs`                                                                                                                                                | Ember 已有较完整 capability surface，不是单一 chat tool 模型                                                                                                                                                   |
| Memory / Search / AGENTS 注入         | 已落地        | `ember-rs/src/services/memory_source_resolver_service.rs`、`internal/aiprompts/overview.md`                                                                                                                                                                       | 记忆与规则文件已进入生产链，Web/MCP/search 也已存在                                                                                                                                                           |
| Context Rot 治理                      | 部分落地      | `ember-rs/src/commands/aster_agent_cmd/command_api/runtime_api.rs`、`ember-rs/crates/aster-rust/crates/aster/src/context_mgmt/mod.rs`、`ember-rs/crates/agent/src/tool_io_offload.rs`、`ember-rs/crates/aster-rust/crates/aster/src/context/compressor.rs` | 已有 compact、tool output compression、tool offload，但产品侧可见性与默认治理还不够强                                                                                                                         |
| Long-Horizon Execution                | 部分落地      | `ember-rs/src/commands/aster_agent_cmd/runtime_turn.rs`、`ember-rs/src/commands/aster_agent_cmd/tool_runtime/subagent_tools.rs`、`ember-rs/src/commands/aster_agent_cmd/command_api/runtime_api.rs`                                                         | 已有 auto continue、provider continuation、subagent、queue/resume，但还没形成统一 completion loop                                                                                                             |
| Verification / Replay / Review        | 部分落地      | `internal/aiprompts/harness-engine-governance.md`、`ember-rs/src/commands/aster_agent_cmd/command_api/runtime_api.rs`、`src/lib/agentRuntime/harnessVerificationPresentation.ts`、`ember-rs/src/services/runtime_review_decision_service.rs`                     | evidence / replay / analysis / review 已成链；前端 verification 展示语义已收敛到共享 helper，review template 也开始直接携带同一份 structured verification summary，但验证结果尚未成为所有后续动作的默认硬约束 |
| Just-in-Time Tool / Context Assembly  | 待加强        | `ember-rs/src/agent_tools/catalog.rs`、`internal/aiprompts/skill-standard.md`、`internal/aiprompts/command-runtime.md`                                                                                                                                                | 已有 surface/profile/skill progressive disclosure，但仍偏静态 catalog，不够按任务即时裁剪                                                                                                                     |
...
- `ember-rs/src/services/memory_profile_prompt_service.rs`
  证明 Ember 已把用户画像与 memory prompt 合并进 system prompt，而不是只靠前端临时拼接。
- `ember-rs/src/services/memory_source_resolver_service.rs`
  证明 Ember 已支持 user memory、durable memory、project rule、多层目录记忆来源解析。

### 5.2 Workspace / Tool Surface / Execution

- `ember-rs/src/agent_tools/catalog.rs`
  证明 Ember 已有 tool catalog、surface profile、capability、lifecycle、permission plane 这些 Harness 级抽象。
- `ember-rs/src/agent_tools/execution.rs`
  证明 Ember 已把 warning policy、restriction profile、sandbox profile 做成统一执行策略，而不是 scattered 规则。
- `ember-rs/src/commands/aster_agent_cmd/tool_runtime/workspace_tools.rs`
...
- `internal/aiprompts/skill-standard.md`
  证明 Ember 对 skill 的理解已经是 bundle，而不是单一 Markdown 提示词。
- `internal/aiprompts/command-runtime.md`
  证明 Ember 已把 `@`、`/`、`scene`、`ServiceSkill`、tool/runtime binding 做成明确产品主链。
...
- `ember-rs/src/commands/aster_agent_cmd/command_api/runtime_api.rs`
  证明 Ember 已有 `agent_runtime_compact_session`、resume thread、thread read model、evidence export 这类 runtime 操作主链。
- `ember-rs/crates/aster-rust/crates/aster/src/context_mgmt/mod.rs`
  证明 Aster 已有 continuation message 与 compact 后续写逻辑，Ember 不是完全没有 continuation。
- `ember-rs/crates/aster-rust/crates/aster/src/context/compressor.rs`
  证明 tool output 已有 head/tail compression。
- `ember-rs/crates/agent/src/tool_io_offload.rs`
  证明 Ember 已有通用 tool arguments/results offload、preview、eviction policy 与 `offload_file` 协议。
- `ember-rs/src/commands/aster_agent_cmd/runtime_turn.rs`
  证明 Ember 已有 auto continue、provider continuation state 恢复与 runtime 级 continuation 配置。

### 5.5 Evidence / Replay / Review / UI

- `internal/aiprompts/harness-engine-governance.md`
  证明 Ember 已明确要求 evidence pack 作为事实源，replay / analysis / review / UI 都应复用它。
...
- `ember-rs/src/services/runtime_review_decision_service.rs`
...
这意味着 Ember 已经能“看见问题”，但还没有完全做到“看见问题以后所有后续动作都按同一事实推进”。
...
这意味着 Ember 现在更像“支持长任务”，还不完全像“强约束地把长任务做完”。
...
这意味着 Ember 已经知道“哪些能力存在”，但还没有稳定做到“当前任务只拿到真正需要的那一组能力和上下文”。
...
- 让 Ember 对复杂任务不只是“可以继续”，而是“默认会继续直到满足完成标准”
...
这才是 Ember 后续对齐 Claude Code / Codex / LangChain Harness 思路时，真正应该抓住的主线。
