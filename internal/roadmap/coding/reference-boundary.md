...
- 恢复 `ember-rs/src/**` 或旧 command wrapper。
...
| --- | --- | --- | --- | --- |
| patch grammar / lenient parser / streaming parser / fuzzy seek | Apache-2.0 | `ember-rs/crates/patch-apply` + App Server `runtime_backend/coding_events/patch.rs` | `ember-rs/crates/patch-apply/src/{parser,streaming_parser,seek_sequence}.rs`、`ember-rs/crates/app-server/src/runtime_backend/coding_events/patch.rs` | `cargo test --manifest-path "ember-rs/Cargo.toml" -p patch-apply`；`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::coding_events -- --nocapture` |
| patch apply replacement engine / workdir path guard / agent tool entry | Apache-2.0 | `ember-rs/crates/patch-apply` + `ember-rs/crates/agent/src/tools/apply_patch_tool.rs` | `ember-rs/crates/patch-apply/src/apply.rs`、`ember-rs/crates/patch-apply/src/lib.rs`、`ember-rs/crates/agent/src/tools/apply_patch_tool.rs`、`ember-rs/crates/agent/src/agent_tools/{catalog,execution}.rs` | `cargo test --manifest-path "ember-rs/Cargo.toml" -p patch-apply`；`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent tools::apply_patch_tool -- --nocapture`；`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::catalog -- --nocapture` |
