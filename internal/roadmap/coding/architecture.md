...
| 能力                                         | 首选落点                                                            | 说明                                                                   |
| -------------------------------------------- | ------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| JSON-RPC 方法、session 读写、fixture backend | `ember-rs/crates/app-server`                                         | App Server 是 coding turn 的 current API owner。                       |
| 协议结构、schema、生成客户端输入             | `ember-rs/crates/app-server-protocol` + `packages/app-server-client` | 协议改动必须同步 Rust schema、generated TS type 和 client contract。   |
| prompt / context / model turn envelope       | `ember-rs/crates/agent`                                              | 只放 coding profile 所需 envelope，不把所有逻辑塞进中心 runtime 文件。 |
...
- `ember-rs/src/**` 或旧 Tauri command wrapper。
...
迁入时必须落到 `ember-rs/crates/**` current owner，不能恢复旧目录或外部 crate 命名。
