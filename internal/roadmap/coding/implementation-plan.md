...
| ---------------------- | -------------------------------------------------------- | ----------------------------------- | -------------------------------------------------- |
| coding profile resolve | `ember-rs/crates/app-server` + `ember-rs/crates/agent`     | runtime client request builder      | Rust profile unit + client contract。              |
...
- 文档能直接指导下一刀进入 `ember-rs/crates/**` current owner。
...
- `ember-rs/crates/app-server/src/runtime.rs` 已从约 7500 行级中心文件收缩到约 527 行，保留 facade、核心类型、构造和共享 helper。
...
- `cargo fmt --manifest-path "ember-rs/Cargo.toml" -p app-server -p lime-gateway -p lime-websocket --check`
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime::tests::evidence_exports --no-run`
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime::gateway_runner::tests -- --nocapture`
...
- 验证证据：`cargo fmt --manifest-path "ember-rs/Cargo.toml" -p lime-agent --check`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::execution -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::tool_orchestrator -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::tool_policy_inspector -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::tool_events -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::coding_events -- --nocapture`。
- 追加验证证据：`cargo fmt --manifest-path "ember-rs/Cargo.toml" -p lime-agent -p app-server --check`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::tests -- --nocapture`。
- 本轮追加验证证据：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::tool_inventory -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::execution -- --nocapture`。
- 本轮新增验证证据：`cargo test --manifest-path "ember-rs/crates/agent-rust/crates/agent/Cargo.toml" --target-dir "ember-rs/target" --lib handle_approval_tool_requests_should_resume_after_manual_confirmation -- --nocapture` 证明人工确认后 Agent 工具请求会继续入队执行；`cargo fmt --manifest-path "ember-rs/Cargo.toml" -p lime-agent --check`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::execution -- --nocapture` 证明默认 policy rule catalog、PolicyService resolver owner、shell command rule classifier 与基础 sandbox owner 稳定。
- 本轮追加验证证据：`cargo fmt --manifest-path "ember-rs/Cargo.toml" -p lime-core -p lime-agent --check`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-core tool_execution_policy_config -- --nocapture`、`CARGO_TARGET_DIR="/tmp/lime-coding-policy-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::execution -- --nocapture`、`git diff --check` 窄范围检查，证明 `shellCommandRules` config、runtime override metadata 与 classifier 回归稳定。
- 本轮快速落地验证证据：`cargo fmt --manifest-path "ember-rs/Cargo.toml" -p lime-agent --check`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::execution::tests -- --nocapture`、`git diff --check` 窄范围检查，证明组织 / 用户 / runtime / 请求策略层级合并和请求级 shell rule 覆盖稳定。
- 本轮 prefix catalog 验证证据：`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-core tool_execution_policy_config -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::execution::tests -- --nocapture` 证明 `matchType=prefix/exact` 配置解析、roundtrip 与 shell classifier 命中稳定。
- 本轮 sandbox backend 验证证据：`CARGO_TARGET_DIR="/tmp/lime-coding-sandbox-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::execution::sandbox_backend_tests -- --nocapture`、`CARGO_TARGET_DIR="/tmp/lime-coding-sandbox-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::execution -- --nocapture`、`CARGO_TARGET_DIR="/tmp/lime-coding-sandbox-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::tests::injected_workspace_sandbox_config_flows_to_turn_context_metadata -- --nocapture`，证明 backend plan、strict fallback、danger-full-access 例外、request 覆盖 persisted sandbox 配置与 App Server 配置注入稳定。
...
- 2026-06-15 P2-B control API 验证证据：`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::execution::process -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server execution_process -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server processor::tests::execution_process_methods_start_drain_and_report_status -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-protocol protocol::v0::tests::catalog -- --nocapture`、`node scripts/check-app-server-client-contract.mjs`、`npm run check:protocol-types`、`npm --prefix "packages/app-server-client" run build`。
...
- 2026-06-15 P2-B no-sandbox live process 验证证据：`rustfmt --edition 2021 --check "ember-rs/crates/agent/src/agent_tools/tool_orchestrator.rs" "ember-rs/crates/agent/src/agent_tools/tool_orchestrator/tests.rs" "ember-rs/crates/agent/src/agent_tools/execution/process.rs" "ember-rs/crates/agent-rust/crates/agent/src/tools/registry.rs"`、`cargo check --manifest-path "ember-rs/Cargo.toml" -p lime-agent`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::tool_orchestrator -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::execution::process -- --nocapture`、`cargo test --manifest-path "ember-rs/crates/agent-rust/Cargo.toml" -p agent-core tools::registry -- --nocapture`、`git diff --check -- ...`。`cargo fmt --manifest-path "ember-rs/Cargo.toml" -p lime-agent --check` 仍会报告同 package 其他历史脏文件格式差异，本轮未格式化无关文件。
...
- 2026-06-15 P2-B App Server process policy 验证证据：`cargo check --manifest-path "ember-rs/Cargo.toml" -p app-server`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server execution_process -- --nocapture`、`cargo run --manifest-path "ember-rs/Cargo.toml" -p app-server-protocol --bin write_schema_fixtures`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-protocol schema -- --nocapture`、`npm run generate:protocol-types`、`npm run check:protocol-types`、`node scripts/check-app-server-client-contract.mjs`、`npm --prefix "packages/app-server-client" test`、`rustfmt --edition 2021 --check "ember-rs/crates/app-server/src/execution_process.rs" "ember-rs/crates/app-server/src/processor/execution_process.rs" "ember-rs/crates/app-server/src/processor/mod.rs" "ember-rs/crates/app-server-protocol/src/protocol/v0/execution_process.rs"`。
...
- 2026-06-15 P2-B policy/sandbox owner 验证证据：`cargo fmt --manifest-path "ember-rs/Cargo.toml" -p lime-agent -p app-server --check`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::execution -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::tool_orchestrator -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server execution_process -- --nocapture`。
...
- 2026-06-15 P2-B live process start hydrate 验证证据：`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::tool_orchestrator -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::coding_events -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime::tests::coding_events::read_model_projects_active_coding_activity_and_pending_action -- --nocapture`、`cargo fmt --manifest-path "ember-rs/Cargo.toml" --all -- --check`、`git diff --check -- ...`。
...
- 2026-06-26 P2-B live process terminal hydrate 验证证据：`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::tool_orchestrator::tests::execute_planned_shell_tool_emits_process_output_delta_before_terminal_event -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::tool_orchestrator -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::execution::process -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::coding_events -- --nocapture`、`rustfmt --edition 2021 --check "ember-rs/crates/agent/src/agent_tools/tool_orchestrator.rs" "ember-rs/crates/agent/src/agent_tools/tool_orchestrator/tests.rs"`、`git diff --check -- ...`。
...
- 2026-06-26 P2-B live process registry 验证证据：`rustfmt --edition 2021 "ember-rs/crates/agent/src/agent_tools/tool_orchestrator.rs" "ember-rs/crates/agent/src/agent_tools/tool_orchestrator/tests.rs" "ember-rs/crates/agent/src/request_tool_policy/web_search_preflight.rs" "ember-rs/crates/app-server/src/execution_process.rs"`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::tool_orchestrator -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server execution_process -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::coding_events -- --nocapture`。
...
- 2026-06-26 P2-B RuntimeBackend shared process owner 验证证据：`CARGO_TARGET_DIR="/tmp/lime-p2b-live-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::live_execution_process -- --nocapture`、`CARGO_TARGET_DIR="/tmp/lime-p2b-live-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent event_converter -- --nocapture`、`CARGO_TARGET_DIR="/tmp/lime-p2b-live-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server execution_process -- --nocapture`、`CARGO_TARGET_DIR="/tmp/lime-p2b-live-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::coding_events -- --nocapture`、`CARGO_TARGET_DIR="/tmp/lime-p2b-live-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_factory -- --nocapture`、`CARGO_TARGET_DIR="/tmp/lime-p2b-live-target" cargo test --manifest-path "ember-rs/crates/agent-rust/crates/agent/Cargo.toml" --lib test_agent_has_tool_registry -- --nocapture`、`rustfmt --edition 2021 --config skip_children=true --check ...` 和 `git diff --check -- ...` 窄范围检查通过。`npm run test:contracts` 已通过 `check:protocol-types`、`check-app-server-client-contract`、`check-command-contracts`、`check-harness-contracts`、`governance:modality-contracts`、`governance:scripts`、`governance:electron-release-workflow`、`harness:cleanup-report:check`，最终 `docs:boundary` 因无关 `internal/roadmap/plugin/*` 旧内部文档路径引用失败，本轮不改插件路线图。
...
- 2026-06-26 P2-B RuntimeBackend host contract 验证证据：`CARGO_TARGET_DIR="/tmp/lime-p2b-agent-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server --features agent-backend runtime_backend -- --nocapture`、`CARGO_TARGET_DIR="/tmp/lime-p2b-agent-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server --features agent-backend agent_factory_can_share_execution_process_owner_with_runtime_core -- --nocapture`、`CARGO_TARGET_DIR="/tmp/lime-p2b-agent-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server --features agent-backend runtime_backend_json_rpc_agent_flow_smoke_covers_artifact_read_and_action_response -- --nocapture` 均通过。`rustfmt --edition 2021 --config skip_children=true --check "ember-rs/crates/app-server/src/runtime_backend.rs" "ember-rs/crates/app-server/src/runtime_factory.rs" "ember-rs/crates/app-server/src/lib.rs"`、`npx prettier --check "internal/roadmap/coding/README.md" "internal/roadmap/coding/implementation-plan.md"`、`git diff --check -- ...` 已通过；全量 `rustfmt --edition 2021 --check ...` 曾因无关脏文件 `ember-rs/crates/app-server/src/runtime/plugin_worker_turn.rs` 需要格式化而失败，本轮未改该文件。
...
- 2026-06-15 evidence export coding summary 增量：`ember-rs/crates/app-server/src/runtime/evidence_provider.rs` 已把 coding facts 写入 `EvidencePackSummary.observability_summary.coding`，schema 为 `coding-evidence-summary.v1`，包含 file change、patch、failed patch、command、failed command、test、failed test、action required/resolved、recovery request 计数，以及 `outputRefs / diffRefs / checkpointRefs / artifactRefs / evidenceRefs / sourceEventIds`；恢复请求中嵌套的 `harness.coding_workbench_recovery` 也会参与 refs 聚合。
- 2026-06-15 evidence export coding summary 验证证据：`cargo fmt --manifest-path "ember-rs/Cargo.toml" -p app-server --check`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime::tests::evidence_exports -- --nocapture` 覆盖 snapshot artifact、失败 patch、失败 command、失败 test、action lifecycle、recovery context 与 Evidence Pack coding summary。
...
- 2026-06-15 Windows restricted token minimal enforce 验证证据：`cargo check --manifest-path "ember-rs/Cargo.toml" -p agent-core`、`cargo test --manifest-path "ember-rs/crates/agent-rust/crates/agent/Cargo.toml" --target-dir "ember-rs/target" --lib sandbox::executor -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::execution::sandbox_backend_tests -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent agent_tools::tool_orchestrator::tests::workspace_sandbox_config_maps_restricted_token_backend -- --nocapture`。
...
- `cargo fmt --manifest-path "ember-rs/Cargo.toml" -p app-server --check`
- `rustfmt --edition 2021 "ember-rs/crates/app-server/src/runtime_backend/model_registry_metadata.rs" "ember-rs/crates/app-server/src/runtime_backend/model_routing.rs" "ember-rs/crates/app-server/src/runtime_backend.rs" "ember-rs/crates/app-server/src/runtime/read_model.rs" "ember-rs/crates/app-server/src/runtime/tests/read_model.rs" "ember-rs/crates/services/src/model_registry_service.rs" "ember-rs/crates/services/src/model_registry_service/runtime_metadata.rs"`
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend:: -- --nocapture`
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::model_registry_metadata -- --nocapture`
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime::tests::read_model::read_session_projects_model_routing_into_thread_read -- --nocapture`
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-services model_registry_service::runtime_metadata -- --nocapture`
...
- 新增 coding 能力没有进入 `ember-rs/src/**`、legacy desktop facade 或产品页本地 Provider key。
...
下次恢复入口固定为 P2-B live process lifecycle fifth slice：先读 `internal/roadmap/coding/README.md` 的“下次从这里开始”，再从 `ember-rs/crates/agent/src/agent_tools/tool_orchestrator.rs`、`ember-rs/crates/agent/src/agent_tools/execution/process.rs`、`ember-rs/crates/app-server/src/runtime_backend/live_execution_process.rs`、`ember-rs/crates/app-server/src/runtime_factory.rs`、`ember-rs/crates/app-server/src/runtime.rs` 和 `ember-rs/crates/app-server/src/execution_process.rs` 开始。第一目标不是新增 UI 或文档，也不是伪造外部 Agent host，而是补默认 command/test sandbox-aware process runner/control owner；`RuntimeBackendSubmitRequest.process_control` 合同已就绪，只有真实生产 host adapter 出现时才继续消费。同时继续通过 `ToolExecutionDecision.requires_sandboxed_execution()` 防止 sandbox required 命令走裸进程。
