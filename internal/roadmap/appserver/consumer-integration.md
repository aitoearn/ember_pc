...
6. 独立 App 不得调用、复制或依赖 `ember-rs/src/commands/**`；该目录只作为 Lime 内部旧 Tauri wrapper 清理区。需要 Agent 后端能力时消费 App Server JSON-RPC / client；需要窗口、shell、picker、托盘等桌面壳能力时由各 App 的 Desktop Host bridge 承接。
...
```bash
APP_SERVER_BIN=/Users/coso/Documents/dev/ai/aiclientproxy/lime/ember-rs/target/debug/app-server
```
