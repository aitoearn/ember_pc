...
2. 先定义 backend-neutral 合同，再迁 Agent 主链。
3. Agent 只是第一个 `ExecutionBackend`，不能把 Agent DTO 直接抬成公共 DTO。
...
| `ExecutionBackend` | `current target` | 后端适配接口，Agent 和未来后端都走这里。 |
| `RuntimeBackend` | `current target` | 对现有 Agent runtime_turn/tool_runtime 的适配封装。 |
| `ember-rs/crates/agent` | `current reference` | 当前最接近 core 的 crate，后续继续拆公共模型与服务。 |
| `ember-rs/src/commands/agent_cmd/runtime_turn.rs` | `compat cleanup reference` | Agent backend 历史实现参考，只能用于迁出逻辑；不再作为公共 core 或新 runtime 落点。 |
| `ember-rs/src/commands/agent_cmd/tool_runtime/*` | `compat cleanup reference` | Agent tool execution 历史参考，事件需迁到公共 tool facts；不再新增 tool 业务逻辑。 |
| `ember-rs/src/commands/agent_cmd/command_api/*` | `compat cleanup reference` | legacy desktop command DTO / facade 历史参考；迁出到 RuntimeCore / App Server 后撤注册并删除，不在该目录继续保留委托 facade。 |
...
5. backend host 返回值必须是公共 `RuntimeEvent`，不能把 `lime_agent::AgentEvent`、`AgentChatRequest` 或 legacy desktop DTO 带回 `app-server` 公共合同。

首批 backend：

1. `RuntimeBackend`：适配现有 Agent runtime。
...
| Agent prompt / tool orchestration 细节 | RuntimeBackend；迁移完成前可参考旧 `agent_cmd`，但不得在 `ember-rs/src/commands/**` 新增逻辑 |
...
5. 边界测试：`app-server` 公共后端 contract 不得重新出现 Lime/Agent 私有事件类型。
