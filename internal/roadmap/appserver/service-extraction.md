...
> 更新时间：2026-06-08
> 作用：定义如何从现有 Ember runtime 中抽出可被 App Server 和 legacy desktop facade 共用的服务层。
...
4. legacy desktop facade 只允许在迁移期保留参数适配、事件转发和错误映射；不得落回 `ember-rs/src/commands/**` 扩写，也不得把该目录里的 thin facade / stub 当作完成态。
...
8. `ember-rs/src/commands/**` 是旧 Tauri command wrapper 清理区，不再承接新的业务逻辑、API adapter、runtime 分支、领域服务实现、compat wrapper 或退场 stub。
...
| `ember-rs/crates/agent` | `current reference` | 当前最接近 core 的 crate，后续继续拆公共模型与服务。 |
| `ember-rs/src/commands/aster_agent_cmd/runtime_turn.rs` | `compat cleanup reference` | Aster backend 历史实现参考，只能用于迁出逻辑；不再作为公共 core 或新 runtime 落点。 |
| `ember-rs/src/commands/aster_agent_cmd/tool_runtime/*` | `compat cleanup reference` | Aster tool execution 历史参考，事件需迁到公共 tool facts；不再新增 tool 业务逻辑。 |
| `ember-rs/src/commands/aster_agent_cmd/command_api/*` | `compat cleanup reference` | legacy desktop command DTO / facade 历史参考；迁出到 RuntimeCore / App Server 后撤注册并删除，不在该目录继续保留委托 facade。 |
| `ember-rs/src/commands/**` | `compat cleanup` | 旧 Tauri wrapper 清理区；只允许迁出核心逻辑、撤注册、机械编译修复和删除；删不动登记 blocker，不保留 stub / compat wrapper / thin facade。新增后端能力进入 App Server crates / RuntimeCore / services，桌面壳能力进入 Electron Desktop Host。 |
| 前端 `safeInvoke` / `invoke` agent runtime API | `compat client` | Ember Desktop 迁移期继续保留，但可改为 app-server client。 |
...
5. backend host 返回值必须是公共 `RuntimeEvent`，不能把 `ember_agent::AgentEvent`、`AsterChatRequest` 或 legacy desktop DTO 带回 `app-server` 公共合同。
...
| Aster prompt / tool orchestration 细节 | AsterBackend；迁移完成前可参考旧 `aster_agent_cmd`，但不得在 `ember-rs/src/commands/**` 新增逻辑 |
| 未来其他执行引擎细节 | 对应 ExecutionBackend |
| 旧 Ember Agent event payload 解析 | Desktop compat facade |
...
3. Ember Desktop frontend gateway。

这些允许位置只表示迁移期可读 / 可委托，不表示可以继续写新实现。`ember-rs/src/commands/**` 中的旧 facade 必须随 RuntimeCore / App Server / Electron Host 覆盖逐步删除。
...
2. 文本扫描：新 runtime 业务逻辑、API adapter、领域 service 不得落回 `ember-rs/src/commands/**`。
...
5. 边界测试：`app-server` 公共后端 contract 不得重新出现 Ember/Aster 私有事件类型。

## 9. 完成判定

服务抽取完成不是“App Server 能启动”，而是：

1. 至少一个真实 Agent turn 经由 service 被 Ember Desktop 和 App Server 复用。
2. Tool / action / artifact / evidence 至少有同源事件链。
3. legacy desktop command 只做 compat facade。
4. content-studio 这类独立 App 不需要理解 Ember Desktop 内部 command。
