...
2. 让 Lime Desktop 继续可用，同时逐步把 legacy desktop command glue 迁出或下线；确需临时兼容时只能在 App Server / RuntimeCore / Electron Desktop Host current 边界内投影，不能继续落在 `ember-rs/src/commands/**`。
...
3. 抽象 `ExecutionBackend`，让 Agent 和未来更多执行后端都通过同一合同接入。
...
作为 Lime Desktop 用户，
当现有 Chat、Workspace、Plugin 或 automation 入口迁到 App Server 背后时，
...
2. Agent 只是 `ExecutionBackend`，不再污染公共协议。
3. 服务层不依赖具体壳层。
