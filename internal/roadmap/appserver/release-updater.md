...
> 更新时间：2026-06-08
> 作用：固定 Ember Desktop 下线上一代前端宿主后的 release、签名、公证、updater feed 与平稳迁移口径。

## 1. 事实源

Ember Desktop 的发布与更新链路由 Electron current 接管：
...
Codex CLI / `codex-rs` 只作为 App Server protocol / daemon lifecycle / client 分层参考；release、updater、tray、Dock、窗口和桌面产品交互都以 Ember Electron Desktop Host 为事实源，不从 Codex App UI 推断。
...
旧 Rust / Tauri updater command 面同样已按 `dead / deleted` 处理：旧 `update_cmd` 文件已物理删除，`ember-rs/src/commands/mod.rs` 不再声明 `update_cmd`，`ember-rs/src/app/runner.rs` 不再管理 `UpdateInstallSessionState`、不再启动 Rust 后台更新检查，也不再注册旧 updater `generate_handler!`。后续不得用 stub、compat wrapper 或新实现把 updater 放回 `ember-rs/src/commands/`；只允许经 Electron Desktop Host updater current 链路演进。
...
flowchart TB
    ...
    BuiltInUpdater --> Feed["Squirrel-compatible feed<br/>EMBER_ELECTRON_UPDATES_URL"]
    Feed --> R2["Cloudflare R2<br/>ember/stable/<feed>/..."]
...
```text
https://updates.aiearn.me/ember/stable/<feed>/
```
...
```text
ember/stable/<feed>/<asset>
ember/stable/vX.Y.Z/<feed>/<asset>
```
...
| 项 | 值 |
| --- | --- |
| 更新域名 | `https://updates.aiearn.me` |
| DNS | `updates.aiearn.me` → ECS 公网 IP 或 SLB |
| Nginx 示例 | `scripts/electron/nginx/updates.aiearn.me.conf.example` |
| 静态根目录 | `/www/wwwroot/updates.aiearn.me`（宝塔）或 `/var/www/updates` |
| 上传脚本 | `npm run electron:upload-update-feed` |
| SCP 远程根 | `EMBER_UPDATES_REMOTE_ROOT=/www/wwwroot/updates.aiearn.me` |

**SCP 上传到 ECS**：

```bash
# 本地 macOS arm64 会自动推断 target triple 与 package.json 版本
npm run electron:stage-release-assets

# 多平台或 CI 需显式传参
node scripts/electron/stage-release-assets.mjs \
  --forge-dir release-electron \
  --out-dir release-assets/aarch64-apple-darwin \
...
**OSS 上传**（Nginx 回源 OSS 时）：

```bash
EMBER_RELEASES_OSS_BUCKET=lmtest-updates \
EMBER_UPDATES_UPLOAD_MODE=oss \
npm run electron:upload-update-feed -- --version v1.0.0
```
...
| 临时 keychain path           | `EMBER_MACOS_KEYCHAIN`         | Forge packager signing keychain    |
| release workflow             | `EMBER_ELECTRON_SIGN=1`        | 显式打开签名 / 公证                |
...
| -------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------------------ |
| `WINDOWS_SIGNING_CERTIFICATE`          | 写入临时 `EMBER_WINDOWS_SIGNING_CERTIFICATE_FILE` | Authenticode PFX 证书                                                                |
| `WINDOWS_SIGNING_CERTIFICATE_PASSWORD` | `EMBER_WINDOWS_SIGNING_CERTIFICATE_PASSWORD`      | Squirrel signing certificate 密码                                                    |
| release workflow                       | `EMBER_ELECTRON_SIGN=1`                           | Windows 构建允许签名；只有两项 Windows secret 都存在时才传入 Squirrel signing config |

Windows 当前通过 `npx electron-forge make --platform win32 --arch x64 --targets squirrel` 生成 Squirrel installer；两项 Windows signing secret 都存在时，`forge.config.mjs` 将 `certificateFile` / `certificatePassword` 传给 `@electron-forge/maker-squirrel`。如果两项 secret 都未配置，release workflow 继续生成 unsigned Forge Squirrel installer；如果只配置其中一项，则 fail-fast，避免半配置签名在 make 阶段才失败。Windows Squirrel 的 `remoteReleases` 同步只允许通过 `EMBER_WINDOWS_SQUIRREL_REMOTE_RELEASES_URL` 显式启用，默认 release make 不从 `EMBER_ELECTRON_UPDATES_URL` 拉取线上 `RELEASES`，避免首发或空 feed 返回 404 时阻断安装包生成。
...
开发态默认不启用真实 updater。只有显式设置 `EMBER_ELECTRON_ENABLE_DEV_UPDATER=1` 时才允许在开发包里调用 Electron 内置 `autoUpdater`，避免开发环境误连生产 feed。
...
1. `forge.config.mjs#APP_ID` 继续为 `com.embercloud.ember`。
2. `forge.config.mjs#PRODUCT_NAME` 继续为 `Ember`，Dock、菜单、托盘和安装器展示不能退回默认 `Electron`。
3. URL scheme 继续为 `ember`。
4. macOS icon 继续走 `ember-rs/icons/icon.icns`，Windows icon 继续走 `ember-rs/icons/icon.ico`。
