...
`ember-desktop-platform` 正在成为 `zhongcao` 等独立 Product App 的应用中心、设置中心和桌面宿主底座。模型 Provider 设置、API Key 和运行时模型选择不能继续在 Desktop Host、Product App 和 App Server 之间形成多份事实源。
...
1. `ember-desktop-platform` 的 `CredentialBroker` 曾作为模型 key 的本地存储，并同步一份 key 到 App Server，形成双写。当前目标是新 key 只以短程输入进入 App Server provider store；旧 broker key 只允许一次性迁移，迁移成功后删除本地旧凭证文件。
2. App Server 默认数据根仍指向现有 Ember 用户数据目录；新平台 App 若直接启动 sidecar，可能污染现有 Ember 的 `ember.db`。

本 PRD 定义 App Server 侧目标架构和实施计划：App Server provider store 成为唯一模型凭证事实源，同时支持宿主显式指定数据根，让新 App 首次启动时可自动初始化独立 DB，不影响现有 Ember。

## 2. 目标

### 2.1 产品目标

- 新 App（例如 `zhongcao`）首次运行时不需要预置 `ember.db`。
...
- 现有 Ember 默认数据路径和已有用户数据不被新平台 App 影响。

### 2.2 工程目标

- App Server 支持 `--data-dir` 和环境变量 `APP_SERVER_DATA_DIR`。CLI 参数优先级高于 env；两者都没有时保持现有 Ember 默认路径。
...
- 不在 App Server 中读取 `ember-desktop-platform` 的 `model-settings.json` 或旧 `CredentialBroker` 文件。
...
| Existing Ember user data | Existing Ember 默认数据根 | Existing Ember | 不被新平台 sidecar 默认写入 |
...
新 App 没有 `ember.db` 是正常状态。初始化责任在 Desktop Platform 启动的 App Server sidecar：
...
3. App Server 解析 `--data-dir`，将 DB 路径解析为 `<data-dir>/ember.db`。
...
sequenceDiagram
  ...
  participant DB as platform-owned ember.db
  ...
  Paths->>DB: open or create ember.db
...
sequenceDiagram
  ...
  App->>Platform: invokeCapability(ember.agent)
...
- Existing Ember 的 `ember.db`、迁移、legacy database recovery 不改变。
- App Server CLI 新参数必须是 additive，不改变默认行为。

### 8.2 Desktop Platform

- Desktop Platform 必须传显式 data root，避免写入 Existing Ember 默认 DB。
...
- 明确 CLI 参数优先级：`--data-dir` > `APP_SERVER_DATA_DIR` > existing Ember default。

验收：

- 未传参数时现有 Ember 默认路径不变。
- 传入临时 data dir 时创建 `<data-dir>/ember.db`。
...
- Existing Ember 默认 DB 路径和迁移行为不变。
...
| Desktop Platform 未传 `--data-dir` 污染现有 Ember DB | Desktop sidecar resolver 默认补 `--data-dir userData/app-server`，并加单测 |
...
- `compat`：未传 data root 时使用 Existing Ember 默认路径。
