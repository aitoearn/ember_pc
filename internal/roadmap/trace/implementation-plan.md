...
```bash
cargo test --manifest-path "ember-rs/crates/agent-rust/Cargo.toml" -p agent-core direct_answer
cargo test --manifest-path "ember-rs/crates/agent-rust/Cargo.toml" -p agent-core provider_trace
```
...
7. 定向验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server provider_trace`。
...
25. Rust writer 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server trace_writer -- --nocapture`，2 个 writer 用例通过。
...
27. Trace read/list redaction hardening 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server trace`、`npm run test:contracts`、`git diff --check`。
...
45. Support bundle 定向验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server support_bundle -- --nocapture`，2 个用例通过；Trace 主链验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server trace -- --nocapture`，11 个用例通过。
...
49. Protocol / Rust / contract 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-protocol catalog -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-protocol schema_fixtures -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server trace -- --nocapture`、`npm run test:contracts`。
