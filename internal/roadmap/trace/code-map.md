...
| 文件                                                                            | 当前职责                                                         | Trace 设计中的角色                                                                                                                                                                                                            |
| ------------------------------------------------------------------------------- | ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ember-rs/crates/app-server/src/runtime.rs`                                      | `RuntimeEvent` / sink / backend trait                            | 注册 `runtime::trace` 最小模块，不让中心文件继续承载 trace 细节。                                                                                                                                                             |
| `ember-rs/crates/app-server/src/runtime/event_store.rs`                          | RuntimeEvent -> AgentEvent append、schema 校验、projection store | S3 统一出口：从 turn metadata 取 trace context，并装饰已有 AgentEvent payload。                                                                                                                                               |
| `ember-rs/crates/app-server/src/runtime/trace.rs`                                | S3 最小 trace context / checkpoint 映射                          | 采用 Codex 模式：从 existing events 映射 trace vocabulary，不新增第二套业务事件。                                                                                                                                             |
| `ember-rs/crates/app-server/src/trace_context.rs`                                | W3C trace context parser / normalizer                            | S16/S17 共用 `traceparent / tracestate` 合法性规则；拒绝全零 trace id / parent span id，不把 W3C trace id 替代 Lime 内部 trace id。                                                                                           |
| `ember-rs/crates/app-server/src/otel_trace.rs`                                   | OpenTelemetry context / exporter helper                          | S18 参考 Codex `set_parent_from_w3c_trace_context`，把合法 W3C carrier 设置为 OTEL remote parent；OTLP exporter 默认关闭，只能由显式开发者环境配置开启。                                                                      |
| `ember-rs/crates/app-server/src/processor/request_trace.rs`                      | JSON-RPC request span boundary                                   | S17/S18 为 `agentSession/turn/start` 创建 `app_server.request` server span，记录安全 scalar trace/session 字段，并通过测试 exporter 证明 trace id / parent span id 继承。                                                     |
| `ember-rs/crates/app-server/src/runtime/trace_store.rs`                          | App Server raw trace JSONL writer / reader                       | S8-S20 append-only store：从已装饰 `AgentEvent` 投影 summary-only `RawTraceEvent`；writer 缓存 `next_seq`；read/list 投影 diagnostics DTO；provider request id 只作为 safe metrics；support bundle 复用其 JSONL 摘要 parser。 |
| `ember-rs/crates/app-server/src/runtime/trace_store/export.rs`                   | Trace export zip helper                                          | S11/S12 负责显式单条 trace zip manifest / summary / events.jsonl / README 写入；只重序列化 summary-only event，不复制原始 JSONL 字节；fixture 可用 `LIME_TRACE_EXPORT_OUTPUT_DIR` 指向临时输出目录。                          |
| `ember-rs/crates/app-server/src/runtime/trace_store/summary.rs`                  | Trace store summary projector                                    | S10 负责 support bundle 的只读文件摘要、parse error 计数和相对路径投影；不承担 zip export 或 runtime writer。                                                                                                                 |
| `ember-rs/crates/app-server/src/runtime/storage_roots.rs`                        | App Server 数据目录派生                                          | S8 增加 `runtime/traces` 根目录；S10 拆出 `StorageRoots::from_data_root` 供只读诊断复用，避免 hard-code 用户路径。                                                                                                            |
| `ember-rs/crates/app-server/src/runtime/turn_execution.rs`                       | turn start、queue、callback sink                                 | 只作为 trace context 来源链路，不把 checkpoint 分散写到 backend emit 点。                                                                                                                                                     |
| `ember-rs/crates/app-server/src/runtime_backend/tool_events.rs`                  | Agent AgentEvent -> RuntimeEvent 映射                            | S5/S20 已将 `provider_trace` 映射为 `provider.request.started`、`provider.first_event.received`、`provider.first_text_delta.received`、`provider.failed`、`provider.canceled`，并透传 provider request id 安全字段。          |
| `ember-rs/crates/app-server/src/main.rs`                                         | App Server runtime wiring                                        | S8 初始化 `TraceEventWriter` 并注入 `RuntimeCore`；S18 在 stdio transport 前安装 opt-in OTLP guard，默认不安装 subscriber、不向 stdout 写 tracing 日志。                                                                      |
| `ember-rs/crates/app-server/src/runtime_backend/request_context.rs`              | request -> Agent session config                                  | P2/P4 注入 provider trace context / W3C carrier；S19 将合法 `agentUiPerformanceTrace.w3cTraceContext` 投影为 `TurnContextOverride.metadata.w3c_trace_context`，不拼正文。                                                     |
| `ember-rs/crates/app-server/src/runtime/diagnostics.rs`                          | App Server diagnostics runtime                                   | S9-S11 暴露 `list_diagnostics_traces` / `read_diagnostics_trace` / `export_diagnostics_trace`，无 writer 时返回 `available=false`。                                                                                           |
| `ember-rs/crates/app-server/src/processor/diagnostics.rs`                        | diagnostics methods                                              | S9-S11 dispatch `diagnostics/trace/list`、`diagnostics/trace/read`、`diagnostics/trace/export`。                                                                                                                              |
| `ember-rs/crates/app-server/src/local_data_source/diagnostics.rs`                | 本地诊断包 / log storage                                         | diagnostics 数据源入口；support bundle 不承担 trace schema owner。                                                                                                                                                            |
| `ember-rs/crates/app-server/src/local_data_source/diagnostics/support_bundle.rs` | support bundle zip 导出                                          | S10 默认写入 `meta/trace-store-summary.json`，只包含 summary-only 文件清单/计数/首末时间；raw trace JSONL 正文默认不导出。                                                                                                    |
| `ember-rs/crates/app-server/src/runtime/tests/trace_store.rs`                    | Trace store Rust 回归                                            | S10 承接 writer/read/list 与 retention 测试，避免 `runtime/trace_store.rs` 重新越过 800 行预警线。                                                                                                                            |
| `ember-rs/crates/app-server-protocol/src/protocol/v0/observability.rs`           | App Server diagnostics DTO                                       | S9-S11 定义 `DiagnosticsTrace*` list/read/export params / response / redaction policy。                                                                                                                                       |
| `ember-rs/crates/app-server-protocol/src/protocol/v0/method_names.rs`            | App Server method name                                           | S9-S11 固定 `diagnostics/trace/list`、`diagnostics/trace/read`、`diagnostics/trace/export`。                                                                                                                                  |

### 1.3 Agent / provider

| 文件                                                                  | 当前职责                                       | Trace 设计中的角色                                                                                                                               |
| --------------------------------------------------------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `ember-rs/crates/agent-rust/crates/agent/src/agents/agent.rs`          | reply loop、direct answer、tool 分类、标题生成 | S5 观察 provider stream 消费点，发出 `ProviderTrace` 诊断事件；不修改消息渲染语义。中心文件只保留发射接线，不继续承载 trace schema。             |
| `ember-rs/crates/agent-rust/crates/agent/src/agents/provider_trace.rs` | provider trace event model                     | S20 承接 provider trace stage/event、elapsed_ms 与 provider request id context attach，避免把诊断模型继续堆到超大 `agent.rs`。                   |
| `ember-rs/crates/agent-rust/crates/agent/src/agents/reply_parts.rs`    | provider request / stream decode               | 保留现有 TTFT 日志与 stream decode；不把 `MessageStream` 扩成第二套事件流。                                                                      |
| `ember-rs/crates/agent-rust/crates/agent/src/session_context.rs`       | turn context task-local / request correlation  | S19 从 `w3c_trace_context` 读取并校验 `traceparent / tracestate`；S20 维护 provider response context task-local，只保存 header-safe request id。 |
| `ember-rs/crates/agent-rust/crates/agent/src/providers/api_client.rs`  | provider HTTP request builder                  | S19 在统一 `send_request` 路径注入标准 W3C HTTP headers；S20 在 response 返回后记录 provider request id headers，避免每个 provider 手写。        |
| `ember-rs/crates/agent-rust/crates/agent/src/context_mgmt/mod.rs`      | auto compact / context 预算                    | 记录 context management span，尤其 auto_compact skip。                                                                                           |
| `ember-rs/crates/agent-rust/crates/agent/src/agents/prompt_manager.rs` | prompt layers                                  | 记录 capabilities layer on/off，不记录完整 prompt。                                                                                              |
...
```text
ember-rs/crates/app-server/src/runtime/trace.rs
```

S8 已新增内部 raw trace store：

```text
ember-rs/crates/app-server/src/runtime/trace_store.rs
```
...
```text
ember-rs/crates/app-server/src/runtime_trace/
  mod.rs
  config.rs
  context.rs
  event.rs
...
| Agent provider      | `provider_trace` -> `provider.*` 映射；first event / first text delta 只记录一次，失败/取消有诊断事件；provider request id 只来自安全 response headers。                                                                                                                                                                                            |
