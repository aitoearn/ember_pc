...
3. Ember 当前 memory 路线图已经定义了 `Personality Layer`，但还没有把它产品化成普通用户可管理的入口。
4. Ember 已有 `memory.profile`、用户资料、灵感库、知识库和 runtime prompt composition，不能再新增一个平行 `SOUL.md` 系统。

本 PRD 的核心判断：

**Ember 需要全局 Soul 能力，但它必须是 Memory 个性化主链的一个产品化投影，而不是新的文件协议或新的记忆系统。**
...
1. 希望 Ember 的回答更像自己想要的助手。
2. 不想维护 Markdown 文件或理解 prompt 工程。
3. 希望能用几个清晰选项控制语气、直接程度、解释深度。

核心场景：

1. 设置 Ember 默认更直接、更简洁或更有耐心。
2. 让 Ember 少说客套话，多给实际建议。
3. 关闭或重置个性化语气。
4. 确认正式内容没有被助手人格污染。
...
1. 支持导入 / 导出 `SOUL.md`。
2. 导入前展示解析预览和风险提示。
3. 导入后写入 Ember current 配置，而不是运行时读取原文件。
...
1. 普通用户能在设置页理解并修改 Ember 的默认交流方式。
