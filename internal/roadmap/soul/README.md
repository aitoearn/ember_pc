...
> 更新时间：2026-06-03
> 目标：把 OpenClaw / Hermes 的 `SOUL.md` 个性化经验转译成 Ember 的全局人格 / 声线能力，同时继续收敛到 Memory 个性化主链，不新增平行事实源。

## 1. 本路线图回答什么

本目录统一回答下面几类问题：

1. Ember 是否应该引入类似 `SOUL.md` 的个性化能力。
2. Soul 与 `memory`、`profile`、`Generation Brief`、Companion 的边界如何划分。
3. 为什么 Ember 采用“全局 AI 个性 / 声线设置”作为主入口，而不是默认读取裸 `SOUL.md` 文件。
...
| Soul                  | Ember 默认怎么和用户说话、以什么人格互动          | 使用 Memory 主链，不另建事实源                     |
...
| Product Personality   | Ember 产品默认交互语气                            | 只影响交互，不默认进入 artifact                    |
...
**Ember 应该提供全局 AI 个性 / 声线设置，但不应把 `~/.ember/SOUL.md` 作为运行时事实源。**

原因：

1. Ember 是 GUI 桌面产品，普通用户不应被要求理解文件路径心智。
2. Ember 已有 `user_profile`、`memory.profile`、`unified_memory_*` 和 runtime prompt composition 主链。
3. 裸文件事实源会与设置页和记忆系统分叉。
4. 全局人格确实符合“Ember 越来越像我”的产品目标，但应落在 Ember current 配置与记忆事实源里。

### 3.4 `SOUL.md` 只做导入 / 导出

固定判断：

**`SOUL.md` 在 Ember 中是高级迁移格式，不是默认运行时协议。**

允许：

1. 从 OpenClaw / Hermes 风格的 `SOUL.md` 导入到 Ember 全局人格设置。
2. 从 Ember 全局人格设置导出 `SOUL.md`，方便用户迁移或备份。
...
2. Global Soul 回答“Ember 默认怎么和我说话”，Expert 回答“本轮由哪个专业角色处理任务”。
...
2. 把 `SOUL.md` 文件当作 Ember 运行时事实源。
3. 把用户个人资料、长期偏好、品牌声线、产品人格和 companion 人格混成一个大 prompt。
4. 默认让全局产品人格影响正式创作内容。
5. 为了兼容 OpenClaw / Hermes，牺牲 Ember 的 GUI 产品心智。
