...
8. 媒体任务合同 / 索引：`ember-rs/crates/app-server/src/media_runtime_contract.rs`、`ember-rs/crates/app-server/src/media_task.rs`
9. 图片 / 视频媒体 worker 路由执行：`ember-rs/crates/media-runtime/src/model_route.rs`、`ember-rs/crates/media-runtime/src/lib.rs`、`ember-rs/crates/media-runtime/src/video_worker.rs`
10. Skill tool preflight / metadata seed：`ember-rs/crates/agent/src/tools/skill_tool_gate.rs`
11. `voice_generation` metadata-only compat：`src/lib/governance/modalityRuntimeContracts.json`、`ember-rs/crates/app-server/src/media_runtime_contract.rs`
12. Provider/model route resolver：`ember-rs/crates/app-server/src/runtime_backend/model_route_resolver.rs`
13. Runtime task profile / permission projection：`ember-rs/crates/agent/src/session_execution_runtime.rs`

本文件解释字段语义；JSON registry 是校验输入。当前已建立治理事实源与前端 TS resolver，所有 current contract 的 launch metadata 可以携带 `execution_profile` 与 `executor_adapter` 快照；图片 / 视频 worker 已消费 `resolved_route` 与本地 broker `model_route_execution`，转写仍走 `audio_transcription` task / transcript 主链；`voice_generation` 目前只是 GUI / Skill 元数据与 `audio_task` 草稿索引，`route_execution_status=metadata_only`，不能被当成可执行音频 worker 或 `model_route_execution` 事实源。Browser Assist 与通用 Skill 工具层继续消费 runtime contract metadata 做 preflight / metadata seed；`voice_generation_profile` 与 `service_skill:voice_runtime` 只作为 compat 元数据，退出条件是 audio worker 或 RuntimeCore provider protocol mapper 消费 `ResolvedModelRoute` 并写入 `model_route_execution`。LimeCore policy、task profile、permission state、用户锁定能力 gap 与统一媒体任务索引继续消费同一合同摘要；仍不新增 Tauri command 或上层 `@` 事实源。

## 2. 固定原则

1. Execution profile 描述底层运行策略，不描述 `@` 命令。
2. 每个 `current` contract 必须被至少一个 execution profile 覆盖。
3. Contract 的 `routing_slot`、`permission_profile_keys`、`limecore_policy_refs` 与 `artifact_kinds` 必须能在 profile registry 中找到对应声明。
4. 只有可执行 current contract 才必须声明 `executor_binding`；`route_execution_status=metadata_only` 的 compat contract 不得为了通过校验伪造 executor binding / adapter。
5. Executor adapter 的 `adapter_key` 必须等于 `executor_kind:binding_key`，并与可执行 contract 的 `executor_binding` 对齐。
6. Adapter 的 progress / cancel / resume / artifact 支持位必须和可执行 contract 一致，不能伪造能力。
7. `generic_file` 只能作为 compat fallback；新增多模态主结果必须优先落到领域化 artifact kind。
8. LimeCore 在本阶段只作为 catalog / policy / offer / audit 引用，不是默认执行器。
...
| `voice_generation`    | `voice_generation_profile`    | `service_skill:voice_runtime`  | `audio_task` / `audio_output`          | metadata-only compat；只写任务 / 展示元数据，不代表已接入可执行音频 worker         |
...
5. 每个可执行 current contract 的 `executor_binding` 必须存在于 `executor_adapters`。
6. `route_execution_status=metadata_only` 的 contract 必须有 `route_execution_exit_condition`，且 contract / profile / adapter / artifact graph 不得声明 `executor_invoked` 或 current 执行面。
7. Adapter 的支持位、artifact output、permission requirements、failure mapping 必须覆盖可执行 contract 声明。
8. Profile 的 model role、permission、LimeCore policy 与 artifact policy 必须覆盖 contract 声明。
...
4. Executor registry 运行时化：图片 / 视频 worker 已消费 `ResolvedModelRoute` 与本地 broker execution binding，转写继续走 `audio_transcription` task / transcript 主链；`voice_generation` 当前是 metadata-only compat，不声明 `executor_invoked`，只能写任务 / 展示元数据。Browser Assist 工具层已在真实浏览器动作前校验 `browser_control` profile / adapter / binding，并把错误结果作为 `runtime_preflight` 合同阻断写回工具 metadata；`LimeSkillTool` 已覆盖 current Skill 主线的 metadata seed 与显式冲突合同阻断。后续继续接入 audio worker / RuntimeCore provider protocol mapper 与真实 Gateway adapter，并补更完整 allow / ask / deny 解释。
