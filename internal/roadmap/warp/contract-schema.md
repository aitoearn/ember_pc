...
> 更新时间：2026-04-29  
> 目标：定义 Ember 多模态底层运行合同的机器可检查结构，确保 `@` 命令、按钮和 Scene 只能绑定到底层 contract，而不是反过来决定运行事实源。
...
```json
{
  "version": 1,
  "status": "current",
  "owner": "internal/roadmap/warp/contract-schema.md",
  "contracts": []
}
```
...
| `embercore_policy_refs` | string[] | 是 | EmberCore 只作为目录/策略/offer/audit 事实源 |
