...
> 更新时间：2026-04-30
> 目标：把 Warp / ClaudeCode 参考转成 Ember 可持续演进的工程节奏，明确每一阶段先交付什么、如何验收、何时允许接上层入口。
...
5. EmberCore 先做目录和策略控制面，不默认接管本地执行。
...
3. 能在失败时解释是 capability gap、permission denied、executor gap 还是 EmberCore policy 限制。
...
| P6 EmberCore   | catalog/policy/offer/audit 能约束本地执行                            | EmberCore 默认代跑所有入口         |
...
| P6   | `embercore-integration.md`                          | catalog/policy SDK 消费                           | Ember + EmberCore contract test              |
...
3. Gateway / cloud scene 有显式 audit，与 Ember evidence 可关联。
...
6. 是否接入 EmberCore catalog / policy / audit？
...
**每一刀都必须让 Ember 更接近“底层 contract 驱动的多模态系统”，而不是更接近“更多入口和更多旁路”。**
