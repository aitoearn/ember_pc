...
| 能力 | current 或目标落点 |
| --- | --- |
| Workflow domain owner | `ember-rs/crates/app-server/src/runtime/workflow/**` |
...
| workflow event producer | `ember-rs/crates/app-server/src/runtime/plugin_worker_workflow.rs` |
| turn cancel 补偿事件 | `ember-rs/crates/app-server/src/runtime/plugin_worker_workflow_cancel.rs`，只服务 `agentSession/turn/cancel` adapter，不是 `workflow/cancel` current owner |
| worker retry 补偿事件 | `ember-rs/crates/app-server/src/runtime/plugin_worker_workflow_retry.rs`，只服务 Plugin worker retry adapter，不是 `workflow/retry` current owner |
| 图片 workflow audit | `ember-rs/crates/app-server/src/runtime_backend/image_command/mod.rs` |
| evidence workflow ids | `ember-rs/crates/app-server/src/runtime/evidence_provider.rs` |
