...
1. P1 Workflow Contract 已落到 `ember-rs/crates/app-server/src/runtime/workflow/**`，包含 definition、status、events、read model 和 source map。
...
1. `ember-rs/crates/app-server/src/runtime/plugin_worker_workflow.rs` 的 `workflow.run.* / workflow.step.*` 事件族。
2. `ember-rs/crates/app-server/src/runtime/workflow/control.rs` 的 `workflow/cancel`、`workflow/retry` 和 `workflow/respond` current control；retry 通过 source turn 级重提交接回 current `agentSession/turn/start` 主链。
3. `ember-rs/crates/app-server/src/runtime/plugin_worker_workflow_cancel.rs` 与 `plugin_worker_workflow_retry.rs` 的 turn cancel / worker retry 补偿事件 adapter。
4. `ember-rs/crates/app-server/src/runtime_backend/image_command/mod.rs` 中图片命令复用 workflow audit 事件族的方向。
...
```text
ember-rs/crates/app-server/src/runtime/workflow/
```
...
```text
ember-rs/crates/app-server-protocol/src/protocol/v0/workflow.rs
...
| 分类 | 文件 | 后续处理 |
| --- | --- | --- |
| `current` | `ember-rs/crates/app-server/src/runtime/workflow/**` | 新增合同、状态、read model 和 control 的唯一演进位置。 |
| `compat` | `ember-rs/crates/app-server/src/runtime/plugin_worker_workflow*.rs` | 先委托 `runtime/workflow/**`，再逐步收窄为 source adapter。 |
