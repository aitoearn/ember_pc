...
> 更新时间：2026-05-01
> 目标：定义 SenseVoice Small 在 Ember 中的模型分发、配置、运行时接入、UI 状态和验证口径。
...
### 3.1 embercore / 对象存储模型清单

embercore control-plane-svc 提供公开客户端接口：
...
1. 模型对象放在 `/Users/coso/Documents/dev/ai/embercloud/embercore` 对应后端管理范围内，由 control-plane-svc 下发目录。
2. 实际大文件放在对象存储或 CDN 公开域名后；P0 可用阿里云 OSS、Cloudflare R2 或等价静态文件服务，客户端只消费下发 URL。
3. 对象存储 Access Key、Secret、签名逻辑不得进入 Tauri 客户端。
4. 若需要私有桶，后续由 embercore 生成短期 presigned GET URL；P0 先支持公开域名 / 自定义域名。
5. Ember 客户端保留同一组 `model_id`、必需文件与安装目录，不再把 GitHub Release URL 作为产品事实源。
...
| `download.archive.downloadUrl` | embercore 拼装后的模型归档下载地址 |
...
1. 优先读取 embercore 下发的 `voice-model-catalog`。
2. 未配置后端目录或环境变量时，客户端使用上面的 R2 公开基址作为默认下载源；模型仍按需下载到用户数据目录，不进入安装包。
3. 后续切换阿里云 OSS / CDN 时，只需要改 embercore 目录或环境变量，不需要改安装包。
...
embercore 下发验收记录（2026-05-01）：

1. 本地启动 `/Users/coso/Documents/dev/ai/embercloud/embercore/services/control-plane-svc`，使用 `SERVER_PORT=18080` 与 `SERVER_VOICE_MODEL_ASSET_BASE_URL=https://pub-fa568bd8496349bcafe04091e2b02e1e.r2.dev`。
...
4. Ember Rust 定向测试 `voice_model_cmd::tests::voice_models_list_catalog_fetches_configured_embercore_url` 已验证客户端会优先读取配置的后端目录 URL，并把后端 `assetBaseURL + downloadPath` 映射为下载地址。
5. DevBridge 真实下载验收：从 embercore 响应构造 `catalogEntry` 传入 `voice_models_download`，模型安装成功，`installed=true`，`installed_bytes=240194146`。
...
3. P0 为了不扩散安装器复杂度，推荐在 embercore 侧把可运行形态重新打包并托管到阿里云 OSS / CDN 或 R2；若要直连 ModelScope，需新增 direct-file 安装模式，并把 `tokens.json` 转换为 sherpa-onnx 可读的 `tokens.txt`。
...
```text
<ember-data-dir>/models/voice/sensevoice-small-int8-2024-07-17/
```
...
6. `voice_models_download` 优先使用前端从 embercore `voice-model-catalog` 取得的对象存储/CDN 下载 URL；无前端目录时才读取 `EMBER_VOICE_MODEL_CATALOG_URL` / `EMBER_VOICE_MODEL_ASSET_BASE_URL` 这类运行时配置。
...
1. `ember-rs/crates/voice-core/src/threaded_recorder.rs` 在开始录音后按采样率预分配 `30s` sample buffer，callback 内直接 downmix + PCM16 转换并写入同一 buffer。
...
新增语音模型 / ASR 后端能力不得再落到 Tauri command wrapper。后端能力进入 App Server JSON-RPC / RuntimeCore / voice services；打开系统设置、文件选择、麦克风权限等桌面壳能力进入 Electron Desktop Host bridge。若仍发现旧 Tauri 语音命令名，只能作为迁移来源或删除队列处理，不能补 stub、compat wrapper 或 `ember-rs/src/commands/**` 新实现。
...
1. 模型清单解析与平台过滤。
2. 配置了后端目录 URL 时，客户端优先消费 embercore `voice-model-catalog`。
3. sha256 校验失败。
4. 必需文件缺失。
5. 未安装模型时转写失败。
6. 短音频沿用现有错误语义。
...
**让 Ember 的语音输入在无云端 ASR 凭证、无网络的情况下仍能完成转写，同时不牺牲安装包体积。**
