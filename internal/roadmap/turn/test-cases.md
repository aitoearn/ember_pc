...
| TURN-CONTRACT-009 | legacy message ToolEnd 不可冒充 canonical    | `ember-rs/crates/agent/src/event_converter.rs`                             | 存在 `legacy_message_tool_response_metadata` 和成功 / 失败 ToolResponse compat 测试，contract 阻止无标记 legacy `ToolEnd` 回流 |
...
```bash
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent request_tool_policy -- --nocapture
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime::tests::read_model -- --nocapture
npm run test:contracts
...
```bash
cargo fmt --manifest-path "ember-rs/crates/agent-rust/crates/agent/Cargo.toml" --check
cargo test --manifest-path "ember-rs/crates/agent-rust/Cargo.toml" -p agent-core web_fetch -- --nocapture
git diff --check -- "ember-rs/crates/agent-rust/crates/agent/src/tools/web.rs"
```
...
```bash
cargo fmt --manifest-path "ember-rs/crates/agent-rust/crates/agent/Cargo.toml" --check
cargo test --manifest-path "ember-rs/crates/agent-rust/Cargo.toml" -p agent-core conversation::tests -- --nocapture
cargo test --manifest-path "ember-rs/crates/agent-rust/Cargo.toml" -p agent-core agents::moim::tests -- --nocapture
cargo test --manifest-path "ember-rs/crates/agent-rust/Cargo.toml" -p agent-core web_fetch -- --nocapture
...
cargo fmt --all # cwd: ember-rs
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server commit_preserves_codex_tool_command_and_patch_timeline -- --nocapture
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server read_session_projects_imported_web_search_tool_result_as_timeline_item -- --nocapture
npm run electron:build
...
cargo fmt --manifest-path "ember-rs/Cargo.toml" --all -- --check
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent tracker_requires_each_required_tool_to_succeed -- --nocapture
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server research_skill_launch_requires_web_fetch_for_page_confirmation -- --nocapture
npm run test:contracts
npm run smoke:agent-runtime-current-fixture
npm run verify:gui-smoke
npm run smoke:claw-chat-ready-streaming -- --timeout-ms 180000
...
```bash
cargo fmt --manifest-path "ember-rs/Cargo.toml" --all --check
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent event_converter -- --nocapture
npm run test:contracts
```
...
cargo fmt --manifest-path "ember-rs/crates/agent-rust/crates/agent/Cargo.toml" --check
cargo test --manifest-path "ember-rs/crates/agent-rust/Cargo.toml" -p agent-core web_fetch -- --nocapture
git diff --check -- "ember-rs/crates/agent-rust/crates/agent/src/tools/web.rs" "ember-rs/crates/agent-rust/crates/agent/src/tools/web_fetch_content.rs" "ember-rs/crates/agent-rust/crates/agent/src/tools/mod.rs" "internal/roadmap/turn/implementation-plan.md" "internal/roadmap/turn/test-cases.md"
```
...
```bash
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-mcp oauth -- --test-threads=1
