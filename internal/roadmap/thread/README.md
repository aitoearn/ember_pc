...
4. 2026-07-02 已重新执行的贴边界验证：
   ...
   - `cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent request_tool_policy::policy_config`：`10 tests passed`。
   - `cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-scheduler request_tool_policy`：passed。
   - `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server fast_response`：`8 tests passed`。
   - `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server natural_language_news_turn_exposes_search_tool_surface_by_default`：passed。
   - `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server explicit_auto_search_mode_uses_model_tool_choice`：passed。
   - `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server explicit_web_search_false_keeps_search_disabled`：passed。
