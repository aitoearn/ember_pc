# Ember Artifact Workbench 架构蓝图

> 状态：提案
> 更新时间：2026-03-24
> 运行时边界：凡涉及发送边界、runtime metadata、Team 委派、Op/Event 收口，均以 `internal/aiprompts/query-loop.md`、`internal/aiprompts/task-agent-taxonomy.md`、`internal/aiprompts/state-history-telemetry.md` 与 `internal/exec-plans/upstream-runtime-alignment-plan.md` 为准；本文只细化 Artifact Workbench 架构
> 依赖文档：
...
> 目标：把 Ember 高配版 Artifact Workbench 的产品形态、运行链路、时序关系、system prompt 组装策略与实现边界收敛成一份可直接指导开发实施的总装图
...
7. **框架层与产品层分离**
   - Ember 持有 `ArtifactDocument` 与 Workbench 产品能力
...
- `ember-rs/src/services/agent_timeline_service.rs`
...
3. Ember 已具备右侧工作台、Artifact 渲染、Timeline 投影与富文本编辑器底座。
...
flowchart TB
    ...
    subgraph Runtime["Aster Runtime / Ember Runtime Adapter"]
...
以下时序图只表达产品层与运行时层的职责分工，不单独定义 Ember 当前仓库的 on-wire 字段名、命令名或 metadata 归一化细节。
...
这里的接口只描述 Artifact 领域希望表达的最小语义，不等于 Ember 当前请求载荷的最终 wire contract。  
...
- `ember-rs/src/services/artifact_document_service.rs`
- `ember-rs/src/services/artifact_document_validator.rs`
- 可选 `ember-rs/src/services/artifact_generation_orchestrator.rs`

如果同步建设 `aster-rust`，则建议把运行时通用能力下沉为独立 runtime 模块：

- `ember-rs/crates/aster-rust/crates/aster/src/runtime/thread.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/runtime/turn.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/runtime/item.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/runtime/event.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/runtime/prompt.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/runtime/schema.rs`

这部分是框架层远期形态参考，不构成 Ember 当前仓库的直接实施清单。
