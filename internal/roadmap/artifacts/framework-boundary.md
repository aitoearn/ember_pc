# Ember Artifact Workbench 与 Aster Runtime 分层边界

> 状态：提案
> 更新时间：2026-03-24
> 运行时边界：凡涉及发送边界、runtime metadata、Team 委派、Op/Event 收口、状态同步，均以 `internal/aiprompts/query-loop.md`、`internal/aiprompts/task-agent-taxonomy.md`、`internal/aiprompts/state-history-telemetry.md` 与 `internal/exec-plans/upstream-runtime-alignment-plan.md` 为准；本文只定义长期框架分层原则与远期边界
> 关联文档：
...
> 目标：明确 Ember 产品层、Aster 框架层、Blueprint 规划模块三者的长期职责边界，避免后续把产品协议、任务规划、运行时编排混成一团
...
**Ember 负责“交付物产品”，Aster 负责“代理运行时”，Blueprint 负责“长周期规划”。**
...
1. Ember 已经有以 turn 为单位的运行链路。
2. system prompt 已经是后端分层组装，而不是纯前端拼接。
3. 前端只需要透传结构化 metadata，就可以驱动不同运行策略。
4. Ember 当前缺的是正式 Artifact 产品协议与工作台闭环，而不是重新发明 thread/turn 的概念。

## 2.2 Aster Blueprint 现役事实

来自以下文件：

- `ember-rs/crates/aster-rust/crates/aster/src/blueprint/README.md`
- `ember-rs/crates/aster-rust/crates/aster/src/blueprint/types.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/blueprint/task_tree_manager.rs`
- `ember-rs/crates/aster-rust/crates/aster/src/blueprint/worker_executor.rs`
...
| 能力 | Ember 产品层 | Aster Runtime 层 | Blueprint 模块 |
|------|------|------|------|
...
Ember Artifact Workbench 的产物中心是：
...
2. 不照搬终端/TUI 的 UI 心智。
3. 不把 Ember 的文档协议退化成单一 `outputSchema` 结果对象。
...
## 8. Ember 与 Aster 的连接方式

本节时序图只表达长期职责连接，不单独定义 Ember 当前发送协议字段名。

建议 Ember 不直接自己实现第二套完整 runtime，而是成为 Aster runtime 的产品适配层：

```mermaid
sequenceDiagram
    participant FE as Ember Frontend
    participant App as Ember App Service
...
- 但 Ember 当前仓库里的实际发送边界和 runtime 收口步骤，仍按执行效率路线图推进

## 9. 实施顺序建议

本节属于框架层远期演进建议，不覆盖 Ember 当前仓库已确定的 P1 / P2 / P3 / P4 执行顺序。
...
- Ember 将 Stage 1 / Stage 2 接到 Aster runtime turn
...
**Codex 式 runtime substrate + Blueprint 式 planning module + Ember 式 artifact product layer。**
