...
> 更新时间：2026-03-31
> 运行时边界：turn metadata、prompt 组装入口、runtime output schema 注入链以 `internal/aiprompts/query-loop.md`、`internal/aiprompts/state-history-telemetry.md` 与 `internal/exec-plans/upstream-runtime-alignment-plan.md` 为准；本文只定义 `ArtifactDocument v1` 的产品层协议与校验映射
> 依赖文档：`internal/roadmap/artifacts/roadmap.md`
> 架构蓝图：`internal/roadmap/artifacts/architecture-blueprint.md`
> 分层边界：`internal/roadmap/artifacts/framework-boundary.md`
> Prompt 合同：`internal/roadmap/artifacts/system-prompt-and-schema-contract.md`
> 目标：定义 Ember 高配版 Artifact Workbench 的第一版正式协议，包括产品层对象模型、模型输出契约、校验修复规则与渲染映射
