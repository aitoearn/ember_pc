...
> 关联主链：App Server JSON-RPC、RuntimeCore、`ember-rs/crates/core/src/models/model_registry.rs`、`ModelRegistryService`、media task artifact、Agent adapter
...
2. RuntimeCore。
3. `ember-rs/crates/core/src/models/model_registry.rs`。
4. `ModelRegistryService`。
5. `ember-rs/crates/app-server-protocol` typed model protocol。
...
```bash
cargo test --manifest-path "ember-rs/Cargo.toml" -p <affected-crate>
```
