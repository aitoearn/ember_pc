...
把 Agent Workbench 从文档站推进成 Ember AgentUI / AgentRuntime 标准指引和实现种子。所有新增实现必须继续向以下 current 主链收敛：
...
App Server / RuntimeCore / ExecutionBackend
  ...
  -> @embercloud/agent-runtime-projection
  -> @embercloud/agent-runtime-ui
  -> Product Apps / Content Studio / Agent Apps
```

外部 AG-UI、AI SDK、OpenAI Agents JS、LangGraph 等只作为参考；不能成为 Ember runtime owner，也不能让产品应用绕开 App Server / RuntimeCore 直接拥有 Provider key 或 runtime facts。
...
| Contracts seed | done | `@embercloud/agent-ui-contracts` 已有 fixtures、validation、完整 `AgentUiSubagentsModel`、subagent / handoff / review scope ids。 |
| Projection replay seed | done | `@embercloud/agent-runtime-projection` 已能 replay contracts fixtures，并输出 `state.subagents`。 |
| React fixture smoke | done | `@embercloud/agent-runtime-ui` 已用 fixture replay state 做 server render smoke，Subagents 只消费 `state.subagents`。 |
| Runtime client conformance | done | `@embercloud/agent-runtime-client/sessionGateway` 已补完整 runtime client conformance，覆盖 lifecycle、read、evidence export、event dispatch / nextEvent、transport error 和 browser-safe 子路径。 |
| App Server facts runner | done | `@embercloud/agent-runtime-projection` 已有纯 App Server facts adapter，可 replay `agentSession/event`、`agentSession/read`、`evidence/export` 到标准 projection。 |
...
| Artifact / Evidence refs React surface | done | `@embercloud/agent-runtime-ui` 已新增 `ArtifactRefList`、`EvidenceRefList`、`AgentUiRefList`，组合视图输出 refs DOM contract，包级测试通过。 |
...
| Standard package publish gate | done | `@embercloud/app-server-client@1.0.0`、`@embercloud/agent-runtime-client@0.1.1` 已发布到 `@embercloud` organization；无 scope `app-server-client@1.0.0` 和依赖旧名的 `@embercloud/agent-runtime-client@0.1.0` 只作为 compat-misrelease 记录，不作为标准完成证据。 |
| Product app seed adoption | partial | Agent App 真实 Electron 主路径已证明复用标准 `AgentRuntimeClient`、共享 `AgentUiProjectionView` 与 Artifact/Evidence refs；Host action 决策已走 current `respondAction -> agentSession/action/respond`，Host drawer 已成为始终渲染标准 projection 的宿主壳；Content Studio UI 主路径已统一到标准 `AgentUiProjectionSurface`，并已通过 registry 版 `@embercloud/agent-runtime-client@0.1.1/sessionGateway` 包装 App Server turn 主链，GUI smoke 仍待补。 |
...
| current owner | `@embercloud/agent-runtime-client` 只拥有 App Server JSON-RPC、Host bridge、events、read APIs、action response、evidence export。 |
...
| --- | --- |
| 状态 | partial |
| 写集 | 优先 Workbench 文档：`/Users/coso/Documents/dev/ai/embercloud/ember-agent-workbench/docs/tutorials/**`、`docs/profiles/**`；Content Studio 产品接入另行认领 `/Users/coso/Documents/dev/ai/embercloud/content-studio/src/renderer/src/components/agent/**`、`/Users/coso/Documents/dev/ai/embercloud/content-studio/src/renderer/src/components/agents/**`、`/Users/coso/Documents/dev/ai/embercloud/content-studio/scripts/ember-agent-boundary-audit.mjs`。 |
...
```bash
cd /Users/coso/Documents/dev/ai/embercloud/ember-agent-workbench
npm run docs:build
```
...
- `src/features/agent-app/runtime/agentRuntimeAppServerClient.ts` 使用 `@embercloud/agent-runtime-client/sessionGateway` 包装 App Server `agentSession/*` current 方法。
...
- `/Users/coso/Documents/dev/ai/embercloud/ember-agent-workbench/docs/tutorials/content-studio.md` 已补完整 adoption blueprint，明确 Content Studio owns / does not own、标准 surface 映射、compat 迁移表和 blocked 行为。
- `/Users/coso/Documents/dev/ai/embercloud/content-studio/src/renderer/src/components/agent/AgentUiProjectionSurface.tsx` 已作为产品侧标准 surface adapter，统一组合 `@embercloud/agent-runtime-ui` 的 timeline / facts primitives 与 Content Studio artifact/evidence refs。
- Content Studio `AgentsWorkbench` / `AgentSessionPanel` 已改为消费 `AgentUiProjectionSurface`，不再直接散装组合共享 AgentUI primitives。
- `/Users/coso/Documents/dev/ai/embercloud/content-studio/scripts/ember-agent-boundary-audit.mjs` 已增加边界检查，阻止产品页面绕过标准 surface 直接拼共享 primitives，并检查 `.agent-ui-projection` / `.agent-ui-main` / `.agent-ui-sidecar` DOM contract。
- `/Users/coso/Documents/dev/ai/embercloud/content-studio/src/main/services/appServerAgentRuntimeGateway.ts` 已抽出 `ContentStudioAgentRuntimeSessionGateway`，并通过 `@embercloud/agent-runtime-client/sessionGateway` 的 `createAgentRuntimeClientFromSessionGateway(...)` 包装成标准 `AgentRuntimeClient`；本地 gateway 只保留 `agentSession/start`、`artifact/read` 和 sidecar transport 适配。
...
- 标准包发布前门槛已通过：`npm --prefix packages/app-server-client run test`、`npm --prefix packages/agent-runtime-client run test`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-protocol schema_fixtures_match_generated_output`、两个 package 目录内的 `npm pack --dry-run --json --ignore-scripts` 均通过。
- `npm view "@embercloud/app-server-client@1.0.0" version` 与 `npm view "@embercloud/agent-runtime-client@0.1.1" version` 已可查询；`npm view "app-server-client@1.0.0" version` 与 `npm view "@embercloud/agent-runtime-client@0.1.0" version` 只作为误发兼容记录。
...
| current owner | `@embercloud/agent-runtime-ui` 只渲染 `state.artifacts` / `state.evidence` 轻量 refs，并通过 callback 交还宿主。 |
...
- Content Studio 已安装并接入 `@embercloud/agent-runtime-client/sessionGateway`；`package-lock.json` 解析到 `@embercloud/agent-runtime-client@0.1.1` 与间接 `@embercloud/app-server-client@1.0.0` registry tarball。
...
| current owner | `@embercloud/agent-runtime-ui` 只消费 projection state 和 command callbacks。 |
...
- 发布前运行 docs build。
- 若进入 Ember 主仓版本相关文件，必须按仓库发版规则运行版本一致性检查。
...
| 写集 | 发布阶段优先 `packages/app-server-client/**`、`packages/agent-runtime-client/**`；接入阶段另行认领 `/Users/coso/Documents/dev/ai/embercloud/content-studio/package.json`、lockfile、`src/main/services/appServerAgentRuntimeGateway.ts`、`scripts/ember-agent-boundary-audit.mjs`。 |
...
- 保持 Content Studio 固定安装并消费 `@embercloud/agent-runtime-client@0.1.1/sessionGateway`。
- 补 Content Studio GUI smoke / 产品回归。

验证：

```bash
npm view "@embercloud/app-server-client@1.0.0" version
npm view "@embercloud/agent-runtime-client@0.1.1" version
```

Content Studio 接入后再运行：

```bash
npm run verify:ember-agent
npm run test:functional -- --test-name-pattern "Ember Agent 边界审计会阻断 runtime/key/UI 协议回流"
...
- Content Studio 主路径实际调用 `@embercloud/agent-runtime-client/sessionGateway`。
- `npm run verify:ember-agent`、定向 functional 和 `npx tsc --noEmit --pretty false` 已通过。
