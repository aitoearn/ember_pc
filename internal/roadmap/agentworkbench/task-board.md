...
| Standard package publish gate | done | `@embercloud/app-server-client@1.66.0`、`@embercloud/agent-runtime-client@0.1.1` 已发布到 `@embercloud` organization；无 scope `app-server-client@1.66.0` 和依赖旧名的 `@embercloud/agent-runtime-client@0.1.0` 只作为 compat-misrelease 记录，不作为标准完成证据。 |
...
- `src/features/plugin/runtime/agentRuntimeAppServerClient.ts` 使用 `@embercloud/agent-runtime-client/sessionGateway` 包装 App Server `agentSession/*` current 方法。
...
- `/Users/coso/Documents/dev/ai/limecloud/content-studio/src/renderer/src/components/agent/AgentUiProjectionSurface.tsx` 已作为产品侧标准 surface adapter，统一组合 `@embercloud/agent-runtime-ui` 的 timeline / facts primitives 与 Content Studio artifact/evidence refs。
...
- `npm view "@embercloud/app-server-client@1.66.0" version` 与 `npm view "@embercloud/agent-runtime-client@0.1.1" version` 已可查询；`npm view "app-server-client@1.66.0" version` 与 `npm view "@embercloud/agent-runtime-client@0.1.0" version` 只作为误发兼容记录。
...
- Content Studio 已安装并接入 `@embercloud/agent-runtime-client/sessionGateway`；`package-lock.json` 解析到 `@embercloud/agent-runtime-client@0.1.1` 与间接 `@embercloud/app-server-client@1.66.0` registry tarball。
