...
> 状态：active
> 目的：允许多个进程同时推进 `ember-agent-workbench`，减少互相覆盖。
...
| 依赖 | AI SDK `UIMessage.parts` 参考、AG-UI event stream 参考、Ember current packages。 |
...
| 暂不触碰 | Ember 主仓 `src/lib/dev-bridge/**`、`ember-rs/**`。 |
...
| 发现文档标准和 Ember current 实现冲突 | 先写 conflict note 到对应页面，不直接改生产代码。 |
