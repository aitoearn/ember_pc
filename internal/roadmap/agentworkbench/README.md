# Ember Agent Workbench 路线图

> 状态：active
> 更新时间：2026-06-11
> 范围：`/Users/coso/Documents/dev/ai/embercloud/ember-agent-workbench` 文档站、Ember AgentUI SDK 标准、AgentRuntime 配合边界、Subagents 标准化。

## 主目标

把 `ember-agent-workbench` 从“参考 AG-UI 的中文文档站”推进成 Ember 内部 AgentUI / AgentRuntime 标准指引，并让后续实现只能向 Ember current 主链收敛：
...
Workbench 不是外部 SDK 套壳，也不是第二套 runtime。外部 AG-UI、AI SDK、OpenAI Agents JS、LangGraph 等只能作为参考；Ember 的事实源仍是 App Server、RuntimeCore、ExecutionBackend、AgentUI Projection、Evidence / Replay / Review 和治理分类。
...
3. TypeScript 包线拆为四个 current 包，`@embercloud/agent-ui` 只允许作为 future facade。
...
6. 文档站必须和 Ember 现有代码、路线图、App Server / AgentRuntime / AgentUI 事实源对齐；不能写成外部标准翻译稿。
...
| v0.2.0 | done | 去掉非标准树形过程方向，初步对齐 AG-UI / AI SDK / Ember runtime 现实。 |
| v0.3.0 | active | 补齐 Subagents、SDK 包边界、runtime client、projection conformance、Ember 现有实现迁移路线。 |
| v0.4.0 | active | 从文档标准推进到 Ember 主仓包实现、guard、Artifact/Evidence refs surface、Agent App 真实 smoke 和 Content Studio 标准 UI surface adoption。 |
...
- `ember-agent-workbench` GitHub Pages 中文标准站。
- Ember App Server JSON-RPC + RuntimeCore + ExecutionBackend runtime facts。
- `RuntimeEvent / ThreadReadModel / TaskSnapshot / EvidencePack`。
- `@embercloud/agent-ui-contracts`、`@embercloud/agent-runtime-projection`、`@embercloud/agent-runtime-ui`、`@embercloud/agent-runtime-client`。
...
- Content Studio `AI agents` 工作台和通用 `AgentSessionPanel` 已统一通过 `AgentUiProjectionSurface` 消费共享 projection read model，并输出 `.agent-ui-projection` / `.agent-ui-main` / `.agent-ui-sidecar` 标准 DOM surface；其 standalone/dev App Server turn 主链已通过 `@embercloud/agent-runtime-client/sessionGateway` 包装为标准 `AgentRuntimeClient`。标准 npm 链路已发布并接入 `@embercloud/app-server-client@1.0.0` 与 `@embercloud/agent-runtime-client@0.1.1`，Content Studio 不能安装误发的无 scope 链路。
- `@embercloud/agent-runtime-ui` 已有 `ArtifactRefList` / `EvidenceRefList`，引用内容由宿主 artifact workspace / evidence pack owner 打开。
- `@embercloud/agent-runtime-ui` 已有 `SubagentsView`，只消费 `state.subagents`。

### compat

- Ember 现有 `packages/app-server-client` 作为 `@embercloud/app-server-client` 的本地开发事实源。
- 未来 `@embercloud/agent-ui` facade 作为非 current 可选聚合入口。
