...
| 协议参考 | AG-UI、AI SDK、OpenAI Agents JS 等只作为 reference，不成为 Ember owner | `docs/agentic-protocols.md` |
...
| 禁止旧词 | 不把非标准树形过程术语写成 Ember 标准 | 全站搜索 |
...
| 包 | current 职责 | 禁止 |
| --- | --- | --- |
| `@embercloud/agent-ui-contracts` | types、schemas、fixtures、version constants | React、transport、provider、产品应用依赖 |
| `@embercloud/agent-runtime-projection` | projector、selectors、hydration、replay、reconciliation、Subagents model | React DOM、Electron、Provider SDK、App Server DB |
| `@embercloud/agent-runtime-ui` | React primitives、surfaces、callbacks、i18n label contract、CSS / DOM contract | runtime truth、Provider key、JSON-RPC transport |
| `@embercloud/agent-runtime-client` | App Server JSON-RPC、Host bridge、events、read APIs、action response、evidence export | UI projection state、React component、Provider Store owner |
| `@embercloud/agent-ui` | future facade，只 re-export UI 侧包 | runtime transport、业务实现 |
...
| 现有路径 | 目标 | 验收 |
| --- | --- | --- |
| `packages/app-server-client` | `@embercloud/agent-runtime-client` seed | 不生成 UI projection，不带 React。 |
| `packages/agent-runtime-projection` | `@embercloud/agent-runtime-projection` current | projector / selectors 在 React 外可测，输出 `state.subagents`。 |
| `packages/agent-runtime-ui` | `@embercloud/agent-runtime-ui` current | 只消费 projection state，Subagents 不重算 runtime facts，Artifact/Evidence refs 不读取大 payload。 |
...
| 85% | Ember 主仓 seed 包实现可被至少一个产品应用消费。 |
...
| 98.5% | Content Studio App Server turn 主链已抽到标准 session gateway 形状，`nextEvent()` 保持 `agentSession/event` notification 合同，sidecar service 已委托 gateway；标准包已发布到 `@embercloud` organization：`@embercloud/app-server-client@1.0.0` 与 `@embercloud/agent-runtime-client@0.1.1`。Content Studio 已固定安装 registry 版 `@embercloud/agent-runtime-client@0.1.1`，并通过 `@embercloud/agent-runtime-client/sessionGateway` 包装现有 gateway；首轮误发的无 scope `app-server-client@1.0.0` 和依赖旧名的 `@embercloud/agent-runtime-client@0.1.0` 只作为 compat-misrelease 记录。GUI smoke 仍未补齐，因此不提升到 100%。 |
