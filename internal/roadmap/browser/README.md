...
- **CDP adapter**：`ember-rs/crates/browser-runtime`，负责外部 Chrome / CDP session、自动化动作、观测事件和证据事实。
...
3. `ember-rs/crates/browser-runtime` 继续承接 CDP / 外部 Chrome session / action runtime，不直接拥有 Right Surface `WebContentsView`。
...
| Rust Browser Runtime                | `ember-rs/crates/browser-runtime/src/**`                                                                                       | CDP session manager、event buffer、action execution                           | P4 以后承接 Browser Assist 可见执行、action trace 和 evidence facts 的 runtime 部分                                           |
| App Server Browser Session protocol | `ember-rs/crates/app-server-protocol/src/protocol/v0/browser_session.rs`                                                       | `browserSession/*` params / response / schema                                 | P4 最小 current method：target list、open、read、close、event list、action execute                                            |
| App Server Browser Session runtime  | `ember-rs/crates/app-server/src/runtime/browser_session.rs`、`processor/browser_session.rs`                                    | RuntimeCore 到 `BrowserRuntimeManager` 的 thin owner 和 JSON-RPC dispatch     | 业务逻辑不塞回 `processor/mod.rs`；中心文件只保留最小 dispatch                                                                |
| Rust App Server client              | `ember-rs/crates/app-server-client/src/lib.rs`                                                                                 | Rust typed request helper / protocol re-export                                | 保持和 `app-server-protocol` 同步，供 smoke / sidecar / 后续 Rust consumer 复用                                               |
...
| App Server Right Surface protocol   | `ember-rs/crates/app-server-protocol/src/protocol/v0/right_surface.rs`                                                         | Right Surface request / pending contract                                      | P1 尽量复用 `metadata` 传 browser intent；字段稳定后再提升为强类型                                                            |
| App Server Right Surface runtime    | `ember-rs/crates/app-server/src/runtime/right_surface.rs`、`processor/right_surface.rs`                                        | pending request 存储、消费、通知                                              | 继续作为 Right Surface 请求事实源；不新增 browser 专用 pending 队列                                                           |
...
```text
ember-rs/crates/app-server-protocol/src/protocol/v0/browser_session.rs
ember-rs/crates/app-server/src/runtime/browser_session.rs
ember-rs/crates/app-server/src/processor/browser_session.rs
...
```text
ember-rs/crates/app-server-protocol/src/protocol/v0.rs
...
ember-rs/crates/app-server-protocol/src/protocol/v0/schema_types.rs
...
`ember-rs/crates/browser-runtime` 不删除，也不继续孤立成调试页面专属 runtime。它在统一 Browser Runtime 中承担三类职责：

| 职责                          | owner                                                           | 与内嵌浏览器关系                                                                                              |
| ----------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| CDP session / external Chrome | `ember-rs/crates/browser-runtime`                                | 用于需要真实 Chrome profile、扩展、远程调试或站点适配的场景                                                   |
...
| Browser action trace 需要进入 EvidencePack / Replay                                | 在 `ember-rs/crates/browser-runtime/src/` 增加 `trace.rs` 或 `evidence.rs`       | 只做可见浏览器时不加 Rust 事实模型 |
...
| CDP action          | `ember-rs/crates/browser-runtime`                | `BrowserEvent`、`BrowserActionResult`、event buffer                  | action 有 `actionId/sessionId/status/duration`，失败有 reason            | `evidence_gap`   |
