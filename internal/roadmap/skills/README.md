...
1. Agent 初始化会把 Lime skills 加载到 agent-rust `global_registry`，代码注释甚至写着“使 AI 能够自动发现和调用这些 Skills”，但同一初始化路径又用 `LimeSkillTool` 覆盖默认 SkillTool，避免通用对话默认暴露全部本地 Skills。见：
   - `ember-rs/crates/agent/src/agent_state.rs`
   - `ember-rs/crates/agent/src/agent_state_support.rs`
   - `ember-rs/crates/agent/src/tools/skill_tool_gate.rs`
2. `LimeSkillTool` 的 current 目标是安全 gate：未启用 session access 时拒绝执行；启用后再按 allowlist 裁剪。这保证安全，但没有提供“自然发现、自然选择、按需读取正文”的体验。
3. `ember-rs/crates/skills/src/skill_loader.rs` 已能解析 `SKILL.md`、`allowed_tools`、`when_to_use`、workflow 等字段，但 `load_skills_from_directory` 只按目录直接扫描一级 skill，缺少 Codex 那种 per cwd / per config snapshot、禁用规则、路径 alias、预算渲染和隐式索引。
4. `ember-rs/crates/skills/src/skill_matcher.rs` 只有关键词式 `when_to_use_config` 匹配，且没有成为 Agent turn 的 current 决策入口；它更像未接主链的辅助器。
5. `ember-rs/crates/agent-rust/crates/agent/src/agents/skills_extension.rs` 有 `loadSkill` 工具和简版 instructions，但这仍是 extension 内部能力，不等于 Lime App Server runtime 的统一事实源。
...
| `ember-rs/crates/services/src/skill_service.rs` | `current` | 技能安装、管理、检查、本地/远程包 UI 数据源 |
| `ember-rs/crates/skills/src/skill_loader.rs` | `current`，但需收敛 | 作为 Agent Skills metadata / body parser 的基础；需要补 snapshot、scope、policy 和预算渲染 |
...
1. 在 `ember-rs/crates/skills` 或独立子模块中实现 `AgentSkillSnapshotService`，复用 `skill_loader`，避免再新增平行 parser。
...
```bash
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-skills
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent skill
...
1. 在 `ember-rs/crates/skills` 新增 `AgentSkillSnapshot`、`AgentSkillMetadata`、`AgentSkillRoot`、`AgentSkillScope`。
...
```bash
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-skills agent_
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server session_config_appends_project_agent_skills_metadata_to_system_prompt
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server agent_skills_context
```
...
1. 在 `ember-rs/crates/skills` 新增 `AgentSkillSelection`、`AgentSkillSelectionTrigger`、`AgentSkillBodyLocator`、`AgentSkillBody`。
...
```bash
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-skills agent_
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-skills agent_selection
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server agent_skills_context
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server session_config_appends_explicit_agent_skill_body_to_system_prompt
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server session_config_does_not_append_skill_body_without_explicit_selection
```
...
```bash
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server skill_runtime_enable
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent skill_tool_gate
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server agent_skills_context
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server session_config_appends_explicit_agent_skill_body_to_system_prompt
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server session_config_does_not_append_skill_body_without_explicit_selection
...
1. 在 `ember-rs/crates/skills` 新增 `select_agent_skills_by_name_candidates(...)`，把结构化候选选择收敛到 P2 selector，而不是在 App Server 里重复匹配逻辑。
...
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server agent_skills_context
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server skill_runtime_enable
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server session_config_projects_selected_skill_allowed_tools_to_turn_scope
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server session_config_does_not_project_tool_scope_for_unknown_skill_metadata
```
...
```bash
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server agent_skills_telemetry
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-skills agent_selection
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server agent_skills_context
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server skill_runtime_enable
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server session_config_projects_selected_skill_allowed_tools_to_turn_scope
...
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server agent_skills_telemetry
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server skill_runtime_enable
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server export_evidence_records_skill_invocation_from_tool_metadata
...
1. 在 `ember-rs/crates/skills` 新增 `agent_search` 纯逻辑模块，提供 `search_agent_skills(...)` 与 `reorder_agent_skill_snapshot_for_query(...)`。
...
```bash
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-skills agent_search
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server agent_skills_context
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server agent_skills_telemetry
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server skill_runtime_enable
...
1. 在 `ember-rs/crates/agent/src/tools/skill_search_tool.rs` 新增 `skill_search` Agent native tool，并在 Agent 初始化时注册到 current runtime 工具面。
...
```bash
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent skill_search_tool
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent test_tool_catalog_entries_for_surface_counts_and_lifecycle_boundaries
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent test_build_tool_inventory_marks_visibility_and_mappings
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-skills agent_search
```
...
1. `verify:local` 完整通过：
   ...
   - `cargo test --manifest-path "ember-rs/Cargo.toml"` 通过；真实联网 ASR / embedding / live GitHub smoke 按设计 ignored。
...
```bash
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server agent_skills_context expert
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server skill_runtime_enable expert
npm run test:contracts
```
...
```bash
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server queue -- --nocapture
cargo fmt -p app-server  # workdir: ember-rs
node --check scripts/agent-runtime/claw-chat-current-fixture-smoke.mjs
...
```bash
cargo fmt --manifest-path "ember-rs/Cargo.toml" --package lime-skills --package app-server --check
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-skills agent_selection
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server agent_skills_context
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server agent_skills_telemetry
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server skill_runtime_enable
