...
cargo fmt --manifest-path "ember-rs/Cargo.toml" --package app-server --check
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server scaffold
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server workspace_skill
