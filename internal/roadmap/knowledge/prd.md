# Ember Agent Knowledge PRD

> 状态：v1 / legacy current reference；v2 current 事实源见 `internal/roadmap/knowledge/prd-v2.md`
> 更新时间：2026-05-08
> 目标：把团队的 Agent Knowledge 标准接入 Ember，并让普通用户通过现有 Agent 输入框资料图标、File Manager 和首页引导完成项目资料的生成、沉淀、确认与使用闭环。
> v2 覆盖范围：目录结构、架构、时序、命令边界、Builder Skill、document-first profile、persona/data runtime mode 与运营类知识库路线，以 `internal/roadmap/knowledge/prd-v2.md` 和 `internal/roadmap/knowledge/prd-v2-diagrams.md` 为准。

## 1. 背景与结论

Ember 现有 `internal/knowledge` 已经沉淀了 Markdown-first 项目知识库、个人 IP 知识库 Builder Skill 原型、Skill 与知识包边界等早期方案。它们证明了一个方向：
...
1. 目录结构仍偏 Ember 自定义，例如 `knowledge.md`、`pack.json`、`source/`。
...
**Ember 接入 Agent Knowledge 标准，把 `KnowledgePack` 作为显式知识资产事实源；产品上表现为“项目资料”模块，主使用入口回到现有 Agent，资料管理页只负责检查、确认和维护。**

当前产品判断：

1. File Manager 是用户把本地资料沉淀为项目资料的自然入口。
2. 输入框底栏资料图标是用户选择和使用资料的主入口；`@资料` 只作为旧路由和快捷发现的兼容入口。
3. 首页引导负责把新用户带入“先添加资料，再让 Ember 生成”的路径。
...
4. v1 历史 legacy alias `knowledge_resolve_context` 曾按 v0.5.0 runtime standard 输出 bounded fenced context；current App Server method 为 `knowledgeContext/resolve`，v2 继续沿用 `context-resolution.schema.json`，但 selection 必须额外消费 `profile` 与 `runtime.mode`。
...
作为内容运营，我希望导入访谈稿、简历、历史文案和公开资料，让 Ember 生成一份个人 IP 知识包。确认后，我在写短视频脚本、沙龙开场白、朋友圈文案时，可以选择这份知识包，输出稳定体现人物经历、观点、语气和禁忌边界。
...
Ember 的知识包目录采用 Agent Knowledge 标准：
...
| Ember 用户模板 | Agent Knowledge `type` | Ember metadata |
| --- | --- | --- |
| 个人 IP | `personal-profile` | `metadata.emberTemplate: personal-ip` |
| 品牌产品 | `brand-product` | `metadata.emberTemplate: brand-product` |
| 组织 Know-how | `organization-knowhow` | `metadata.emberTemplate: organization-knowhow` |
| 增长策略 | `growth-strategy`（v0.6 标准；v1 曾使用 `custom:ember-growth-strategy`） | `metadata.emberTemplate: growth-strategy` |
...
interface KnowledgePack {
  ...
  metadata?: {
    ...
    emberTemplate?: "personal-ip" | "brand-product" | "organization-knowhow" | "growth-strategy" | string;
  };
}
...
interface KnowledgeContextResolutionRun {
  ...
  resolver?: {
    tool: "ember-knowledge-resolver" | string;
    version?: string;
...
```text
ember-rs/crates/knowledge/
  src/lib.rs        # KnowledgePack 文件事实源、编译、解析、测试

ember-rs/crates/app-server/
  # 暴露 knowledgePack/*、knowledgeContext/* current JSON-RPC method，不承载重复领域逻辑

ember-rs/src/commands/knowledge_cmd.rs
  # 已删除；旧 knowledge_* desktop facade 不再是实现入口
```
...
1. `ember-knowledge` 是后端领域事实源。
2. 旧 `knowledge_cmd.rs` / `knowledge_*` desktop facade 已删除，不承载领域逻辑；新能力不得继续落到 `ember-rs/src/commands/**`。
...
7. App Server `knowledgeContext/resolve` 是唯一 runtime context resolver 边界；任何页面、Skill、Memory 或 Inspiration 路径都不得自行拼装知识 prompt。
...
flowchart TB
  ...
  AppServer --> Domain["ember-knowledge crate"]
...
sequenceDiagram
  ...
  participant API as knowledgePack/source/import
  participant Compiler as knowledgePack/compile
...
sequenceDiagram
  ...
  participant Agent as 现有 Agent 输入框
  participant Catalog as knowledgePack/list
...
Ember runtime 的 v1 基线对齐 Agent Knowledge v0.5.0；v2 继续保留下列安全流程，但标准字段以 `prd-v2.md` 的 v0.6.0 `profile` / `runtime.mode` / `metadata.producedBy` 为准：
...
| Method | 职责 |
| --- | --- |
| `knowledgePack/source/import` | 导入文件或粘贴文本，写入 `sources/` 和导入记录。 |
| `knowledgePack/compile` | 通过 App Server Builder Skill runtime binding / compiler，生成或刷新 `wiki/`、`compiled/`、`runs/`。 |
| `knowledgePack/list` | 读取 catalog metadata，用于总览和选择器。 |
| `knowledgePack/read` | 读取单个知识包详情、状态、风险和运行时视图。 |
| `knowledgePack/default/set` | 设置 workspace 默认知识包。 |
| `knowledgePack/status/update` | 更新知识包状态。 |
| `knowledgeContext/resolve` | 按任务、状态、信任、预算解析 fenced context。 |
| `knowledgeContextRun/validate` | 校验 `runs/context-*.json` 是否符合 Agent Knowledge context-resolution schema；用于开发、CI 和诊断面板，不进入普通用户主路径。 |

命令边界变更时必须同步：

1. 前端 `src/lib/api/knowledge.ts` App Server 网关。
2. App Server JSON-RPC protocol / client / runtime 处理器。
3. Electron Desktop Host 只保留通用 `app_server_handle_json_lines` / `app_server_drain_events` bridge，不恢复专用 `knowledge_*` facade 或 IPC 白名单。
...
**后续知识包能力只允许向 `KnowledgePack + App Server knowledgePack/* / knowledgeContext* + Knowledge Context Resolver` 收敛；`project_memory_get` 只保留项目资料附属层职责，不能继续定义知识包主链。旧 `knowledge_*` desktop facade 已退场，只能作为负向守卫或历史记录出现，不得新增逻辑。**

命令契约补充：

1. `knowledgePack/list` 只返回 catalog metadata，不返回完整 `KNOWLEDGE.md` 正文。
2. `knowledgePack/read` 可以读取 guide、状态、风险和维护视图，但默认不返回 raw `sources/` 全文。
3. `knowledgeContext/resolve` 返回 fenced context、warnings、selectedFiles、sourceAnchors、missing、tokenEstimate，并可选择写入 `runs/context-*.json`。
4. `knowledgeContextRun/validate` 只校验诊断记录，不参与模型上下文组装。
...
2. 让 memory runtime 自行扫描 `.ember/knowledge/packs/` 组装另一套上下文。
...
`ember-rs/resources/default-skills/personal-ip-knowledge-builder/` 在 v2 中升级为 P1 current Builder Skill 事实源；`ember-rs/resources/default-skills/knowledge_builder/SKILL.md` 仅保留为 v1 历史实现 / compat 候选，不再承接新的章节模板或运营类扩展。
...
4. Builder Skill 由 App Server `knowledgePack/compile` current runtime binding 调用；导入、状态、默认包和诊断分别通过 `knowledgePack/source/import`、`knowledgePack/status/update`、`knowledgePack/default/set`、`knowledgeContextRun/validate` 维护；最终回答仍通过 `knowledgeContext/resolve` 读取已维护知识工件。
...
9. App Server `knowledgePack/*` / `knowledgeContext*` 最小 method 面，包括 `knowledgeContext/resolve` 和 `knowledgeContextRun/validate`。
10. `ember-rs/resources/default-skills/personal-ip-knowledge-builder/`（v2 P1 current Builder Skill）。
...
1. `internal/knowledge/ember-knowledge-base-construction-blueprint.md`。
2. `internal/knowledge/markdown-first-knowledge-pack-plan.md`。
3. `internal/knowledge/ember-project-knowledge-base-solution.md`。
...
7. `ember-rs/resources/default-skills/knowledge_builder/SKILL.md`（v1 内置整理器，v2 迁移为 compat / deprecated 候选）。

退出条件：

1. 新知识包实现落地后，旧文档只保留为 research / archive，README 中指向本 PRD。
2. App Server `knowledgePack/compile` 通过 current runtime binding 真实委托专用 Builder Skill 后，`knowledge_builder` 不再作为运行时 skill source。
...
本 PRD 不直接删除旧文件。后续如果发现无入口、无引用、无迁移价值的旧知识库草案，再按 `internal/aiprompts/governance.md` 进入 `dead` 分类和删除流程。
...
1. 新建 `.ember/knowledge/packs/<pack-name>/` 标准目录。
...
7. Runtime 通过 App Server `knowledgeContext/resolve` 注入 fenced context。
8. File Manager、输入框资料图标、`@资料` 兼容入口、首页引导和 Agent 输出沉淀纳入同一资料模块路径。
9. `knowledgeContext/resolve` 输出可选 `runs/context-*.json`，并能用 `knowledgeContextRun/validate` 校验。
10. Ember 用户模板与 Agent Knowledge 标准类型分离，`metadata.emberTemplate` 保留产品模板来源。
...
8. `knowledgeContextRun/validate` 只校验诊断记录，不进入普通用户主路径。
...
3. App Server `knowledgeContext/resolve` 是唯一运行时知识上下文解析入口。
4. 知识上下文必须 fenced。
...
```bash
test -f internal/roadmap/knowledge/prd.md
rg -n "File Manager|@资料|沉淀为项目资料|knowledgePack/source/import|knowledgeContext/resolve|KnowledgePack" internal/roadmap/knowledge/prd.md
rg -n "v0.5.0|context-resolution|knowledgeContextRun/validate|Agent Knowledge runtime" internal/roadmap/knowledge/prd.md
```
...
1. Agent Knowledge `v0.5.0` release：`https://github.com/embercloud/agentknowledge/releases/tag/v0.5.0`
2. `/Users/coso/Documents/dev/ai/embercloud/agentknowledge/docs/zh/specification.md`
3. `/Users/coso/Documents/dev/ai/embercloud/agentknowledge/docs/zh/client-implementation/runtime-standard.md`
4. `/Users/coso/Documents/dev/ai/embercloud/agentknowledge/docs/zh/client-implementation/runtime-context-resolver.md`
5. `/Users/coso/Documents/dev/ai/embercloud/agentknowledge/docs/zh/agent-knowledge-vs-skills.md`
6. `/Users/coso/Documents/dev/ai/embercloud/agentknowledge/docs/public/schemas/context-resolution.schema.json`
7. `/Users/coso/Documents/dev/ai/embercloud/agentknowledge/docs/zh/versions/v0.5.0/overview.md`
