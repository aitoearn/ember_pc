...
本目录承载 Ember Desktop 客户端侧的专家功能路线图。
...
| 分类 | 对象 | 说明 |
|---|---|---|
| current | `internal/roadmap/zuanjia/prd.md` | 专家广场、详情、添加和对话体验的产品方案。 |
| current | `internal/roadmap/zuanjia/architecture.md` | 客户端投影、缓存、安装 overlay、运行时绑定和失败模式。 |
| current | `internal/roadmap/zuanjia/implementation-plan.md` | P0-P5 客户端实施顺序、测试和验收。 |
| current | `/Users/coso/Documents/dev/ai/embercloud/embercore/internal/roadmap/zuanjia` | EmberCore 专家云市场控制面方案。 |
...
4. 专家目录来自 EmberCore；客户端 seeded catalog 只做离线兜底和开发 fixture。
5. 用户私有记忆、对话内容、workspace 数据不进入公共专家目录。
6. 新增用户可见文案必须覆盖 `zh-CN / zh-TW / en-US / ja-JP / ko-KR`。
7. GUI 改动必须按 Ember 桌面产品视觉语言落地，并做最小 GUI 冒烟。
