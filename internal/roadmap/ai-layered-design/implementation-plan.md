...
> 状态：proposal  
> 更新时间：2026-05-05  
> 目标：把 `LayeredDesignDocument`、原生分层生成、Canvas 编辑、扁平图拆层和导出能力拆成可执行阶段，确保实现持续回到 Ember current 主链。
...
1. 新增 `internal/research/ai-layered-design/` 研究拆解。
2. 新增 `internal/roadmap/ai-layered-design/` 路线图、架构、实施计划和图纸。
...
目标：用户上传已有图片后，Ember 能拆出主要对象并建立可编辑工程。

### 5.1 用户流

```text
用户上传海报 flat.png
  -> Ember 识别人像、Logo、烟雾、文字和背景
  -> 生成主要对象 mask
  -> 抠出 RGBA 图层
  -> 修补 clean background
  -> 普通文字转 TextLayer
...
**每个阶段都要证明 Ember 正在获得“设计工程能力”，而不是只多了一种图片生成方式。**
