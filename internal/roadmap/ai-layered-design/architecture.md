...
> 状态：proposal  
> 更新时间：2026-05-05  
> 目标：定义 Ember AI 图层化设计的文档协议、数据流、图层类型、编辑行为和 provider 边界，让后续实现不落成单图输出旁路。
...
```text
.ember/designs/<design_id>/design.json
.ember/designs/<design_id>/assets/<asset_id>.png
.ember/designs/<design_id>/previews/latest.png
```
