...
- `TeamDefinition`、`selectedTeam`、`recent_team_selection`、`teamWorkspaceRuntime`、`useTeamWorkspaceRuntime`、`ember.chat.team_selection.v1.general` 仍是历史兼容命名，只能承载现有 runtime metadata / session projection。
...
- 自定义 Team 克隆 / 本地写入入口：`cloneTeamDefinitionAsCustom`、`saveCustomTeams`、`buildWorkspaceSettingsWithCustomTeams`、`ember.chat.custom_teams.v1`。
