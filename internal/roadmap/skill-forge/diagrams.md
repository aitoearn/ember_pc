# Skill Forge 启发下的 Ember 架构图与流程图
...
4. 真实执行必须回到 Ember current runtime。
...
1. 来源格式只提供原料。
2. Ember 标准仍是 Skill Bundle / Adapter Spec。
...
flowchart LR
    ...
    Outcome --> Roadmap[internal/roadmap / internal/exec-plans]
...
flowchart LR
    ...
    Source[外部来源<br/>API / CLI / Website / MCP] --> Compile[编译到 Ember 标准]
    Compile --> Current
...
2. 图中执行节点必须能对应到 Ember 现有 Query Loop、tool_runtime、workspace 或 evidence 主链。
