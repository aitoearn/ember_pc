# Skill Forge 启发下的 Ember 实施计划

> 状态：P0-P10 第六刀完成；Read-Only HTTP API policy 已有 runtime evidence pack 端到端审计回归，并通过 GUI smoke
> 更新时间：2026-05-07
> 目标：把 Skill Forge / workspace-local generated skill / Agent envelope 的落地拆成可执行阶段，确保实现不偏离 Ember current 主链。
...
3. 注册结果只落到当前 workspace 的 `.agents/skills/<skill_directory>/`，并写入 draft 侧 `registration/latest.json` 与 registered skill 侧 `.ember/registration.json`。
4. P3B 已新增 `capability_draft_list_registered_skills`：显式传入 `workspaceRoot`，扫描当前项目 `.agents/skills`，只返回带 `.ember/registration.json` 的 P3A 注册能力。
...
2. **P3B：workspace registered discovery**
   ...
   - 只投影带 `.ember/registration.json` 的 P3A 注册能力。
...
2. P3B discovery 只返回带 `.ember/registration.json` 的当前 workspace 注册项。
...
7. 不允许来源 API / CLI schema 反向定义 Ember runtime 协议。
