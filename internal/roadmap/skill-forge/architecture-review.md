...
> 状态：review gate  
> 更新时间：2026-05-06
> 目标：在进入实现前，重新检查 Skill Forge 启发下的 Ember 方案是否缺层、缺闭环，或误把 Skill / 目标续跑当成完整 Agent 系统。
...
2. Ember 不新增平行 AI PM / AB / telemetry / evidence 系统。
...
[../../research/pi-mono-coding-agent/README.md](../../research/pi-mono-coding-agent/README.md) 已确认 `pi-mono` 的默认 coding tools 是 `read / write / edit / bash`，另有 read-only tools `read / grep / find / ls`，并支持 `noTools / tools allowlist / customTools`。这说明 Ember 的首期不应该直接给模型完整 coding tools，而应该按 profile 给能力：
...
Skill Forge 访谈强调 AB testing 和日志反馈，但 Ember 不能把它们塞进 evidence pack 里。
...
Ember 当前是桌面 GUI 产品，需要明确三种情况：

1. **关闭浏览器页签**
   - 如果 Ember App 仍在，automation job 可以继续。
