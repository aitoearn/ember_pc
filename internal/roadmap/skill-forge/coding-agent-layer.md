...
你指出的问题是对的：如果只有 `Managed Objective`，Ember 得到的是一个“目标续跑控制层”；但 Skill Forge 案例最关键的不是续跑本身，而是：
...
所以 Ember 的完整方案必须有三条互相衔接、但不能混成一条的链：
...
2. Ember 的 generated capability 未来会进入 skill catalog、automation job 和 evidence 主链，风险会扩展到账号、API、业务数据、外部发布、花钱、删除和可调度重复执行。
...
[../../research/pi-mono-coding-agent/README.md](../../research/pi-mono-coding-agent/README.md) 说明 `pi-mono` 可以作为本层的 engineering reference，但不能作为 Ember 的新产品形态。
...
1. 不新增 `coding_agent_runtime`。
2. 不把 JSONL session 变成 Ember 第二事实源。
3. 不开放全仓库 `bash / write / edit` 给 draft 生成任务。
4. 不让 pi-style extension runtime 替代 Ember governance / evidence。
5. 不把终端命令系统搬进 Ember 前台。
...
**Ember 这层弱，不代表要补成通用 Coding Agent；先补成可治理的 capability authoring 子集。**
...
4. 把外部 API schema 直接变成 Ember runtime 协议。
