...
> 更新时间：2026-06-15  
> 范围：固定 catalog 来自 `ember-rs/crates/agent/src/agent_tools/catalog.rs`，前端展示来自 `src/components/agent/chat/utils/toolDisplayInfo.ts` 与 `ToolCallDisplay*` 测试。动态 MCP / browser 工具在本文只按工具卡渲染和 inventory snapshot 评测；完整 MCP 系统见 `mcp.md`。
