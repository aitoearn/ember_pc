...
| 本地默认包 | `ember-rs/resources/default-skills/**/SKILL.md` | 当前发现 `33` 个默认 `SKILL.md`，覆盖分析、调研、写作、媒体生成、知识构建、站点检索等 |
...
| Runtime gate | `ember-rs/crates/agent/src/tools/skill_tool_gate.rs` | 有 session access gate、allowed skills、allowed skill sources、source metadata、allow / deny 测试 |
| Runtime 执行 | `ember-rs/crates/agent/src/skill_execution.rs` | 有 workflow / prompt execution、`skill-exec-*` runtime turn、allowed_tools 转发、TextDelta / runtime.status；回合终态由 App Server `turn.completed / turn.failed / turn.canceled` 收口 |
