...
| Lime tool catalog | `ember-rs/crates/agent/src/agent_tools/catalog.rs`、`inventory.rs`、`runtime_backend/tool_inventory.rs` | 工具评测必须以 catalog / runtime inventory 为事实源；本轮盘点出 60 个固定 catalog 条目，并将动态 MCP / browser 工具单独按 runtime snapshot 评估 |
