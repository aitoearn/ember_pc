...
| App Server 协议 | `packages/app-server-client/src/protocol.ts`、`ember-rs/crates/app-server-protocol/src/protocol/v0/mcp.rs` | current 方法包括 `mcpServer/*`、`mcpServerStatus/list`、`mcpTool/*`、`mcpPrompt/*`、`mcpResource/*` |
| App Server 实现 | `ember-rs/crates/app-server/src/processor/mcp.rs`、`runtime/mcp.rs`、`runtime/app_data/mcp.rs` | MCP processor 与 runtime 已进入 App Server current 主链 |
| Rust MCP crate | `ember-rs/crates/mcp/src/manager.rs`、`client.rs`、`types.rs`、`tool_converter.rs` | 有 connection manager、stdio 启停、工具名正规化、工具/提示词/资源调用、progress/logging handler |
| Agent Runtime | `ember-rs/crates/agent/src/mcp_bridge.rs`、`agent_tools/inventory.rs`、`ToolSearch`、`ListMcpResourcesTool`、`ReadMcpResourceTool` | 动态 MCP 工具和 resource helper 已能进入 tool inventory / tool UI 分类 |
...
| MCP runtime manager | `ember-rs/crates/mcp/src/manager.rs` | 3.5 | streamable HTTP / OAuth / list_changed / elicitation 的 current 产品证据 |
