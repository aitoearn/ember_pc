...
判断官方 `i18next-cli` 是否可以作为 Ember P3 的默认抽取 / lint / locale sync / type generation 工具链候选。
...
`i18next-cli` 适合作为后续统一工具链候选，但当前不能直接替换 Ember 的治理脚本。

原因很直接：

1. Ember 现在不仅需要抽取和 locale sync，还需要动态前缀保护、unused key 家族分桶、legacy Patch 退出门禁。
...
- Ember 自研 `detect-missing-translations` 在同一 fixture 上只负责资源结构一致性，不替代 extraction / lint / types。
- Ember 自研 `i18n-unused` 现在也纳入同一 benchmark：在干净 fixture 上能输出 `unusedKeyCount=1` 与 `protectedKeyCount=1`，补齐官方 CLI 当前没有直接覆盖的 unused / protected dynamic family 视角。
