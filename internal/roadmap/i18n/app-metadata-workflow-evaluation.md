...
- 根级 `package.json`、`ember-rs/Cargo.toml`、`forge.config.mjs` 与 `ember-rs/capabilities/plugin-shell.json` 是当前 installer / app metadata 审阅事实源。
...
- `package.json` 与 `ember-rs/Cargo.toml` 的 description 仍是单一英文描述，不是分 locale 的 metadata bundle。
- `plugin-shell.json` 的 description 仍是单一中文说明，没有 companion 版本。
...
- [ember-rs/Cargo.toml](/Users/coso/Documents/dev/ai/aiclientproxy/lime/ember-rs/Cargo.toml)
- [forge.config.mjs](/Users/coso/Documents/dev/ai/aiclientproxy/lime/forge.config.mjs)
- [ember-rs/capabilities/plugin-shell.json](/Users/coso/Documents/dev/ai/aiclientproxy/lime/ember-rs/capabilities/plugin-shell.json)
