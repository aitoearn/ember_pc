# Ember installer / app metadata i18n 评估
...
判断 Ember 的 installer、应用元数据和壳层描述，是否已经具备独立的多语言工作流。

## 当前事实

- 根级 `package.json`、`ember-rs/Cargo.toml`、`forge.config.mjs` 与 `ember-rs/capabilities/agent-app-shell.json` 是当前 installer / app metadata 审阅事实源。
