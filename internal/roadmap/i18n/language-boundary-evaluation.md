...
- 新增 `scripts/i18n/i18n-language-boundary-report.ts`，默认扫描 `src/` 与 `ember-rs/`，并把 language-like marker 分类为：
  - `uiLocale`
...
| 文件 | 数量 | 边界判断 |
| ---- | ---- | ---- |
| `ember-rs/src/commands/media_task_cmd.rs` | 57 | media task 产物语言 / 内容语言 |
...
| `ember-rs/crates/knowledge/src/lib.rs` | 19 | Knowledge 内容语言元数据 |
...
3. Browser Environment 里的 `Accept-Language` 已经是独立事实源，不能跟随 Ember UI locale 自动变化。
