# Ember AgentRuntime Profile 路线图

> 状态：implementation-audited
> 更新时间：2026-05-17
> 目标：把 Ember 已有的运行时、任务、模型路由、权限、证据、GUI read model 与 AgentRuntime 标准收敛成一条可执行、可测试、可回放的 current 主链。
> 上游标准草案：`/Users/coso/Documents/dev/ai/embercloud/agentruntime`，当前对齐 `Agent Runtime v0.4.0` 与 `Ember AgentRuntime Profile`。
...
Ember 当前最大问题不是功能不足，而是功能已经很多，却缺少统一运行事实源，导致：

1. 任务、会话、子代理、自动化 job、模型路由、证据导出各自有局部真相。
2. GUI、review、analysis、dashboard 会二次拼状态。
3. 测试只能验证局部函数，难以证明一次 agent turn 从提交到证据导出的完整闭环。
4. 标准协议很多，但没有一条强制绑定 Ember current 主链的 profile。

本路线图回答：

**Ember 如何用 AgentRuntime Profile 把 Objective / Session / Thread / Turn / Task / Tool / Action / Evidence 收敛成一条主链，并让 GUI、review、replay、analysis 只消费这条主链。**
...
2. **Ember 先实现 Ember Profile，不直接追完整公开标准。**
   公开 AgentRuntime 保持跨产品通用；Ember Profile 收紧 required ids、event families、snapshot、fixtures 和验收场景。
...
7. **Chat、Claw、Agent App、Automation 都只是 runtime surface。**
   完整 AI 能力只能向 AgentRuntime facts 收敛；内容工厂这类 App 不能把 `EMBER_GATEWAY_*`、模型 API 或嵌入通用 Chat 当成 Agent 能力边界。App 内 `ember.agent` / `ember.workflow` 必须通过 Agent App Runtime Surface 复用 Aster / Claw / Skills / Tools / Evidence 主链，详见 [./app-surface-runtime.md](./app-surface-runtime.md)。
...
本路线图完成时，Ember 至少应该能做到：

1. 任意 agent turn 都能导出符合 Ember AgentRuntime Profile 的核心事件和 thread read snapshot。
