# Ember AgentRuntime Profile 架构蓝图

> 状态：implementation-audited
> 更新时间：2026-05-16
> 作用：定义 AgentRuntime Profile 在 Ember current 主链中的分层、owner、事实源和接口边界。
...
5. 让公开 AgentRuntime 标准通过 Ember Profile 可测试地落地。
...
| model routing | `request_model_resolution.rs`、`internal/roadmap/task/` |
...
`agent_app_cmd.rs` 当前属于 Agent App 生命周期 owner，不是完整 AgentRuntime owner。`EMBER_GATEWAY_*` / `OPENAI_BASE_URL` 注入只能视为低阶模型 executor 或开发期 fallback；完整 `ember.agent` / `ember.workflow` 必须进入 AgentRuntime Surface。
...
1. Schema fixture 校验：验证 Ember Profile fixtures。
