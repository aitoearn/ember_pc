# Ember AgentRuntime Profile 测试用例

> 状态：implementation-audited
> 更新时间：2026-05-12
> 作用：把 AgentRuntime Profile 的 PRD、架构和落地计划翻译成可执行测试矩阵，避免 Ember 继续靠局部测试或 GUI 观感判断运行主链是否正确。
...
| 层级 | 目的 | 主要对象 | 推荐执行时机 |
| --- | --- | --- | --- |
| Schema / Fixture | 验证 Ember Profile 事件和 snapshot 形状 | AgentRuntime profile fixtures | 改 schema、事件字段、snapshot 字段时 |
...
首期用例对齐 AgentRuntime 标准项目中的 Ember Profile fixtures：

| Fixture | 覆盖场景 | Ember 对应能力 |
| --- | --- | --- |
...
Ember 仓库落地实现时，真实 runtime 输出可以不直接复制 fixture 内容，但必须满足同一结构约束和语义断言。
...
| `agentui-profile-event-compat.json` | AgentUI 接收 Ember Profile dotted event，不告警、不创建 UI-owned truth | AgentUI |

## 4. P0 文档与标准冻结用例

| ID | 用例 | 前置条件 | 操作 | 期望结果 |
| --- | --- | --- | --- | --- |
| AR-P0-DOC-001 | roadmap 文档入口完整 | `internal/roadmap/agentruntime/` 存在 | 检查 README 链接 | PRD、architecture、diagrams、implementation-plan、test-cases 全部可达 |
...
| ID | 用例 | 输入 | 断言 |
| --- | --- | --- | --- |
| AR-SCHEMA-001 | submit turn event 符合 Ember Profile | `submit-turn-event.json` | 必含 `eventId/schemaVersion/runtimeId/sessionId/threadId/turnId/type/timestamp/sequence/payload` |
...
| AR-UI-LINK-001 | AgentUI 静默兼容 P1 profile event | `schemaVersion=ember-profile-0.4.0` 且 `type=turn.submitted` | unknown event controller 不告警，保持 stream 活跃 |
...
这三类测试能最快证明 AgentRuntime 不是新文档，而是 Ember current runtime 的可执行主链。
