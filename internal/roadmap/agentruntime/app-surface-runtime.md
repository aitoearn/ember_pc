...
> 状态：in progress
> 更新时间：2026-05-16
> 作用：定义 Agent App 如何作为 AgentRuntime 的业务 surface 复用 Ember Agent / Claw / Aster 主链，而不是把 App 做成模型 API 壳或 Chat UI 旁路。

## 1. 一句话结论

Agent App 是业务工作台，AgentRuntime 是智能体运行事实主链，Claw 是一组已落地的能力入口和能力治理经验。内容工厂这类 App 不能只拿模型 API，也不能跳回通用 Chat 完成主流程；它必须通过统一 Agent Runtime Surface 调用 Ember 已有 Agent 能力，并把进度、追问、人工确认、产物和证据回投到 App 页面内。
...
Content Factory / Agent App
  ...
  -> Aster / ember_agent / Skills / Tools / MCP / Browser / Evidence
...
6. 将最终文案、素材需求、脚本、报告写入 `ember.storage`、`ember.artifacts` 和 `ember.evidence`。

直接注入 `EMBER_GATEWAY_*` 或给 App 一个 OpenAI-compatible token 只能提供低阶模型生成；它无法复用 Ember 的工具治理、知识绑定、Evidence Pack、review、runtime queue 和 capability gap。后续它只能作为 degraded fallback 或受控模型 executor，不是完整 Agent 能力边界。
...
`thinking / text delta / tool input-output / Skill / routing / cost / usage / artifact / evidence / blocked / completed` 这类 Claw 式运行过程属于 Ember 主 App 的 runtime projection，不属于垂直业务 App。Agent App 只允许消费 Host Bridge 下发的 `runtimeProcess` / `process` 标准视图，并用它做业务展示：
...
## 3.1 全量 `ember.*` 能力与 AgentRuntime 的关系

Agent App 侧全量能力路线图见 [`../agentapp/p18-7-full-ember-capability-surface.md`](../agentapp/p18-7-full-ember-capability-surface.md)。本文只固定 AgentRuntime owner 边界：凡涉及 AI 执行、模型、Skill、工具、记忆、用量、证据的能力，生产事实必须继续向 AgentRuntime / ToolRuntime / Desktop Host 收敛。

Agent App 不再只抽象 `ember.agent / ember.workflow` 两个入口。完整 Ember AI 能力必须拆成三类，并统一登记在 `src/features/agent-app/sdk/capabilityCatalog.ts`：

| 类型       | Capability                                                                                                                     | AgentRuntime 关系                                                                             | App 边界                                                                |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| 执行主链   | `ember.agent`、`ember.workflow`、`ember.automation`、`ember.tasks`                                                                 | 进入 AgentRuntime session/thread/turn/task/queue。                                            | App 只定义业务输入、状态递进和结果写回。                                |
| 智能体资源 | `ember.models`、`ember.skills`、`ember.memory`、`ember.context`、`ember.usage`                                                      | 从 AgentRuntime request metadata、runtime facts、read model、telemetry 投影。                 | App 不保存 provider key，不复制 Skill runtime，不自建 token/cost 统计。 |
| 工具与集成 | `ember.tools`、`ember.mcp`、`ember.browser`、`ember.search`、`ember.documents`、`ember.media`、`ember.terminal`、`ember.connectors`    | 经 ToolRuntime / MCP bridge / Browser / Media / Document runtime 接入 AgentRuntime evidence。 | App 声明需求和消费结构化结果，不直连底层服务。                          |
| 数据与证据 | `ember.storage`、`ember.files`、`ember.knowledge`、`ember.artifacts`、`ember.evidence`                                              | 与 AgentRuntime task provenance、artifact refs、knowledge refs 关联。                         | App 定义业务对象和产物类型，Ember 负责命名空间、引用和审计。             |
| 治理与宿主 | `ember.ui`、`ember.events`、`ember.workspace`、`ember.policy`、`ember.secrets`、`ember.settings`、`ember.review`、`ember.capabilities` | 作为 Host / policy / readiness projection 约束 runtime 调用。                                 | App 做展示和降级，不绕过 Host Bridge。                                  |

事实源声明补强：

```text
所有可给 Agent App 使用的 Ember 功能，名称和边界先进入 ember.* capability catalog。
...
这解决内容工厂当前暴露出来的问题：如果 App 需要完整 Ember AI 能力，不是把 Claw UI 嵌进去，也不是让 App 调模型 API，而是把 Claw 已有的图片、搜索、研报、PDF、总结、PPT、浏览器、发布等能力拆成 `ember.skills / ember.tools / ember.search / ember.media / ember.documents / ember.browser / ember.artifacts / ember.evidence` 等能力，再由 `ember.agent.startTask` 或 `ember.workflow.start` 编排。缺失的能力只允许在 catalog/profile 中显示为 unavailable，不允许由业务 App 临时 mock 成成功。
...
3. `ember.agent` / `ember.workflow` 是 App 面向 SDK 的 capability facade，内部必须委托 AgentRuntime Surface。
...
3. `EMBER_GATEWAY_*` 可作为低阶模型 executor 或开发期 fallback，但不能被文档或 UI 宣称为完整 Agent 能力。

当前进展：`agent_app_runtime_start_task` 等后端 facade 已可用，`AgentAppRuntimePage` 的 Host Bridge 已通过 `AgentRuntimeCapabilityHost` 将 `ember.agent` 的 start / stream / get / cancel / retry / submitHostResponse 接到该 facade；本地 in-memory adapter 只继续承接 storage / artifact / evidence / knowledge 的前端样板与离线测试，不再冒充完整 Agent 任务执行事实源。
