...
> 更新时间：2026-05-12
> 作用：把 `agentruntime`、`agentcontext`、`agentevidence`、`agentpolicy` 与 `agentui` 的 owner 边界写清楚，避免 Ember 把标准协议继续叠成多条事实源。

## 1. 标准来源

| 标准项目 | 本地路径 | Ember 使用方式 |
| --- | --- | --- |
| AgentRuntime | `/Users/coso/Documents/dev/ai/embercloud/agentruntime` | runtime facts 主链，拥有 session/thread/turn/task/event/snapshot/control-plane。 |
| AgentContext | `/Users/coso/Documents/dev/ai/embercloud/agentcontext` | context envelope、context item、source ref、selection、budget、assembly、injection、compaction 与 missing context 的 owner。 |
| AgentEvidence | `/Users/coso/Documents/dev/ai/embercloud/agentevidence` | evidence pack、claim/source/support/provenance、verification、review、replay、redaction 与 completeness 的 owner。 |
| AgentPolicy | `/Users/coso/Documents/dev/ai/embercloud/agentpolicy` | policy decision、risk scope、approval request、permission grant、redaction/retention obligation、waiver 与 policy trace 的 owner。 |
| AgentUI | `/Users/coso/Documents/dev/ai/embercloud/agentui` | runtime-first projection surface，只读 runtime/context/policy/evidence/artifact facts，通过受控动作写回 owner。 |

2026-05-11 已补读：

- `/Users/coso/Documents/dev/ai/embercloud/agentcontext/docs/zh/specification.md`
- `/Users/coso/Documents/dev/ai/embercloud/agentcontext/docs/zh/authoring/runtime-profile-test-cases.md`
- `/Users/coso/Documents/dev/ai/embercloud/agentevidence/docs/zh/specification.md`
- `/Users/coso/Documents/dev/ai/embercloud/agentevidence/docs/zh/authoring/runtime-profile-test-cases.md`
- `/Users/coso/Documents/dev/ai/embercloud/agentpolicy/docs/zh/specification.md`
- `/Users/coso/Documents/dev/ai/embercloud/agentpolicy/docs/zh/authoring/runtime-profile-test-cases.md`
...
AgentContext 规范要求实现为重要 turn、task、tool call 与 handoff 导出 context envelope，并保留 selected、omitted、budget、assembly、injection、compaction、missing context 事实。Ember read model 只能保留 refs/summary，不能把大 context item content 复制进 runtime snapshot。
...
Ember 当前 P1 只要求 runtime event 保留 `session/thread/turn` correlation；P2/P4 再把 memory budget、missing context、compaction summary 接入 read model。
...
P1 允许 AgentUI 对 `schemaVersion = ember-profile-0.4.0` 的 dotted profile event 静默保持 stream 活跃；P2 开始必须把这些 facts 归入 `ThreadReadModel`，再由 AgentUI 只读展示。
...
| 阶段 | 相邻标准目标 | Ember 最小产物 |
| --- | --- | --- |
| P1 | Runtime event envelope 可被 AgentUI 安全接收 | `ember-profile-0.4.0` stream event 静默兼容与结构测试 |
...
| 测试 | 断言 |
| --- | --- |
| Runtime/Profile fixture | `schemaVersion=ember-profile-0.4.0`、核心 ids、sequence 单调。 |
