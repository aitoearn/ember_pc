...
在正式 `agent-apps` runtime surface 中建立标准 Host Bridge：App UI 运行在受控 iframe / sandbox 内，通过 `ember.agentApp.bridge` 与 Ember Host 同步主题、语言、入口上下文、导航、通知、下载和 capability 请求；App 不再拥有私有 `postMessage` 协议，也不能直接访问 Tauri / Node / 宿主 DOM。
...
上游事实源是 `/Users/coso/Documents/dev/ai/embercloud/agentapp` 的 Agent App v0.3 Host Bridge v1 文档。Ember 客户端只做标准实现，不重新定义协议。
...
| Theme sync            | 订阅 Ember theme / color scheme / system mode，广播 `theme:update`。                           | 不让 App 自己猜 Ember 主题。                           |
...
| Agent task            | 通过 `ember.agent.startTask` / `ember.workflow.start` 在 App 内运行任务、流式事件和结构化结果。 | 不跳转通用 Chat 完成核心工作，不让 App 自建模型网关。 |
| Structured write-back | App 确认后通过 `ember.storage` / `ember.artifacts` / `ember.evidence` 写回业务对象和证据。       | 不让聊天文本成为唯一结果，不要求用户手工复制。        |
...
Agent task 事件通过 `capability:invoke` 的标准请求 / 响应承载，不新增私有协议。`ember.agent.startTask` 返回 `taskId` / `traceId` 后，后续 stream 至少要表达：
...
App 收到后只写入自身 `document.documentElement.style`，并设置 `data-ember-theme`、`data-ember-theme-effective`、`data-ember-color-scheme`。主题 token 不是业务数据，不进入 App storage、Artifact 或 Evidence。
...
flowchart TD
  ...
  Bridge --> Theme[Ember Theme / Color Scheme]
  ...
  Guard --> HostCaps[Ember Host Capabilities]
...
| Appearance helpers          | 读取当前 Ember CSS variables，产出 theme payload。                                                             |
| Capability dispatcher       | 把 `capability:invoke` 转成现有 SDK / adapter 调用或 blocked error。                                          |
| Agent task dispatcher       | 把 `ember.agent.startTask` / stream / cancel / retry 绑定到 App 作用域 task，附加 appId / entryKey / traceId。 |
...
- 切换 Ember 主题 / 配色后，已打开的 App iframe 内颜色同步变化。
...
| P17.4.2 | 已完成：Agent task stream contract hardening。 | `ember.agent.startTask / streamTask / getTask / cancelTask / retryTask` 与 Host Bridge response 证据。 | App 内可 start / stream / get / cancel / retry，不回跳通用 Chat。                    |
...
2. 不新增 legacy desktop facade，不改 `src/lib/api/agentApps.ts`，不改变 Cloud / EmberCore control-plane 边界。
...
1. 本轮只认领客户端 Host Bridge runtime 任务流回归，不触碰 `agentknowledge`、Ember Cloud / EmberCore、上游标准仓库或 AgentApp 其他 roadmap 资料。
...
1. `AgentAppRuntimePage.test.tsx` 增加 Host Bridge 集成回归，模拟同一 iframe / origin / appId / entryKey 下连续发起 `ember.agent.startTask`、`ember.agent.streamTask`、`ember.agent.cancelTask`。
...
1. 本轮只认领 P17.4.2 Host Bridge task contract，不触碰 AgentApp 其他 roadmap 总纲资料、`agentknowledge`、Ember Cloud / EmberCore 或上游标准仓库。
...
4. `capabilityDispatcher` 支持 `ember.agent.retryTask`，继续只通过 Host Capability SDK 路径调用，不新增私有 bridge method。
...
1. 本轮只认领内容工厂 App bootstrap 和 Host Bridge request / response 样板，不触碰 SceneApp 清理、安装生命周期和 Cloud / EmberCore 控制面。
...
2. `content-factory-app/src/ui/app.js` 在 `confirmCurrentStep()` 成功后调用 Host Bridge，同步执行 `ember.agent.startTask -> ember.agent.streamTask -> ember.storage.set -> ember.artifacts.create -> ember.evidence.record`。
...
| 命令 / 证据                                                                                                                                                                                                                                                                      | 结果                                                                                                                                         |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `/Users/coso/Documents/dev/ai/embercloud/content-factory-app: npm run verify`                                                                                                                                                                                                     | 通过，build、41 tests、validate、readiness 均 OK；readiness 在未绑定 host setup 时保持 `needs-setup`。                                       |
...
1. 本轮只认领客户端 Host 侧 structured write-back guard，不触碰上游 `content-factory-app`、Ember Cloud / EmberCore 或其他 AgentApp roadmap 总纲资料。
...
2. `ember.storage.set/delete` 写回前要求 manifest 声明 storage namespace；没有 storage 声明时返回 `WRITEBACK_NOT_DECLARED`。
3. `ember.artifacts.create` 写回前校验 `input.kind` 必须命中 `projection.artifactTypes[].key`，未声明 artifact kind 返回 `WRITEBACK_NOT_DECLARED`。
4. `ember.evidence.record` 写回前校验 `input.kind` 必须命中 `projection.evals[].key`，未声明 evidence subject 返回 `WRITEBACK_NOT_DECLARED`。
...
| `nice -n 10 npm run smoke:agent-service-skill-entry`                                                                               | 通过，Skill Forge 前端 metadata、8 个 Rust 定向测试、服务技能入口路由、Agent 对话 A2UI 挂起主链均通过，并生成 `.ember/qc/skill-forge-runtime-transcript-current.json`。                            |
...
| `rg -n "port\|3030\|1420\|health-url\|invoke-url" scripts/verify-gui-smoke.mjs ember-rs`                                          | 已确认 smoke 脚本可替换 health / invoke URL，但当前 DevBridge 端口仍由 Tauri 后端固定为 3030；没有不碰现有 Tauri dev 的隔离端口路径。                                                             |
...
| `lsof -nP -iTCP:3030 -sTCP:LISTEN` | `target/debug/ember` 正在监听 3030。 |
...
3. 本轮只收口 P17.4.5 验证，不新增 runtime 功能、不改 Cloud / EmberCore。
