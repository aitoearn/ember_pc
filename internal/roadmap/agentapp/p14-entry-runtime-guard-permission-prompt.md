...
上游 Agent App 标准已经把宿主最低职责明确为：`Discover → Validate → Project → Check readiness → Authorize → Inject capabilities → Isolate data → Clean up`。Ember 客户端 P0-P13 已经覆盖到 package、projection、readiness、setup、persistence、cache、loader，但还缺少独立的 `Authorize` 运行前 gate。
...
5. Cloud / EmberCore 仍只提供 catalog、release、tenant enablement、metadata；不运行 Agent、不渲染 UI、不接管本地 storage。
...
| `src/features/agent-app/index.ts` | 仅导出 P14 public test seam，不暴露 Ember internal。 |
...
| `git diff --check -- internal/roadmap/agentapp src/features/agent-app src/i18n/resources src/vite-env.d.ts` | 通过。 |
...
git diff --check -- internal/roadmap/agentapp src/features/agent-app src/i18n/resources
...
- 不新增 Tauri command。
- 不让 Cloud / EmberCore 运行默认 Agent Runtime 或渲染 App UI。
