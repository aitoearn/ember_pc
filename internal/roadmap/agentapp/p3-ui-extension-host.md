...
| 范围 | 做 | 不做 |
|---|---|---|
| UI Runtime Flag | 新增 `VITE_EMBER_AGENT_APP_UI_RUNTIME=1` 与 `uiRuntimeEnabled`。 | 不默认开启，不自动进入正式导航。 |
| Capability Profile | `buildUiRuntimeCapabilityProfile` 将 `ember.ui` 标记为 `native`，可叠加 mock 或 adapter capability。 | 不把 worker runtime 一起打开。 |
...
| SDK Bridge | 输出 injected SDK bridge descriptor，只暴露 readiness 允许的 capability。 | App 不能 import Ember internal modules。 |
...
```text
VITE_EMBER_AGENT_APP_UI_RUNTIME=1
```
...
- `buildUiRuntimeCapabilityProfile` 只把 `ember.ui` 变成 native；其他能力仍来自 mock / adapter。
- `AgentAppLabPage` 只显示 P3 实验入口，不把 App entry 写入主产品路由。

## Host Bridge 补充

P3 的原始目标是证明 App 可以拥有受控 UI surface；它只输出 sandbox policy 和 injected SDK bridge descriptor。进入正式 `agent-apps` runtime 后，必须补上 Host Bridge v1，否则 App UI 会变成孤立 iframe，无法跟随 Ember 主题，也无法把导航、提示、下载和 capability 调用纳入 Host policy。
