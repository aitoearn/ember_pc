...
| 范围 | 做 | 不做 |
|---|---|---|
| SDK facade | 定义 `LimeAppSdk`、storage、artifact、evidence capability 类型。 | 不暴露 Ember internal path。 |
...
```text
VITE_EMBER_AGENT_APP_MOCK_SDK=1
```
...
4. P1 mock host 不 import Ember 主业务 store，只依赖 Agent App projection / readiness / cleanup plan。
