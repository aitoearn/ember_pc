...
P15-H 把 P15 的 Lab-only 安装启动闭环从“组件和单测可用”推进到“真实 GUI 可演示、失败可定位、清理可复演”。它仍然不把 Agent App 接入正式主导航，也不把 EmberCore / Cloud 变成 runtime。

## 为什么需要 P15-H

P15 已经串起 `source → install review → verify/cache → installed state → setup/readiness → permission prompt → launch → cleanup preview`，但这还不能证明用户真的能在 Ember Desktop 里完整走完流程。P15-H 专门补三类证据：
...
| GUI smoke feature flags | 已落地 | `resolveAgentAppHostFlags()` 支持 `ember.agentAppHost.flags` localStorage JSON，仅服务本地 Lab / smoke。 |
...
| 证据输出 | 已落地 | `.ember/qc/gui-evidence/agent-app-lab/agent-app-lab-smoke-summary.json` 与截图。 |
...
sequenceDiagram
  ...
  participant App as Ember Desktop Web
...
| 命令 | 结果 |
|---|---|
| `npm run smoke:agent-app-lab -- --timeout-ms 180000` | 通过；summary 输出到 `.ember/qc/gui-evidence/agent-app-lab/agent-app-lab-smoke-summary.json`，截图输出到 `.ember/qc/gui-evidence/agent-app-lab/agent-app-lab-smoke.png`。 |
