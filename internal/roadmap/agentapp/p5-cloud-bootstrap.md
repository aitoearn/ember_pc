...
P5 的目标不是把 Ember Desktop 变成 Cloud 的执行器，而是让客户端在保持本地运行、可离线、可清理的前提下，消费 EmberCore 下发的 catalog / release / tenant enablement 元数据，并把它收敛到现有 `package source -> verify -> manifest -> projection -> readiness -> runtime` 链路。

## 背景

`/Users/coso/Documents/dev/ai/embercloud/agentapp` 已升级到 Agent App v0.3 的完整标准层。相对早期草案，新增的约束不只是字段更多，而是把宿主实现从“能展示一个 APP.md”推进到“能安全安装、投影、授权、运行、升级和清理一个可执行应用包”。

对 Ember Desktop 的直接影响：
...
| Typed SDK expectations：error code、cancel、retry、idempotency、trace、mock | P5 接 Cloud 前要保证 runtime call 继续穿过 Capability SDK，而不是直接调 Ember internal。 |
...
| 层 | 负责 | 不负责 |
|---|---|---|
| EmberCore / Cloud | Catalog、Release、package metadata、tenant enablement、license、ToolHub availability、policy defaults、audit metadata。 | 不运行 Agent、不渲染 UI、不执行 worker、不管理本地 storage。 |
| Ember Desktop | 下载、校验、缓存、manifest 解析、projection、readiness、authorization、SDK 注入、UI / workflow / worker runtime、storage namespace、artifact / evidence、cleanup。 | 不实现 Cloud catalog 管理台、不保存租户发布状态事实源、不把 App 写进 Ember Core。 |
| Agent App package | UI bundle、workflow、worker、storage schema、业务代码、entries、permissions、overlay templates。 | 不 import Ember internal、不携带客户数据、不携带明文 secret。 |
...
flowchart TD
  ...
  EmberCore[EmberCore Bootstrap Payload] --> Payload[CloudBootstrapPayload]
...
```mermaid
sequenceDiagram
  participant Cloud as EmberCore
  participant Client as Ember Desktop
...
```mermaid
flowchart TD
  Start[启动 Ember Desktop] --> HasCache{本地已有 installed package?}
...
- 记录 Ember Desktop 当前 projection / readiness 和 reference CLI 的差异。
- 对差异分类：必须实现、可降级、标准候选反馈、暂缓。

验收：

```bash
npm --prefix /Users/coso/Documents/dev/ai/embercloud/agentapp run cli -- validate docs/examples/content-factory-app
npm --prefix /Users/coso/Documents/dev/ai/embercloud/agentapp run cli -- project docs/examples/content-factory-app
npm --prefix /Users/coso/Documents/dev/ai/embercloud/agentapp run cli -- readiness docs/examples/content-factory-app
```
...
P5.5 当前完成 reference CLI cross-check 和差异分类，不把 reference CLI 输出直接当 Ember runtime 实现。

| 检查项 | Reference CLI | Ember Desktop 当前 | 分类 |
|---|---|---|---|
...
1. EmberCore payload 返回 `content-factory-app@0.3.0`、`registrationRequired=true`、`registrationState`、tenant policy defaults。
