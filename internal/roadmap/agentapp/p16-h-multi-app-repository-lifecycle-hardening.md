...
| reference | `/Users/coso/Documents/dev/ai/embercloud/embercore/internal/roadmap/agentapp` | Cloud / EmberCore 只提供 catalog、release、tenant metadata，不运行 Agent App。 |
...
- `Residual Audit` 只检查 Agent App namespace，不扫描或删除 Ember 主业务数据。
...
| P16-H.4 | 已完成最小实现：residual audit：检查 Agent App namespace 内 cache / storage / artifact / evidence / setup 残留。 | 不扫描 Ember 主业务目录。 |
...
| `internal/roadmap/agentapp/*` | 记录 P16-H 验证证据和正式入口 gate 结论。 |
...
git diff --check -- internal/roadmap/agentapp src/features/agent-app scripts/agent-app/lab-smoke.mjs package.json src/i18n/resources
...
5. Cloud / EmberCore 仍只提供 catalog / release / tenant metadata，不运行 Agent、不渲染 UI、不接管本地 storage。
