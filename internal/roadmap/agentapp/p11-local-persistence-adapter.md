...
1. 重启 Ember Desktop 后无法恢复已安装 App。
...
```mermaid
sequenceDiagram
  participant App as Ember Desktop Startup
  participant Repo as InstalledStateRepository
  participant File as Local Persistence Adapter
...
如果需要真实文件系统能力，优先复用 Ember 现有客户端存储封装；不能在 Agent App 内直接 `safeInvoke` / `invoke`，也不能新增 Agent App 专属 Tauri command。
...
| 当前 localStorage driver 面向 Lab persistence；真实文件 driver 仍需通过 Ember 平台存储封装注入。 | 不在 Agent App 内新增 Tauri command 或直接 `safeInvoke`。 |
