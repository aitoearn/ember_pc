...
| SDK 边界 | 只通过 `CapabilityHost` / `LimeAppSdk` 调用 storage、knowledge、agent、artifacts、evidence。 | 不 import Ember 内部 store、AgentChat 或 Artifact 主 schema。 |
| Storage | 写入 `projects/*`、`knowledge-bindings/*`、`content_scenarios/*`、`content-assets/*`。 | 不新增 Ember Core 业务表。 |
...
flowchart TD
  ...
  Host --> SDK[LimeAppSdk]
  SDK --> Storage[ember.storage namespace]
  SDK --> Knowledge[ember.knowledge search]
  SDK --> Agent[ember.agent task]
  SDK --> Artifact[ember.artifacts.create]
  SDK --> Evidence[ember.evidence.record]
...
1. 内容工厂只是 Agent App 平台验证样板，不进入 Ember Core 垂直功能。
