...
P2 在 P1 mock host 之后增加一个仍默认关闭的 adapter host：不执行 App UI / worker、不改 AgentChat 主流程，只把 `ember.storage`、`ember.artifacts`、`ember.evidence` 从硬编码 mock 行为推进到可替换的本地 adapter store，并证明产物可以按 `agent_app` provenance 查询和卸载清理。
...
| Knowledge Adapter | `ember.knowledge.search` 从 App knowledge bindings 中只读解析候选资料。 | 不访问真实项目知识库、不做网络检索。 |
| Agent Adapter | `ember.agent.startTask / cancelTask / getTask` 记录本地任务 trace。 | 不修改 AgentChat 主流程、不提交真实模型 turn。 |
...
| Feature Flag | 新增 `VITE_EMBER_AGENT_APP_REAL_ADAPTER=1` 与 `realAdapterEnabled`。 | 默认不启用，不影响主产品路径。 |
...
```text
VITE_EMBER_AGENT_APP_REAL_ADAPTER=1
```
