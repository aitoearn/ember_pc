...
P17.0 不发布 marketplace，也不扩展 Cloud 管理台；它只把 Ember Desktop 现有 `agent-apps` 受控入口定成 current 主链：用户在这里安装、注册、启动、禁用、卸载演练和进入 runtime；`agent-app-lab` 继续作为研发验证入口，负责 fixture、smoke、flag-off 和实验断言。

## 背景

P16-H 已完成多 App repository、selected launcher、持久化 lifecycle、cleanup evidence、residual audit、Agent App Lab GUI smoke、flag-off 回归和 P17 Gate 审计。结合 `/Users/coso/Documents/dev/ai/embercloud/agentapp` v0.3 标准，客户端下一步不能继续停留在 Lab，也不能跳到公开市场，而是要先固定正式入口契约。

这次计划更新的关键判断：

1. Agent App 是完整可安装应用，不是专家卡片、prompt 包或 Markdown 目录。
2. Ember Desktop 是运行宿主，负责安装、cache、projection、readiness、SDK 注入、UI / workflow runtime、storage namespace 和 cleanup。
3. Ember Cloud / EmberCore 只负责 catalog、release、license、tenant enablement、gateway 和 ToolHub metadata，不运行默认 Agent、不渲染 App UI、不接管本地 storage。
...
| 明确能力边界 | App 只能通过 Capability SDK 使用 Ember 能力，不能 import Ember 内部模块或直接调用 legacy desktop facade。 |
...
7. 不允许 Agent App 绕过 Capability SDK 直接使用 Ember internal store、`safeInvoke` 或 `invoke`。
...
| reference | `/Users/coso/Documents/dev/ai/embercloud/agentapp` | Agent App v0.3 标准事实源。 |
| reference | `/Users/coso/Documents/dev/ai/embercloud/embercore/internal/roadmap/agentapp` | 服务端 / Cloud 控制面路线图。 |
...
6. 如触碰 Electron Desktop Host IPC / App Server JSON-RPC / legacy desktop facade 命令边界，必须同步前端 gateway、治理目录册、mock priority / default mocks，并执行 `npm run test:contracts`；不得在 `ember-rs/src/commands/**` 新增业务逻辑。
