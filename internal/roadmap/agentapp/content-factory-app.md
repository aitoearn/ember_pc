...
基于 Ember Agent App 平台开发一个独立垂直产品：`内容工厂`。它不是“内容专家聊天框”，而是有自己 UI、storage、worker、workflow、Knowledge 绑定、Artifact 和数据复盘闭环的完整业务应用，安装到 Ember Desktop 后本地运行。

验收心智：运营用户应在内容工厂内完成资料整理、场景规划、批量生产、交付和复盘；Ember Agent 在背后作为 `ember.agent` / `ember.workflow` 能力执行任务。用户不应为了关键步骤跳回 Ember 通用 Chat，也不允许内容工厂自建一套模型、凭证、权限、证据和工具底座来绕过 Ember。
...
`APP.md` 只声明这些能力和入口；真实功能在 runtime package 中实现，并通过 `@ember/app-sdk` 调用 Ember 能力。

内容工厂内的 Expert 只是协作者入口：它可以读取当前项目、资料版本、内容批次和复盘指标，也可以触发 App workflow；但它不能成为“用户把结果从聊天框复制回内容表”的旁路。

后端事实源：内容工厂的 AI 任务必须通过 `internal/roadmap/agentruntime/app-surface-runtime.md` 定义的 Agent App Runtime Surface 进入 AgentRuntime 主链。`EMBER_GATEWAY_*` / 模型 API 只能作为低阶 executor 或降级能力，不能替代 Ember Agent 的工具、知识、权限、证据和 artifact 治理。

当前实现进展：Ember Host 的 `AgentAppRuntimePage` 已把 iframe Host Bridge 里的 `ember.agent.startTask / streamTask / getTask / cancelTask / retryTask / submitHostResponse` 接入 `agent_app_runtime_*` facade，并由后端复用 `AgentRuntimeThreadReadModel` 投影 `taskStatus / taskEvents`。这意味着内容工厂页面内发起“生成文案 / 场景规划”等 Agent task 时，第一跳已经进入完整 Ember AgentRuntime 主链；当 runtime 追问上下文、等待确认或要求工具授权时，App 也能在页面内回传响应，而不是跳回通用 Chat。Host Bridge / `AgentRuntimeCapabilityHost` 负责把 Claw 式过程进一步封装为标准 `runtimeProcess` / `process`：包含思考、正文流、执行流、Skill、工具、模型、Token、费用、产物、证据、终态与默认折叠策略。内容工厂只消费该视图做业务展示，不再维护自己的底层运行过程解析器。当前剩余缺口是更多真实多 capability 编排、后端/cross-surface capability policy owner，以及重启宿主后的端到端业务流覆盖。

新增硬验收：内容工厂不是“本地 Web + 生成 API”。知识库整理、场景地图、内容战役、脚本 / 图片提示词、交付包和复盘只要触发 AI 业务动作，就必须构造 `ContentFactoryAgentTask` 并通过 `ember.agent` 进入 AgentRuntime。页面必须展示 AI 运行现场：思考、执行、流式输出、Skill、工具、模型、Token、费用、artifact、evidence；完成后可以折叠，但过程不能消失。Host 已连接时，`/api/scene/generate`、`/api/copy/batch`、`/api/script/generate`、`/api/strategy/analyze`、`/api/review/submit` 等本地生成接口只能作为开发 fallback，不能承接正式生成。
...
这不是 Ember Core 的垂直功能，而是验证 Agent App 平台能力的标杆 App。
...
| 交付顾问       | 为客户交付可运行系统，而不是复制 prompt 或 fork Ember。                             | 交付可复制，官方 App 可升级，客户数据不被覆盖。 |
| 团队管理员     | 控制 App 可见性、模型、工具、默认知识模板、质量门禁。                              | 团队能力标准化、权限可控。                      |
| Ember 平台      | 验证 App SDK、UI extension、storage namespace、worker runtime、artifact/evidence。 | 新行业通过 App 扩展，不污染 Core。              |

## 非目标

- 不把行业内容系统写进 Ember Core。
- 不把客户真实资料、私有 SOP、品牌语气、投放数据打进官方 App 包。
- 不在 Ember Cloud 增加默认云端 Agent Runtime。
...
requires:
  ...
  capabilities:
    ...
    ember.files: "^0.3.0"
    ember.agent: "^0.3.0"
    ember.knowledge: "^0.3.0"
    ember.tools: "^0.3.0"
    ember.artifacts: "^0.3.0"
    ember.workflow: "^0.3.0"
    ember.evidence: "^0.3.0"
runtimePackage:
  ui:
    path: ./dist/ui
  worker:
...
```mermaid
flowchart TD
  Upload[上传 PDF / Word / Excel] --> Parse[ember.files + ember.tools 解析]
  Parse --> Extract[ember.agent 结构化抽取]
  Extract --> Review[人工确认]
  Review --> Store[ember.storage 写入 knowledge_items]
  Store --> Bind[ember.knowledge 绑定 / 导出]
  Bind --> Evidence[ember.evidence 记录来源和版本]
```
...
```mermaid
flowchart TD
  Select[选择项目 / 内容场景 / 平台 / 数量] --> Retrieve[ember.knowledge 检索]
  Retrieve --> Generate[ember.agent 批量生成]
  Generate --> Score[去 AI 味 + 事实支撑 Eval]
  Score --> Grade[A/B/C 分级]
  Grade --> Save[ember.storage 保存内容资产]
  Save --> Artifact[ember.artifacts 生成内容表]
```
...
全量 Ember 能力抽象、App / Ember 主 App 边界和后端接线顺序见 [P18.7 Full Ember Capability Surface](./p18-7-full-ember-capability-surface.md)。内容工厂只作为第一个业务验证 App，不拥有底层 AgentRuntime / ToolRuntime 能力事实源。

内容工厂只实现内容业务层，不实现 Ember 底层 AI 平台层。所有 Ember 功能先看 `internal/roadmap/agentapp/capability-sdk.md` 的全量能力地图；内容工厂 MVP 消费其中一部分，后续扩展也必须继续走 `ember.*` capability，而不是新增 `content_factory_*` 专用后端命令或裸模型 API。

| 业务功能                                | 主要 Capability                                               | 说明                                                                                           |
| --------------------------------------- | ------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| 文件上传和解析                          | `ember.files`、`ember.documents`、`ember.tools`                  | App 选择资料类型和业务落点；Ember 负责 file ref、解析器、工具执行和证据。                       |
| 三层知识库管理                          | `ember.storage`、`ember.knowledge`、`ember.evidence`             | App 管 IP / 项目 / 素材三层业务模型；Ember 管 namespace、检索、binding、版本和来源。            |
| 结构化抽取 / 场景 / 文案 / 脚本         | `ember.agent`、`ember.skills`、`ember.models`、`ember.memory`     | App 组织 task input 和 expected output；AgentRuntime 负责模型、Skill、上下文、记忆和运行事实。 |
| 批量任务、进度、中断                    | `ember.workflow`、`ember.tasks`、`ember.events`                  | App 展示阶段和人工确认；Ember 托管 checkpoint、任务中心、事件订阅和恢复。                       |
| 运行过程、思考、工具、模型、Token、费用 | `ember.agent`、`ember.usage`、`ember.capabilities`               | App 只消费 Host 下发的 `runtimeProcess`；模型、用量和可用性来自 Ember。                         |
| 内容表、报告、PPT、交付物               | `ember.artifacts`、`ember.documents`、`ember.media`              | App 定义交付结构；Ember 负责 artifact、导出、图片/音频/视频/文档 runtime。                      |
| 外部调研和网页采集                      | `ember.search`、`ember.browser`、`ember.mcp`、`ember.connectors`  | App 给出调研问题和筛选规则；Ember 负责搜索、浏览器、MCP、连接器和审计。                         |
| 凭证、权限、成本、风险、审核            | `ember.secrets`、`ember.policy`、`ember.settings`、`ember.review` | App 解释用途并处理降级；Ember 托管 secret、策略、设置 overlay 和发布/风险审核。                 |
| 产物证据和复盘                          | `ember.evidence`、`ember.usage`、`ember.tasks`                   | App 展示来源和复盘指标；Ember 从 runtime facts / telemetry 归因。                               |

边界结论：内容工厂可以有自己的多页面、多工作流和业务数据库视图，但模型路由、Skill 调用、MCP、浏览器、搜索、媒体、终端、凭证、Token 费用、Evidence Pack 和运行过程都必须由 Ember 主 App capability 提供。

## App 内 Agent 任务闭环

内容工厂的 Agent 能力必须作为业务任务嵌入 App，而不是跳转到通用 Chat：

| 业务任务        | App 输入                             | Ember Agent / Capability                                                                         | App 内写回                                                  |
| --------------- | ------------------------------------ | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| 资料整理        | 用户授权文件、资料类型、项目目标。   | `ember.files` / `ember.tools` 解析，`ember.agent.startTask` 结构化抽取，`ember.evidence` 记录来源。 | `knowledge_items` 草稿、资料健康度、人工确认状态。          |
| 场景规划        | 已确认知识库、目标平台、人群和数量。 | `ember.knowledge.search` 检索，`ember.agent.startTask` 生成结构化场景。                           | `content_scenarios` 表、场景 Artifact、引用证据。           |
| 批量文案 / 脚本 | 场景、平台规则、数量、风格约束。     | `ember.agent.startTask` + writer skills，Eval 检查事实支撑和去 AI 味。                           | `content_assets` / `script_batches`、A/B/C 分级、质量记录。 |
| 策略交付        | 内容批次、项目事实、竞品资料。       | `ember.agent.startTask` 生成报告结构，`ember.artifacts` 持久化交付物。                            | 策略报告、PPT 大纲、交付状态。                              |
| 数据复盘        | CSV / Excel 指标、历史内容资产。     | `ember.tools` 归一化，`ember.agent.startTask` 归因分析。                                          | `review_reports`、下轮动作、选题权重更新。                  |

每个任务都必须在内容工厂页面内展示进度、引用、工具调用、失败原因、重试和人工确认；最终结果以结构化对象写回 App storage，再生成 Artifact / Evidence。

当前 App Host 第一刀已具备：Host Bridge 的 `ember.agent` 调用会经 `AgentRuntimeCapabilityHost` 转入 `agent_app_runtime_start_task`，`streamTask / getTask` 先消费 `agent_app_runtime_get_task` 的 snapshot events，`cancelTask` 委托 `agent_app_runtime_cancel_task`，`retryTask` 复用同一 runtime session 重新提交 App task，`submitHostResponse` 委托 `agent_app_runtime_submit_host_response`。前端本地 adapter 继续只作为 storage / artifact / evidence / knowledge 的样板实现，不能再被用来证明内容工厂已经具备完整 Agent 能力。
...
```text
写文案表单状态
  -> ember.agent.startTask / ember.workflow.start
...
| --------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------- |
| 生产前资料补齐        | `ember.capability.research.search` / `ember.capability.summary.generate`       | `research_skill_launch` / `summary_skill_launch`     |
| 竞品和策略分析        | `ember.capability.report.generate`                                            | `report_skill_launch`                                |
| PDF / 资料读取        | `ember.capability.pdf.read`                                                   | `pdf_read_skill_launch`                              |
| 配套素材 / 海报 brief | `ember.capability.image.generate` / `ember.capability.cover.generate`          | `image_skill_launch` / `cover_skill_launch`          |
| 交付 PPT / 报告       | `ember.capability.presentation.generate` / `ember.capability.webpage.generate` | `presentation_skill_launch` / `webpage_skill_launch` |
...
| 场景                  | Bridge 事件                     | 验收                                                                                          |
| --------------------- | ------------------------------- | --------------------------------------------------------------------------------------------- |
| 跟随 Ember 主题 / 配色 | `host:snapshot`、`theme:update` | 主 App 切换主题后，内容工厂 iframe 内字体、颜色、背景和选中态同步变化。                       |
| 非技术提示            | `host:toast`                    | App 内操作结果用 Ember Host 提示，不暴露 Gateway、Artifact、RAG 等技术词。                     |
...
内容工厂 App 内部只保留主题 fallback，方便脱离 Ember Host 做本地预览；真实嵌入 Ember 时，主题、语言、入口上下文和 Host actions 都以 `ember.agentApp.bridge` 快照为准。
...
| 阶段             | 目标                                                      | 验收                                                   |
| ---------------- | --------------------------------------------------------- | ------------------------------------------------------ |
| P0 App Host 验证 | 用 mock SDK 跑通 App UI + storage + workflow + artifact。 | 不修改 Ember Core 业务代码即可打开 App 页面和保存数据。 |
...
| 检查                       | 级别               | 说明                                       |
| -------------------------- | ------------------ | ------------------------------------------ |
| `ember.storage` 缺失        | blocking           | App 无法保存项目和内容资产。               |
| `ember.files` 缺失          | blocking           | 无法上传 / 解析资料。                      |
...
- App 不作为 Ember Core 垂直功能出现；业务 UI 来自 App runtime package。
...
| 编号  | 用户故事                                                                 | 验收标准                                                                |
| ----- | ------------------------------------------------------------------------ | ----------------------------------------------------------------------- |
| US-01 | 作为运营，我可以创建一个内容项目，并选择行业、目标平台和是否需要 IP 库。 | `projects` 写入 App storage namespace；不新增 Ember Core 业务表。        |
| US-02 | 作为运营，我可以上传产品资料，系统生成三层知识库草稿。                   | 文件通过 `ember.files` 授权读取；结构化结果进入 `knowledge_items`。      |
...
| US-07 | 作为管理员，我可以看到缺失能力或缺失知识绑定的 readiness 提示。          | 缺少 `project_knowledge`、`ember.files` 或 writer skill 时不能静默运行。 |
...
| 责任               | 放在 App 实现 | 放在 Ember 平台                                     |
| ------------------ | ------------- | -------------------------------------------------- |
| 三层知识库 UI      | 是            | 提供 UI host 和 storage capability。               |
| 文案生成业务规则   | 是            | 提供 Agent Runtime、Skills 调用、Evidence。        |
| 去 AI 味规则库     | 是            | 提供 Eval 执行和结果记录。                         |
| 文件权限与解析入口 | 编排          | `ember.files` 和 `ember.tools` 提供授权与工具执行。  |
| 数据表物理存储     | 否            | `ember.storage` 管 namespace、CRUD、migration。     |
| Artifact 持久化    | 定义内容      | `ember.artifacts` 管创建、打开、导出和 provenance。 |
| 凭证               | 否            | `ember.secrets` 托管。                              |
...
| 页面         | 必备组件                                             | Capability                                      |
| ------------ | ---------------------------------------------------- | ----------------------------------------------- |
| 项目首页     | 项目列表、知识健康度、最近 Artifact、快捷操作。      | `ember.storage`、`ember.artifacts`                |
| 三层知识库   | IP / 项目 / 素材 tabs、分区表、字数警告、来源查看。  | `ember.files`、`ember.storage`、`ember.knowledge`  |
| 内容场景规划 | 内容场景表、维度筛选、人工编辑、批量保存。           | `ember.agent`、`ember.storage`                    |
| 内容工厂     | 平台选择、数量、内容场景选择、批量结果、评分与分级。 | `ember.agent`、`ember.artifacts`、`ember.evidence` |
| 交付物       | 内容表、报告、导出、版本历史。                       | `ember.artifacts`、`ember.evidence`               |
