...
状态：P18.0 文档计划已完成；P18.1 SDK facade / stable error / mock host 已完成最小代码契约；P18.2 Host Bridge typed router / stable error response 已完成；P18.3 Core capability adapters 已完成；P18.4 App-scoped Agent task SDK facade 已完成；P18.4-H AgentRuntime handoff gate 已完成；P18.5.1 Ember-side 内容工厂 SDK 回归已完成；P18.5.2 package-side read-only tests 已通过；P18.5-S Host Bridge SDK client 已完成 capability invoke、subscription、Host action 和 Host event contract；P18.5.3 package-side SDK facade / verify / dist 同步已完成；P18.6 Raw Worker 前 Gate 已完成；P17.5 Formal entry GUI smoke 已通过；完整 `verify:local` 已于 2026-05-16 07:33 通过，2026-05-16 10:53 SDK seam / handoff core 定向测试 5 files / 17 tests passed，`typecheck`、`test:contracts` 与 `lint` 当前会话复核通过；P18 功能完成，剩余 owner handoff / git 写集收口。

## P18.7 重新打开原因

2026-05-16 追加：用户要求“将 Ember 所有功能都抽象出来”，因此 P18 不再只以 typed SDK gate 完成为终点。新的 current 路线图是 [P18.7 Full Ember Capability Surface](./p18-7-full-ember-capability-surface.md)：先把全量 `ember.*` 能力、App / Ember 主 App 边界、v0.6 标准兼容、Host discovery 与后端接线顺序落成文档事实源，再继续代码。原 P18.1-P18.6 仍是已完成基础层，不代表整体目标完成。
...
| P18 / 完成候选          | 上游 Agent App v0.4 Host Bridge 标准对齐、Typed Capability SDK gate、App-scoped Agent task 契约。P18.1 已落 SDK 类型、stable error、mock host 和 contract seam；P18.2 已收敛 Host Bridge typed router；P18.3 已完成 core adapters；P18.4 已完成 `ember.agent` SDK facade；P18.4-H 已完成 handoff gate；P18.5.1 已完成 Ember-side 内容工厂 SDK 回归；P18.5.2 已完成 package-side read-only tests；P18.5-S 已补 Host Bridge SDK client；P18.5.3 已完成真实 package verify 和 dist 同步；P18.6 已完成 raw worker 前 gate。 | 不启停隔壁 Tauri / Vite / DevBridge，不改 `ember-rs/*` / runtime facade，不改 `src/features/agent-app/runtime/agentRuntimeCapabilityHost*`，不进入 raw worker；后续只做 owner handoff / 提交边界，不再扩 P18 功能面。 |
...
| ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `ember.agent` task facade                 | `start / stream / get / cancel / retry / submitHostResponse / listTasks` 到 `agent_app_runtime_*`，并映射 `sessionId / turnId / requestId`。                                                                                                    | P18.1 已定义 SDK 类型和 mock 行为；P18.4 已包装成 typed adapter；P18.4-H 已消费隔壁运行证据。                                               | P18 不改 Rust command、不改 `src/lib/api/agentAppRuntime.ts`。                                                       |
...
| storage / artifact / evidence write-back | 内容工厂已用声明类型写回 `ember.storage / ember.artifacts / ember.evidence`，AgentRuntime 负责事实投影。                                                                                                                                           | P18.3 只包装 typed adapter，自动带 provenance。                                                                                             | 不新增第二套 artifact / evidence store，不写垂直 `content_factory_*` adapter。                                       |
...
| capability catalog                       | `internal/roadmap/agentruntime/claw-capability-sharing.md` 规定 Chat `@命令`、Agent App `ember.agent.startTask`、Automation job 都只是 surface adapter；首批 capability hints 已可映射 Claw `*_skill_launch` metadata，独立 catalog service 未完成。 | P18 先在 SDK 层定义 `ember.tools / ember.knowledge / ember.agent` 能力声明与错误语义；把 capability sharing 作为 AgentRuntime owner 输入消费。 | 不复制 Claw `*_skill_launch.rs`，不把 capability hint 内置成 App 专用逻辑，不新增 `content_factory_*` 垂直后端能力。 |
...
- `internal/roadmap/agentruntime/*` 已补充 Agent App Runtime Surface 事实：内容工厂实际 App 已把主生产结果和确认链结果写回 `ember.storage / ember.artifacts / ember.evidence`。
- `agent_app_runtime_start_task / cancel / get / submit_host_response` facade、Host Bridge `ember.agent` 适配、`artifact:created` refs 和跨刷新 task 恢复已经进入 done / first-cut 口径。
- `internal/roadmap/agentruntime/claw-capability-sharing.md` 新增 Claw capability 共享草案：Chat `@命令`、Agent App `ember.agent.startTask`、Automation job 都应作为 surface adapter 进入 AgentRuntime capability catalog；首刀已将 image / cover / research / report / pdf / summary capability hint 写入现有 Claw `*_skill_launch` metadata。
...
- P18 不接管 Claw capability catalog owner，不复制 `*_skill_launch.rs`，也不新增 `content_factory_*` 垂直后端能力；内容工厂只能通过 `ember.agent` / capability intent 消费共享主链。
...
上游 `/Users/coso/Documents/dev/ai/embercloud/agentapp` 已发布 `agentapp-ref@0.4.0`，把 Host Bridge v1 明确为 sandboxed Agent App UI 的标准运行时事件协议。P17.4 已在 Ember Desktop 内完成 runtime surface / Host Bridge 的生产硬化，P17.5 已证明正式 `agent-apps` 入口能独立跑通 install、registration、launch、disable、uninstall rehearsal、runtime surface 和 flag-off。

P18 的问题不是继续扩大页面能力，而是把已经出现的 `ember.agentApp.bridge` 固定为 App 可依赖的 Typed Capability SDK：

- App 拥有业务 UI、workflow、storage schema、结构化结果写回和人工确认。
- Ember Desktop 拥有 Host runtime、Agent Runtime、Tool Broker、Knowledge、Artifact、Evidence、Policy、Secrets、cleanup 和 provenance。
- Ember Cloud / EmberCore 只作为 catalog、release、tenant enablement、policy defaults、ToolHub / gateway metadata 的 control-plane 输入，不成为默认 Agent Runtime。
- App 不能 import Ember 内部模块，也不能绕过 SDK 自建模型网关、凭证系统、权限系统、证据系统或工具调度器。

## 一句话目标

在扩展 raw worker 或更多业务 App 前，把 `@ember/app-sdk`、`ember.agentApp.bridge` 和 Ember host services 之间的 typed contract 固定下来，让 Agent App 可以稳定调用 Ember 能力，同时不依赖内部实现。
...
1. **SDK 暴露能力，不暴露实现**：App 只依赖 `@ember/app-sdk` 类型和稳定错误码。
2. **Bridge 只做传输**：`ember.agentApp.bridge` 是跨 iframe / sandbox 的消息协议，不保存执行事实。
...
5. **结果结构化写回**：Agent task 输出进入 `ember.storage`、`ember.artifacts`、`ember.evidence`，不靠用户复制聊天文本。
...
flowchart TD
  ...
  Cloud[Ember Cloud Control Plane] --> Installer[Desktop Installer / Release Metadata]
  Installer --> Guard
```
...
| 层                    | 负责                                                                                    | 不负责                                                    |
| --------------------- | --------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| `@ember/app-sdk`       | TypeScript facade、调用参数类型、稳定错误码、mock host。                                | 不 import Ember internal path，不执行模型 / 工具。         |
...
| Ember Cloud / EmberCore | catalog、release、tenant enablement、license、policy defaults。                         | 不默认运行 Agent，不渲染 App UI，不接管本地 storage。     |
...
| ---------------- | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `ember.ui`        | `toast`、`navigate`、`openExternal`、`download`、`getSnapshot`                                     | 走 Host Bridge action；外链 / 下载必须校验协议、origin、权限。                                          |
| `ember.storage`   | `table.get`、`table.insert`、`table.update`、`table.query`                                         | 只写 App namespace；自动附加 provenance；schema 不匹配返回 stable error。                               |
| `ember.files`     | `pick`、`readRef`                                                                                  | 只接收 host file ref，不暴露 raw path。                                                                 |
| `ember.agent`     | `startTask`、`streamTask`、`getTask`、`cancelTask`、`retryTask`、`submitHostResponse`、`listTasks` | 映射到 AgentRuntime Surface；不跳通用 Chat；P18.4 已补 typed adapter，P18.4-H 已对齐 handoff 运行证据。 |
| `ember.knowledge` | `search`、`bindStatus`                                                                             | 只能访问 manifest / setup 声明的 Knowledge binding。                                                    |
| `ember.tools`     | `invoke`、`getProgress`                                                                            | 通过 Tool Broker / ToolHub；policy 和 secret handle 由 Host 管。                                        |
| `ember.artifacts` | `create`、`open`、`export`                                                                         | artifact 带 appId、entryKey、packageHash、manifestHash。                                                |
| `ember.workflow`  | `start`、`checkpoint`、`awaitHuman`                                                                | P18 仍是 typed workflow；raw worker 进入后续 gate。                                                     |
| `ember.policy`    | `check`、`requestPermission`                                                                       | UI prompt 不是最终授权；bridge 层必须强制。                                                             |
| `ember.secrets`   | `getRef`、`requestBinding`                                                                         | App 只能拿 scoped handle，不能拿 secret value。                                                         |
| `ember.evidence`  | `record`、`linkArtifact`                                                                           | 记录 task、tool、knowledge、artifact、eval 和 cleanup provenance。                                      |
...
interface LimeAgentAppBridgeMessage {
  protocol: "ember.agentApp.bridge";
  version: 1;
  type: string;
  requestId?: string;
  appId: string;
...
sequenceDiagram
  ...
  participant SDK as @ember/app-sdk
  participant Bridge as Host Bridge
  participant Host as Ember Host

  App->>SDK: getEmberRuntime()
  ...
  SDK-->>App: typed ember runtime handles
```
...
sequenceDiagram
  ...
  User->>App: 点击生成内容策略
  App->>SDK: ember.agent.startTask(request)
  SDK->>Host: capability:invoke(ember.agent.startTask)
  ...
  User->>App: 确认写回
  App->>SDK: ember.storage.update + ember.artifacts.create + ember.evidence.record
...
| App 开发者  | 我只依赖 `@ember/app-sdk` 类型和 mock host，就能本地测试 App 工作流。  | 不 import Ember internal path；mock / real host 的错误码一致。                       |
...
1. **内容工厂批量文案**：App 调 `ember.agent.startTask`，流式展示资料引用、工具调用和结构化草稿；用户确认后写入 `content_assets` 表和 `content_table` Artifact。
2. **知识绑定检查**：App 调 `ember.knowledge.bindStatus` / `search`，缺客户 Knowledge pack 时返回 `readiness_blocked` 或 `secret_required`，不落假数据。
3. **工具调用**：App 调 `ember.tools.invoke` 生成搜索或分析结果；Tool Broker 负责权限、进度和证据。
4. **受控下载**：App 调 `ember.ui.download` 导出报告；Host 校验同源 runtime asset / artifact permission 后执行。
...
| P18.3   | Core capability adapters  | 已完成：`ember.ui`、`storage`、`artifacts`、`evidence`、`knowledge`、`tools` typed adapter facade。                                                                                      | adapter contract tests、namespace / provenance tests、typecheck、contracts。                         |
| P18.4   | App-scoped Agent Task     | 已完成：`ember.agent.startTask` / stream / get / cancel / retry / submitHostResponse / listTasks typed adapter，并覆盖 `task:*`、`artifact:created`、`evidence:*` 事件契约。             | App task adapter contract tests；不回跳通用 Chat；不重复实现 runtime read model。                    |
...
| P18.5   | 内容工厂 SDK 化回归       | 已完成 Ember-side SDK regression、Host Bridge SDK client contract 和 package-side 只读 `npm test`；内容工厂 task / write-back / evidence 可由通用 facade 表达，也可穿过 Host Bridge v1。 | 下一刀等待 package owner 稳定后做 P18.5.3 package-side SDK facade / verify；不复刻内容工厂后端能力。 |
...
边界：P18.1 未修改 `ember-rs/*`、`src/lib/api/agentAppRuntime.ts`、`src/features/agent-app/runtime/agentRuntimeCapabilityHost*`、GUI smoke 脚本或 Cloud / EmberCore。
...
git diff --check -- src/features/agent-app/sdk src/features/agent-app/runtime src/features/agent-app/ui/AgentAppRuntimePage.test.tsx src/features/agent-app/index.ts internal/roadmap/agentapp
rg -n "SceneApp|contentEngineering|sceneapp_|safeInvoke|invoke\\(|new Worker|Worker\\(" src/features/agent-app || true
```

边界：P18.2 未修改 `ember-rs/*`、`src/lib/api/agentAppRuntime.ts`、GUI smoke 脚本、Cloud / EmberCore，也不抢隔壁 AgentRuntime 的 Rust / facade / GUI 验证链路；`agentRuntimeCapabilityHost*` 的并行改动由隔壁任务负责。

## P18.3 实施记录

P18.3 的最小目标是先把 core capabilities 包成 App 可调用的 typed adapter facade，而不是新增第二套 store、ToolHub 或 UI runtime。本轮落地：

- `src/features/agent-app/sdk/capabilityAdapters.ts`：新增 `createEmberCoreCapabilityAdapters`，把 `ember.ui / ember.storage / ember.artifacts / ember.evidence / ember.knowledge / ember.tools` 包成 typed facade。
...
nice -n 10 npm run typecheck -- --pretty false
nice -n 10 npm run test:contracts
git diff --check -- src/features/agent-app/sdk src/features/agent-app/index.ts internal/roadmap/agentapp
rg -n "SceneApp|contentEngineering|sceneapp_|safeInvoke|invoke\\(|new Worker|Worker\\(" src/features/agent-app || true
```

边界：P18.3 未新增真实 store、ToolHub、GUI smoke 脚本、Cloud / EmberCore，也未改 `agentRuntimeCapabilityHost*` 的实现逻辑；只对共享 `AgentAppTaskRequest` 做 additive 类型对齐。

## P18.4 实施记录

P18.4 的最小目标是把 `ember.agent` 暴露成 App 可依赖的 typed task adapter，而不是把 AgentRuntime read model、Rust facade 或 GUI smoke 验证链路搬进 SDK。本轮落地：
...
- `src/features/agent-app/sdk/capabilityAdapters.ts`：`createEmberCoreCapabilityAdapters` 新增 `agent` facade，统一走 P18.1 `LimeCapabilityInvoker` 和 P18.2 typed invoke envelope，自动带默认 provenance。
...
边界：P18.4 未修改 `ember-rs/*`、`src/lib/api/agentAppRuntime.ts`、`src/features/agent-app/runtime/agentRuntimeCapabilityHost*`、GUI smoke 脚本、DevBridge / Tauri / Vite 进程、Cloud / EmberCore；真实 runtime push subscribe、workspace patch producer、capability catalog 和桌面 GUI smoke 仍归 P18.4-H / 隔壁 AgentRuntime 收口。

## P18.4-H 实施记录

P18.4-H 的最小目标是做 Agent App 消费侧 handoff gate，而不是抢隔壁 AgentRuntime owner。本轮落地：

- `internal/roadmap/agentapp/p18-4-h-agentruntime-handoff-gate.md`：新增 prompt-to-artifact checklist，把 `ember.agent` facade、task request options、task event union、Host response、Artifact / Evidence refs、workspace patch、跨刷新恢复、no-private-bridge 和协作边界逐项映射到代码 / 测试 / 文档证据。
...
边界：P18.4-H 未修改 `ember-rs/*`、`src/lib/api/agentAppRuntime.ts`、`src/features/agent-app/runtime/agentRuntimeCapabilityHost*`、GUI smoke 脚本、DevBridge / Tauri / Vite 进程、Cloud / EmberCore。

## P18.5 实施记录

P18.5 的最小目标是证明内容工厂 App 可以只依赖 Ember 公开 SDK facade 完成 App 内业务闭环。本阶段先做不打架的 Ember-side 回归和 Host Bridge SDK client 目标收敛：

- `src/features/agent-app/sdk/contentFactorySdkRegression.test.ts`：新增内容工厂 SDK contract regression，模拟 `content.copy.generate` task、`task:missingContextRequested` Host response、`artifact:created` / `evidence:recorded` stream、`contentFactoryWorkspacePatch` payload、`ember.storage.set`、`ember.artifacts.create(content_batch)`、`ember.evidence.record(fact_grounding)`。
...
- `src/features/agent-app/sdk/index.ts` 与 `src/features/agent-app/sdk/publicSdkSurface.test.ts`：新增 SDK-only public surface，固定后续正式 `@ember/app-sdk` / package-local shim 的窄导出清单，避免从 feature 总入口误导出 UI、installer、repository 或 runtime host 内部实现。
- `src/features/agent-app/index.test.ts`：固定 feature public entry 导出 `createEmberCoreCapabilityAdapters`、`createLimeHostBridgeCapabilityInvoker`、`EMBER_AGENT_APP_BRIDGE_PROTOCOL` 与 `EMBER_AGENT_APP_BRIDGE_VERSION`，防止外部 package 后续只能依赖深路径。
...
- `internal/roadmap/agentapp/p18-5-content-factory-sdk-regression.md`：新增 P18.5 记录，说明 Ember-side regression 和 package-side read-only tests 已完成，并明确外部 `/Users/coso/Documents/dev/ai/embercloud/content-factory-app` 当前 dirty，暂不跨仓库改 package-side bridge wrapper。
- `internal/roadmap/agentapp/p18-5-3-package-sdk-migration-plan.md`：新增外部 package 迁移计划，固定推荐写集、迁移步骤、验收清单和回滚点。
- `internal/roadmap/agentapp/p18-completion-audit.md`：新增 completion audit，记录 P18.1-P18.6 证据、外部 package 阻塞、完整 `verify:local` 已通过和下一刀判定。
...
- Ember-side SDK seam 最小集：`hostBridgeClient.test.ts`、`contentFactorySdkRegression.test.ts`、`index.test.ts`，3 files / 8 tests passed。
...
- Ember `typecheck` 与 `test:contracts`：均通过。
- 外部 package 仍有 39 个 dirty / untracked 条目；`src/ui/host-bridge.js` 仍包含 `pendingRequests / buildMessage / window.parent.postMessage / requestHostBridge / capability:invoke / capability:subscribe / capability:event`，未出现 `createLimeHostBridgeCapabilityInvoker` 或 `createEmberCoreCapabilityAdapters`。

边界：P18.5.1 / P18.5.2 / P18.5-S 未修改外部 content-factory package、`ember-rs/*`、`src/lib/api/agentAppRuntime.ts`、`src/features/agent-app/runtime/agentRuntimeCapabilityHost*`、GUI smoke 脚本、DevBridge / Tauri / Vite 进程、Cloud / EmberCore；`hostBridge.ts` 仅有 timer 类型修正用于恢复 `typecheck`，`index.test.ts` 只固定 public SDK export seam。后续 P18.5.3 已在 10:55 完成真实 package verify / dist 同步；完整 `verify:local` 已于 2026-05-16 07:33 端到端通过，不再是当前阻塞。
...
- `internal/roadmap/agentapp/p18-6-raw-worker-pre-gate.md`：新增 raw worker 前 gate，映射 runtime policy、workflow runtime、runtime package loader、entry runtime guard、feature flag 和 feature-island boundary scan 证据。
...
边界：P18.6 未新增 worker runtime、未执行 raw worker、未新增 network / filesystem / secret 通道，也未修改 Cloud / EmberCore / legacy desktop facade。

## 验收标准

- App UI / workflow 只能通过 `@ember/app-sdk` / Host Bridge 调 Ember 能力。
...
- `ember.agent` / `ember.workflow` 进入 AgentRuntime Surface，不返回无 provenance 的模型文本。
...
P18 不得在 `ember-rs/src/commands/**` 新增业务逻辑。若触碰 Electron Desktop Host IPC / App Server JSON-RPC / legacy desktop facade 命令边界，必须同步前端 gateway、治理目录册和 mock priority，并执行 `npm run test:contracts`。

## 风险与应对

| 风险                                      | 影响                                        | 应对                                                       |
| ----------------------------------------- | ------------------------------------------- | ---------------------------------------------------------- |
| SDK 过厚，变成 Ember internal API 重导出。 | App 绑定内部实现，升级成本高。              | 只暴露 capability facade；禁止 internal import。           |
...
| Agent task 退化为模型 API。               | 丢失 trace、tool、knowledge、evidence。     | `ember.agent` 必须走 AgentRuntime Surface。                 |
...
- `/Users/coso/Documents/dev/ai/embercloud/content-factory-app`：仅在 package owner 稳定且确认不覆盖隔壁改动后，做 package-side SDK facade / verify。
- `internal/roadmap/agentapp/*`：同步记录内容工厂 SDK 化证据和剩余缺口。

P18.5 暂不触碰：

- `ember-rs/*`、`src/lib/api/agentAppRuntime.ts`、`src/features/agent-app/runtime/agentRuntimeCapabilityHost*`。
- GUI smoke 脚本、DevBridge / Tauri / Vite 进程、Cloud / EmberCore。
