...
P4.2 把 P4.1 的“内置 demo 编排”推进为可执行的受控 workflow runtime：App 仍不能执行 raw worker bundle 或任意 JS，只能通过 Ember Desktop 提供的白名单 DSL step 调用 Capability SDK，并产出可取消、可追溯、可清理的运行记录。
...
- 每个 step 通过 `CapabilityHost` / `LimeAppSdk` 调用能力，不 import Ember 内部模块。
...
4. 不实现完整内容工厂 SaaS。
5. 不让 Ember Cloud / EmberCore 运行默认 Agent。
...
flowchart TD
  ...
  Step --> SDK[LimeAppSdk]
  SDK --> Storage[ember.storage]
  SDK --> Knowledge[ember.knowledge]
  SDK --> Agent[ember.agent]
  SDK --> Artifact[ember.artifacts]
  SDK --> Evidence[ember.evidence]
...
| 角色 | 故事 | 验收 |
|---|---|---|
| App 开发者 | 我希望把内容工厂流程声明成 workflow step，而不是写死在 Ember 内部。 | `runContentFactoryDemo()` 可通过 `WorkflowRuntimeHost` 执行 `content_factory_demo`。 |
| Ember 平台维护者 | 我希望 workflow 只能调用 SDK 能力，不直接碰内部 store。 | runner 只依赖 `CapabilityHost` / `LimeAppSdk`。 |
...
| `src/features/agent-app/runtime/workflowRuntimeCapabilityProfile.ts` | 在 `workerRuntimeEnabled=true` 时把 `ember.workflow` 标记为 native。 |
...
如果路线失败，关闭 `VITE_EMBER_AGENT_APP_WORKFLOW_RUNTIME` / `workerRuntimeEnabled` 即可让 workflow runner 失效；delete-data 仍由 `AdapterCapabilityHost.uninstall()` 清理 storage、Artifact、Evidence、Task。
