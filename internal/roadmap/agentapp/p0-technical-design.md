...
P0 如果能全部在前端完成，就不增加 Tauri command。只有本地 package hash、文件枚举或 app data 路径必须走系统能力时，才增加 `ember-rs/src/agent_app/`。
...
2. 从 entry 收集入口级能力。
3. 从 `storage` 自动推导 `ember.storage`。
4. 从 `runtimePackage.ui` 自动推导 `ember.ui`。
5. 从 `runtimePackage.worker` / workflow 自动推导 `ember.workflow` 或 `ember.agent`，但 P0 只标记，不运行。
...
const p0HostCapabilityProfile: HostCapabilityProfile = {
  ...
  capabilities: {
    'ember.ui': { version: '0.3.0', enabled: false, implementation: 'none' },
    'ember.storage': { version: '0.3.0', enabled: false, implementation: 'none' },
    'ember.agent': { version: '0.3.0', enabled: false, implementation: 'none' },
    'ember.knowledge': { version: '0.3.0', enabled: false, implementation: 'none' },
    'ember.artifacts': { version: '0.3.0', enabled: false, implementation: 'none' },
    'ember.evidence': { version: '0.3.0', enabled: false, implementation: 'none' }
  },
...
| derive storage capability | 有 storage 声明 | requiredCapabilities 包含 `ember.storage`。 |
