...
状态：package-side SDK facade / verify 已完成，待 owner handoff；外部 `/Users/coso/Documents/dev/ai/embercloud/content-factory-app` 的 source-side SDK facade marker 已接入，私有 bridge marker 已清零，真实 `npm run verify` 已通过，`dist/*` 已同步 SDK facade；该仓库仍处于大量 dirty / untracked 状态，需 owner 接收写集。

## 一句话结论

P18.5.3 的目标不是重写内容工厂业务，也不是把 Ember repo 的测试当成外部 package 完成证据；目标是把外部 `content-factory-app/src/ui/host-bridge.js` 从手写 Host Bridge transport 收敛为标准 SDK facade consumer，让业务 helper 继续存在，但底层只调用 `createLimeHostBridgeCapabilityInvoker + createEmberCoreCapabilityAdapters`。截至 2026-05-16 10:55，source-side facade blocker 已解除，真实 package verify 已通过，`dist/*` 已同步；剩余风险转为 owner handoff / git 写集收口。
...
```text
/Users/coso/Documents/dev/ai/embercloud/content-factory-app
```
...
| `invokeCapability(capability, method, input)` | App 自己构造 `capability:invoke` payload。 | App 业务 helper 调用 `ember.agent / ember.storage / ember.artifacts / ember.evidence` facade。 |
| `startHostTask / streamHostTask / getHostTask / submitHostResponse` | 对 `ember.agent` 的私有 wrapper。 | 保留业务语义，但内部改为 `ember.agent.*`。 |
| `writeHostTaskResult / syncHostConfirmation` | 内容工厂业务 helper。 | 可以保留；内部必须只调用 `ember.storage / ember.artifacts / ember.evidence / ember.agent`。 |
...
```bash
git -C /Users/coso/Documents/dev/ai/embercloud/content-factory-app status --short
rg -n "pendingRequests|requestHostBridge|buildMessage|postMessage|createLimeHostBridgeCapabilityInvoker|createEmberCoreCapabilityAdapters|capability:invoke|capability:subscribe|capability:event" /Users/coso/Documents/dev/ai/embercloud/content-factory-app/src/ui/host-bridge.js
```
...
| 是否已接入标准 SDK client | `src/ui/host-bridge.js` 未出现 `createLimeHostBridgeCapabilityInvoker` 或 `createEmberCoreCapabilityAdapters`。 | 未接入。 |
...
当前业务 helper
  ...
  -> ember.agent / ember.storage / ember.artifacts / ember.evidence facade
...
|---|---|---|
| `/Users/coso/Documents/dev/ai/embercloud/agentapp` | 工作区干净，当前 `HEAD=4bef605 fix: quote mermaid sdk labels`；标准文档已扩展到 zh / en 的 authoring、client implementation、reference、examples、what-is-agent-app、agent-app-vs-skills-knowledge 与 mini-program analogy。 | 只作为标准事实源输入；不修改标准仓库，不把标准文档复制进 Ember 客户端计划。 |
| `/Users/coso/Documents/dev/ai/embercloud/content-factory-app` | 仍有 `APP.md`、`README.md`、`dist/*`、`src/ui/*`、`tests/*`、docs、model-generation 等大量 dirty / untracked。 | owner 未稳定，继续不改源码、不改 `dist/*`、不跑会 build 的 verify。 |
| `src/ui/host-bridge.js` | 当时包含 `pendingRequests`、`window.parent.postMessage`、`requestHostBridge`、`capability:invoke`、`capability:subscribe` 等手写 transport 标记，未出现 `createLimeHostBridgeCapabilityInvoker` 或 `createEmberCoreCapabilityAdapters`。 | 当时需替换 transport seam，不重写业务 UI；该项已在 10:55 收口。 |
...
| 检查项 | 最新事实 | P18.5.3 判定 |
|---|---|---|
| 标准仓库 | `/Users/coso/Documents/dev/ai/embercloud/agentapp` 工作区干净，`HEAD=4bef605`。 | 标准事实源稳定；无需改标准仓库。 |
| 外部 package 状态 | `/Users/coso/Documents/dev/ai/embercloud/content-factory-app` 仍有 39 个 dirty / untracked 条目，其中 36 个 tracked modified、3 个 untracked。 | owner 仍未稳定；不接管外部源码、`dist/*` 或 package verify。 |
| Host Bridge SDK marker | `src/ui/host-bridge.js` 仍包含 `pendingRequests`、`window.parent.postMessage`、`requestHostBridge`、`capability:invoke`、`capability:subscribe`、`capability:event`，未出现 `createLimeHostBridgeCapabilityInvoker` 或 `createEmberCoreCapabilityAdapters`。 | 仍是手写 transport；SDK facade 尚未完成。 |
...
```bash
node scripts/agent-app/package-handoff-check.mjs --package-dir /Users/coso/Documents/dev/ai/embercloud/content-factory-app
```
...
hostBridgeSdk=createLimeHostBridgeCapabilityInvoker:2, createEmberCoreCapabilityAdapters:2
uiTestPrivate=none
...
| 检查项 | 当前事实 | P18.5.3 判定 |
|---|---|---|
| Source-side SDK facade | `src/ui/host-bridge.js` 已出现 `createLimeHostBridgeCapabilityInvoker` 与 `createEmberCoreCapabilityAdapters` marker。 | 迁移实现 blocker 已解除。 |
...
| 当前协作动作 | 只更新 Ember 客户端 roadmap，不改外部 package，不启停外部 dev 进程，不重建 `dist/*`。 | 符合“隔壁有任务在跑”的协作边界。 |
...
| missing-dist | `src/ui/ember-app-sdk.js` | `dist/ui/ember-app-sdk.js` | package-local SDK shim 尚未出现在 dist。 |
...
```text
/tmp/embercloud-content-factory-verify.uots6S/content-factory-app
```
...
```bash
cd /Users/coso/Documents/dev/ai/embercloud/content-factory-app
nice -n 10 npm run verify
```
...
| 当前测试面 | 可保留 / 需调整 | 迁移目标 |
|---|---|---|
| `UI接入 Ember Agent App Host Bridge 并跟随宿主主题` | 需调整。当前直接断言 `bridge` 文本包含 `ember.agentApp.bridge`、`app:ready`、`host:getSnapshot`、`capability:invoke`、`capability:subscribe`、`capability:event` 等 transport 字符串。 | 改为断言 package 通过 SDK-style runtime handle 暴露 ready / snapshot / theme / host action / subscription 能力，不鼓励业务文件手写 message type。 |
| `主生产流程在当前页面发起 Ember AI Agent 任务并保留本地兜底` | 大部分可保留。它验证 `runHostAgentTask`、`writeHostTaskResult`、progress event、`content_factory.*` taskKind 与本地 fallback。 | 保留业务 helper 和 App 内闭环断言；只把底层 `bridge` transport 字符串断言替换为 facade 调用断言。 |
...
| `Host Bridge 能在确认链后完成任务、读取事件并写回项目资产` | 可保留行为，需调整 mock。当前 `installHostBridgeWindow()` 直接模拟 `postMessage` 和 `host:response`。 | 改为模拟 SDK invoker / host bridge client，仍断言 capability call sequence：`ember.agent.startTask -> streamTask -> getTask -> submitHostResponse -> storage.set -> artifacts.create -> evidence.record`。 |
| `Host Bridge 能把主生产任务结果写回声明过的资产和证据类型` | 可保留行为，需调整 mock。当前通过 `env.posts` 读取 `capability:invoke` message。 | 改为读取 SDK invoker call log，继续断言 `ember.storage.set -> ember.artifacts.create -> ember.evidence.record`。 |
...
2026-05-16 07:17 再次只读复核 `tests/ui.test.mjs`，当前 Host Bridge 测试仍覆盖 5 个关键用例：主题 / snapshot 接入、manifest 声明资产与证据类型、确认链写回、主生产任务写回、workspace patch 物化；其中仍直接过滤 `message.type === 'capability:invoke'`、模拟 `postMessage`，并构造 `protocol: 'ember.agentApp.bridge'`。因此后续测试迁移仍应把 mock 从私有 message transport 改成 SDK invoker / Host Bridge client call log，而不是删除这些业务断言。
...
| `runHostAgentTask(input, options)` | 主生产流程先启动 Ember Agent task，再按 stream / snapshot / Host response 驱动当前页面。 | 保留业务语义；内部改为 `ember.agent.startTask / streamTask / getTask / submitHostResponse` 与 SDK subscription。 |
| `writeHostTaskResult(...)` | 主生产任务结果写回 storage / artifact / evidence。 | 保留业务语义；内部只调用 `ember.storage / ember.artifacts / ember.evidence` facade。 |
| `syncHostConfirmation(...)` | 确认链后同步项目资产、确认状态和 evidence。 | 保留业务语义；内部复用 `runHostAgentTask` 与 typed facade 写回。 |

`startHostTask / subscribeHostTask / unsubscribeHostTask / streamHostTask / getHostTask / cancelHostTask / retryHostTask / submitHostResponse` 当前只在 `host-bridge.js` 内部或 `window.emberAgentAppBridge` 调试面暴露；P18.5.3 可以继续导出它们作为兼容薄 wrapper，但不能让它们继续维护私有 request lifecycle。
...
|---|---|---|---|
| `initHostBridge()` | 注册 `message` listener，手写 `app:ready`、`host:getSnapshot`，并把调试对象挂到 `window.emberAgentAppBridge`。 | 创建 SDK Host Bridge client / adapters，注册 snapshot、theme、visibility、capability event handler；调试对象只暴露 SDK facade handle。 | 保留。 |
...
| `startHostTask(input)` | `invokeCapability('ember.agent', 'startTask', input)`。 | `ember.agent.startTask(input)`。 | 兼容保留。 |
| `streamHostTask(taskId)` | `ember.agent.streamTask` 的私有 wrapper。 | `ember.agent.streamTask({ taskId })` 或标准 SDK 同名 facade。 | 兼容保留。 |
| `getHostTask(taskId)` | `ember.agent.getTask` 的私有 wrapper。 | `ember.agent.getTask({ taskId })`。 | 兼容保留。 |
| `cancelHostTask(taskId)` | `ember.agent.cancelTask` 的私有 wrapper。 | `ember.agent.cancelTask({ taskId })`。 | 兼容保留。 |
| `retryHostTask(taskId)` | `ember.agent.retryTask` 的私有 wrapper。 | `ember.agent.retryTask({ taskId })`。 | 兼容保留。 |
| `submitHostResponse(input)` | `ember.agent.submitHostResponse` 的私有 wrapper。 | `ember.agent.submitHostResponse(input)`。 | 兼容保留。 |
| `subscribeHostTask(taskId, options)` | 手写 `capability:subscribe`，传 `capability=ember.agent / topic=task / pollIntervalMs / bridgeAction`。 | SDK client `subscribeCapability` / `ember.agent.subscribeTask` 等价薄 wrapper，事件分发交给 SDK。 | 兼容保留。 |
| `unsubscribeHostTask(subscriptionId)` | 手写 `capability:unsubscribe`。 | SDK client `unsubscribeCapability`。 | 兼容保留。 |
| `runHostAgentTask(input, options)` | 业务编排：start、可选 subscribe、stream/get 轮询、Host response、unsubscribe、派发 `ember:host-agent-task-update`。 | 保留业务编排；底层只调用 `ember.agent.*` facade 与 SDK subscription，不直接使用 `requestHostBridge`。 | 必须保留。 |
| `writeHostTaskResult(...)` | 依次调用 `ember.storage.set`、`ember.artifacts.create`、`ember.evidence.record`，写内容工厂 task result。 | 保留业务语义；底层改成 `ember.storage / ember.artifacts / ember.evidence` typed facade。 | 必须保留。 |
...
| `AGENT_APP_BRIDGE_PROTOCOL / VERSION` | 从 package 内常量 `ember.agentApp.bridge / 1` 导出。 | 由 SDK / Host Bridge client 提供；如保留，只作为 compatibility export，不作为业务文件手写协议依据。 | 兼容保留，后续退出。 |
...
| Step | 改动 | 退出条件 |
|---|---|---|
| 1 | 新增或切换到 `src/ui/ember-app-sdk.js` runtime factory，集中创建 `createLimeHostBridgeCapabilityInvoker + createEmberCoreCapabilityAdapters` 等价 handle。 | 业务文件不 import Ember internal path；`postMessage` 只在 SDK client / shim 内。 |
...
| 4 | `initHostBridge()` 只负责初始化 SDK runtime、注册 Host snapshot/theme/visibility/capability event handler，并把调试对象挂到 `window.emberAgentAppBridge`。 | `window.emberAgentAppBridge` 暴露 facade/debug handle，不暴露 `requestHostBridge`。 |
| 5 | `notifyHost / requestHostDownload` 改成 SDK Host action wrapper。 | 不再手写 `host:toast / host:download` message。 |
| 6 | `startHostTask / streamHostTask / getHostTask / cancelHostTask / retryHostTask / submitHostResponse` 改成 `runtime.agent.*` wrapper。 | 保留导出名；底层只走 `ember.agent` facade。 |
| 7 | `subscribeHostTask / unsubscribeHostTask` 改成 SDK subscription wrapper，并继续把 task event payload 派发成 `ember:host-agent-task-update`。 | 业务仍能收到进度事件；测试不再模拟 `postMessage` transport。 |
...
3. 不改 Ember `ember-rs/*`、AgentRuntime facade、GUI smoke 脚本。
4. 不新增垂直 `content_factory_*` Ember command。
5. 不把 `content-factory-app` 迁移成依赖 Ember internal path；App package 只能依赖 SDK facade。
...
content-factory-app/src/ui/app.js
  ...
  -> ember.agent / ember.storage / ember.artifacts / ember.evidence
  -> createEmberCoreCapabilityAdapters
  -> createLimeHostBridgeCapabilityInvoker
  -> ember.agentApp.bridge v1
  -> Ember AgentAppHostBridge
```
...
| 文件 | 动作 | 原因 |
|---|---|---|
| `/Users/coso/Documents/dev/ai/embercloud/content-factory-app/src/ui/host-bridge.js` | 收敛为 SDK facade consumer。 | P18.5.3 主目标。 |
| `/Users/coso/Documents/dev/ai/embercloud/content-factory-app/src/ui/ember-app-sdk.js` 或等价 package-local shim | 仅在正式 `@ember/app-sdk` 尚未可安装时新增，暴露 `createLimeHostBridgeCapabilityInvoker + createEmberCoreCapabilityAdapters` 等价最小接口。 | 当前外部 package `dependencies / devDependencies` 为空，不能 import Ember repo 内部路径；Ember-side `src/features/agent-app/index.test.ts` 已固定 public export 面，shim 仍是临时退出条件，不是第二套长期 SDK。 |
| `/Users/coso/Documents/dev/ai/embercloud/content-factory-app/tests/ui.test.mjs` | 更新断言：从“手写 bridge 包含某些字符串”改为“调用标准 facade 并产生相同 capability call 序列”。 | 防止测试继续鼓励私有 bridge。 |
| `/Users/coso/Documents/dev/ai/embercloud/content-factory-app/package.json` | 仅在需要引入正式 SDK package / 本地 workspace alias 时修改。 | 避免未声明依赖。 |
| `/Users/coso/Documents/dev/ai/embercloud/content-factory-app/dist/ui/*` | 只在明确运行 build / verify 后更新。 | 避免和隔壁 dist 产物打架。 |
...
2. `/Users/coso/Documents/dev/ai/embercloud/content-factory-app` 的 relevant 写集稳定：`src/ui/host-bridge.js`、`tests/ui.test.mjs`、`package.json`、`dist/ui/*` 没有新的未知改动，且当前 dirty 状态可归因。
...
```bash
git -C /Users/coso/Documents/dev/ai/embercloud/content-factory-app status --short
rg -n "pendingRequests|requestHostBridge|buildMessage|postMessage|createLimeHostBridgeCapabilityInvoker|createEmberCoreCapabilityAdapters|capability:invoke|capability:subscribe|capability:event" /Users/coso/Documents/dev/ai/embercloud/content-factory-app/src/ui/host-bridge.js /Users/coso/Documents/dev/ai/embercloud/content-factory-app/tests/ui.test.mjs
cd /Users/coso/Documents/dev/ai/embercloud/content-factory-app && nice -n 10 npm test
```
...
| 检查项 | 结果 | Gate 判定 |
|---|---|---|
| 标准仓库 | `/Users/coso/Documents/dev/ai/embercloud/agentapp` 工作区干净，`HEAD=4bef605 fix: quote mermaid sdk labels`；标准资料继续固定 Agent App 是完整应用包、Expert 只是 `expert-chat` entry、能力调用走 `@ember/app-sdk` / Capability SDK。 | 标准事实源稳定；不需要改标准仓库。 |
| 外部 package 状态 | `/Users/coso/Documents/dev/ai/embercloud/content-factory-app` 仍有 39 个 dirty / untracked 条目，其中 36 个 tracked modified、3 个 untracked；`src/ui/host-bridge.js` 与 `dist/ui/host-bridge.js` 仍未跟踪。 | 不满足 owner 稳定条件；继续只读。 |
| 私有 bridge marker | `src/ui/host-bridge.js` 仍包含 `pendingRequests / buildMessage / window.parent.postMessage / requestHostBridge / capability:invoke / capability:subscribe / capability:event`，未出现 `createLimeHostBridgeCapabilityInvoker` 或 `createEmberCoreCapabilityAdapters`；`tests/ui.test.mjs` 仍直接过滤 `message.type === 'capability:invoke'` 并模拟 `postMessage`。 | SDK facade 尚未完成。 |
| 外部 `npm test` | 2026-05-16 08:19 低优先级只读运行，46 tests passed。 | 只读业务测试仍绿；不能替代 package-side SDK facade / verify。 |
| Ember-side SDK seam | 2026-05-16 08:19 低优先级运行 `npm test -- src/features/agent-app/sdk/hostBridgeClient.test.ts src/features/agent-app/sdk/contentFactorySdkRegression.test.ts src/features/agent-app/index.test.ts`，3 files / 8 tests passed。 | Ember-side Host Bridge SDK client、内容工厂 SDK regression 与 public SDK export seam 当前仍绿；不等于外部 package 已迁移。 |
...
| SDK marker | 未出现 `createLimeHostBridgeCapabilityInvoker` 或 `createEmberCoreCapabilityAdapters`。 | package-side 仍不是标准 SDK facade consumer。 |
...
| Ember-side 回归 | 2026-05-16 09:09 `npm test -- src/features/agent-app`，35 files / 173 tests passed；09:04 `typecheck` 与 `test:contracts` 均通过。 | Ember-side 当前绿；不等于外部 package 已迁移。 |
...
| Ember-side SDK seam | 低优先级重跑 `npm test -- src/features/agent-app/sdk/hostBridgeClient.test.ts src/features/agent-app/sdk/contentFactorySdkRegression.test.ts src/features/agent-app/index.test.ts`，3 files / 8 tests passed；`src/features/agent-app` 越界扫描无输出。 | Ember SDK target 仍可用；仍不能替代外部 package 迁移。 |
| 运行面 | 当前未再发现 cwd 在 `content-factory-app` 的 `npm run dev` 进程，但 Ember Vite 与 `tauri:dev:headless` 仍在。 | 进程空闲不是 handoff；未获明确接管前继续只读。 |
...
| 发现正式 `@ember/app-sdk` 可安装方式和 package-local shim 设计冲突。 | 可能会形成第二套 SDK。 | 优先正式 SDK；shim 只保留临时退出条件。 |

接管后的验证顺序固定为：

1. 外部 package：`nice -n 10 npm test`。
2. 外部 package：确认允许更新 `dist/*` 后，运行 `nice -n 10 npm run verify`。
3. Ember repo：P18 SDK / Host Bridge 定向 tests。
4. Ember repo：`npm run typecheck -- --pretty false` 与 `npm run test:contracts`。
5. Ember repo：根据改动范围决定是否重跑完整 `npm run verify:local`。
...
本草图只用于后续 owner handoff 后实施，不直接写入外部 package。若正式 `@ember/app-sdk` 已可安装，应优先删除 shim，改为从正式包 import。

```js
// src/ui/ember-app-sdk.js
export {
...
```js
// src/ui/ember-app-sdk.js，伪代码；临时退出条件：正式 @ember/app-sdk 可安装后删除。
export const EMBER_AGENT_APP_BRIDGE_PROTOCOL = 'ember.agentApp.bridge';
export const EMBER_AGENT_APP_BRIDGE_VERSION = 1;

export function createContentFactoryEmberRuntime({ appId, entryKey, storageNamespace }) {
  const invoker = createHostBridgeInvoker({ appId, entryKey });
  return createCoreCapabilityAdapters({ invoker, storageNamespace });
}
```

`createHostBridgeInvoker / createCoreCapabilityAdapters` 是 package-local shim 内对 `createLimeHostBridgeCapabilityInvoker / createEmberCoreCapabilityAdapters` 的最小等价实现；不得扩展出新的长期 SDK 语义。

2026-05-16 08:37 只读复核 Ember repo SDK 发布边界：根 `package.json` 仍为 private 桌面 App 包，`packages/` 下只有 `packages/ember-cli-npm`，未发现独立 `@ember/app-sdk` / `ember-app-sdk` package。P18.5.3 不能把外部 App 指向 `src/features/agent-app/*` 深路径；如果 owner handoff 时正式 SDK package 仍不可安装，package-local shim 是允许的临时手段，但必须保持退出条件：正式 SDK package 可安装后删除 shim，改为从正式包 import。

`src/ui/host-bridge.js` 迁移后的结构应收敛为：

```js
import { createContentFactoryEmberRuntime } from './ember-app-sdk.js';

let runtime;

function getRuntime() {
  if (!runtime) {
    runtime = createContentFactoryEmberRuntime({
...
export async function writeHostTaskResult(input) {
  const ember = getRuntime();
  const storageEntry = await ember.storage.set({ key: input.storageKey, value: input.content });
  const artifact = await ember.artifacts.create({ kind: input.kind, title: input.title, content: input.content });
  const evidence = await ember.evidence.record({ kind: input.evidenceKind, message: input.message, refs: input.refs });
...
3. **引入 SDK facade seam**：
   - 如果已有可安装 `@ember/app-sdk`，从 SDK package import。
   - 如果还没有发布包，先用 package-local shim 只暴露与 `createLimeHostBridgeCapabilityInvoker + createEmberCoreCapabilityAdapters` 等价的最小接口，并在文档中标记退出条件。
4. **改写 transport 层**：
...
7. **运行验证**：
   ...
   - 回到 Ember repo 跑 P18 SDK / Host Bridge regression。
...
| Capability 调用序列不变。 | tests 仍断言 `ember.agent.startTask -> streamTask -> getTask -> submitHostResponse -> storage.set -> artifacts.create -> evidence.record`。 |
...
| Ember-side regression 通过。 | Ember P18 SDK / Host Bridge tests、typecheck、diff check 通过。 |

## 回滚点

如果外部迁移失败，只需要回滚外部 package 的 P18.5.3 写集；Ember-side `hostBridgeClient.ts`、`hostBridgeClient.test.ts` 和本计划仍可保留，因为它们是标准 SDK client 的独立 contract evidence，不依赖内容工厂 package 的改造进度。
...
4. Ember P18 SDK / Host Bridge regression 通过。
