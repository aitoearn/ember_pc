...
2. `scripts/agent-app/apps-smoke.mjs` 在 active bootstrap 阶段注入受控 `__EMBER_OEM_CLOUD__` / `__EMBER_SESSION_TOKEN__`，并用 Playwright route mock `/client/agent-apps`，避免访问真实网络。
...
- Summary：`.ember/qc/gui-evidence/agent-apps/agent-apps-smoke-summary.json`
- 主截图：`.ember/qc/gui-evidence/agent-apps/agent-apps-smoke.png`
- Flag-off 截图：`.ember/qc/gui-evidence/agent-apps/agent-apps-smoke-flag-off.png`
