...
- `ember-rs/crates/app-server/src/runtime_backend/skill_runtime_enable.rs` 仍在写 session 级 skill allowlist。
- `ember-rs/crates/agent/src/tools/skill_tool_gate.rs` 仍以会话允许列表决定 `Skill(image_generate)` 能否执行。
- `src/components/image-gen/useImageGen.ts` 仍承担生成调度和 provider 执行，但已经直接消费统一 catalog。
- `ember-rs/resources/default-skills/image_generate/SKILL.md` 仍承担迁移期兼容入口。
...
2026-07-02 `@配图` 只输出文字、不出现图片任务框的断点已收口到 App Server Agent Runtime：`image_skill_launch.skill_name` 会参与 selected Agent Skill body 注入，让 `image_generate` 的 `allowed_tools` 进入 turn `tool_scope`；同时 `image_skill_launch` 会强制使用完整工具面而非 fast-response direct answer，并在 turn context 写入 `runtime_control.stop_after_tool_result`，匹配 `task_type=image_generate` 且带 `task_id/artifact_path/path` 的工具结果后立即收口，避免图片任务已创建后继续闲聊。验证：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server image_skill_launch -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server appends_model_skill_launch_body_from_image_metadata -- --nocapture`、`cargo check --manifest-path "ember-rs/Cargo.toml" -p app-server`、`npm run smoke:claw-chat-current-fixture -- --scenario image-command --timeout-ms 180000`、`npm run smoke:claw-chat-current-fixture -- --scenario expert-panel-skills-runtime --timeout-ms 180000` 均通过；`npm run smoke:agent-runtime-current-fixture` 另在 Coding Workbench fixture 的 `externalFixtureBackendUsed` 断言失败，归为非图片链路剩余门禁。
...
| 运行时授权     | `ember-rs/crates/app-server/src/runtime_backend/skill_runtime_enable.rs`、`ember-rs/crates/agent/src/tools/skill_tool_gate.rs`           | 图片能力不再由 session allowlist 决定，只保留普通 Skill gate。                         |
| 兼容 skill     | `ember-rs/resources/default-skills/image_generate/SKILL.md`                                                                             | 作为迁移期 facade 保留，最终不再承担权限事实源。                                       |
