...
- 新增 App Server intent parser：
  - [x] `ember-rs/crates/app-server/src/runtime_backend/image_command/mod.rs` 首期解析器
...
```text
ember-rs/crates/app-server/src/runtime_backend/image_command/
  mod.rs
  intent.rs
  gate.rs
  route.rs
...
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server image_command -- --nocapture`
