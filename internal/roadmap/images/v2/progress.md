...
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::image_command --lib` 通过，19 个图片命令 / presentation 测试。
...
- `ember-rs/crates/app-server/src/runtime_backend/image_command/**` 中已无 `ImageCommandIntent::from_scope_only`，对应 dead_code warning 未复现。
...
- `cargo build --manifest-path "ember-rs/Cargo.toml" -p app-server --bin app-server` 通过，未复现 `ImageCommandIntent::from_scope_only` dead_code warning；当前源码已无 `from_scope_only` 符号。
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server workflow -- --nocapture` 通过，42 个 workflow 相关测试。
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server image_command -- --nocapture` 通过，20 个图片命令测试，`media_task_jsonrpc` 中 image command 集成用例同步通过。
- `cargo fmt --manifest-path "ember-rs/Cargo.toml" --all -- --check` 通过。
...
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server image_command -- --nocapture` 通过，20 个测试。
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server --test media_task_jsonrpc image_command -- --nocapture` 通过，1 个测试。
...
- App Server sidecar：`ember-rs/target/debug/app-server --stdio --backend runtime --data-dir "$HOME/Library/Application Support/lime/app-server"`。
...
- `cargo fmt --manifest-path "ember-rs/Cargo.toml" --all -- --check` 通过。
- `git diff --check -- "ember-rs/crates/app-server/src/runtime_backend/image_command/mod.rs" "ember-rs/crates/app-server/src/runtime_backend/image_command/presentation.rs" "ember-rs/crates/agent/src/lib.rs" "ember-rs/crates/services/src/lib.rs"` 通过。
- `npm run test:rust:unit -- -p app-server image_command` 通过，19 个测试。
- `cargo build --manifest-path "ember-rs/Cargo.toml" -p app-server --bin app-server` 通过。
...
- `rustfmt --edition 2021 --check "ember-rs/crates/app-server/src/runtime_backend/image_command/presentation.rs" "ember-rs/crates/app-server/src/runtime_backend/image_command/mod.rs"` 通过。
...
- `npm run test:contracts` 通过。
- `cargo fmt --manifest-path "ember-rs/crates/media-runtime/Cargo.toml" --check` 通过。
- `cargo fmt --manifest-path "ember-rs/crates/app-server/Cargo.toml" --check` 通过。
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime image_worker --lib` 通过，17 个测试。
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server media_task_worker --lib` 通过，13 个测试。
...
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server read_model::workflow -- --nocapture` 通过，3 个测试。
- `cargo build --manifest-path "ember-rs/Cargo.toml" -p app-server --bin app-server` 通过，确保 Electron fixture 使用最新 App Server binary。
...
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server article_workspace_worker_evidence -- --nocapture` 通过，1 个测试。
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server read_session_materializes_content_factory_workspace_patch_into_article_workspace -- --nocapture` 通过，1 个测试。
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server article_workspace_turn_runs_installed_worker_and_materializes_workspace_patch -- --nocapture` 通过，1 个测试。
- `npx vitest run "scripts/agent-runtime/claw-chat-current-fixture-smoke.test.mjs" --silent=passed-only` 通过，23 个测试。
- `cargo build --manifest-path "ember-rs/Cargo.toml" -p app-server --bin app-server` 通过。
...
- `cargo fmt --manifest-path "ember-rs/Cargo.toml" -p app-server -- --check` 通过。
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::image_command::presentation -- --nocapture` 通过。
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server plugin_worker_turn -- --nocapture` 通过。
- `cargo build --manifest-path "ember-rs/Cargo.toml" -p app-server` 通过。
...
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server plugin_worker_turn -- --nocapture` 通过，24 个测试。
...
- `cargo fmt --manifest-path "ember-rs/Cargo.toml" -p lime-agent -- --check` 通过。
- `cargo check --manifest-path "ember-rs/Cargo.toml" -p lime-agent` 通过。
- `npm run verify:gui-smoke` 通过；覆盖 renderer build、Electron host build、App Server sidecar 初始化、Claw workbench shell ready 与 memory settings ready。
...
- `cargo fmt --manifest-path "ember-rs/Cargo.toml" -p app-server -- --check` 通过。
- `cargo check --manifest-path "ember-rs/Cargo.toml" -p app-server` 通过；此前 `NormalizedImageCreateParams` 编译阻塞在当前工作树未复现。
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::image_command --lib` 通过，20 个测试。
