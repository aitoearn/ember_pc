...
- `current`
  - `ember-rs/src/agent_tools/catalog.rs` 中的 `LIME_SITE_*_TOOL_NAME`
...
- `ember-rs/src/agent_tools/catalog.rs` 已把 `lime_site_*` 标记为 `current`，`lime_run_service_skill` 标记为 `compat`。
