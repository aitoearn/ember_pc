...
```bash
cargo test --manifest-path "ember-rs/Cargo.toml" diff_summary -- --nocapture
cargo test --manifest-path "ember-rs/Cargo.toml" restore_file_checkpoint_should -- --nocapture
```
