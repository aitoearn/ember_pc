...
native 注册事实源：`ember-rs/crates/aster-rust/crates/aster/src/tools/mod.rs`
...
- `ember-rs/crates/aster-rust/crates/aster/src/tools/task_list_tools.rs`
...
- 当前 git 状态：工作区全局很脏，本轮只修改相关文件：`ember-rs/crates/aster-rust/crates/aster/src/tools/task_list_tools.rs`、`src/components/agent/chat/components/InlineToolProcessStep.tsx`、`src/components/agent/chat/components/StreamingRenderer.test.tsx`、`src/components/agent/chat/components/messageListItemProjection.unit.test.ts`、本文件。`internal/exec-plans/` 目录当前为未跟踪状态，属于既有执行计划产物。
...
- 发现问题：
  - `cargo test --manifest-path "ember-rs/Cargo.toml" -p aster-core ...` 不可用，因为 `aster-core` 被 `ember-rs` workspace exclude；正确入口是 `ember-rs/crates/aster-rust/Cargo.toml`，并显式设置 `CARGO_TARGET_DIR` 到 `ember-rs/target`。
