...
native 注册事实源：`ember-rs/crates/agent-rust/crates/agent/src/tools/mod.rs`
...
- `ember-rs/crates/agent-rust/crates/agent/src/tools/task_list_tools.rs`
...
cargo test --manifest-path "ember-rs/Cargo.toml" test_register_default_tools -- --nocapture
```
...
- 当前 git 状态：工作区全局很脏，本轮只修改相关文件：`ember-rs/crates/agent-rust/crates/agent/src/tools/task_list_tools.rs`、`src/components/agent/chat/components/InlineToolProcessStep.tsx`、`src/components/agent/chat/components/StreamingRenderer.test.tsx`、`src/components/agent/chat/components/messageListItemProjection.unit.test.ts`、本文件。`internal/exec-plans/` 目录当前为未跟踪状态，属于既有执行计划产物。
...
- 已执行验证：
  ...
  - `CARGO_TARGET_DIR="/Users/coso/Documents/dev/ai/aiclientproxy/lime/ember-rs/target" cargo test --manifest-path "ember-rs/crates/agent-rust/Cargo.toml" -p agent-core task_list_tools -- --nocapture`：`task_list_tools` 15 tests passed；其余 test targets 在同一 filter 下为 0 tests。
...
- 发现问题：
  - `cargo test --manifest-path "ember-rs/Cargo.toml" -p agent-core ...` 不可用，因为 `agent-core` 被 `ember-rs` workspace exclude；正确入口是 `ember-rs/crates/agent-rust/Cargo.toml`，并显式设置 `CARGO_TARGET_DIR` 到 `ember-rs/target`。
