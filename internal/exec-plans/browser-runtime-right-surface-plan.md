...
主目标：先把 Browser 模块骨架落到 current 主链，再按阶段补 Chrome 类细节、CDP current 接线、Browser Assist 可见执行和 evidence。右侧栏内嵌浏览器是用户可见主路径，`ember-rs/crates/browser-runtime` 是外部 Chrome / CDP 自动化和观测事实源，二者通过 `browserSessionId / launchUrl / action trace / evidence refs` 收敛，不新增第二套 browser backend。
...
| current    | `ember-rs/crates/app-server*/**` 的 `workspaceRightSurface/*`                               | Browser surface intent 的 App Server pending 事实源                  |
| current    | `ember-rs/crates/browser-runtime/src/**`                                                    | 外部 Chrome / CDP session、action、event buffer、后续 evidence facts |
...
目标：让 `ember-rs/crates/browser-runtime` 从孤立 crate 进入 App Server / Runtime current method，不恢复旧 `launch_browser_session`。
...
- [x] 高风险动作走 action.required / confirmation / human takeover 闭环：当前仅 `navigate / read_page / read_console_messages / read_network_requests` 自动执行，`click / type / form_input / submit / upload / download / javascript` 等动作在 `browser-runtime` CDP executor 内 fail-closed，不触发页面 CDP mutation；返回 `tool_confirmation` 请求、`permission_facts`、`action.required` metadata，并把 session 切到 `human_controlling / human`，由 EvidencePack `browser_action_index` 记录 `status=pending / confirmation_request_id / control_mode / lifecycle_state / human_reason`。
...
| CDP action             | `ember-rs/crates/browser-runtime`                       | Rust unit、App Server method、event buffer                 | action 有 session/action/result/error            | `evidence_gap`   |
...
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-browser-runtime
```
...
- `ember-rs/crates/app-server/src/processor/mod.rs` 已超过 `1000` 行；本轮只保留 method dispatch 接线，业务逻辑已拆到 `processor/browser_session.rs`。退出条件：后续新增 Browser Session method 时继续进入分域 processor，不再扩中心文件。
- `ember-rs/crates/browser-runtime/src/manager.rs` 已从 `1216` 行拆为 `344` 行 facade；`manager/session.rs` 只保留 `CdpSessionHandle` core / CDP command helpers，`manager/session_reader.rs` 承接 CDP reader，`manager/session_events.rs` 承接 event buffer / collection，`manager/session_stream.rs` 承接 screencast / screenshot fallback，`manager/session_lifecycle.rs` 承接 control lifecycle，`manager/cdp_targets.rs` 承接 CDP target discovery，`profile_scope.rs` 承接 task scoped profile owner / cleanup plan。退出条件：后续新增 trace、evidence、profile policy 时继续进分域子模块，不回填 `manager.rs` 或 `manager/session.rs`。
- `ember-rs/crates/app-server/src/runtime/evidence_provider/browser.rs` 已拆为 `35` 行 facade，`browser/action_index.rs` 承接既有 action index 逻辑，`browser/file_artifacts.rs` 承接 network / console / screenshot 文件级 evidence。退出条件：`browser/action_index.rs` 不再继续追加 network / console / screenshot 细节；后续新增 evidence 维度必须进入同级子模块。
...
- [x] `git diff --check`
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-protocol browser_session`
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server browser_session`
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-protocol`
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-client`
- [x] `rustfmt --edition 2021 --check "ember-rs/crates/app-server-protocol/src/protocol/v0/browser_session.rs" "ember-rs/crates/app-server/src/runtime/browser_session.rs" "ember-rs/crates/app-server/src/processor/browser_session.rs" "ember-rs/crates/app-server-client/src/lib.rs"`
...
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-browser-runtime`（P5 高风险动作 CDP executor fail-closed，16 tests passed）
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server browser_session_action_trace_preserves_action_required_state`（P5/P6 高风险动作 pending trace）
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server evidence_exports`（P5/P6 高风险动作 confirmation / human takeover evidence，12 tests passed）
- [x] `npx vitest run "src/components/agent/chat/components/harnessEvidenceViewModel.unit.test.ts"`（P6 高风险动作 replay meta 透传，9 tests passed）
- [x] `npx eslint "src/lib/api/agentRuntime/types.ts" "src/lib/api/agentRuntime/normalizers.ts" "src/components/agent/chat/components/harnessEvidenceViewModel.ts" "src/components/agent/chat/components/harnessEvidenceViewModel.unit.test.ts" --max-warnings 0`（P6 高风险动作 evidence TS 窄写集）
- [x] `rustfmt --edition 2021 --check "ember-rs/crates/browser-runtime/src/action.rs" "ember-rs/crates/browser-runtime/src/action_policy.rs" "ember-rs/crates/browser-runtime/src/lib.rs" "ember-rs/crates/app-server/src/runtime/browser_session.rs" "ember-rs/crates/app-server/src/runtime/evidence_provider/browser/action_index.rs" "ember-rs/crates/app-server/src/runtime/tests/evidence_exports/browser.rs"`（P5/P6 高风险动作 Rust 格式检查）
- [x] `npx prettier --check "src/lib/api/agentRuntime/types.ts" "src/lib/api/agentRuntime/types.d.ts" "src/lib/api/agentRuntime/normalizers.ts" "src/components/agent/chat/components/harnessEvidenceViewModel.ts" "src/components/agent/chat/components/harnessEvidenceViewModel.unit.test.ts"`（P6 高风险动作前端格式检查）
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server evidence_exports`（P6 browser evidence 第一刀，11 tests passed）
- [x] `rustfmt --edition 2021 --check "ember-rs/crates/app-server/src/runtime/evidence_provider.rs" "ember-rs/crates/app-server/src/runtime/evidence_provider/browser.rs" "ember-rs/crates/app-server/src/runtime/tests/evidence_exports.rs" "ember-rs/crates/app-server/src/runtime/tests/evidence_exports/browser.rs"`（P6 browser evidence 第一刀）
- [x] `git diff --check -- "ember-rs/crates/app-server/src/runtime/evidence_provider.rs" "ember-rs/crates/app-server/src/runtime/evidence_provider/browser.rs" "ember-rs/crates/app-server/src/runtime/tests/evidence_exports.rs" "ember-rs/crates/app-server/src/runtime/tests/evidence_exports/browser.rs"`（P6 browser evidence 第一刀）
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server browser_session_action_result`（P6 BrowserActionTrace 第二刀，2 tests passed）
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server evidence_exports`（P6 BrowserActionTrace 第二刀，11 tests passed）
...
- [x] `rustfmt --edition 2021 --check "ember-rs/crates/app-server/src/runtime/browser_session.rs" "ember-rs/crates/app-server/src/runtime/evidence_provider/browser.rs" "ember-rs/crates/app-server/src/runtime/tests/evidence_exports/browser.rs"`（P6 BrowserActionTrace 第二刀）
...
- [x] `git diff --check -- "ember-rs/crates/app-server/src/runtime/browser_session.rs" "ember-rs/crates/app-server/src/runtime/evidence_provider/browser.rs" "ember-rs/crates/app-server/src/runtime/tests/evidence_exports/browser.rs" "src/lib/api/agentRuntime/types.ts" "src/lib/api/agentRuntime/types.d.ts" "src/lib/api/agentRuntime/normalizers.ts" "src/components/agent/chat/components/harnessEvidenceViewModel.ts" "src/components/agent/chat/components/harnessEvidenceViewModel.unit.test.ts"`（P6 BrowserActionTrace 第二刀）
...
- [x] `rustfmt --edition 2021 --check "ember-rs/crates/app-server/src/runtime/evidence_provider/browser.rs" "ember-rs/crates/app-server/src/runtime/evidence_provider/browser/action_index.rs"`（P6 evidence provider action index 拆分）
- [x] `git diff --check -- "ember-rs/crates/app-server/src/runtime/evidence_provider/browser.rs" "ember-rs/crates/app-server/src/runtime/evidence_provider/browser/action_index.rs"`（P6 evidence provider action index 拆分）
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server evidence_exports`（P6 evidence provider action index 拆分，11 tests passed）
- [x] `rustfmt --edition 2021 --check "ember-rs/crates/app-server/src/runtime/evidence_provider.rs" "ember-rs/crates/app-server/src/runtime/evidence_provider/browser.rs" "ember-rs/crates/app-server/src/runtime/evidence_provider/browser/action_index.rs" "ember-rs/crates/app-server/src/runtime/evidence_provider/browser/file_artifacts.rs" "ember-rs/crates/app-server/src/runtime/tests/evidence_exports.rs" "ember-rs/crates/app-server/src/runtime/tests/evidence_exports/browser.rs"`（P6 browser file evidence）
- [x] `git diff --check -- "ember-rs/crates/app-server/src/runtime/evidence_provider.rs" "ember-rs/crates/app-server/src/runtime/evidence_provider/browser.rs" "ember-rs/crates/app-server/src/runtime/evidence_provider/browser/action_index.rs" "ember-rs/crates/app-server/src/runtime/evidence_provider/browser/file_artifacts.rs" "ember-rs/crates/app-server/src/runtime/tests/evidence_exports.rs" "ember-rs/crates/app-server/src/runtime/tests/evidence_exports/browser.rs"`（P6 browser file evidence）
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server evidence_exports`（P6 browser file evidence，11 tests passed；`lime-agent/request_tool_policy.rs` 仅剩 `unused import: uuid::Uuid` warning，未阻塞本轮）
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-browser-runtime profile_scope`（P3 task scoped profile owner skeleton，2 tests passed）
- [x] `rustfmt --edition 2021 --check "ember-rs/crates/browser-runtime/src/manager.rs" "ember-rs/crates/browser-runtime/src/manager/session.rs" "ember-rs/crates/browser-runtime/src/manager/cdp_targets.rs" "ember-rs/crates/browser-runtime/src/lib.rs" "ember-rs/crates/browser-runtime/src/profile_scope.rs"`（BrowserRuntimeManager 拆分）
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-browser-runtime`（BrowserRuntimeManager 拆分 / profile scope，8 tests passed）
- [x] `git diff --check -- "ember-rs/crates/browser-runtime/src/manager.rs" "ember-rs/crates/browser-runtime/src/manager/session.rs" "ember-rs/crates/browser-runtime/src/manager/cdp_targets.rs" "ember-rs/crates/browser-runtime/src/lib.rs" "ember-rs/crates/browser-runtime/src/profile_scope.rs"`（BrowserRuntimeManager 拆分）
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server evidence_exports`（BrowserRuntimeManager 拆分下游回归，11 tests passed）
- [x] `rustfmt --edition 2021 --check "ember-rs/crates/browser-runtime/src/manager.rs" "ember-rs/crates/browser-runtime/src/manager/session.rs" "ember-rs/crates/browser-runtime/src/manager/session_events.rs" "ember-rs/crates/browser-runtime/src/manager/session_lifecycle.rs" "ember-rs/crates/browser-runtime/src/manager/session_reader.rs" "ember-rs/crates/browser-runtime/src/manager/session_stream.rs" "ember-rs/crates/browser-runtime/src/manager/cdp_targets.rs"`（BrowserRuntimeManager session 二次拆分）
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-browser-runtime`（BrowserRuntimeManager session 二次拆分，12 tests passed）
- [x] `rustfmt --edition 2021 --check "ember-rs/crates/browser-runtime/src/action.rs" "ember-rs/crates/browser-runtime/src/evidence.rs" "ember-rs/crates/browser-runtime/src/lib.rs" "ember-rs/crates/app-server/src/runtime/browser_session.rs" "ember-rs/crates/app-server/src/runtime/tests/evidence_exports/browser.rs"`（P6 browser-runtime action result file evidence metadata）
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-browser-runtime`（P6 browser-runtime action result file evidence metadata，12 tests passed）
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server browser_session_action`（P6 action trace file evidence refs，3 tests passed）
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server evidence_exports`（P6 action result -> evidence pack 回归，11 tests passed）
...
- [x] `rustfmt --edition 2021 --check "ember-rs/crates/app-server/src/runtime.rs" "ember-rs/crates/app-server/src/runtime/browser_session.rs"`（BrowserProfileScope App Server owner 生命周期）
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server browser_profile`（BrowserProfileScope App Server owner 生命周期，2 tests passed）
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server browser_session`（BrowserProfileScope close cleanup + browser action trace 回归，8 tests passed）
...
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-browser-runtime`（DOM/accessibility file evidence metadata，14 tests passed）
- [x] `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server evidence_exports`（DOM/accessibility file evidence pack 导出，11 tests passed）
...
- [x] `npm test -- "electron/embeddedBrowserHost.test.ts"`（主窗口刷新清理内嵌 `WebContentsView`，15 tests passed）
- [x] `npm run typecheck:electron`（主窗口刷新清理内嵌 `WebContentsView`）
- [x] `npm run verify:gui-smoke`（主窗口刷新清理内嵌 `WebContentsView`；真实 Electron renderer、App Server、Claw shell、memory settings 启动通过）
...
- 2026-06-24：P6 evidence provider 防膨胀拆分完成：`ember-rs/crates/app-server/src/runtime/evidence_provider/browser.rs` 从 788 行收敛为 20 行 facade，既有 browser action index / artifact extraction 逻辑移动到 `browser/action_index.rs`；`evidence_provider.rs` 对外函数名和 `evidence/export` JSON 行为不变。`rustfmt --check`、`git diff --check` 和 `cargo test -p app-server evidence_exports` 已过。
...
- 2026-06-25：高风险 Browser action confirmation / human takeover 证据闭环落地：`browser-runtime` CDP executor 对 `click / type / form_input / submit / upload / download / javascript` 等 mutation / script 动作 fail-closed，先返回 `action.required + tool_confirmation + permission_facts`，并将 session 切到 `human_controlling / human`，避免 Claw 或 MCP 直接调用 `browserSession/action/execute` 绕过前端 skeleton；App Server `browser_action_trace` 保留 `status=pending / success=false / requestId / controlMode / lifecycleState / humanReason`，`browser_action_index` 增加 `confirmation_request_id / control_mode / lifecycle_state / human_reason`，前端 evidence normalizer 和 Browser replay artifact meta 同步透传。为守住 800 行体量边界，本轮同时拆出 `ember-rs/crates/browser-runtime/src/action_policy.rs`，让 `action.rs` 回落到 694 行；`browser/action_index.rs` 仍为 992 行，后续若再扩展必须先拆分。未新增 App Server method、Electron IPC、私有确认 UI 或确认后恢复状态机。
- 2026-06-25：Agent App Browser intent 第一刀落地：新增 `src/features/agent-app/ui/agentAppBrowserIntentLaunch.ts`，把 `lime.browser/open` dispatcher 返回的 `requires_agent_task` intent 转成 `workspaceRightSurface/request` 的 `surfaceKind=browser` foreground pending request；workspace/session 只取宿主 `AgentAppRightSurfaceLaunchTarget`，不信任 App intent payload 自带 target。metadata 只保留 app/entry、`lime.browser/open`、URL 和 `shared / waiting_for_human` 控制态，复用 Right Surface browser pending 消费链路，不新增 App Server method、Electron IPC、浏览器 UI 副本或 CapabilityHost 胖接口。定向 Vitest / ESLint / Prettier 已过；尚未接入 Agent App runtime page 的实际投递触发，因此完整验收项继续保持未完成。
- 2026-06-25：Agent App Browser intent 主路径接线完成：`AgentAppsPage` 已把宿主 `rightSurfaceTarget` 透传到 `agent-app` 路由，`AgentAppRuntimePage` 通过 `wrapAgentAppCapabilityDispatchWithBrowserIntentLaunch` 在 `lime.browser/open` 后自动请求 Right Surface browser pending；定向 Vitest / ESLint / Prettier 已过。完整产品验收仍只差真实网页工作流、Playwright 和多 tab/profile。
- 2026-06-25：主窗口刷新后内嵌浏览器残留修复：`ElectronEmbeddedBrowserHost` 现在为每个 entry 监听宿主窗口 `webContents.did-start-loading`，主窗口刷新、强刷或 renderer 重新加载时立即销毁对应 native `WebContentsView`，避免 React cleanup 未执行时旧网页继续盖在新 renderer 上；destroy / reparent 会同步摘掉 `closed` 和 `did-start-loading` 监听。未新增 IPC command、App Server method、renderer mock 或浏览器 backend；定向 Host Vitest、`typecheck:electron` 和标准 `verify:gui-smoke` 已过。

当前非本轮阻塞：`npm run test:contracts` 仍会在 `scripts/check-app-server-client-contract.mjs` 阶段失败，失败项均指向 `ember-rs/crates/agent/src/request_tool_policy.rs` 的 WebSearch preflight contract snippet 缺口；本轮 Browser Host 右键菜单 / 下载没有修改该文件，也没有新增命令。Browser 相关命令边界已用 `node scripts/check-command-contracts.mjs` 单独验证通过。

## 7. 下一刀

1. Agent App Browser intent UI 接入：App 触发 `lime.browser/open` 后由宿主把 intent 投递到 Right Surface Browser，并用 GUI/fixture 证明浏览器可见打开。
2. Playwright 覆盖真实网页工作流：打开 URL、Agent observe、人工接管、继续执行、生成 evidence。
