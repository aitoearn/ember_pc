...
| Wave     | 状态    | 当前证据/下一步                                                       |
| -------- | ------- | --------------------------------------------------------------------- |
| Wave 0   | pending | 上一候选已失效；完成 parity closeout 和扩大验证后生成全新候选         |
...
- 已删除 `runtime_backend/tests.rs` 中零引用的 `TestMcpAutostartDataSource` 死夹具。权威
  `npm run test:rust:related -- ember-rs/crates/app-server/src/runtime_backend/tests.rs` 通过
...
- 2026-07-16 并行写集复核：相邻进程独占
  `ember-rs/crates/app-server/src/runtime/conversation_import/**` 及其直接 read-model 修复，本 Gate
