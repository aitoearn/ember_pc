...
- `release metadata`：`package.json`、`packages/ember-cli-npm/package.json`、`ember-rs/Cargo.toml`、`ember-rs/Cargo.lock`、`RELEASE_NOTES.md`、`RELEASE_NOTES.en.md` 与本计划。
...
- `cargo check --manifest-path "ember-rs/Cargo.toml" -p agent-runtime`：通过；修复 provider turn 新增 `FinishReason` 映射时遗漏的 import。
