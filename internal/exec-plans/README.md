...
- Ember 多模态运行合同实施计划：`internal/exec-plans/multimodal-runtime-contract-plan.md`
...
- 端自动化统一（agent-device + scrcpy，去 AutoGLM sidecar）执行计划：`internal/exec-plans/device-automation-unify-agent-device-scrcpy-plan.md`
- 端自动化统一进度日志：`internal/exec-plans/device-automation-unify-agent-device-scrcpy-progress.md`
- 移动端性能监控 P1 执行计划：`internal/exec-plans/device-performance-monitor-plan.md`
- 确定性可复现测试流与自愈回放执行计划：`internal/exec-plans/deterministic-flow-self-healing-plan.md`
