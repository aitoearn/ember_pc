...
```bash
npm run test:rust:related -- ember-rs/crates/app-server ember-rs/crates/core
```
...
- 验证记录：
  ...
  - 未通过但非本轮写集：`npm run test:contracts` 失败在 App Server / Agent / MCP contract 缺口，缺失文件和字符串集中于 `ember-rs/crates/app-server/**`、`ember-rs/crates/agent/**`、`ember-rs/crates/app-server-protocol/**`、`packages/app-server-client/**`，不涉及本轮专家入口写集。
...
- 验证记录：
  ...
  - 通过：`npm run typecheck`。
  - 通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server read_session_projects_thread_expert_role_switch_metadata_into_items_and_evidence`，1 个定向测试通过。
  ...
  - 未完成但非代码失败：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime::tests::read_model::` 等待 `ember-rs/target` artifact 锁超过数分钟；进程盘点显示已有其它 Cargo 测试 / 构建占用同一 target，本轮中断该额外模块级验证。
...
- P2-C 验证记录：
  - 通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server automation_execution`，4 个自动化执行测试通过。
  - 通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server validate_agent_turn_payload`，2 个自动化写入边界测试通过。
  - 通过：`npx vitest run "src/lib/governance/projectThreadFirstBoundary.test.ts"`，1 个文件 / 7 个测试通过。
  - 通过：`npm run typecheck`。
  - 通过：`rustfmt --edition 2021 --check "ember-rs/crates/app-server/src/automation_execution.rs" "ember-rs/crates/app-server/src/local_data_source/automation.rs"`。
- P2-C 前端创建入口收口：
...
- P2-D Skills evidence/export 验证记录：
  - 通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server export_evidence_records_skill_invocation_from_tool_metadata`，1 个 evidence export 定向测试通过。
...
- P2-E 插件验证记录：
  - 通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server export_evidence_pack_includes_plugin_agent_task_events_and_refs`，1 个 evidence export 定向测试通过。
...
- P2-F Browser evidence/export 验证记录：
  - 通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server export_evidence_pack_includes_browser_session_and_snapshot_artifacts`，1 个 Browser evidence export 定向测试通过。
...
- P3-B 后端 evidence 验证记录：
  - 通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server export_evidence_pack_includes_multi_agent_team_facts`，1 个 Team facts evidence/export 定向测试通过。
...
- P2-G Automation smoke 分层验证记录：
  ...
  - 通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent register_lime_project_skill_from_directory_registers_project_namespace`。
  - 通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server apply_runtime_enable_registers_workspace_skill_source`。
  - 通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server export_evidence_marks_completed_with_workspace_skill_and_artifact`。
  - 通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server export_evidence_records_skill_invocation_from_tool_metadata`。
