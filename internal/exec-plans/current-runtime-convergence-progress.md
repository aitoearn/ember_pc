...
- [x] `cargo metadata --manifest-path "ember-rs/Cargo.toml" --no-deps`：34 个 workspace package，未发现已退役 runtime 或 compat package。
...
- [x] `CARGO_TARGET_DIR="/tmp/lime-current-runtime-check" cargo check --manifest-path "ember-rs/Cargo.toml" -p lime-agent`：通过；当前 runtime owner 可独立编译。
- [x] `CARGO_TARGET_DIR="/tmp/lime-current-app-server-check" cargo check --manifest-path "ember-rs/Cargo.toml" -p app-server`：通过；`RuntimeRequest` 已替代 host JSON 中的 provider/model/turn 配置。
...
- [ ] 收回 `scripts/check-command-contracts.mjs` 中把已删除 `ember-rs/src/commands/**` 机械改名为虚构 `agent_cmd.rs` 的 guard；改为对已删除路径的负向存在性与 App Server current owner 守卫。当前文件由并行线程持有，不夹写。
