...
- `current`：`model/list`、`modelProvider/*`、`modelProviderKey/*` App Server JSON-RPC methods，`ModelRegistryService`，`ember-rs/crates/core/src/models/model_registry.rs`，RuntimeCore / App Server protocol。
...
- `dead`：旧 Tauri Provider facade、`ember-rs/src/**`、`ember-rs/resources/models` 本地 catalog、旧 `get_api_key_providers` / `get_system_provider_catalog` 命令族。
...
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-protocol`
- `cargo check --manifest-path "ember-rs/Cargo.toml" -p app-server`
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server model_route_assembly -- --nocapture`
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::model_route_resolver -- --nocapture`
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::model_route_contract -- --nocapture`
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::model_routing -- --nocapture`
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server model_task_contract -- --nocapture`
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server media_task_payload -- --nocapture`
...
- `cargo check --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime`
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime model_route -- --nocapture`
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime --test model_route_execution -- --nocapture`
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime capability_gap -- --nocapture`
...
- 验证结果：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-protocol`、`cargo check --manifest-path "ember-rs/Cargo.toml" -p app-server`、`npm run check:protocol-types`、`npm run test:contracts`、`npm run typecheck:electron`、`npx vitest run "electron/hostCommands.test.ts"`、`npx vitest run "src/lib/api/modelRegistry.test.ts" "src/lib/api/apiKeyProvider.test.ts"`、`cargo fmt --manifest-path "ember-rs/Cargo.toml" --all --check`、`npx prettier --check ...` 均通过。
...
- 验证结果：`cargo fmt --manifest-path "ember-rs/Cargo.toml" --all --check`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::model_route_contract -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server media_task::tests -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime::tests::read_model::read_session_projects_model_routing_into_thread_read -- --nocapture`、`cargo check --manifest-path "ember-rs/Cargo.toml" -p app-server`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-protocol`、`npm run check:protocol-types`、`npm run test:contracts` 均通过。
...
- 验证结果：`cargo fmt --manifest-path "ember-rs/Cargo.toml" --all --check`、`cargo check --manifest-path "ember-rs/Cargo.toml" -p app-server`、`cargo check --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server model_task_contract -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server media_task_payload -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::model_route_contract -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime::tests::read_model::read_session_projects_model_routing_into_thread_read -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime capability_gap -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-protocol`、`npm run check:protocol-types`、`npm run test:contracts` 均通过。
...
- 验证结果：`cargo fmt --manifest-path "ember-rs/Cargo.toml" --all --check`、`cargo check --manifest-path "ember-rs/Cargo.toml" -p app-server`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::model_route_resolver -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::model_route_contract -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::model_routing -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime::tests::read_model::read_session_projects_model_routing_into_thread_read -- --nocapture` 均通过。
...
- 验证结果：`cargo fmt --manifest-path "ember-rs/Cargo.toml" --all --check`、`cargo check --manifest-path "ember-rs/Cargo.toml" -p app-server`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server model_route_assembly -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::model_route_resolver -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::model_route_contract -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime::tests::read_model::read_session_projects_model_routing_into_thread_read -- --nocapture`、`npm run test:contracts` 均通过。
...
- 验证结果：`cargo fmt --manifest-path "ember-rs/Cargo.toml" --all --check`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server model_route_assembly -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server media_task_payload -- --nocapture`、`cargo check --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime model_route -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime --test model_route_execution -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime capability_gap -- --nocapture` 均通过。
...
- 验证结果：`cargo fmt --manifest-path "ember-rs/Cargo.toml" --all --check`、`cargo check --manifest-path "ember-rs/Cargo.toml" -p app-server`、`cargo check --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server model_route_execution -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server media_task_payload -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime model_route -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime --test model_route_execution -- --nocapture` 均通过。
...
- 验证结果：`rustfmt --edition 2021 --check "ember-rs/crates/media-runtime/src/model_route.rs" "ember-rs/crates/media-runtime/src/lib.rs" "ember-rs/crates/media-runtime/src/video_worker.rs" "ember-rs/crates/media-runtime/tests/model_route_execution.rs" "ember-rs/crates/app-server/src/model_route_execution.rs"`、`CARGO_TARGET_DIR="/tmp/lime-model-route-target" cargo check --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime`、`CARGO_TARGET_DIR="/tmp/lime-model-route-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime model_route -- --nocapture`、`CARGO_TARGET_DIR="/tmp/lime-model-route-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime --test model_route_execution -- --nocapture`、`CARGO_TARGET_DIR="/tmp/lime-model-route-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime capability_gap -- --nocapture`、`CARGO_TARGET_DIR="/tmp/lime-model-route-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server model_route_execution -- --nocapture`、`CARGO_TARGET_DIR="/tmp/lime-model-route-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server media_task_payload -- --nocapture` 均通过。使用独立 target 目录是为了避开当前工作区已有长时间 cargo 进程持有共享 target 锁。
...
- 验证结果：`npx prettier --check "internal/exec-plans/models-runtime-unification-plan.md" "internal/roadmap/models/prd.md"`、`rustfmt --edition 2021 "ember-rs/crates/app-server/src/local_data_source/media_tasks.rs" "ember-rs/crates/app-server/src/local_data_source/impls/media.rs" "ember-rs/crates/app-server/src/media_task.rs" "ember-rs/crates/app-server/src/media_task_payload.rs"`、`CARGO_TARGET_DIR="/tmp/lime-model-route-target" cargo check --manifest-path "ember-rs/Cargo.toml" -p app-server`、`CARGO_TARGET_DIR="/tmp/lime-model-route-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server media_task_payload -- --nocapture`、`CARGO_TARGET_DIR="/tmp/lime-model-route-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server model_route_execution -- --nocapture` 均通过。
...
- 验证结果：`rustfmt --edition 2021 "ember-rs/crates/app-server/src/media_runtime_contract.rs" "ember-rs/crates/app-server/src/lib.rs" "ember-rs/crates/app-server/src/media_task_payload.rs" "ember-rs/crates/app-server/src/media_task.rs"`、`CARGO_TARGET_DIR="/tmp/lime-model-route-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server media_runtime_contract -- --nocapture`、`CARGO_TARGET_DIR="/tmp/lime-model-route-target" cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server media_task_payload -- --nocapture`、`CARGO_TARGET_DIR="/tmp/lime-model-route-target" cargo check --manifest-path "ember-rs/Cargo.toml" -p app-server`、`git diff --check -- "ember-rs/crates/app-server/src/media_runtime_contract.rs" "ember-rs/crates/app-server/src/media_task_payload.rs" "ember-rs/crates/app-server/src/media_task.rs" "ember-rs/crates/app-server/src/lib.rs"` 均通过。
...
- Active 文档清理：`internal/roadmap/warp/execution-profile.md`、`implementation-plan.md`、`README.md`、`internal/aiprompts/commands.md`、`quality-workflow.md` 不再宣称配音媒体 worker / 本地 ServiceSkill 已真实执行音频；`internal/aiprompts/prompt-foundation.md` 顺手收掉已删除 `ember-rs/src/**` 作为 Prompt current owner 的旧路回流，改指向 App Server request context 与 `ember-rs/crates/agent`。
