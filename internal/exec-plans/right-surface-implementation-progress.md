...
```bash
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server workspace_right_surface
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-client
cargo run --manifest-path "ember-rs/Cargo.toml" -p app-server-protocol --bin write_schema_fixtures
...
npm run generate:protocol-types
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server workspace_right_surface
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-client workspace_helpers_use_current_methods
...
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-client reexports_protocol_method_catalog_for_consumers
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server workspace_right_surface
npx vitest run "packages/app-server-client/tests/client.test.mjs"
npx vitest run "src/lib/api/workspaceRightSurface.test.ts" "src/lib/api/appServer.test.ts"
node "scripts/check-app-server-client-contract.mjs"
cargo fmt --manifest-path "ember-rs/Cargo.toml" --all
npm run test:contracts
git diff --check
```
...
- 已通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server workspace_right_surface`，10 个 Right Surface server 测试通过。
- 已通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-client workspace_helpers_use_current_methods`，1 个 Rust app-server-client workspace helper 测试通过。
...
- 已通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-protocol app_server_method_catalog_keeps_request_and_notification_methods_together`，1 个 protocol catalog 测试通过。
- 已通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-client reexports_protocol_method_catalog_for_consumers`，1 个 Rust client catalog 测试通过。
- 已通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server workspace_right_surface`，10 个 Right Surface server 测试通过。
...
- 已通过：`node "scripts/check-app-server-client-contract.mjs"`，`app-server-client-contract` 282 项通过；已通过：`cargo fmt --manifest-path "ember-rs/Cargo.toml" --all`、`npm run test:contracts` 与 `git diff --check`。
...
- 已通过：`cargo fmt --manifest-path "ember-rs/Cargo.toml" --all`。
- 已通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server workspace_right_surface`，11 个 Right Surface server / processor 测试通过。
- 已通过：`git diff --check`。
...
- 已通过：`npx vitest run "scripts/agent-runtime/expert-skills-live-runner.test.mjs"`，8 个测试通过；已通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::agent_skills_context -- --nocapture`，9 个测试通过；已通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_backend::agent_skills_telemetry -- --nocapture`，4 个测试通过；已通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server session_config_does_not_project_expert_runtime_enable_allowed_tools_to_turn_scope -- --nocapture`，1 个测试通过；已通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent tools::skill_search_tool -- --nocapture`，3 个测试通过；已通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent tools::skill_tool_gate -- --nocapture`，11 个测试通过。
- 真实 live Provider 结果：
...
- 完成 P4 objectCanvas `persistRequested` 正式 snapshot store 第一刀：
  ...
  - 已通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server local_data_source::tests::right_surface_object_canvas_snapshot_persists_in_local_sqlite_data_source -- --nocapture`，1 个真实 Local SQLite 测试通过。
  - 已通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime::tests::right_surface -- --nocapture`，11 个 Right Surface Runtime 测试通过。
  - 已通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server right_surface -- --nocapture`，14 个 Right Surface Runtime / processor / Local SQLite 测试通过。
...
- 完成 P4 objectCanvas `replayRequested` Runtime consumer 骨架第一刀：
  ...
  - 已通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server right_surface -- --nocapture`，16 个 Right Surface Runtime / processor / Local SQLite 测试通过。
- 完成 P4 objectCanvas `replayRequested` Runtime dry-run / audit 骨架：
  ...
  - 已通过：`cargo fmt --manifest-path "ember-rs/Cargo.toml" --all`。
  - 已通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server right_surface -- --nocapture`，19 个 Right Surface Runtime / processor / Local SQLite 测试通过。
