...
3. 不恢复 `ember-rs/src/**`、旧 Tauri wrapper、旧 `agent_runtime_*` 生产事实源或 mock fallback。
...
- `ember-rs/crates/app-server/src/runtime_backend/plan_events.rs`
- `ember-rs/crates/app-server/src/runtime_backend/reasoning_events.rs`
...
- 2026-06-23：
  ...
  - `npx vitest run "src/components/agent/chat/utils/planState.unit.test.ts" "src/components/agent/chat/utils/modelReasoningState.unit.test.ts" --silent=passed-only --disableConsoleIntercept`
    - 结果：通过，2 个文件 4 个测试。
  - `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server model_capability`
    - 结果：通过，3 个测试。
  - `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server plan_events`
    - 结果：通过，1 个测试。
  - `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server reasoning_events`
    - 结果：通过，1 个测试。
  - `git diff --check -- ".gitignore" "ember-rs/crates/app-server/src/runtime_backend.rs"`
    - 结果：通过，已跟踪改动无空白错误。
  - `rg -n "[ \t]+$" <P0 新增文件>`
    - 结果：无匹配，新增文件无尾随空白。
  ...
  - `cargo fmt --manifest-path "ember-rs/Cargo.toml" --all`
    - 结果：通过。
  - `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server proposed_plan_parser`
  ...
  - `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server tool_events`
  ...
  - `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server model_effective`
  ...
  - `cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent test_convert_item_started_reasoning_runtime_item_preserves_summary`
    - 结果：通过，1 个测试；覆盖 `ItemRuntimePayload::Reasoning.metadata` 透传到 `AgentThreadItemPayload::Reasoning.metadata`。
  - `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server reasoning_item_payload_metadata_is_preserved_in_thread_items`
    - 结果：通过，1 个测试；覆盖 App Server thread read model 保留 reasoning item payload 中的 `metadata.provider_metadata` 与 provider 原生字段。
  - `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server reasoning_final_provider_metadata_is_projected_to_thread_item_metadata`
    - 结果：通过，1 个测试；覆盖标准 `reasoning.final` payload 中的 `providerMetadata` 进入历史 thread item metadata，并标记 completed。
  - `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server runtime_thinking_delta_emits_reasoning_lifecycle_events`
    - 结果：通过，1 个测试；确认 `reasoning.started/delta/final/ended` lifecycle 事件序列保持稳定。
  - `CARGO_TARGET_DIR="/Users/coso/Documents/dev/ai/aiclientproxy/lime/ember-rs/target" cargo test --manifest-path "ember-rs/crates/agent-rust/Cargo.toml" -p agent-core test_project_message_emits_reasoning_summary_runtime_item`
    - 结果：通过，1 个测试；覆盖 Agent projector 将 thinking signature 保真投影到 reasoning metadata。显式设置 `CARGO_TARGET_DIR`，未生成 `ember-rs/crates/agent-rust/target`。
  ...
  - `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server plan_item_preserves_revision_source_and_plan_metadata`
