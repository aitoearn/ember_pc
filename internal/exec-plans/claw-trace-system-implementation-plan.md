...
- `ember-rs/crates/app-server/src/runtime/trace_store/export.rs`
- `ember-rs/crates/app-server/src/runtime/trace_store/summary.rs`
- `ember-rs/crates/app-server/src/runtime/storage_roots.rs`
- `ember-rs/crates/app-server/src/local_data_source/diagnostics/support_bundle.rs`
- `ember-rs/crates/app-server/src/local_data_source/diagnostics/support_bundle/trace_attachment.rs`
- `ember-rs/crates/app-server/src/runtime_backend/tool_events.rs`
- `ember-rs/crates/app-server/src/runtime/tests/turn_lifecycle.rs`
...
- `ember-rs/crates/agent-rust/crates/agent/src/agents/agent.rs`
- `ember-rs/crates/agent-rust/crates/agent/src/agents/provider_trace.rs`
- `ember-rs/crates/agent-rust/crates/agent/src/agents/mod.rs`
- `ember-rs/crates/agent-rust/crates/agent/src/agents/subagent_handler.rs`
- `ember-rs/crates/agent/src/protocol.rs`
...
- `ember-rs/crates/agent-rust/crates/agent/src/agents/agent.rs` 当前仍约 `9117` 行，远超仓库体量边界。本轮 S20 已先把 provider trace event model 拆到 `agents/provider_trace.rs`，避免继续把 trace schema 堆进中心 reply loop 文件。
...
```bash
cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server trace
```
...
- 完成 S3 App Server 最小 checkpoint：
  - 新增 `ember-rs/crates/app-server/src/runtime/trace.rs`，从 `turn_runtime_options.metadata.agentUiPerformanceTrace` 抽取 `traceId / runId / requestId / submittedAt`。
...
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server trace_metadata_is_attached_to_runtime_events`。
- 验证通过：`rustfmt --edition 2021 --check "ember-rs/crates/app-server/src/runtime/trace.rs" "ember-rs/crates/app-server/src/runtime/event_store.rs" "ember-rs/crates/app-server/src/runtime/tests/turn_lifecycle.rs"`。
...
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server provider_trace`，2 个 provider trace 定向用例通过。
...
- 验证通过：`rustfmt --edition 2021 "ember-rs/crates/app-server/src/runtime/trace_store.rs"`。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server trace_writer -- --nocapture`，2 个 writer 用例通过。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server trace`，9 个 trace 相关用例通过。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server storage_roots`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server initialize_database_uses_configured_data_dir`，覆盖 `runtime/traces` 路径派生与初始化接线。
...
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server support_bundle -- --nocapture`，2 个 support bundle 用例通过。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server trace -- --nocapture`，11 个 trace 相关用例通过。
...
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-protocol catalog -- --nocapture`、`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-protocol schema_fixtures -- --nocapture`。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server trace -- --nocapture`，13 个 trace 相关用例通过。
- 验证通过：`npm run test:contracts`。
...
- 验证通过：`cargo fmt --manifest-path "ember-rs/Cargo.toml" --package app-server-protocol --package app-server`。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server support_bundle -- --nocapture`，3 个 support bundle 用例通过。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server trace -- --nocapture`，14 个 trace 相关用例通过。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server-protocol schema_fixtures -- --nocapture`，4 个 schema fixture 用例通过。
- 验证通过：`npm run test:contracts`。
...
- 验证通过：`cargo fmt --manifest-path "ember-rs/Cargo.toml" --package app-server`。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server support_bundle -- --nocapture`，4 个 support bundle 用例通过。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server trace -- --nocapture`，15 个 trace 相关用例通过。
- 验证通过：`npx vitest run "scripts/agent-runtime/claw-chat-current-fixture-smoke.test.mjs"`，20 个 smoke guard 用例通过。
...
- 验证通过：`cargo fmt --manifest-path "ember-rs/Cargo.toml" --package app-server`。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server trace -- --nocapture`，16 个 trace 相关用例通过。
- 验证通过：`npm run test:contracts`。
...
- 验证通过：`cargo fmt --manifest-path "ember-rs/Cargo.toml" --package app-server`。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server trace_context -- --nocapture`，4 个用例通过，覆盖共用 W3C parser 与既有 runtime invalid propagation。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server request_trace -- --nocapture`，3 个 request span metadata 用例通过。
...
- 验证通过：`cargo fmt --manifest-path "ember-rs/Cargo.toml" --package app-server`。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server otel_trace -- --nocapture`，1 个 OTEL context 用例通过。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server request_trace -- --nocapture`，5 个 request trace 用例通过。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server trace -- --nocapture`，24 个 trace 相关用例通过。
- 验证通过：`npm run test:contracts`。
- 验证通过：`npm run smoke:agent-runtime-current-fixture`，覆盖 current Agent Runtime / Claw GUI Electron fixture；`liveProviderUsed=false`。
...
- 验证通过：`cargo fmt --manifest-path "ember-rs/Cargo.toml" --package app-server`、`cargo fmt --manifest-path "ember-rs/crates/agent-rust/Cargo.toml" --package agent-core`。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server w3c_trace_context -- --nocapture`，5 个 W3C / trace context 相关用例通过。
- 验证通过：`CARGO_TARGET_DIR="ember-rs/target" cargo test --manifest-path "ember-rs/crates/agent-rust/crates/agent/Cargo.toml" w3c_trace_context -- --nocapture`、`test_request_correlation_headers_injection`、`test_request_correlation_context_rejects_invalid_w3c_traceparent`。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server trace -- --nocapture`，26 个 trace / OTEL / support bundle / provider phase 用例通过。
- 验证通过：`npm run test:contracts`。
- 验证通过：`npm run smoke:agent-runtime-current-fixture`，覆盖 current Agent Runtime / Claw GUI Electron fixture；`liveProviderUsed=false`。
...
- 验证通过：`cargo fmt --manifest-path "ember-rs/Cargo.toml" --package app-server --package lime-agent`、`cargo fmt --manifest-path "ember-rs/crates/agent-rust/Cargo.toml" --package agent-core`。
- 验证通过：`CARGO_TARGET_DIR="ember-rs/target" cargo test --manifest-path "ember-rs/crates/agent-rust/crates/agent/Cargo.toml" provider_response_context -- --nocapture`，3 个 request id / provider context 用例通过。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server provider_trace -- --nocapture`，2 个 provider trace 用例通过。
- 验证通过：`cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-agent provider_trace -- --nocapture`，协议 crate provider_trace 过滤编译通过。
