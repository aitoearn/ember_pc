...
> 状态：Draft
> 更新时间：2026-04-06
> 适用范围：`image_generate` skill、`ember-cli`、媒体任务协议、聊天区、图片工作台
...
- `ember media image generate --json`
- `ember task create image --json`（兼容入口，但必须复用同一条执行链）
...
2. `ember media image generate --json` 与 `ember task create image --json` 都返回稳定 JSON，且共享同一条图片执行链。
3. `ember task status` 能读到标准状态。
4. `ember task result` 能读到标准结果结构。
