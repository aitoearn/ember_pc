...
> 更新时间：2026-06-07
> Owner：Ember Runtime / App Server / Claw / Agent Apps

## 1. 一句话目标

把 Ember 打造成以 App Server / RuntimeCore 为底座、以 sandbox-first 权限模型为执行安全主轴、以 Claw 为旗舰体验、以共享 AgentRuntime projection 和 UI primitives 为端侧复用层、可演进到服务端运行形态和移动 / 小程序入口的多端 Agent 平台。

## 2. 产品目标

1. 让 Ember Desktop / Claw 继续承载最完整的 Agent 工作台体验。
2. 让 content-studio 和未来独立 App 不需要自建 AgentRuntime，就能获得会话、turn、工具、审批、artifact、evidence 和事件流能力。
3. 让 Agent Apps 既可以在 Ember Desktop 中运行，也可以向独立壳演进，但 runtime 能力仍受 Ember App Server 治理。
...
| 独立 App 开发者 | 快速接入 Ember Agent 能力 | TypeScript client、protocol schema、projection SDK 和可选 UI 组件。 |
...
2. 用户身份通过微信 OpenID / unionId 与 Ember 账号或租户绑定。
...
5. 独立 App 不直接链接 Ember Rust workspace。
...
4. 不让独立 App 直接 import Ember Rust crate 或 Claw 内部组件树。
