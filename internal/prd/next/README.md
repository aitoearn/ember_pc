...
> 更新时间：2026-06-07
> Owner：Ember Runtime / App Server / Claw / Agent Apps / 独立 App 集成

## 1. 定位

本目录定义 Ember 下一阶段的北极星：把 Ember 从单一桌面产品内的 Agent 工作台，升级为以 `ember-rs` App Server 为底座、以 Claw 为旗舰体验、以 Agent Apps、独立 App、移动 App、微信小程序和服务端运行形态为扩展面的多端 Agent 平台。

本目录不是替代 `internal/roadmap/appserver/` 的执行路线图，而是上层产品与架构对齐文档。具体 App Server current 实施仍以 `internal/roadmap/appserver/` 为准。

## 2. 一句话方向

Ember Next 要把 `ember-rs` 抽成可被 Ember Desktop、Claw、Agent Apps、content-studio、移动 App、微信小程序、未来独立 App 和受控服务端运行形态复用的 Agent Runtime 服务底座；安全模型采用 sandbox-first，客户端和服务端都必须通过 permission profile、sandbox manager、approval / escalation 和 audit 执行工具；前端复用以 headless projection 和 UI primitives 为核心，不复制 Claw 整页产品壳。
...
| 分类 | 对象 | 说明 |
| --- | --- | --- |
| `current` | `internal/prd/next/*` | Ember Next 北极星、产品目标、共享边界和验收原则。 |
| `current` | `internal/roadmap/appserver/*` | App Server current 执行路线图和协议事实源。 |
| `current target` | `ember-rs` App Server / RuntimeCore / ExecutionBackend | 跨 App 复用的服务端底座。 |
...
| `current shell` | Claw / Ember Desktop | 旗舰 Agent 工作台和多 App 桌面壳。 |
| `current shell` | content-studio / Agent App shell | 通过 client、projection 和 UI primitives 消费 runtime 的独立业务壳。 |
...
| `dead` | 独立 App copy Claw 整页 UI 或自建完整 Agent runtime | 不再作为 Ember 生态扩展方向。 |

## 4. 文档索引

| 文档 | 作用 |
| --- | --- |
| [background.md](./background.md) | 背景、Codex-rs 借鉴、Ember 当前问题和战略判断。 |
...
| [sequences.md](./sequences.md) | 初始化、Claw、Agent App、content-studio、projection、artifact/evidence 时序图。 |
...
| [prototype.md](./prototype.md) / [prototype.html](./prototype.html) | Ember Next 多端产品原型、交互契约和可打开的静态可视化原型。 |
...
1. 先读 [background.md](./background.md)，确认为什么 Ember Next 必须服务化和组件化。
...
1. `ember-rs` 从桌面内部实现升级为跨 App、跨端、可服务端化的 Runtime 底座。
2. Claw 从单一页面升级为 Ember AgentRuntime 的旗舰体验和共享组件来源。
3. 客户端和服务端执行安全统一到 permission profile、sandbox manager、approval / escalation 与 audit，而不是散落在各端。
4. Agent Apps / content-studio / 移动 App / 微信小程序 / 未来独立 App 不再复制 runtime，而是复用协议、client、projection、远程 gateway 和可组合 UI。
...
App Server facts
  ...
  -> UI primitives
  -> Claw / Agent App / content-studio shell adapters
```
