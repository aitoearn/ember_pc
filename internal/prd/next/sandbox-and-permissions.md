...
> 更新时间：2026-06-07  
> Owner：Ember Runtime / App Server / Security Boundary

## 1. 结论

Ember Next 必须采用 **sandbox-first** 架构。Sandbox 不是服务端专属能力，也不是 Docker / Kubernetes 的同义词；它是客户端、本地 sidecar、服务端 worker、移动审批入口和小程序 gateway 共同遵守的执行安全主轴。
...
| `protocol/src/permissions.rs` | permission profile、FS / network policy、protected metadata、deny-read fail-closed | Ember 需要明确 profile、FS、network、metadata 保护和拒读语义。 |
...
| `sandboxing/src/manager.rs` | 选择 macOS Seatbelt、Linux seccomp / bubblewrap、Windows restricted token，并把 command + permissions transform 成 exec request | Ember 需要平台 sandbox manager，而不是在 backend 里散落判断。 |
...
Codex 的另一个关键点：当 filesystem policy 有 denied reads 时，不能简单绕过 sandbox 运行。因为 denied-read 只有在 sandbox 内才可执行，绕过 sandbox 会悄悄扩大读取范围。Ember Next 必须保留这个 fail-closed 口径。

## 3. Ember Next Sandbox 分层图

```mermaid
flowchart TB
    subgraph Entry["入口层"]
        Desktop["Claw / Desktop / Agent App"]
...
客户端也必须有 sandbox。没有客户端 sandbox，Ember Desktop / Claw / 本地 Agent App 就会在用户机器上以 UI 壳权限直接执行工具，无法证明 workspace、metadata、secret 和网络边界。
...
4. protected metadata 默认只读：`.git`、`.agents`、`.codex` 或 Ember 等价目录。
...
Ember Next 不应把 Docker / Kubernetes 当成 sandbox 抽象本身。建议模型：
