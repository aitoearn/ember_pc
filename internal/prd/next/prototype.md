...
> 更新时间：2026-06-07  
> 目标：把 Ember Next 从架构文字推进到可讨论的产品界面原型，覆盖 Claw 桌面端、AgentRuntime 共享 UI、服务端控制台、移动 App、微信小程序和客户端 / 服务端基础设施边界。
...
Ember Next 的产品原型不是一个新首页，也不是营销页。它是一套多端工作面：
...
主页面骨架：
桌面端保留 Ember 左侧导航 + 顶部上下文 + 主工作区 + 右侧详情栏。
...
目标：让团队一眼看到 Ember Next 是一条 Runtime、多端入口、两类 adapters。
