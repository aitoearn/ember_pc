...
过去这些能力主要服务 Lime Desktop 和 Claw 工作台。随着 Plugins、content-studio、独立 App、服务端运行形态、移动 App、微信小程序和更多垂直工作流出现，继续把 runtime 绑定在单个桌面壳内会带来明显问题：
...
6. 后续替换壳层、接入独立 App、扩展移动端或升级 Plugin 发布形态时，每次都要重新适配 runtime。
...
app-server-protocol
  ...
  -> RuntimeCore
  -> ExecutionBackend
  -> Lime Desktop / Claw / Plugins / content-studio / Mobile / Mini Program / Server Mode
```
...
1. **服务端资产**：`ember-rs`、App Server crate 家族、RuntimeCore、ExecutionBackend、RuntimeBackend、Tool / Skill / Evidence / Artifact 能力。
...
5. Plugins / content-studio / 未来独立 App 通过 client、projection、UI primitives 复用能力。
...
5. 不让 Plugins 用 UI runtime 生命周期替代 Agent turn runtime。
...
4. Claw shell、Plugin shell、content-studio shell、移动 shell、小程序 shell 各自负责导航、业务对象和宿主能力。
