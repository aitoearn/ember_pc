...
Claw 整页 shell 继续留在 Ember Desktop；Agent Apps、content-studio、未来独立 App、移动 App 和微信小程序只复用 projection、状态机、设计合同和可组合 UI。移动 App 与小程序不强行复用 React DOM 组件。
...
| Claw workspace shell / sidebar / model selector / settings entry | 不共享 | 绑定 Ember Desktop 产品壳和导航。 |
| Electron bridge / `safeInvoke` / legacy command | 不共享 | 属于宿主适配层，不是 UI primitive。 |
| 业务对象面板 | 不共享 | content-studio / Agent Apps 各自拥有业务语义。 |
...
5. Agent App UI runtime start/status/stop 被当作对话 runtime。
