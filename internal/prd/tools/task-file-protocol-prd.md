# Ember 统一 Task File 协议 PRD

更新时间：2026-04-03

## 1. 背景

Ember 当前已经具备一套轻量 task file 底座，能够覆盖：
...
现有协议已经解决了“任务创建、状态读取、简单重试”的基础问题，但当 Ember 开始把图片生成、图片编辑、视频生成、视频编辑、自动化工作流、聊天区动态渲染统一收口到同一条主线时，现有 task file 还存在四类缺口：
...
```json
{
  ...
  "logs_ref": ".ember/tasks-logs/task_01HQ.../attempt_2.jsonl"
}
```
  ...
  "images": [
    {
      ...
      "size_bytes": 734002,
      "storage_ref": "s3://...",
      "preview_ref": ".ember/previews/asset_img_01.json"
    }
  ],
  "billing": {
    "provider_cost": 0.04
...
[
  {
    ...
    "path": ".ember/task-artifacts/task_01/preview.json"
  },
  {
    "artifact_id": "artifact_log_01",
    "kind": "attempt_log",
    "path": ".ember/task-logs/task_01/attempt_2.jsonl"
  }
]
```
...
- `.ember/tasks/image/image_generate/<task_id>.json`
- `.ember/tasks/image/image_edit/<task_id>.json`
- `.ember/tasks/video/video_generate/<task_id>.json`
- `.ember/tasks/video/video_edit/<task_id>.json`

相关外置目录：

- `.ember/task-logs/<task_id>/attempt_<n>.jsonl`
- `.ember/task-artifacts/<task_id>/...`
- `.ember/previews/<asset_id>.json`
...
- `ember task create <type>`
- `ember task status <task-ref>`
- `ember task result <task-ref>`
- `ember task attempts <task-ref>`
- `ember task retry <task-ref>`
- `ember task cancel <task-ref>`
- `ember task list --family image --type image_edit --status running`
- 可选：`ember task graph <task-ref>`

## 13.2 前端 / Tauri

前端统一通过 API 网关读取任务：

- `ember_task_get_status`
- `ember_task_get_result`
- `ember_task_get_attempts`
- `ember_task_list`
- `ember_task_retry`
- `ember_task_cancel`
...
task file 应该被设计成 Ember 的统一任务协议，而不是某个业务临时写出的 JSON 记录。
...
这样 Ember 才能把图片生成、图片编辑、视频生成、视频编辑、聊天区动态渲染、CLI、worker、统一任务面板全部收敛到同一条主链。
