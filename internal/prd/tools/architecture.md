...
路径：`ember-rs/crates/core/src/tool_calling.rs`
...
任何地方如果还在自己读 `x-ember` / `x_ember`，都应视为治理退化。

### 2.2 native 目录事实源

路径：`ember-rs/src/agent_tools/catalog.rs`
...
路径：`ember-rs/src/agent_tools/execution.rs`
...
路径：`ember-rs/crates/mcp/src/manager.rs`
...
路径：`ember-rs/src/commands/aster_agent_cmd.rs`
...
- 把 `ember-rs/src/agent_tools/catalog.rs`
- 把 `ember-rs/src/agent_tools/execution.rs`
- 把 `ember-rs/src/agent_tools/inventory.rs`

以模块方式复用到 `ember-agent` crate 内，供纯逻辑单测执行。

注意：

- 它不是新的 runtime 事实源
- 它只是测试载体，避免 `ember` 主包为 Tauri/App wiring 做超大链接
...
graph TD
    ...
    CMD --> META[ember_core::tool_calling.rs]
    CMD --> MCP[ember_mcp::manager.rs]
    CMD --> AGENT[Aster Agent]
    TEST[ember-agent test carrier] -. 复用纯逻辑 .-> CAT
    TEST -. 复用纯逻辑 .-> EXEC
    TEST -. 复用纯逻辑 .-> INV
...
下面是 Ember 启动 Agent 并把 MCP 工具面注入 Aster 的主链路。
...
- 这个工具属于哪个产品 surface
- 是 current 还是 compat
- 是 Aster builtin、Ember injected 还是 Browser compatibility
...
Ember 不需要一比一复制 Codex，但要学到这三个原则：
...
| Codex                           | Ember 对应实现                                                        |
| ------------------------------- | -------------------------------------------------------------------- |
| `DynamicToolSpec.defer_loading` | `x-ember.deferred_loading` + `McpToolDefinition.deferred_loading`     |
