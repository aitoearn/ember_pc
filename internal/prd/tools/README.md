...
- **native 工具目录事实源**：`ember-rs/crates/agent/src/agent_tools/catalog.rs`
- **执行权限事实源**：`ember-rs/crates/agent/src/agent_tools/execution.rs`
- **MCP runtime 工具事实源**：`ember-rs/crates/mcp/src/manager.rs`
- **Agent 注入工具面事实源**：`ember-rs/crates/agent` + App Server `RuntimeBackend`
- **工具库存 / 审计快照事实源**：`ember-rs/crates/agent/src/agent_tools/inventory.rs`
...
| current        | `ember-rs/crates/agent/src/agent_tools/catalog.rs`                 | 完整 native tool 目录与默认授权子集                                                                                    |
| current        | `ember-rs/crates/agent/src/agent_tools/execution.rs`               | 统一 execution 层的 warning / sandbox / 参数限制事实源                                                                 |
| current        | `ember-rs/crates/mcp/src/manager.rs`                               | MCP tools runtime registry                                                                                             |
| current        | `ember-rs/crates/app-server/src/runtime_backend/tool_inventory.rs` | App Server `agentSession/toolInventory/read` 到 runtime inventory builder 的桥接                                       |
| current        | `ember-rs/crates/agent/src/agent_tools/inventory.rs`               | runtime 工具库存快照                                                                                                   |
...
| dead-candidate | `ember-rs/crates/agent/src/tool_permissions.rs`                    | 已退出 `lime-agent` 的 `lib.rs` 编译图，仅通过 `ember-rs/crates/agent/tests/legacy_permission_surfaces.rs` 测试夹具加载 |
| dead-candidate | `ember-rs/crates/agent/src/shell_security.rs`                      | 已退出 `lime-agent` 的 `lib.rs` 编译图，仅通过 `ember-rs/crates/agent/tests/legacy_permission_surfaces.rs` 测试夹具加载 |
...
`ember-rs/crates/agent/src/agent_tools/catalog.rs` 已升级为完整目录，覆盖：
...
- **有效策略解析**：`ember-rs/crates/agent/src/agent_tools/execution.rs::resolve_tool_execution_policy`
...
- **说明**  
  Core surface 现已按 current surface 收敛；精确数量与分类以 `ember-rs/crates/agent/src/agent_tools/catalog.rs` 为准。
