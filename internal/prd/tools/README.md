...
当前 Ember 的工具体系已经不只是单一的 Aster native tools，还同时包含：

- Aster 默认内置工具
- Ember 注入工具
- Workbench 专属工具
- Browser Assist 兼容工具面
- Ember MCP runtime tools
- Aster ExtensionManager 注入后的 prefixed tools

这套能力本身已经接近 Tool Calling 2.0，但过去存在两个核心问题：

1. **工具事实源分裂**
   - MCP schema metadata、Aster runtime registry、Ember 注入 extension、provider 转换层分别做了解析
...
本次治理的目标，是把 Ember 的工具系统收敛到一条清晰主链路，参考 Codex 的思路：
...
- **工具元数据事实源**：`ember-rs/crates/core/src/tool_calling.rs`
- **native 工具目录事实源**：`ember-rs/src/agent_tools/catalog.rs`
- **执行权限事实源**：`ember-rs/src/agent_tools/execution.rs`
- **MCP runtime 工具事实源**：`ember-rs/crates/mcp/src/manager.rs`
- **Aster 注入工具面事实源**：`ember-rs/src/commands/aster_agent_cmd.rs`
- **工具库存 / 审计快照事实源**：`ember-rs/src/agent_tools/inventory.rs`

### 2.3 当前 / 兼容 / 待清理分类

| 分类           | 路径 / 对象                                      | 说明                                                                                                                     |
| -------------- | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------ |
| current        | `ember_core::tool_calling`                        | 统一 metadata 读取与打分                                                                                                 |
| current        | `ember-rs/src/agent_tools/catalog.rs`           | 完整 native tool 目录与默认授权子集                                                                                      |
| current        | `ember-rs/src/agent_tools/execution.rs`         | 统一 execution 层的 warning / sandbox / 参数限制事实源                                                                   |
| current        | `ember-rs/crates/mcp/src/manager.rs`            | MCP tools runtime registry                                                                                               |
| current        | `ember-rs/src/commands/aster_agent_cmd.rs`      | Aster runtime 注入、`ToolSearch`、inventory 命令                                                                         |
| current        | `ember-rs/src/agent_tools/inventory.rs`         | runtime 工具库存快照                                                                                                     |
| compat         | `workspace_allowed_tool_names(...)`              | 当前保留为旧调用入口别名，实际委托默认授权目录                                                                           |
| dead-candidate | `ember-rs/crates/agent/src/tool_permissions.rs` | 已退出 `ember-agent` 的 `lib.rs` 编译图，仅通过 `ember-rs/crates/agent/tests/legacy_permission_surfaces.rs` 测试夹具加载 |
| dead-candidate | `ember-rs/crates/agent/src/shell_security.rs`   | 已退出 `ember-agent` 的 `lib.rs` 编译图，仅通过 `ember-rs/crates/agent/tests/legacy_permission_surfaces.rs` 测试夹具加载 |
...
- `ember-rs/crates/providers/src/providers/openai_custom.rs`
- `ember-rs/crates/providers/src/providers/claude_custom.rs`

### 3.2 native 工具目录补全

`ember-rs/src/agent_tools/catalog.rs` 已升级为完整目录，覆盖：
...
- **持久化覆盖**：`ember-rs/crates/core/src/config/types.rs` -> `NativeAgentConfig.tool_execution`
- **运行时覆盖**：`request.metadata.harness.executionPolicy` / `execution_policy`
- **有效策略解析**：`ember-rs/src/agent_tools/execution.rs::resolve_tool_execution_policy`
...
为避免 `ember` 主包在本地因 Tauri 链接过大而降低回归效率，已补一条对齐 Codex 思路的轻量测试通道：

- `ember-rs/crates/agent/src/lib.rs`
- `ember-rs/crates/agent/src/agent_tools/mod.rs`
...
- 不再通过 `ember-agent` crate 根对外 `pub mod`
- 不再通过 `ember-agent` crate 根对外 `pub use`
- 不再进入 `ember-agent` 的 `lib.rs` 编译图
- 不再进入正常 `cargo check` / 运行时编译图
- 只通过 `ember-rs/crates/agent/tests/legacy_permission_surfaces.rs` 测试夹具加载，并继续复用文件内自测

同时新增了两层守卫：

- `scripts/report-legacy-surfaces.mjs`：防止旧权限模块重新公开、重新挂回 `lib.rs` 编译图，或被上层重新依赖
- `src/lib/governance/legacyToolPermissionGuard.test.ts`：防止 `ember-agent` 再次把这两套旧权限逻辑挂回 `lib.rs`，并约束测试夹具边界
...
- **说明**  
  Core surface 现已按 current surface 收敛；精确数量与分类以 `ember-rs/src/agent_tools/catalog.rs` 为准。
...
- `internal/prd/tools/architecture.md`：架构、时序、流程图、Codex 对照
- `internal/prd/tools/inventory.md`：工具盘点、分类、库存命令说明
- `internal/prd/tools/development-plan.md`：开发计划、验收标准、下一刀
