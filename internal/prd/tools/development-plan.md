...
- [x] 新增 `ember-rs/crates/agent/src/agent_tools/execution.rs`
...
# MCP / Provider 定向测试
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-mcp empty_query_prioritizes_always_visible_then_name -- --nocapture
cargo test --manifest-path "ember-rs/Cargo.toml" -p lime-providers supports_x_lime_alias -- --nocapture
