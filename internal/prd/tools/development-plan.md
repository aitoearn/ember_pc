...
把 Ember 的工具系统从“多源并存、权限混写、上下文不可审计”收敛为：
...
- [x] 把 tool metadata 解析统一到 `ember_core::tool_calling`
...
- [x] 输出 `internal/prd/tools/README.md`
- [x] 输出 `internal/prd/tools/architecture.md`
- [x] 输出 `internal/prd/tools/inventory.md`
- [x] 输出 `internal/prd/tools/development-plan.md`
...
- [x] 补齐 provider 层对 `x-ember` / `x_ember` alias 与去重行为测试
- [x] 补齐前端 `agentRuntime.ts` inventory helper 默认参数测试
- [x] 新增 `ember-agent` 轻量测试载体，复用 `catalog.rs` / `inventory.rs` 纯逻辑模块，绕开主包 Tauri 大链接
- [x] 把 `tool_permissions.rs` / `shell_security.rs` 迁出 `ember-agent` 的 `lib.rs` 编译图，改为独立 integration test 夹具加载，并补治理守卫与 Vitest 护栏

## Phase F：执行权限事实源收口（本轮完成）

- [x] 新增 `ember-rs/src/agent_tools/execution.rs`
...
- **小包优先**：优先跑 `ember-providers`、`ember-mcp` 与前端 `agent.test.ts`
- **轻量逻辑优先**：`catalog.rs` / `inventory.rs` 的纯逻辑单测优先从 `ember-agent` 执行，测试名使用模块前缀：
  - `agent_tools::catalog::tests::...`
  - `agent_tools::inventory::tests::...`
- **主包降级为编译检查**：`ember` 主包测试会触发超大链接；在当前环境磁盘仅余约 `4.4Gi` 时，优先做 `cargo check` / 定向编译验证
- **边界说明**：若后续 CI 或本地磁盘空间恢复，应追加一次 `ember` 主包的完整定向测试，把 `catalog.rs` / `inventory.rs` / `aster_agent_cmd.rs` 新增测试全部实跑
...
# MCP / Provider 定向测试
cargo test --manifest-path "ember-rs/Cargo.toml" -p ember-mcp empty_query_prioritizes_always_visible_then_name -- --nocapture
cargo test --manifest-path "ember-rs/Cargo.toml" -p ember-providers supports_x_ember_alias -- --nocapture
...
- 已从 `ember-agent` crate 根移除公开表面
- 已退出 `ember-agent` 的 `lib.rs` 编译图，仅通过 `tests/legacy_permission_surfaces.rs` 测试夹具加载
...
1. 在 `catalog.rs` 有记录
2. metadata 走 `ember_core::tool_calling`
...
只要守住这三条，Ember 的工具数继续增长，也不会再回到“上下文失控 + 权限混乱”的状态。
