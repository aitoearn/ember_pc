# Ember 如何借鉴 Codex 的工程方法：从“已有基础”到“快而稳的迭代系统”

> 目的不是照抄 Codex 的技术栈，而是借鉴它背后的工程方法，并结合 Ember 当前仓库的真实现状，收敛出一条适合我们的落地路径。
...
2. 对 Ember 当前仓库的工程入口、脚本、工作流、测试和发布链路做了核对。

结论先写在前面：

**Ember 并不缺“零起点能力”，而是已经有不少关键积木。**

我们现在更需要的，不是继续堆更多脚本，而是把已有能力收敛成一条清晰、分层、可执行的开发与交付路径。

如果要把这些工程方法进一步落实到 Ember 的 Agent / Harness 改造，请继续阅读：

- `internal/tech/harness/README.md`
...
因为 Ember 当前已经不是一个简单的单体项目，而是一个同时包含：

- React + TypeScript 前端
- Electron Desktop Host 桌面壳
- App Server JSON-RPC 后端事实入口
- Rust workspace
...
从 `ember-rs/Cargo.toml` 看，当前 Rust workspace 已经有 `20` 个 crate；  
从 `package.json` 看，前端、桥接、契约、发布、本地验证、AI 验证、治理脚本都已经存在。

这意味着：

**Ember 已经进入“需要系统化工程节奏”的阶段。**

## 2. Ember 当前已经具备的基础

在讨论“借鉴 Codex”之前，先明确 Ember 不是从零开始。
...
- 全部输出与文档统一使用中文
- 涉及 UI 先看 `internal/aiprompts/design-language.md`
- 默认同时考虑 macOS / Windows
- 优先使用 `npm`、`cargo`、Electron / App Server 命令与项目内封装
- 明确给出了构建、测试、代码检查入口

这意味着 Ember 已经有“仓库级规则入口”，这是非常好的基础。
...
也就是说，Ember 已经不是“完全依赖人工自觉”的状态，而是开始把工程约束前移到提交流程。
...
- `ember-rs/Cargo.toml`
- Electron 发布配置
- App Server release manifest

的版本是否一致。

后者会检查：

- 前端 `safeInvoke` / `invoke` 实际调用了哪些 Electron Host / App Server 命令
- Electron Host 白名单、App Server JSON-RPC protocol / client 与 legacy desktop facade 是否一致
...
- 版本同步到 Electron 发布配置与 App Server manifest
- release notes 生成
- GitHub Release 上传
...
说明 Ember 已经不是没有交付能力，而是已经有了一个基础发布流水线。

## 3. Ember 当前最明显的缺口

和 Codex 对比后，我认为 Ember 当前最核心的缺口不是“工具不够多”，而是下面三件事还没有完全收敛。
...
这意味着，Ember 当前很多重要校验虽然**本地可跑**，但还没有成为 PR 上稳定、清晰、统一的 required status，例如：
...
Ember 现在更像是“积木已经有了，但还没拼成系统”。

### 3.3 契约检查已有基础，但还没有完全上升为“关键接口演进规范”

`check-command-contracts.mjs` 已经很有价值，但它目前更像一个检查脚本，而不是一整套“接口演进流程”的中心。

对 Ember 这种项目来说，真正高风险的契约不止是 Electron Host command 名称，还包括：

- Electron Host command / App Server JSON-RPC 参数结构
- 前后端共享数据结构
- bridge 交互协议
...
这正是 Ember 接下来最该补齐的一层。

## 4. Codex 最值得 Ember 借鉴的五个方法

下面只写“适合 Ember 的”，不写那些和我们技术栈不匹配的。
...
这样 Ember 就不需要额外引入 `justfile` 一类新工具，仍然符合我们仓库“优先使用 `npm` / `cargo` / Electron / App Server 命令”的跨平台约束。
...
- Rust：`cargo test --manifest-path "ember-rs/Cargo.toml"`
...
其中如果明确把 Ember 看作 **GUI 桌面产品**，我更建议把高风险路径的最小专项护栏优先收口成一个统一入口：

- `npm run verify:gui-smoke`

它适合作为：

- Electron Desktop Host 启动验证
- App Server sidecar 初始化验证
- DevBridge 健康检查
...
### 4.3 借鉴三：把“命令契约”升级成 Ember 的正式工程边界

Codex 很强调契约一致性。

Ember 现在已经有 `check-command-contracts.mjs`，这是非常好的起点。
...
2. **凡是修改 `ember-rs/src/app/runner.rs`、前端 `safeInvoke` 调用、bridge mock 命令集合的改动，都要求契约检查通过**
...
Ember 当前的 `release.yml` 已经有不错的发布骨架。

但如果借鉴 Codex，我们可以把“发布”进一步拆成两个层级：

#### A. 稳定版交付

继续保留当前正式 tag 驱动的 GitHub Release、Homebrew 更新、Electron 构建发布。
...
## 5. 结合 Ember 实际，哪些不要照抄

借鉴 Codex，不代表要复制它的全部做法。

以下几件事我认为对 Ember 当前阶段不适合直接照搬。
...
Ember 当前已经有 bridge / smoke / headless 相关脚本，这很好。
...
1. `safeInvoke` / Electron Host / App Server command 契约
2. bridge / Electron smoke 相关路径
...
如果只从表面看，Codex 和 Ember 的仓库差异很大。

但从工程方法看，Ember 其实已经有不少和 Codex 同方向的基础：
...
所以 Ember 现在最需要做的，不是“再发明一套全新的方法论”，而是：

**把已有能力收口成一条清晰、稳定、分层的工程路径。**

如果用一句话概括这份文档的核心建议，那就是：

**对 Ember 来说，借鉴 Codex 的重点不是“学他们用了什么工具”，而是“把我们的规则、脚本、测试、治理和发布真正串成系统”。**

一旦这件事完成，Ember 的迭代速度和稳定性都会一起上一个台阶。
