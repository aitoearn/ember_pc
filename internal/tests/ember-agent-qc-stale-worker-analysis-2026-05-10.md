# Ember Agent QC stale worker 根因分析（2026-05-10）
...
当前 Ember Agent QC 整体目标未完成的直接原因不是测试标准缺失，而是官方 P0 Evidence Pack 没有真实 8/8 pass。当前阻断项是 isolated full P0 job `1778405842243079000` 中的 `browser-runtime-site-adapter`：worker 长时间 running，但没有 stdout/stderr。
...
| binary provenance | 当前 qcloop serve 不是 `.ember/qc/bin/qcloop-timeout-fixed` | 已准备的 timeout fix 未作用于当前进程 |

因此不能覆盖 `.ember/qc/agent-qc-evidence.json`，不能启动新的 full GUI P0，也不能把 isolated partial pass 当作 release pass。
...
权威 sidecar：`.ember/qc/qcloop-db-lease-isolated-p0-full-v1.md`。

2026-05-10 21:02 复核补充：等待超过上述 `lock_expires_at` 后，qcloop 并未自然释放该 item；只读 SQLite 显示 `lock_expires_at` 已延长到 `2026-05-10T21:17:05+08:00`，active attempt 仍为 `running` 且 stdout/stderr `0 / 0`。权威 sidecar：`.ember/qc/qcloop-db-lease-isolated-p0-full-v1-after-expiry.md`。
...
```text
active_binary=/Users/coso/Documents/dev/ai/embercloud/qcloop/qcloop
active_sha256=177bf7fa79212d065c5cb6cc5cdbb153d3079cec3252a157993f70ab35dc5fe1
fixed_binary=.ember/qc/bin/qcloop-timeout-fixed
fixed_sha256=2895e33d068a7109607d3e45b6e3bebe1807b6f71d387a3b62eb669a3d4314c7
```

权威 sidecar：`.ember/qc/qcloop-runtime-binary-provenance-18080.md`。
...
- 修改 `.ember/qc/qcloop-isolated-worker-preflight.db`。
- 启动新的 full GUI P0。
...
- 更新 `.ember/qc/*` sidecar 和 `internal/tests` 分析文档。
...
2. 使用 `.ember/qc/bin/qcloop-timeout-fixed` 或等价重建后的 qcloop binary，启动隔离 qcloop server 与独立 DB。
3. 生成或复核 `.ember/qc/qcloop-p0-structured-evidence-v1-payload.json`：8 个 P0、`max_qc_rounds=1`、`max_executor_retries=0`。
4. 提交 P0 structured evidence job。
5. 等待 8/8 qcloop `success`。
6. 只在该 job 真实 pass 后导出 `.ember/qc/agent-qc-evidence.json`。
