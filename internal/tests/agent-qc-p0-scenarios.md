# Ember Agent QC P0 场景执行手册

> 本文件是 `internal/tests/agent-ops-qc.md` 的执行层补充，专门描述 P0 场景如何被 Agent / qcloop / Playwright MCP 执行和验收。
...
正式发布门禁只接受覆盖全部 P0 scenario id 的 `.ember/qc/agent-qc-evidence.json`。如果只是验证 qcloop 导出链路，应写入 `.ember/qc/agent-qc-evidence.partial.json`，不能把 partial evidence 当成发布通过证据。
...
正式 `release.yml` 默认也会读取 `.ember/qc/agent-qc-evidence.json`。如果团队把 evidence pack 放在其他路径，使用 workflow dispatch 时必须填写 `agent_qc_evidence_path`；否则发布会被 Agent QC 硬门禁阻断。
...
| `skill-forge-register-bind-enable` | `npm run test:contracts` + `npm run smoke:agent-service-skill-entry`                                                 | draft、verify/register、runtime binding、`.ember/qc/skill-forge-runtime-transcript-current.json` 中的 SkillTool request/decision/result/source metadata | 把 registered 误判为 executable、metadata 自动启用 skill                                                                 |
...
- DevBridge 是否健康。
- GUI session owner / isolation statement：说明本次是否独占 Ember GUI / DevBridge 会话，或列出仍在 running/stale 的 GUI qcloop sidecar。
- 关键用户路径的页面状态。
- console error 摘要。
- network error 摘要。
