# Ember Agent 自主测试执行矩阵

> 本文件是 `internal/tests/ember-agent-autonomous-testing-plan.md` 的执行层补充：把“应该如何测试 Ember”落成 Agent 可以按步骤执行的矩阵。它不替代通用标准，也不替代 qcloop Evidence Pack；它只定义 Ember 作为样本产品时，测试 Agent 如何选择场景、采集证据、阻断发布和回写回归。

## 1. 适用范围

Ember 是桌面 GUI + Agent Runtime + 工具/技能/浏览器能力的组合产品，因此不能用单一测试手段证明可运营。一次 Ember Agent 自主测试至少覆盖四类事实：
...
npm run agent-qc:qcloop-status -- \
  ...
  --format json \
  --output "./.ember/qc/qcloop-status.current.json"
...
npm run agent-qc:process-owner-check -- \
  ...
  --watch-history-output "./.ember/qc/raw-process-owner-watch-history.jsonl"

npm run agent-qc:qcloop-db-lease -- \
  --db "./.ember/qc/qcloop-isolated-worker-preflight.db" \
  --job-id "<running-qcloop-job-id>" \
  --format json \
  --output "./.ember/qc/qcloop-db-lease-current.json" \
  --markdown-output "./.ember/qc/qcloop-db-lease-current.md"
...
- 官方 `.ember/qc/agent-qc-evidence.json` 不是同一批次 8/8 P0 pass。
...
5. 只有 8/8 P0 pass 后，才覆盖 `.ember/qc/agent-qc-evidence.json`。
...
- Ember 分阶段计划：`internal/tests/ember-agent-autonomous-testing-plan.md`
- 当前阻断：`internal/tests/ember-agent-qc-current-blockers.md`
- qcloop 运维：`internal/tests/ember-agent-qc-qcloop-operations.md`
- Evidence 契约：`internal/tests/ember-agent-qc-evidence-contract.md`
...
| official Evidence Pack | fail | `.ember/qc/agent-qc-evidence.json` 不能发布 |
...
`skill-forge-register-bind-enable` 的最新前置证据是 `.ember/qc/skill-forge-runtime-transcript-current.json`：它覆盖 deterministic-smoke 与 runtime-transcript 两层、8 个事件，并证明 Rust exact tests 不再是 `running 0 tests`。随后 qcloop job `1778445676473687000` 已把该场景跑成 `success`，sidecar `.ember/qc/agent-qc-evidence.skill-forge-runtime-transcript.json` 为单场景 `pass`。但该 sidecar 缺少其余 7 个 P0 场景，release summary `--check` 仍正确失败；下一轮 full P0 必须在同一批次内重新覆盖 8/8，而不是拼接 partial pass。
