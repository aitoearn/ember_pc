# Ember Agent QC 当前 P0 阻断记录

> 本文件记录 Ember 作为样本产品执行 Agent QC P0 时发现的真实阻断。它不是发布放行说明；只要本文件中的 P0 blocker 未清空，`.ember/qc/agent-qc-evidence.json` 就不应被视为绿色发布证据。
...
```text
.ember/qc/agent-qc-evidence.json status=fail scenarios=8/8
```
...
| `.ember/qc/agent-qc-evidence.json`                                                     | `fail`                                | 官方默认 Evidence Pack，覆盖 8/8，但不能发布                                                                                                                                                                                                                                                                                                                                                    |
| `.ember/qc/agent-qc-evidence.p0-v3.json`                                               | `fail`                                | v3 sidecar，覆盖 8/8                                                                                                                                                                                                                                                                                                                                                                            |
| `.ember/qc/agent-qc-evidence.p0-rerun-v4-running.json`                                 | `fail` / `blocked`                    | v4 sidecar，当前 6 个重跑场景未完成或失败                                                                                                                                                                                                                                                                                                                                                       |
| `.ember/qc/agent-qc-evidence.p0-rerun-v4-completed.json`                               | `fail`                                | 2026-05-10 15:35 从 completed v4 job 导出的 sidecar，6/6 exhausted；不是官方发布证据                                                                                                                                                                                                                                                                                                            |
| `.ember/qc/agent-qc-evidence.p0-rerun-v5-current.json`                                 | `fail`                                | v5 sidecar；`tool-approval-sandbox-boundary` qcloop item 已 failed，同时 GUI / Skill / Browser 类场景因 `QCLOOP_WORKER_RESULT=BLOCKED` 保持 blocked 语义；不是官方发布证据                                                                                                                                                                                                                      |
| `.ember/qc/qcloop-executor-env-20260510.json`                                          | env finding                           | qcloop serve PID `80248` 仅设置了 `QCLOOP_CODEX_BIN`，未设置 `QCLOOP_CODEX_SANDBOX=off` / bypass / approval policy；解释了 worker localhost 被 Codex 默认沙箱阻断的高概率原因                                                                                                                                                                                                                   |
| `.ember/qc/qcloop-devbridge-health-after-v5.json`                                      | env finding                           | v5 completed 后宿主 shell 直连 `http://127.0.0.1:3030/health` 也失败，`lsof` 未发现 3030 listener；这是宿主 DevBridge 恢复前的历史阻断证据                                                                                                                                                                                                                                                      |
| `.ember/qc/qcloop-devbridge-health-restored.json`                                      | env finding                           | 2026-05-10 16:59 后 headless Tauri 启动成功，宿主 `npm run bridge:health` 与 `agent-qc:qcloop-preflight -- --require-devbridge` 均为 pass；官方 evidence 未变                                                                                                                                                                                                                                   |
| qcloop worker preflight `1778403715309891000`                                         | `completed` / `blocked` sidecar       | 宿主 DevBridge 已恢复后创建 1 个只读 preflight item；worker cwd/tmp 通过，但 DevBridge health 仍 `fetch failed`，输出 `QCLOOP_WORKER_RESULT=BLOCKED`；`.ember/qc/qcloop-status.worker-devbridge-preflight.json` 归类为 `blocked`                                                                                                                                                                 |
| isolated qcloop worker preflight `1778404260108641000`                                | `completed` / sidecar `complete`      | 使用隔离 qcloop server `127.0.0.1:18080`、独立 DB `.ember/qc/qcloop-isolated-worker-preflight.db`、`QCLOOP_CODEX_SANDBOX=off` 后，worker preflight 通过；证明 qcloop worker 权限问题可通过正确启动环境解除                                                                                                                                                                                       |
...
| direct host `tool-approval-sandbox-boundary` live runtime check                       | command `pass`                        | `.ember/qc/backend-evidence/approval-sandbox-live-current-2026-05-10/summary.json` verdict=`pass`；denied / resolved 两条 flow 均生成 `runtime_permission_confirmation:*`，并证明 `approvalPolicy=on-request`、`sandboxPolicy=workspace-write` 已随 turn 提交；仍是 sidecar，不覆盖官方 Evidence Pack                                                                                            |
...
| `.ember/qc/gui-evidence/product-backend-ux-e2e-2026-05-10.json`                        | `pass` + deep flow finding `fail`     | 基础 GUI surface 通过，但 Claw streaming deep flow 标记为 product blocker                                                                                                                                                                                                                                                                                                                       |
...
由于当前仍有多个 GUI / DevBridge 相关 qcloop 批次 running 或 stale，后续不能再并发启动新的 full P0 GUI 批次。新的 manifest 已把 `GUI session owner / isolation statement` 加入 GUI P0 evidence，并把 `parallel GUI smoke interference` 加入 failure mode；这用于区分真实产品回归和多个 worker 抢同一个 Ember GUI 会话造成的测试失真。
...
`workspace-ready-session-restore` 的细化证据已导出到 `.ember/qc/design-canvas-failure-fastmini-workspace-extract.json`。关键日志为：
...
当前唯一可安全推进的是继续维护标准、sidecar 和 runbook。必须等待 PID `69738` 自然释放，或由 owner 明确确认处置后，才能按 `internal/tests/ember-agent-qc-stale-owner-intervention.md` 执行 post-stale runbook。确认前不得直接处理进程或修改 qcloop DB。
...
新增 sidecar：`.ember/qc/qcloop-db-lease-isolated-p0-full-v1.md`。只读 SQLite 与进程树显示：
...
新增 sidecar：`.ember/qc/qcloop-runtime-binary-provenance-18080.md`。只读 `lsof` 与 checksum 显示：

| 二进制                  | 路径                                                   | 大小       | SHA-256               | 结论           |
| ----------------------- | ------------------------------------------------------ | ---------- | --------------------- | -------------- |
| 当前 18080 qcloop serve | `/Users/coso/Documents/dev/ai/embercloud/qcloop/qcloop` | `13321106` | `177bf7fa...35dc5fe1` | 当前正在运行   |
| timeout-fixed sidecar   | `.ember/qc/bin/qcloop-timeout-fixed`                    | `13426082` | `2895e33d...3d4314c7` | 已构建但未运行 |
...
新增 sidecar：`.ember/qc/qcloop-db-lease-isolated-p0-full-v1-after-expiry.md`。等待超过先前记录的 `lock_expires_at=2026-05-10T21:02:05+08:00` 后，只读复核显示：
...
`agent-qc:gui-owner-check -- --format json` 现在会在 stale owner 场景输出 `ownerIntervention`，当前 `.ember/qc/gui-owner-current.json` 显示：
...
新增 sidecar：`.ember/qc/stale-owner-watch-history.jsonl`。该文件按 JSONL 记录只读观察序列，当前包含 3 条：
...
2026-05-10 21:21 后续观察已改为脚本化追加：`agent-qc:gui-owner-check -- --watch-history-output ./.ember/qc/stale-owner-watch-history.jsonl` 已写入第 5 条记录，后续无需手工拼 JSONL。
...
- `.ember/qc/qcloop-status.isolated-p0-full-v1-current.json`
- `.ember/qc/gui-owner-current.json`
- `.ember/qc/stale-owner-watch-history.jsonl`
- `.ember/qc/qcloop-db-lease-isolated-p0-full-v1-latest.json`
- `.ember/qc/qcloop-db-lease-isolated-p0-full-v1-latest.md`

这些证据只支持 owner 决策，不构成处理授权。确认前仍不得处理 PID `69738`、修改 qcloop DB、启动新的 full GUI P0，或覆盖官方 `.ember/qc/agent-qc-evidence.json`。

### 2.10 `verify:local` 与 Claw streaming 复核（2026-05-11 01:02）

最新完整 `npm run verify:local` 仍为失败，且已把 `.ember/qc/verify-local-current.json` 刷新到最新失败口径：

```text
status=fail
failedStage=verify:gui-smoke / smoke:claw-chat-ready-streaming
evidence=.ember/qc/verify-local-sensenova-2026-05-11-0023.log
```
...
| 证据                                                                                                                                                    | 状态    | 含义                                                                                                       |
| ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ---------------------------------------------------------------------------------------------------------- |
| `.ember/qc/gui-evidence/claw-chat-ready-streaming-post-refresh-fallback-2026-05-11/claw-chat-ready-streaming-post-refresh-fallback-summary.json`         | `pass`  | runtime 已持久化 `复原完成` 时，刷新 / 会话恢复 fallback 可以让 GUI 重新呈现恢复结果                       |
| `.ember/qc/gui-evidence/claw-chat-ready-streaming-sensenova-session-restore-2026-05-11/claw-chat-ready-streaming-sensenova-session-restore-summary.json` | `fail`  | 复测期间 DevBridge 中途不可达，恢复 turn 未进入 completed；这是环境 / runtime 稳定性证据，不能替代完整门禁 |
| `.ember/qc/claw-chat-ready-streaming-current.json`                                                                                                       | `mixed` | 汇总当前 Claw deep flow 的 pass / fail 侧证据                                                              |
...
```text
command=EMBER_AGENT_QC_PROVIDER=custom-f74b38b5-6d3f-44ef-aa0f-99d70c3482ed EMBER_AGENT_QC_MODEL=sensenova-6.7-flash-lite npm run verify:gui-smoke -- --reuse-running --timeout-ms 240000
status=pass
...
- `.ember/qc/verify-gui-smoke-current.json`
- `.ember/qc/verify-gui-smoke-2026-05-11-0112-sensenova-pass.md`
- `.ember/qc/claw-chat-ready-streaming-current.json`
...
但 `.ember/qc/gui-process-owner-current.json` 仍显示 raw process owner 为 `busy`：存在长时间 `smoke:design-canvas` 进程、stale qcloop Codex worker，以及多个 Cargo / Rust 编译进程。因此仍不应在当前窗口启动新的完整 `verify:local` 或新的 full GUI P0 qcloop 批次；`local-verify-gate` 只能在这些 owner 自然释放后用完整 `npm run verify:local` 关闭。

### 2.12 只读续刷与 owner 取证脚本化（2026-05-11 01:32）

按“Ember 不要推送、还有其他进程在跑”的约束，本轮仍未执行 commit / push / tag / release，也没有 kill / pause / interrupt / restart qcloop、Codex worker、Tauri、Cargo 或 GUI smoke 进程。只读刷新后的当前状态：
...
- `.ember/qc/gui-owner-current.json`
- `.ember/qc/stale-owner-watch-history.jsonl`
- `.ember/qc/gui-process-owner-current.json`
- `.ember/qc/gui-process-owner-current.md`
- `.ember/qc/qcloop-db-lease-isolated-p0-full-v1-latest.json`
- `.ember/qc/qcloop-db-lease-isolated-p0-full-v1-latest.md`
...
| `tool-approval-sandbox-boundary`   | default qcloop failed；direct-host live runtime sidecar pass                 | v5 attempt 1 和 repair attempt 2 的确定性 smoke 不足以通过 verifier；本轮 direct-host `smoke:agent-runtime-approval-sandbox` 已补真实 live runtime transcript：denied / resolved 两条 flow 均生成 `runtime_permission_confirmation:*`，并证明 approval / sandbox policy 进入 turn config；证据在 `.ember/qc/backend-evidence/approval-sandbox-live-current-2026-05-10/summary.json`                                                                   | 该场景从“缺 live transcript”降级为“待 full P0 qcloop 同批次采信”；不要覆盖官方 `.ember/qc/agent-qc-evidence.json`，也不要在 stale GUI owner 未释放前重启 full GUI P0                                                                                                                                                               |
...
| `qcloop-batch-verifier-repair`     | worker / verifier 协议仍在收敛                                               | 已发现 qcloop generic repair prompt 会要求 worker “修复目标工作区”，这不适合发布证据批次；已把 worker prompt 改为只读，并生成 `.ember/qc/qcloop-readonly-p0-v1-payload.json` / `.ember/qc/qcloop-fastmini-readonly-p0-v2-payload.json`（`max_qc_rounds=1`）待用； no-MCP P0 v2 的 `command-bridge-contract` attempt 1 真实执行通过，verifier 正确拒绝了“只给命令通过与日志路径、未逐项解释 evidence / failure mode”的输出；repair attempt 已补足并通过 | worker prompt 已强化为必须逐项列出 `evidence_required` 和 `failure_modes`；`command-bridge-contract` verifier 已允许 no-change surface evidence 使用 checked surface counts + contract pass，并已生成待用 payload `.ember/qc/qcloop-isolated-nomcp-p0-v3-after-evidence-prompt-payload.json`；继续观察后续 P0 是否复现同类证据不足 |
...
- `agent-qc:audit` 已能区分官方 `.ember/qc/agent-qc-evidence.json` 与 sidecar evidence，避免 partial 或 fail sidecar 被误读为完成。
...
- 已提交 qcloop rerun v5 `1778398587521627000`，payload 来自 `.ember/qc/qcloop-p0-rerun-v5-verifier-evidence-ready-payload.json`，覆盖当前 6 个未通过 / 未完成 P0，并确认 worker prompt 含 preflight、verifier prompt 含 `{{stdout}}` / `{{attempt_status}}` / `{{exit_code}}`。
- 已用 `npm run tauri:dev:headless` 恢复宿主 DevBridge，并把恢复证据写入 `.ember/qc/qcloop-devbridge-health-restored.json`。
...
- 已启动 isolated P0 full v1 `1778405842243079000`。该批次目前 4/8 success，但 `browser-runtime-site-adapter` item 长时间 running 且无 stdout/stderr，`.ember/qc/qcloop-status.isolated-p0-full-v1-stale-check.json` 已标记 `stale`；只读 `ps` 显示内层 `codex exec` 仍在跑、尚未出现 npm 子命令输出，后续 3 个 P0 pending；按 qcloop 运维规则只读观察，不 kill、不 pause、不覆盖官方 evidence。
...
3. `npm run agent-qc:release-summary -- --evidence ./.ember/qc/agent-qc-evidence.json --require-scenario-manifest internal/test/agent-qc-scenarios.manifest.json --require-risk P0 --check` 通过。
4. `npm run agent-qc:audit -- --format json` 返回 `complete`。

在此之前，Ember Agent QC 整体目标不得标记完成。

### 5.1 手动 Playwright Agent UI / Skills 非抢占续测（2026-05-10 20:54）

新增证据目录：`.ember/qc/gui-evidence/agent-ui-manual-e2e-2026-05-10/`。
...
| Playwright evidence | `.ember/qc/gui-evidence/skill-preflight-reference-sanitized-2026-05-10/summary.json`，verdict=`pass`            |
...
| Playwright evidence | `.ember/qc/gui-evidence/skills-search-local-hit-2026-05-10/summary.json`，verdict=`pass`                        |
...
| Playwright evidence | `.ember/qc/gui-evidence/advanced-settings-plan-label-cn-2026-05-10/summary.json`，verdict=`pass`                                   |
...
| Evidence | `.ember/qc/gui-evidence/design-canvas-project-roundtrip-current-2026-05-10/summary.json`，verdict=`pass`                     |
...
| 历史基线   | 未发现既有 `.ember/harness/history` / `reports` 样本；已把本轮结果记录为第一条真实 baseline，并生成 summary / trend / cleanup / dashboard 报告 |
| Evidence   | `.ember/qc/backend-evidence/harness-replay-regression-current-2026-05-10/summary.json`，verdict=`pass_with_trend_seed_limit`                   |
...
| Evidence        | `.ember/qc/backend-evidence/approval-sandbox-live-current-2026-05-10/summary.json`，verdict=`pass`                                                                                                                                                                                                                                                                                                                         |
| 剩余            | 仍需 full P0 qcloop verifier 在同批次采信；当前 active GUI owner `1778405842243079000` 仍 stale，不能新开 full GUI P0                                                                                                                                                                                                                                                                                                     |

该项从“缺 live runtime transcript”降级为 direct-host backend sidecar pass；它不改变官方 `.ember/qc/agent-qc-evidence.json` 仍为 fail，也不关闭 `real-qcloop-evidence` 和 `local-verify-gate` 两个 completion audit 缺口。
...
| qcloop status    | `.ember/qc/qcloop-status.isolated-p0-full-v1-current.json` 为 `verdict=stale` | 4 success / 1 running / 3 pending / 1 stale                                                                                                               |
...
| running evidence | `.ember/qc/verify-local-2026-05-10-2340-running.md`                           | 只读记录当前进程与日志，不覆盖最终 gate                                                                                              |
...
| evidence     | `.ember/qc/verify-local-2026-05-10-2350-cargo-lock.md` | 只读记录，不覆盖最终 `verify-local-current.json`                                                                                               |

该状态不能关闭 `local-verify-gate`，但也不再等同于 22:39 的 Vitest 失败；最终口径以当前 wrapper 自然结束后写出的 `.ember/qc/verify-local-current.json` 为准。
...
| evidence         | `.ember/qc/verify-local-2026-05-10-2355-gui-smoke-timeout.md`、`.ember/qc/completion-audit-2026-05-10-2356.json`                                                            | 当前仍不可发布                                                                                                                       |
...
- `.ember/qc/verify-gui-smoke-current.json`
- `.ember/qc/verify-gui-smoke-reuse-2026-05-11-0006.log`
- `.ember/qc/gui-evidence/claw-chat-ready-streaming/claw-chat-ready-streaming-summary.json`
- `.ember/qc/smoke-claw-chat-ready-streaming-deepseek-chat-2026-05-11-0008.log`
- `.ember/qc/gui-evidence/claw-chat-ready-streaming-deepseek-chat-2026-05-11/claw-chat-ready-streaming-deepseek-chat-summary.json`
...
本轮只修复 smoke 脚本的证据采集和空快照健壮性，没有重跑完整 GUI smoke；原因是仍有其他 Ember / Cargo / qcloop 进程在运行，且用户明确要求不要推送、不要干预其他进程。关闭条件是：在无并发 GUI owner 阻断的环境下，重跑 `npm run verify:gui-smoke -- --reuse-running` 或等价 qcloop P0 item，取得 `claw-chat-ready-streaming` 的 pass summary，并包含 runtime transcript、interrupt scope、恢复 turn、console/network 摘要。
...
- `.ember/qc/verify-gui-smoke-current.json`
- `.ember/qc/verify-gui-smoke-2026-05-11-0025-sensenova-pass.md`
- `.ember/qc/verify-gui-smoke-reuse-sensenova-2026-05-11-0020.log`
- `.ember/qc/gui-evidence/claw-chat-ready-streaming/claw-chat-ready-streaming-summary.json`
...
| qcloop official P0 Evidence Pack | 仍未关闭；官方 `.ember/qc/agent-qc-evidence.json` 仍为 fail，且 stale qcloop GUI owner 未释放 |
...
| release summary against official evidence | exit `1`                                                                              | 当前官方 `.ember/qc/agent-qc-evidence.json` 仍为 fail，发布被正确阻断                                 |
...
| `.ember/qc` secret scan                    | `pass`                                                                                | `fileCount=315`、`findingCount=0`                                                                    |
...
| 门禁                       | 当前值           | 结论                                                                                                             |
| -------------------------- | ---------------- | ---------------------------------------------------------------------------------------------------------------- |
| objective completion audit | `achieved=false` | `.ember/qc/objective-completion-audit-current.json` 明确失败项为 `real-p0-qcloop-evidence` 与 `local-verify-gate` |
...
| 通用标准口径               | 已补强           | `agent-ops-qc.md` 与 `ai-agent-testing-guide.md` 不再以 Ember 作为标准标题；Ember 样本仍在 `ember-*` 文档中         |
...
| objective completion audit | `achieved=false`                           | `.ember/qc/objective-completion-audit-current.json` schema `v3` 失败项仍为 `real-p0-qcloop-evidence` 与 `local-verify-gate` |

该口径不改变核心发布事实：没有真实 8/8 P0 qcloop pass 和完整 `verify:local` pass 时，仍不能把当前状态标记为整体完成，也不能覆盖官方 `.ember/qc/agent-qc-evidence.json`。

### 5.16 当前门禁刷新与 stale owner 收口（2026-05-11 02:42）

本轮只执行只读 / sidecar 取证与低冲突脚本修正；未执行 git commit / push / tag / release，未修改 qcloop SQLite DB，未覆盖官方 `.ember/qc/agent-qc-evidence.json`，也未启动新的 full GUI P0 或完整 `verify:local`。
...
| sidecar Evidence Pack                     | `.ember/qc/agent-qc-evidence.isolated-p0-full-v1-after-intervention.json` status=`fail` | 只作为失败证据；不覆盖官方 Evidence Pack                               |
...
npm run agent-qc:audit -- --format json --output ./.ember/qc/objective-completion-audit-current.json
npm run agent-qc:release-summary -- --evidence ./.ember/qc/agent-qc-evidence.isolated-p0-full-v1-after-intervention.json --require-scenario-manifest internal/test/agent-qc-scenarios.manifest.json --require-risk P0 --tag sidecar-isolated-p0-full-v1-after-intervention --check
```
...
最新 sidecar：`.ember/qc/gui-process-owner-current.json` / `.md`。当前 `ownerIntervention.status=requires_owner_confirmation`，确认文本为：
...
- `.ember/qc/stale-raw-gui-owner-intervention-request.md`
...
- `.ember/qc/objective-completion-checklist-current.json`
- `.ember/qc/objective-completion-checklist-current.md`
...
`agent-qc:process-owner-check` 新增 `--watch-history-output`，已追加 `.ember/qc/raw-process-owner-watch-history.jsonl`。最新记录：`status=busy`，`activeGuiSmoke=1`，`staleActiveGuiSmoke=1`，`ownerIntervention=requires_owner_confirmation`，PID `59011` 运行约 `7h16m`。该 JSONL 只记录观察，不授权处理进程。
...
npm run agent-qc:objective-checklist -- --format markdown --output ./.ember/qc/objective-completion-checklist-current.md
...
`internal/tests/ember-agent-qc-stale-owner-intervention.md` 新增第 8 节，专门覆盖 `gui-owner-check` 已通过但 `process-owner-check` 仍因 raw GUI smoke stale 的场景。该 runbook 明确：确认前不处理进程；确认后只处理 owner 明确确认的 PID / PGID；不得顺手清理 passive qcloop serve、passive desktop runtime 或 observer shell；处理后必须先通过 `agent-qc:process-owner-check -- --check` 与 `agent-qc:gui-owner-check -- --check`，再跑完整 `verify:local`。

### 5.25 local verify gate 已关闭（2026-05-11 03:11）

外部完整门禁已把 `.ember/qc/verify-local-current.json` 与 `.ember/qc/verify-gui-smoke-current.json` 刷新为 `status=pass`。重新运行 `agent-qc:audit` 后，completion audit 从 `15/17` 提升为 `16/17`，剩余目标级缺口只剩 `real-qcloop-evidence`。Objective checklist 从 `4/7` 提升为 `5/7`，其中 `verify:local` 已 PASS。
...
观察到本地已有隔离 P0 full v2 批次 `1778440541478632000`（`ember-agent-qc-isolated-p0-full-v2-2026-05-11-0315`）运行在 `127.0.0.1:18086`。本轮只做只读监控、sidecar 导出和 release gate 校验；未覆盖官方 `.ember/qc/agent-qc-evidence.json`，未执行 git commit / push / tag / release。

| 门禁                    | 当前值                                                                                         | 结论                                                                                              |
| ----------------------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| qcloop status           | `failed`，3 success / 5 exhausted                                                              | 批次无 stale，但仍不能发布                                                                        |
| sidecar Evidence Pack   | `.ember/qc/agent-qc-evidence.isolated-p0-full-v2-2026-05-11-0315.json` status=`fail`            | 2 pass / 4 fail / 2 blocked                                                                       |
| sidecar release summary | `.ember/qc/release-agent-qc.sidecar-isolated-p0-full-v2-2026-05-11-0315.md`，`--check` exit `1` | release gate 正确拒绝                                                                             |
...
随后观察到隔离 P0 full v3 批次 `1778442773271496000`（`ember-agent-qc-isolated-p0-full-v3-2026-05-11-0354`）运行在 `127.0.0.1:18087`。该批次修正了 worker 内 GUI session owner / isolation 口径，避免 worker 把同批次 GUI owner 误判为外部抢占；本轮仍只做只读监控和 sidecar 导出。

| 门禁                    | 当前值                                                                                         | 结论                                                                        |
| ----------------------- | ---------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| qcloop status           | `failed`，4 success / 4 exhausted                                                              | 批次已终态，无 running / pending / stale；仍不能发布                        |
| sidecar Evidence Pack   | `.ember/qc/agent-qc-evidence.isolated-p0-full-v3-2026-05-11-0354.json` status=`fail`            | 4 pass / 1 fail / 3 blocked                                                 |
| sidecar release summary | `.ember/qc/release-agent-qc.sidecar-isolated-p0-full-v3-2026-05-11-0354.md`，`--check` exit `1` | release gate 正确拒绝                                                       |
...
当前不能把 v3 sidecar 升格为官方发布证据。关闭条件仍是：PID `59011` 自然释放，或 owner 明确确认处理 stale raw GUI owner 后，重新执行 single-owner full P0，并得到官方 `.ember/qc/agent-qc-evidence.json` 的真实 8/8 P0 pass。
...
- 为 root Tauri 测试显式使用 `-p ember`
- 为 `ember-agent` SkillTool gate 测试显式使用 `-p ember-agent`
...
| 证据                                                                                | 状态   | 说明                                                                     |
| ----------------------------------------------------------------------------------- | ------ | ------------------------------------------------------------------------ |
| `.ember/qc/smoke-agent-service-skill-entry-after-rust-exact-fix-2026-05-11-0430.log` | `pass` | 当前 `smoke:agent-service-skill-entry` 已通过                            |
| Rust exact tests                                                                    | `pass` | 日志中关键 root / `ember-agent` 测试均出现 `running 1 test` 与 `1 passed` |
...
观察到单项 qcloop job `1778445171616868000`（`ember-agent-qc-skill-forge-rust-exact-fix-2026-05-11-0432`）运行在 `127.0.0.1:18087` 并进入终态 `failed`，`0 success / 1 exhausted / 0 stale`。该批次只覆盖 `skill-forge-register-bind-enable`，不是 full P0，也不能覆盖官方 Evidence Pack。

| 证据                                                              | 状态     | 说明                                                       |
| ----------------------------------------------------------------- | -------- | ---------------------------------------------------------- |
| `.ember/qc/qcloop-status.skill-forge-rust-exact-fix-current.json`  | `failed` | 单项 job 已终态，无 stale                                  |
| `.ember/qc/agent-qc-evidence.skill-forge-rust-exact-fix.json`      | `fail`   | deterministic smoke 证据充分，但 runtime-transcript 层仍缺 |
| `.ember/qc/release-agent-qc.sidecar-skill-forge-rust-exact-fix.md` | `fail`   | release summary 正确拒绝单项 sidecar 作为 full P0 发布证据 |
...
| official Evidence Pack | `fail` | `.ember/qc/agent-qc-evidence.json` 仍不能发布              |

同时观察到 `.ember/qc/skill-forge-runtime-transcript-current.json` 已生成：
...
继续只读观察后，发现第二个单项 qcloop job `1778445676473687000`（`ember-agent-qc-skill-forge-runtime-transcript-2026-05-11-0441`）已在 `127.0.0.1:18087` 终态 `completed`，`1 success / 0 fail / 0 stale`。本轮只导出 sidecar，并运行 release summary 检查；未覆盖官方 Evidence Pack。

| 证据                                                                  | 状态       | 说明                                               |
| --------------------------------------------------------------------- | ---------- | -------------------------------------------------- |
| `.ember/qc/qcloop-status.skill-forge-runtime-transcript-current.json`  | `complete` | `skill-forge-register-bind-enable` 单项 success    |
| `.ember/qc/agent-qc-evidence.skill-forge-runtime-transcript.json`      | `pass`     | 只覆盖 `skill-forge-register-bind-enable` 1 个场景 |
| `.ember/qc/release-agent-qc.sidecar-skill-forge-runtime-transcript.md` | `fail`     | 缺少其余 7 个 P0 场景，release gate 正确拒绝       |

这说明 `skill-forge-register-bind-enable` 的 runtime-transcript 层已能被 qcloop worker / verifier 采信；该缺口从“需要补 transcript artifact”推进为“需要在下一轮 full P0 同批次重新覆盖”。它仍不能关闭官方发布门禁，因为官方 `.ember/qc/agent-qc-evidence.json` 仍是旧的 8/8 `fail`，且当前 raw process owner 仍被 PID `59011` 阻断。
...
- `.ember/qc/gui-process-owner-current.json`
- `.ember/qc/gui-process-owner-current.md`
- `.ember/qc/raw-process-owner-watch-history.jsonl`
- `.ember/qc/stale-raw-gui-owner-intervention-request.json`
- `.ember/qc/stale-raw-gui-owner-intervention-request.md`
...
| 产物                                                         | 状态         | 说明                                          |
| ------------------------------------------------------------ | ------------ | --------------------------------------------- |
| `.ember/qc/qcloop-p0-single-owner-ready-2026-05-11-0454.json` | `valid=true` | 覆盖 8 个 P0 scenario                         |
| `.ember/qc/qcloop-p0-single-owner-ready-2026-05-11-0454.md`   | ready note   | 记录 scenario IDs、启动前置门禁和当前 blocker |
...
- `.ember/qc/qcloop-p0-single-owner-ready-coverage-2026-05-11-0455.json`
- `.ember/qc/qcloop-p0-single-owner-ready-coverage-2026-05-11-0455.md`
...
- `.ember/qc/post-owner-clear-full-p0-runbook-2026-05-11-0457.md`
- `.ember/qc/qcloop-p0-single-owner-ready-submit-curl-2026-05-11-0457.txt`
...
5. 只在 qcloop 8/8 P0 success 后覆盖官方 `.ember/qc/agent-qc-evidence.json`。
...
npm run agent-qc:payload-coverage -- \
  --payload "./.ember/qc/qcloop-p0-single-owner-ready-2026-05-11-0454.json" \
  --format json \
  --output "./.ember/qc/qcloop-p0-single-owner-ready-coverage-current.json" \
  --check
```
...
该项只证明 ready payload 覆盖正确，不代表 full P0 已执行。最终发布门禁仍只接受官方 `.ember/qc/agent-qc-evidence.json` 的真实 8/8 P0 pass。

### 5.39 执行矩阵纳入 payload coverage gate（2026-05-11 05:04）

`internal/tests/ember-agent-autonomous-test-execution-matrix.md` 的执行前 Owner Gate 已补充 `npm run agent-qc:payload-coverage`。后续 Agent 在启动 full P0 前，不仅要确认 GUI / raw process / DB lease 清空，还必须确认 ready payload 与 P0 manifest 一致。
...
`internal/tests/ember-agent-qc-stale-owner-intervention.md` 已新增 8.6，明确确认前可以做只读进程树取证，但不能发送 signal。该节把以下 sidecar 纳入标准处置链：

- `.ember/qc/stale-raw-gui-owner-process-tree-current.json`
- `.ember/qc/stale-raw-gui-owner-recursive-tree-current.json`
- `.ember/qc/stale-raw-gui-owner-intervention-plan-current.json`
