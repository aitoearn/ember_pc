# Ember Agent 自主化测试体系示例计划

> 本文件把通用 Agent QC 标准落到 Ember 这个实际产品上：目标不是让人多点几遍 GUI，而是让测试 Agent 能读改动、选场景、执行验证、收集证据、独立复核，并把失败回写为长期回归资产。

## 1. 问题判断

Ember 即将运营，测试策略必须从“开发者人工确认”升级为“Agent 自己证明自己可运营”。人工测试仍有价值，但只能用于设计场景、审核 waiver 和校准语义评测，不应作为发布通过的主要证据。
...
结论：Ember 的测试体系要围绕“证据”建，而不是围绕“命令跑过”建。每个场景都必须回答：谁执行、执行了什么、看到了什么、为什么可以判定通过、失败后下次怎么提前发现。
...
Ember 当前最需要覆盖的不是“所有代码行”，而是用户运营路径和 Agent 自主执行路径：
...
单一测试方式无法覆盖 Agent 产品。Ember 默认使用组合策略：
...
- `internal/test/agent-qc-scenarios.manifest.json` 固定场景、命令、证据和失败模式。
- `internal/test/agent-qc-gui-flows.manifest.json` 固定 GUI / Playwright MCP flow。
- `internal/test/agent-qc-evidence.schema.json` 固定 Evidence Pack 形状。
...
npm run agent-qc:qcloop-job -- \
  ...
  --output "./.ember/qc/qcloop-p0-job.json" \
  --check
```
...
npm run agent-qc:export-evidence -- \
  ...
  --output "./.ember/qc/agent-qc-evidence.json" \
...
- qcloop 进程 cwd 可能不是 Ember，payload 必须显式传 `--cwd`。
...
Ember 是桌面 GUI 产品，所以 GUI 主路径仍以 `verify:gui-smoke` 和 Playwright MCP 为主；如果未来提供 TUI 或 WebUI，也应复用同一套 Evidence Pack schema，而不是另起一套口头流程。
...
npm run agent-qc:release-summary -- \
  ...
  --require-risk P0 \
  --tag "<release-tag>" \
  --output "./.ember/qc/release-agent-qc.md" \
  --check
```
...
- 官方 `.ember/qc/agent-qc-evidence.json` 存在。
...
- `.ember/qc/agent-qc-evidence.json` 当前仍是 `fail`，不能发布。
...
- v5 completed 后宿主 shell 直连 `127.0.0.1:3030/health` 也失败；本轮已通过 headless Tauri 恢复宿主 DevBridge，并写入 `.ember/qc/qcloop-devbridge-health-restored.json`。
...
- 已启动 isolated full P0 v1 `1778405842243079000`：当前 4/8 success，`browser-runtime-site-adapter` running 且无 stdout/stderr，`--stale-minutes 1` 只读检查已标记 stale，后续 3 个 P0 pending；这仍是 sidecar running/stale 状态，不能覆盖官方 `.ember/qc/agent-qc-evidence.json`。
...
- `smoke:design-canvas` 的细化证据已落到 `.ember/qc/design-canvas-failure-fastmini-workspace-extract.json`：页面已进入 `CANVAS:DESIGN 专属 GUI SMOKE`，DevBridge 为 `ok`，但 `project-roundtrip-save-open` 阶段没有出现 `已保存图层设计工程`，说明“页面可见”还不能证明“工程保存链路可用”。
...
- 2026-05-11 01:13 观察到外部 `verify:gui-smoke -- --reuse-running --timeout-ms 240000` 自然通过，且 Claw streaming / interrupt / resume 在 latest summary 中为 `verdict=pass`、`recoveryVisibleSource=live-stream`。这证明 GUI smoke 主链已经恢复到可通过状态，但 `.ember/qc/gui-process-owner-current.json` 仍显示 raw process owner `busy`，且完整 `verify:local` 尚未重跑通过。
- 2026-05-11 01:32 已把 raw process owner 检查脚本化为 `npm run agent-qc:process-owner-check`，把 qcloop SQLite lease 取证脚本化为 `npm run agent-qc:qcloop-db-lease`，并新增 `internal/tests/ember-agent-autonomous-test-execution-matrix.md` 作为 Ember 样本的执行矩阵。最新 sidecar 仍显示 `qcloop stale` 与 `raw process owner busy`，因此本轮仍不能启动完整 `verify:local` 或新的 full GUI P0。
- 2026-05-11 03:11 后，`.ember/qc/verify-local-current.json` 与 `.ember/qc/verify-gui-smoke-current.json` 已恢复为 `status=pass`，completion audit 提升到 `16/17`；剩余目标级缺口收敛为真实 qcloop P0 Evidence Pack。
...
- `skill-forge-register-bind-enable` 的 Rust exact filter 缺口已修正：`smoke:agent-service-skill-entry` 现在拒绝 `running 0 tests`，并产出 `.ember/qc/skill-forge-runtime-transcript-current.json`。随后单项 qcloop `1778445676473687000` 已采信该 runtime transcript 并导出 `.ember/qc/agent-qc-evidence.skill-forge-runtime-transcript.json`，该场景为 `pass`；但它只覆盖 1 个场景，不能覆盖官方 8/8 P0 Evidence Pack。
- 官方 `.ember/qc/agent-qc-evidence.json` 仍为 `status=fail`。当前测试体系已经能把“本地 smoke 通过”“sidecar 通过/失败”“官方发布证据通过”分开处理，这正是运营级测试要保留的硬边界。
...
8. 在 `.ember/qc/gui-process-owner-current.json` 不再显示长时间 raw GUI smoke / Cargo owner 后，重新跑完整 `npm run verify:local`；只有该统一门禁继续通过，`local-verify-gate` 才能保持关闭。
...
10. 只有 8/8 P0 全部 pass 后，才把结果导出为官方 `.ember/qc/agent-qc-evidence.json`，再运行 `npm run agent-qc:release-summary -- --check` 和 `npm run agent-qc:audit -- --format json`。

## 8. 与其他文档的关系

- 通用运营标准：`internal/tests/agent-ops-qc.md`。
- P0 场景执行手册：`internal/tests/agent-qc-p0-scenarios.md`。
- Ember 落地路线：`internal/tests/ember-agent-qc-rollout-plan.md`。
- Ember 执行矩阵：`internal/tests/ember-agent-autonomous-test-execution-matrix.md`。
- 当前 P0 阻断：`internal/tests/ember-agent-qc-current-blockers.md`。
- 机器可读场景：`internal/test/agent-qc-scenarios.manifest.json`。
- 机器可读证据 schema：`internal/test/agent-qc-evidence.schema.json`。

本文件负责回答“Ember 作为实际 Agent 产品，应该如何一步步从现有丰富测试升级为自主化运营测试体系”。
