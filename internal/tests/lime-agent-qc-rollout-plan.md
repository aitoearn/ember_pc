# Ember Agent 运营级测试落地计划

> 本文件把 `internal/tests/agent-ops-qc.md` 的标准落到 Ember 当前产品上：Ember 只是标准协议的一个实现样本，但它需要在上线运营前证明 Agent Runtime、Agent UI、GUI 桌面壳、工具调用、发布包和 qcloop 证据链都能长期自测。

## 1. 建设目标

Ember 不能继续依赖“人工点一遍”来判断是否可运营。目标是让每次高风险改动都能自动进入以下闭环：
...
当前最关键的缺口不是“没有测试命令”，而是还没有把完整 P0 qcloop 批次稳定导出为 `.ember/qc/agent-qc-evidence.json`。
...
- `internal/test/agent-qc-scenarios.manifest.json` 有 P0/P1 场景、lane、命令、证据要求和 verifier。
- `internal/test/agent-qc-gui-flows.manifest.json` 有 GUI / Playwright MCP 步骤和断言。
...
npm run agent-qc:qcloop-job -- \
  ...
  --output "./.ember/qc/qcloop-command-bridge-job.json" \
  --check
```
...
npm run agent-qc:export-evidence -- \
  ...
  --output "./.ember/qc/agent-qc-evidence.partial.json" \
...
- `.ember/qc/agent-qc-evidence.json` 的 `verdict.status` 是 `pass`。
...
目标：不只证明命令通过，还要证明 Ember 的 GUI 主路径真的可用。
...
需要真实交互时，用 Playwright MCP 复用已有桌面 Chrome 会话，避免打断本地桌面工作流。证据保存到 `.ember/qc/gui-evidence/`，再由 release summary 引用。

### Phase 4：Runtime / Tool / Approval 深测

目标：让 Ember Agent 不是“能回答”，而是工具链、授权、sandbox 和恢复行为都可靠。
...
npm run agent-qc:release-summary -- \
  ...
  --require-risk P0 \
  --harness-summary "./.ember/harness/reports/harness-eval-summary.json" \
  --harness-trend "./.ember/harness/reports/harness-eval-trend.json" \
  --tag "<release-tag>" \
...
4. 等本地环境空闲后再执行 Phase 2，把 `.ember/qc/agent-qc-evidence.json` 作为正式发布门禁证据。
