...
本文件定义 Ember 当前 `State / History / Telemetry` 的唯一主链，主要回答：
...
后续 Ember 的 `State / History / Telemetry` 只允许向下面这条主链收敛：
...
补充迁移边界：`agent_runtime_get_session`、`agent_runtime_get_thread_read`、`agent_runtime_export_*` 等命令名仍是 current 读模型 / 导出 surface，但 `ember-rs/src/commands/**` 不是新增状态、历史或遥测实现目录。本文提到的 `aster_agent_cmd/**` 只作为现有行为锚点和迁移来源；新增状态投影、request telemetry、evidence / replay / review 能力应进入 App Server / RuntimeCore / services / `ember-rs/crates/agent`。旧 telemetry Tauri wrapper 已删除，不得恢复 stub 或 compat wrapper。

补充边界：

[Codex `/goal`](../research/codex-goal/README.md) 这类目标续跑模式如果在 Ember 演进为 `Managed Objective`，其状态投影也必须消费 `SessionDetail / AgentRuntimeThreadReadModel / evidence pack`。不得让 objective UI、automation job、review/handoff 各自重建“目标是否完成”的第二套真相。
...
- `ember-rs/src/agent/aster_agent.rs` 的 `get_runtime_session_detail(...)`
...
- `ember-rs/src/commands/aster_agent_cmd/dto.rs`
  - `AgentRuntimeSessionDetail`
  - `AgentRuntimeThreadReadModel`
  - `build_pending_requests(...)`
  - `build_last_outcome(...)`
  - `build_incidents(...)`
- `ember-rs/src/services/thread_reliability_projection_service.rs`
...
- `ember-rs/crates/server/src/handlers/api.rs` 的 `attach_request_correlation_metadata(...)`
- `ember-rs/crates/server/src/lib.rs` 的 `record_request_telemetry(...)`
- `ember-rs/crates/infra/src/telemetry/types.rs` 的 `RequestLog`
...
- `ember-rs/src/services/runtime_handoff_artifact_service.rs`
- `ember-rs/src/services/runtime_evidence_pack_service.rs`
- `ember-rs/src/services/runtime_replay_case_service.rs`
- `ember-rs/src/services/runtime_analysis_handoff_service.rs`
- `ember-rs/src/services/runtime_review_decision_service.rs`
- `ember-rs/src/commands/aster_agent_cmd/command_api/runtime_api.rs` 的：
...
- `ember-rs/crates/agent/src/session_store.rs`
- `ember-rs/src/agent/aster_agent.rs`
- `ember-rs/src/commands/aster_agent_cmd/dto.rs`
- `ember-rs/src/commands/aster_agent_cmd/command_api/runtime_api.rs` 的 `get_session / get_thread_read / replay_request / export_*`
- `ember-rs/src/services/thread_reliability_projection_service.rs`
- `ember-rs/crates/server/src/handlers/api.rs`
- `ember-rs/crates/server/src/lib.rs`
- `ember-rs/crates/infra/src/telemetry/types.rs`
- `ember-rs/src/services/runtime_handoff_artifact_service.rs`
- `ember-rs/src/services/runtime_evidence_pack_service.rs`
- `ember-rs/src/services/runtime_replay_case_service.rs`
- `ember-rs/src/services/runtime_analysis_handoff_service.rs`
...
- `internal/roadmap/ember-aster-codex-alignment-roadmap.md`
...
- `ember-rs/src/commands/telemetry_cmd.rs`

旧 request log / stats / token stats Tauri wrapper 已删除；如果后续需要用户可见统计，只能补 App Server current method / Electron Host current bridge 或下线入口，不能在 `ember-rs/src/commands/**` 恢复 telemetry wrapper / stub。后续若 `reliability` 专项文档或旧报表分支完全失去入口，再单独转为 `dead`。
