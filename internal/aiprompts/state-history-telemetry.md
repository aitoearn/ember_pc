...
补充迁移边界：会话详情读取 current surface 是 App Server `agentSession/read`；`agent_runtime_get_session` 已退为 `dead / retired guard-only`，不得重新接回生产入口、DevBridge truth、mock priority 或 generated manifest。旧 `agent_runtime_get_thread_read` 与 `agent_runtime_export_*` 只允许作为受控迁移面、retired guard 或历史 evidence，不得承接新业务事实；current 读模型 / 导出必须走 App Server `agentSession/read`、`evidence/export` 与 `agentSession/*/export`。`ember-rs/src/commands/**` 已删除，不是新增状态、历史或遥测实现目录；新增状态投影、request telemetry、evidence / replay / review 能力应进入 App Server / RuntimeCore / services / `ember-rs/crates/agent`。旧 telemetry Tauri wrapper 已删除，不得恢复 stub 或 compat wrapper。
...
- `ember-rs/src/agent/agent_agent.rs` 的 `get_runtime_session_detail(...)`
...
- `ember-rs/crates/app-server/src/runtime/exports.rs`
