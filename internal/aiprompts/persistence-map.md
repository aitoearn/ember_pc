...
路径边界：旧 `agent_runtime_*file_checkpoint*` 文件快照命令名只允许作为迁移期读取 surface、retired guard 或历史 evidence；`ember-rs/src/**` 与 `ember-rs/src/commands/**` 已删除，不是新增持久化实现目录。新增 file checkpoint、artifact sidecar、export / replay 持久化逻辑应进入 `ember-rs/crates/**` 下的 App Server / RuntimeCore / services / core / agent 或协议 client，不得恢复旧 command wrapper。DB 瘦身与平台落盘主线见 `internal/roadmap/db/README.md` 与 `internal/exec-plans/db-slimming-codex-alignment-plan.md`。
...
- 迁移目标：`ember-rs/crates/app-server/src/runtime/projection_store.rs` / `projection_repair.rs` 或后续独立 projection store crate
...
- `ember-rs/crates/app-server/src/runtime/exports/**`
...
- `ember-rs/crates/app-server/src/runtime/exports/**`
- `ember-rs/crates/app-server/src/runtime/**`
- `ember-rs/crates/app-server/src/local_data_source/current_timeline.rs`（compat，迁移到 projection reader 前）
