...
本文件定义 Ember 当前运行时 `Query Loop` 的唯一主链，主要回答：
...
- `internal/roadmap/task/runtime-integration.md`

## 固定主链

当前 Ember 的 Query Loop 统一按下面这条链理解：
...
`agent_runtime_*` 等命令名在迁移期仍可作为前端网关 surface，但 `ember-rs/src/commands/**` 不是新的 Query Loop 实现目录。本文列出的 `runtime_turn.rs`、`tool_runtime.rs`、`command_api/runtime_api.rs` 等路径是现有行为锚点和迁移来源；新增运行时能力应进入 App Server JSON-RPC、RuntimeCore、ExecutionBackend、services 或 `ember-rs/crates/agent`。当核心逻辑迁出后，旧 wrapper 应撤 runner / DevBridge / catalog / mock 注册并删除；不得在 `commands/**` 新增 compat wrapper、退场 stub 或平行 runtime 分支。

### Managed Objective / `/goal` 类能力边界

[Codex `/goal`](../research/codex-goal/README.md) 证明了 “persistent objective -> idle continuation turn” 可以由 runtime 管理，但 Ember 不能因此新增第二条 Query Loop。
...
- `ember-rs/crates/agent/src/runtime_queue.rs`
...
- `agent_runtime_compact_session`
- `ember-rs/src/commands/aster_agent_cmd/action_runtime.rs::agent_runtime_respond_action`
...
- `ember-rs/src/commands/agent_cmd.rs::agent_generate_title`
...
它允许为本地 auxiliary session 附带最小 `ember_runtime` metadata，用于记录 `task_profile / routing_decision / cost_state` 一类辅助任务分类事实；必要时也可以把该 auxiliary session 的 `execution_runtime` 诊断快照回传到命令结果，但这份快照只服务该一次性会话自己的诊断与可观测性，不进入 current Query Loop 的 thread / turn 真相。
...
- `ember-rs/src/commands/theme_context_cmd.rs::aster_agent_theme_context_search`
...
> Ember 当前 Query Loop 的唯一主链，是 `agent_runtime_submit_turn` 驱动的 turn 组包、runtime queue、tool runtime、流式执行与 evidence 下游消费链；任何新能力都只能往这条链收敛，不能平行再长一套执行真相。
