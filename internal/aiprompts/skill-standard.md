...
本文件定义 Ember 仓库里 `skill` 能力的统一工程标准，主要回答：

- Ember 在 Agent Skills 之上认可的技能包标准与运行时 profile 长什么样
- `skill`、`adapter`、`runtime binding` 的边界分别是什么
- 为什么 Agent Skills 应该成为 Ember 的包格式标准，但不能直接等同于 Ember 的运行时协议
...
**Agent Skills 是 Ember 唯一对齐的技能包格式标准；Ember 只在这个标准之上定义自己的运行时与产品 profile。**
...
- `Ember` 负责回答“技能包进入产品后怎么分发、怎么补参、怎么执行、怎么交付”
...
**Ember 不直接执行 `SKILL.md`，而是把 Agent Skills 包编译成自己的目录投影与运行时绑定。**

这也是为什么 Ember 可以兼容 Agent Skills，但不需要把产品层降级成“SkillToolset 产品”：

`Agent Skill Bundle -> 标准解析 / 校验 -> SkillBundle 摘要层 -> ServiceSkillCatalog / SkillCatalog / SceneCatalog 投影 -> Runtime Binding 执行`

固定结论：

1. `SKILL.md` 不是 Ember 的最终产品对象。
2. `ServiceSkill` / `Scene` 也不是新的包格式标准。
3. `ServiceSkill` / `Scene` 是 Ember 在 Agent Skills 之上的产品投影层。
...
除了分层边界，Ember Skill 在设计上还应遵守下面几条补充原则。
...
1. 不再新造第二种 Skill 包格式。
2. Ember 私有字段统一进入 `metadata.Ember_*`，不新增顶层私有 frontmatter。
3. 包标准层只回答“技能包长什么样”，不回答“在 Ember 里怎么执行”。
...
但说明层不是 Ember 的运行时事实源。

### 3. 输入与产品投影层

作用：

- 把标准 Skill 包投影成 Ember 可消费的输入、展示和产品对象
...
- `ServiceSkillItem` 是 Ember 客户端产品投影，不是 Agent Skills 原始包
...
如果后续需要把这些模式暴露给 Ember 的产品投影层，优先把原始包信息放在：

- `metadata.Ember_pattern_primary`
- `metadata.Ember_pattern_stack`
- `metadata.Ember_interaction_mode`
- `metadata.Ember_checkpoint_policy`
...
## Ember Runtime Profile v1

这一节定义的不是新的包格式，而是：

**一个 Agent Skills 兼容包进入 Ember 之后，最少需要被编译成哪些产品投影与运行时字段。**
...
- 如果原始包要携带这些信息，优先通过 `metadata.Ember_*` 命名空间进入编译层。
...
- 标准摘要层统一收敛到 `skillBundle.metadata.Ember_output_destination`
...
Google ADK 对 Ember 最值得借鉴的，不是“再做一个 ADK SkillToolset”，而是它的分层方式。
...
4. **激活后再开放额外工具**
   ...
   - Ember 可借鉴为 `metadata.Ember_additional_tools`
5. **脚本前资源物化**
   - 在临时工作目录里重建 skill bundle，再执行脚本

ClaudeCode 对 Ember 最值得借鉴的，不是“再做一个 ClaudeCode skills 系统”，而是它把 skills 当成宿主治理对象来处理。
...
1. 不把 `SkillToolset` 当成 Ember 最终产品形态。
2. 不把已激活 skill 直接等同于 `ServiceSkill` / `Scene`。
3. 不把 Skill 包原文直接当成 `service_scene_launch`、`browser_assist`、`automation_job` 等运行时协议。
4. 不把 ClaudeCode 的 `.claude/skills`、`/commands/`、plugin-only policy、bare mode 直接当成 Ember 的产品事实源。
...
- `skillBundle`
  ...
  - 以及 Ember 运行时真正需要的标准状态：`resourceSummary`、`standardCompliance`
- `ServiceSkillCatalog` / `ClientServiceSkillCatalog`
  - Ember 面向 Claw / 工作区 / 对话内 A2UI 卡的**产品投影层**
  - 负责表达：卡片文案、补参表单、执行绑定、结果去向、主题目标、自动化入口等业务语义

强约束：

- 不要把 `ServiceSkillItem` 上的产品展示字段误认为标准本体
- 也不要把外部 `SKILL.md` 原文直接当成 Ember 客户端协议
...
- Ember 客户端必须保留 seeded catalog 作为 offline / degrade 兜底，不能只依赖服务端在线目录

## 外部 `SKILL.md` 参考边界

外部 `SKILL.md` 仓库对 Ember 的帮助不只在“说明书结构”，还包括：
...
> 外部 `SKILL.md` 可以是 Ember 的技能包标准来源，但不能直接拿走 Ember 的运行时与产品定义权。
...
- [embercore-collaboration-entry.md](embercore-collaboration-entry.md)
