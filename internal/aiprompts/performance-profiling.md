...
- renderer DevTools 通过 `EMBER_ELECTRON_OPEN_DEVTOOLS=1 npm run electron:dev` 打开
...
- 即使手动设置 `EMBER_PROFILE=*`，release / 生产构建也会忽略这些开发诊断开关

### 1. Rust Trace 导出

开发环境下如果设置 `EMBER_PROFILE=trace`，并以 `dev-profiling` feature 启动，应用会把 span 导出为 Perfetto / Chrome Trace 可读的 JSON 文件。

- 输出目录：应用运行时目录下的 `profiles/`
- 自定义路径：`EMBER_PROFILE_TRACE_PATH=/abs/path/to/trace.json`
- 调试时自动打开主窗口 DevTools：`EMBER_ELECTRON_OPEN_DEVTOOLS=1`

### 2. Tokio Console 遥测

开发环境下如果设置 `EMBER_PROFILE=console`，并且以 `tokio-console` feature + `RUSTFLAGS=--cfg tokio_unstable` 编译，应用会暴露 Tokio Console 诊断端点。
...
`safeInvoke` 会在浏览器 Performance 面板中写入 `ember:safeInvoke:*` 的 User Timing 条目，便于把前端交互和 Electron IPC / App Server / legacy adapter 耗时对齐。
...
```bash
EMBER_PROFILE=trace EMBER_ELECTRON_OPEN_DEVTOOLS=1 APP_SERVER_BIN=/abs/path/to/app-server npm run electron:dev
```
...
```bash
EMBER_PROFILE=console APP_SERVER_BIN=/abs/path/to/app-server npm run electron:dev
```

如果还要同时看 trace，把 `EMBER_PROFILE` 设为 `trace,console`。
...
2. 查看 `ember:safeInvoke:*` 的 User Timing 条目
