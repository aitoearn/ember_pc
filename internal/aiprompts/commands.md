...
`internal/exec-plans/**`、旧路线图和 git history 中的旧 `ember-rs/src/**` 路径默认是历史 evidence，不是 current owner 引用；只在它们出现在当前规则段落、当前状态摘要或 active checklist 并被写成现役落点时才清理。
...
- 端自动化改动时同步 `internal/aiprompts/device-automation.md`
