...
- 调整 App Server / RuntimeCore / `ember-rs/crates/agent` 子代理 spawn、send input、wait、resume、close 能力
...
补充迁移边界：旧 `agent_runtime_*`、`automation_*`、`execution_run_*` 这类命令名只允许作为 retired guard、历史 evidence、测试 fixture 或受控迁移面；`ember-rs/src/commands/**` 已删除，不是新的 taxonomy、coordinator 或 runtime 实现目录。新增长时执行、子代理、自动化或执行摘要能力应进入 RuntimeCore / services / App Server protocol，不得恢复旧 wrapper。
...
- `ember-rs/crates/agent` Team / subagent runtime support
...
- `internal/aiprompts/query-loop.md`
- `ember-rs/crates/agent/src` 与 `ember-rs/crates/app-server/src` 承接协作 runtime；新逻辑按职责进入 RuntimeCore / services
