...
Lime 的后端业务事实源已经从旧 `ember-rs/src/services/**` 收敛到 App Server workspace crates、RuntimeCore、domain crates 与 Electron Desktop Host 壳能力。

新增业务逻辑不要恢复 `ember-rs/src/**`，也不要把 Electron main / preload 当第二套后端。默认路径是：

`前端 API 网关 -> App Server JSON-RPC -> ember-rs/crates/** domain owner`
...
| 能力                                             | current owner                                                 | 入口                                                            |
| ------------------------------------------------ | ------------------------------------------------------------- | --------------------------------------------------------------- |
| MCP server / tools / prompts / resources / OAuth | `ember-rs/crates/mcp` + App Server `mcp*` processor / protocol | `src/lib/api/mcp.ts`                                            |
| Agent session / turn / read model / evidence     | `ember-rs/crates/app-server` runtime 与 `ember-rs/crates/agent` | `src/lib/api/agentRuntime/**`                                   |
...
`src/lib/api/mcp.ts -> AppServerClient.request(...) -> app_server_handle_json_lines -> App Server JSON-RPC -> ember-rs/crates/mcp`

要点：

- server 配置、stdio / streamable HTTP transport、OAuth、tools/prompts/resources 都由 `ember-rs/crates/mcp` 管理。
...
```bash
cargo test --manifest-path "ember-rs/Cargo.toml" -p <crate> <filter>
```
