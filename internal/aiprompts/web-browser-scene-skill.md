# Ember Web / Browser Scene Skill 设计提案

## 这份文档回答什么

本文件把外部 `web-access` 一类 skill 仓库里值得借鉴的内容，翻译成 Ember 自己可长期演进的设计语言，主要回答：

- 外部 Web skill 对 Ember 到底有什么帮助
- 哪些能力适合吸收进 Ember，哪些不应该直接引入
- `Scene Skill`、`site-adapter`、浏览器运行时、`a2ui`、产物保存之间应该如何解耦
- 面向网页读取、登录态访问、长文导出、素材保存这类场景时，Ember 应该如何做一套可复用主线
...
- 想参考外部 `SKILL.md` 仓库改 Ember 的网页能力
- 想把某个网页导出需求做成长期产品能力，而不是一次性脚本
- 想给 `site-adapter` 增加预热、滚动、懒加载等待、媒体下载等通用能力

## 设计结论

### 1. 外部 `web-access` / Agent Skills 对 Ember 有帮助，但不能直接替代 Ember 运行时
...
- 外部工具仓库定义的运行时协议
- 外部环境变量约定和执行入口
- 把外部 `SKILL.md` 或 SkillToolset 直接当成 Ember 产品事实源

### 2. Ember 的长期真相仍然是 `Scene Skill`

网页类 slash 场景的长期真相必须保持为：

`scene -> gate request -> a2ui -> Scene Skill -> site-adapter -> Ember runtime -> artifact/viewer`
...
- Ember 浏览器运行时负责 CDP / session / tab 生命周期
...
这里的 `SKILL.md` 应优先理解为 **Agent Skills 兼容包入口**，不是 Ember 最终运行时协议。
...
运行时预检查应该是 Ember 自己的过程步骤，而不是外部脚本真相。
...
外部 skill 里“先滚动、等稳定、再提取”的思路非常值得保留，但应下沉为 Ember runtime helper，而不是保留外部 daemon。

### 4. 最小侵入原则

默认不打扰用户已有 tab、尽量使用受控 tab、完成后回收临时上下文，这类原则适合写进 Ember 的浏览器场景规范。

## 明确不要做的事情

- 不引入第二套浏览器 runtime
- 不在 Ember 内部再接一套外部 daemon
- 不把外部 `SKILL.md` 当成 Ember 协议
- 不把站点逻辑写死在 slash 组件里
...
> 外部 `web-access` 最值得借鉴的是“怎么设计强 skill”，不是“把它的 proxy 搬进 Ember”。
