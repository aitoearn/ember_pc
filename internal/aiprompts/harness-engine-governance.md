# Ember Harness Engine 治理规范

## 这份文档回答什么

本文件定义 Ember 的 harness Engine 应该如何长期演进，主要回答：
...
- 当 Ember 对齐 Claude Code 的治理方式时，哪些原则应该真正落到代码里
...
Managed Objective 的详细路线图见 `internal/roadmap/managed-objective/README.md`。在 harness 语境中，它只能是 evidence pack 的消费方和审计派生物，不能成为新的证据导出链。
...
因此 Ember 必须把下面这些键视为 harness 级基础设施，而不是锦上添花：
...
- `.ember/harness/sessions/<session_id>/evidence/runtime.json`
- `.ember/harness/sessions/<session_id>/evidence/artifacts.json`
- `.ember/harness/sessions/<session_id>/replay/input.json`
- `.ember/harness/sessions/<session_id>/analysis/analysis-context.json`
