...
- `npm run governance:file-size` 当前失败，但失败来自本轮外 Rust 写集：`ember-rs/crates/app-server/src/runtime/conversation_import/codex/events.rs` 新文件 1048 行超 800，以及 `ember-rs/crates/app-server/src/runtime/thread_item_projection.rs` 839→981 超基线；本轮前端主文件仍下降，未新增前端体量违例。该 Rust 体量漂移应单独回到 R-60/轴 B 处理，不在本刀顺手修。
