...
Ember 的 Skills 集成基于 aster-rust 框架的 `SkillTool`：

```
用户消息 → AI Agent → SkillTool → global_registry → 执行 Skill
                                        ↑
                                        |
                    load_ember_skills() 加载 ~/.ember/skills/
```

关键组件：
- `AsterAgentState::load_ember_skills()` - 启动时加载 Skills
- `AsterAgentState::reload_ember_skills()` - 安装/卸载后刷新
...
1. 创建测试 Skill：
```bash
mkdir -p ~/.ember/skills/test-greeting
cat > ~/.ember/skills/test-greeting/SKILL.md << 'EOF'
---
name: test-greeting
...
4. **预期结果**：
   - 日志中应显示 `[AsterAgent] 成功加载 1 个 Ember Skills 到 global_registry`
...
1. 在 Ember 运行时，创建新 Skill：
```bash
mkdir -p ~/.ember/skills/test-calculator
cat > ~/.ember/skills/test-calculator/SKILL.md << 'EOF'
---
...
2. 在 Ember Skills 页面点击刷新（或重新进入页面）
...
1. 删除测试 Skill：
```bash
rm -rf ~/.ember/skills/test-greeting
```

2. 在 Ember Skills 页面点击刷新
...
```bash
rm -rf ~/.ember/skills/test-greeting
rm -rf ~/.ember/skills/test-calculator
```
