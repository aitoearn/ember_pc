# 开发 Ember 之后，我越来越相信：做 AI 软件，最难的不是追模型，而是追工程系统

> Ember 这段时间最重要的变化，不是又补了几个工具，也不是又修了几个 UI 细节，而是我们开始把自己当成一个大型软件来治理：参考 Codex，也参考 Claude Code，但最终要长出 Ember 自己的事实源。

最近开发 Ember，有一个感受越来越强：
...
Ember 这段时间做的事情，本质上就是：一边做自己的 GUI 桌面产品，一边把 Codex 和 Claude Code 这类大型软件里的成熟工程经验，尽可能吸收到 Ember 的 current 主链里。
...
我们优先参考 Codex，是因为它仍在持续更新维护，而且它的仓库规则、协议、测试和治理方式更适合长期跟随。但 Ember 不是 CLI，Ember 是桌面 GUI 产品。很多地方不能照搬目录结构，也不能照搬终端输出形态。
...
Ember 后来把很多规则也写进了自己的 `AGENTS.md` 和 `internal/aiprompts/`：
...
## 我们把 Ember 拆成几条 current 主链，而不是到处补功能
...
Claude Code 对 Ember 的影响，更多是在产品形态和长期运行时上。
...
回头看，Ember 现在已经有一些地方不是“像”，而是真的开始对齐大型软件的工程方式。
...
这是我们现在做 Ember 时越来越明确的原则：

**观点写成文章，差距写成计划，规则写进仓库，验证交给工具。**

## 真正学到的东西：大型 AI 软件不是靠灵感堆出来的

开发 Ember 越久，我越觉得，大型 AI 软件的难点不在“模型会不会回答”。
...
这也是 Ember 对齐 Codex 和 Claude Code 最大的收获。
...
这个问题问对了，Ember 才会越来越像一个能长期维护的大型软件。
