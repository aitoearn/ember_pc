...
- `release-package-startup-smoke` 低成本门槛通过：`npm run verify:app-version` 与 `npm run verify:gui-smoke` 通过；首次 smoke 暴露 `app-server` 编译阻断，current worktree 中的 orchestration-based 修正复核后，`cargo check --manifest-path "ember-rs/Cargo.toml" -p app-server` 与重跑 `verify:gui-smoke` 通过；当前仅证明 source-tree startup，不证明 installer artifact。
...
- 用正确包名执行 `cargo check --manifest-path "ember-rs/Cargo.toml" -p lime-media-runtime` 通过，确认此前 `media-runtime` unclosed delimiter 阻断不是当前可复现编译错误。
...
- 首次执行 `npm run verify:gui-smoke` 时，`electron:build:app-server` 暴露 current `app-server` Rust 编译阻断；复核 current worktree 中的 orchestration-based 修正后，`cargo check --manifest-path "ember-rs/Cargo.toml" -p app-server` 通过，重跑 `npm run verify:gui-smoke` 自然收口。
