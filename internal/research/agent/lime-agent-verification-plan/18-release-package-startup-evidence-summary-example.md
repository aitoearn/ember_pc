...
结论：前两项通过，第三项通过且明确是 source-tree scope，第四项仍不通过。首次执行 `verify:gui-smoke` 时，`electron:build:app-server` 暴露 `ember-rs/crates/app-server/src/runtime/plugin_worker_turn.rs` 的编译阻断；当前工作树中的 orchestration-based 修正已存在，`cargo check --manifest-path "ember-rs/Cargo.toml" -p app-server` 通过后，`verify:gui-smoke` 重跑通过。
...
```json
{
  ...
  "commands": [
    ...
    {
      "command": "cargo check --manifest-path \"ember-rs/Cargo.toml\" -p app-server",
      "status": "pass",
  ...
  "initial_blocker": {
    "command": "npm run verify:gui-smoke",
    "failure": "electron:build:app-server 阶段因 app-server Rust 编译失败中断，错误集中在 ember-rs/crates/app-server/src/runtime/plugin_worker_turn.rs 的 PaneActionWorkerTurn 初始化缺字段，以及缺少 manifest_workflow_hook_policy。",
    "resolution": "current worktree 中的 orchestration-based 修正已存在；`cargo check --manifest-path \"ember-rs/Cargo.toml\" -p app-server` 通过后，重新执行 `verify:gui-smoke` 通过。"
  },
  "runtime_fix": {
