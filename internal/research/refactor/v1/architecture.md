...
| 后端入口 | `codex-rs/app-server` 是统一后端 | `packages/server` + protocol groups | `ember-rs/crates/app-server` 是 JSON-RPC 后端事实源 | 采用 Codex 的单后端入口；不采用 opencode server 架构 |
...
| App Server processor | `app-server/src/message_processor.rs`、`request_processors/*` | `ember-rs/crates/app-server/src/processor/*` | processor 薄分发，业务进 runtime/domain |
...
- `ember-rs/crates/app-server-protocol/src/protocol/v0/*`
- `ember-rs/crates/app-server-protocol/src/schema_export.rs`
...
| Rust runtime domain | `npm run test:rust:related -- <paths>` 或定向 `cargo test --manifest-path "ember-rs/Cargo.toml"` |
