...
1. Lime current 主链已经从旧 Tauri wrapper 收敛到 `React GUI -> src/lib/api/* -> AppServerClient / safeInvoke -> Electron Desktop Host -> App Server JSON-RPC -> ember-rs/crates/** -> read model / projection -> GUI`。
2. `ember-rs/src/**` 当前不存在，已属于 `dead / deleted / forbidden-to-restore`；Rust current owner 是 `ember-rs/crates/**`。
...
| Rust workspace | `ember-rs/Cargo.toml`、`ember-rs/crates/**` |
| App Server protocol | `ember-rs/crates/app-server-protocol/src/protocol/v0/**`、`schema_export.rs`、`schema_export/registry.rs`、`schema_fixtures.rs` |
| App Server runtime | `ember-rs/crates/app-server/src/processor/**`、`ember-rs/crates/app-server/src/runtime/**` |
| Agent runtime | `ember-rs/crates/agent/src/**`、`ember-rs/crates/runtime-core/src/**` |
| 多模型 / 多模态 | `ember-rs/crates/model-provider/**`、`src/lib/types/modelRegistry.ts`、`runtime-core/src/llm_protocol/**` |
...
flowchart TB
    ...
    Server[App Server JSON-RPC<br/>ember-rs/crates/app-server]
...
```mermaid
flowchart LR
    Cargo[ember-rs/Cargo.toml<br/>members = crates/*]

    subgraph Current[ember-rs/crates/** current owner]
    ...
    Deleted[ember-rs/src/**<br/>dead / deleted / forbidden-to-restore]
...
| 分类 | 路径 / crate | 判断 |
| --- | --- | --- |
| `current` | `ember-rs/crates/app-server` | App Server JSON-RPC 后端事实源 |
| `current` | `ember-rs/crates/app-server-protocol` | protocol v0、domain types、method names、schema fixtures |
| `current` | `ember-rs/crates/app-server-client`、`app-server-transport`、`app-server-daemon`、`app-server-test-client` | client / sidecar / test client 链 |
| `current` | `ember-rs/crates/agent`、`agent-protocol`、`agent-runtime` | Agent runtime、turn execution、tool、context、projection adapter |
| `current` | `ember-rs/crates/runtime-core` | provider-neutral LLM protocol、runtime primitives |
...
| `dead / deleted / forbidden-to-restore` | `ember-rs/src/**`、`ember-rs/src/commands/**` | 当前文件系统不存在，不得恢复旧 Tauri wrapper |

体量风险：

| 文件 | 当前行数 | 现状判断 |
| --- | ---: | --- |
| `ember-rs/crates/app-server/src/processor/dispatch.rs` | 692 | 接近 800 行预警，后续只应薄 dispatch |
| `ember-rs/crates/app-server/src/runtime.rs` | 693 | 接近 800 行预警，新增逻辑应进 `runtime/**` 子模块 |
...
| 分类 | 路径 / 能力 | 当前判断 |
| --- | --- | --- |
| `current` | `ember-rs/crates/**` | Rust 后端、runtime、protocol、domain owner |
| `current` | `ember-rs/crates/app-server-protocol/src/protocol/v0/**` | App Server JSON-RPC v0 是当前协议，不为对齐 Codex 改名 |
...
| `dead / deleted / forbidden-to-restore` | `ember-rs/src/**`、`ember-rs/src/commands/**` | 当前文件系统不存在，不能恢复 bootstrap、runner、Tauri wrapper、stub 或 compat facade |
| `dead` | 旧 `agent_runtime_*` production command surface | 不得重新接回 Electron Host、App Server current、DevBridge truth、mock fallback |
| `dead` | 旧 Tauri command wrapper | 不得恢复到 `ember-rs/src/commands/**` |
...
| 旧 `ember-rs/src/**` | 当前已删除且仓库规则禁止恢复 | `dead / deleted / forbidden-to-restore` |
