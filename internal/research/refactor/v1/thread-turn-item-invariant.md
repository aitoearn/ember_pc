...
| Dead path | 是否会恢复 `agent_runtime_*`、`ember-rs/src/**` 或生产 mock fallback？ |

## 6. Current / Compat / Dead

| 分类 | 允许 |
| --- | --- |
| `current` | `ember-rs/crates/**`、App Server JSON-RPC、RuntimeCore / agent、ProjectionStore、read model、`src/lib/api/*`、GUI projection |
...
| `dead / forbidden-to-restore` | `ember-rs/src/**`、旧 Tauri command wrapper、旧 `agent_runtime_*` production surface、生产 mock fallback |
...
- Forbidden path check：<agent_runtime_* / ember-rs/src / mock fallback 是否未回流>
