...
src/components / features
  ...
  -> ember-rs/crates/app-server JSON-RPC
...
| 事实 | 路径 |
| --- | --- |
| App Server method 与 scope catalog 已存在 | `ember-rs/crates/app-server-protocol/src/protocol/v0/catalog.rs:1-39,1306-2698` |
| schema fixture 消费 scope | `ember-rs/crates/app-server-protocol/src/schema_fixtures.rs:1-72` |
| Desktop Host 只承接 sidecar/IPC | `electron/appServerHost.ts`、`electron/preload.ts`、`src/lib/dev-bridge/safeInvoke.ts` |
| runtime owner 已拆分 | `ember-rs/crates/agent-runtime/src/**`、`ember-rs/crates/runtime-core/src/**`、`ember-rs/crates/app-server/src/runtime/**` |
| projection/read model 已成形 | `ember-rs/crates/app-server/src/runtime/projection_store.rs`、`read_model/**`、`thread_item_projection/**` |
| GUI projection 已有独立目录 | `src/components/agent/chat/projection/**`、`components/**/timeline-utils/**` |
| provider-neutral content/event 已存在 | `ember-rs/crates/runtime-core/src/llm_protocol/**`、`runtime_content.rs` |

## 体量与聚合风险

| 文件 | 当前行数 | v2 判断 |
| --- | ---: | --- |
| `ember-rs/crates/app-server-protocol/src/protocol/v0/catalog.rs` | 2698 | 已集中 method/scope，但仍是巨型事实源；拆成按 domain 的声明文件并派生总 catalog |
| `ember-rs/crates/app-server/src/runtime.rs` | 710 | 接近 800 行门槛；只保留组装和 re-export |
| `ember-rs/crates/app-server/src/processor/dispatch.rs` | 725 | 只允许薄分派；新增 handler 不得继续堆叠 |
...
| 分类 | 当前事实 | v2 处理 |
| --- | --- | --- |
| `current` | `ember-rs/crates/**`、`packages/app-server-client`、`src/lib/api/*`、`electron/**`、ProjectionStore/read model | 继续收敛，不新增平级 owner |
...
| `dead` | 旧 `ember-rs/src/**`、旧 Tauri wrapper、旧 `agent_runtime_*` 生产命令、生产 mock fallback | 删除并加负向 guard；禁止恢复 |
...
wc -l "ember-rs/crates/app-server-protocol/src/protocol/v0/catalog.rs" \
