...
| --- | --- | --- | --- |
| JSON-RPC method/schema/notification | `ember-rs/crates/app-server-protocol` | `app-server-protocol/src/protocol/common.rs`、`v2/*` | Electron、React、provider wire |
| 请求分派 | `ember-rs/crates/app-server/src/processor/**` | `app-server/src/message_processor.rs`、`request_processors/*` | 巨型 `dispatch.rs` 业务逻辑 |
| Thread/Turn 执行 | `ember-rs/crates/agent-runtime` + `agent` | `core/src/session/**`、`core/src/tasks/**` | App Server handler、Renderer |
| canonical content/event/context | `ember-rs/crates/runtime-core` | `core/context`、OpenCode `packages/llm/src/schema/**` | provider body、GUI JSX |
| provider route/lowering/stream | `ember-rs/crates/model-provider` | OpenCode `packages/llm/src/route/**`、`protocols/**` | RuntimeCore mapper、Renderer |
| tool/approval/sandbox/dispatch | `ember-rs/crates/tool-runtime` | Codex `tools/**`、`core/src/tools/**` | UI 文案、Electron 业务命令 |
| MCP connection/catalog | `ember-rs/crates/mcp` | Codex `codex-mcp/connection_manager.rs` | Renderer 直连、provider lowering |
| skills metadata/selection | `ember-rs/crates/skills` | Codex `core-skills`、`ext/skills` | GUI 私有文本和 path ID |
| Thread/Turn/Item storage | `ember-rs/crates/thread-store` + App Server repository | Codex `thread-store/**` | rollout 文件成为 runtime truth |
