...
- `cargo test --manifest-path ember-rs/Cargo.toml -p thread-store`: pass, 21/21.
- `cargo check --manifest-path ember-rs/Cargo.toml -p thread-store`: pass.
- Scoped `rustfmt --check` and `git diff --check`: pass.
- `RUST_MIN_STACK=8388608 cargo test --manifest-path ember-rs/Cargo.toml -p app-server
