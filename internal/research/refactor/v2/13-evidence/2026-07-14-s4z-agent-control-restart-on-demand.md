...
- `cargo check --manifest-path ember-rs/Cargo.toml -p app-server --lib -q`：通过。
- `cargo test --manifest-path ember-rs/Cargo.toml -p app-server agent_control --lib -q`：12/12 通过，其中 restart-on-demand 为单条端到端 focused test。
