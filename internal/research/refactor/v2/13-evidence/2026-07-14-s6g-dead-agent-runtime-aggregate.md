...
- `cargo test --manifest-path ember-rs/Cargo.toml -p lime-agent --lib -q`：262 tests 通过；
- `cargo check --manifest-path ember-rs/Cargo.toml -p lime-agent`：通过；
- `cargo check --manifest-path ember-rs/Cargo.toml -p lime-agent -p app-server`：通过；
