...
- 保留边界：`ember-rs/crates/app-server/src/runtime/agent_control.rs` 与 gateway 的 `parent_session_id` 仅用于 AgentControl session lookup，不进入 projection 或 Evidence facts。
...
- `cargo test --manifest-path "ember-rs/Cargo.toml" -p app-server --lib export_evidence_pack_includes_multi_agent_team_facts`：1/1 通过。
