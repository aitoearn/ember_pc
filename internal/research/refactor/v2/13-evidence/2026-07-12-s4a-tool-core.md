...
- `CARGO_TARGET_DIR=/tmp/lime-refactor-v2-s4a-tool-core-target cargo test --manifest-path ember-rs/Cargo.toml -p tool-runtime tool_`: pass, 97 passed, 0 failed, 165 filtered.
- exact rustfmt check: pass.
- `cargo fmt --manifest-path ember-rs/Cargo.toml -p tool-runtime --check`: pass.
