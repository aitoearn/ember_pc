...
删除 `ember-rs/crates/agent/src/session_store.rs`、11 个 `session_store_*` sibling 和 `session_state_snapshot.rs`，同时移除 `agent/src/lib.rs` 的 module declaration 与 crate-root CRUD/read/DTO export。
...
- 搜索 `lime_agent::{create_session_sync, get_runtime_session_detail, list_sessions_sync, SessionDetail, SessionInfo, SessionStateSnapshot}` 等完整公开 surface，在 `ember-rs/crates/agent` 外为零生产命中。
...
- `cargo test --manifest-path ember-rs/Cargo.toml -p lime-agent --lib -q`：通过，265 tests。
- `cargo check --manifest-path ember-rs/Cargo.toml -p lime-agent`：通过。
- `cargo check --manifest-path ember-rs/Cargo.toml -p app-server -p lime-scheduler -p lime-server`：通过。
