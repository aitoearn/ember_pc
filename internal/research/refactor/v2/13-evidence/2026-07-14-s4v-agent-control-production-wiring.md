...
- `rustfmt --edition 2021 --check ember-rs/crates/app-server/src/runtime.rs ember-rs/crates/app-server/src/runtime/agent_control.rs`: passed.
- `git diff --check -- ember-rs/crates/app-server/src/runtime.rs ember-rs/crates/app-server/src/runtime/agent_control.rs`: passed.
- `CARGO_TARGET_DIR=/tmp/lime-s4v-target cargo check --manifest-path ember-rs/Cargo.toml -p app-server --lib`: passed. The only S4v warnings are expected dead-code warnings because this narrow internal boundary intentionally has no gateway caller yet.
- `RUST_MIN_STACK=8388608 cargo test --manifest-path ember-rs/Cargo.toml -p app-server agent_control --lib`: pass, 6/6. `2026-07-14-s4r8-local-data-source-fixture-sync` 已补齐 shared `LocalAppDataSource` fixture 的 router field，原并行 MCP 编译阻塞已解除。
