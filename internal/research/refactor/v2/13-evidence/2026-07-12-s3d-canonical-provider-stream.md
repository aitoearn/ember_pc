...
- `CARGO_TARGET_DIR=/tmp/lime-refactor-v2-verify-target cargo test --manifest-path ember-rs/Cargo.toml -p runtime-core -p model-provider --lib`
  - result: pass
  - `model-provider`: 115 passed（consumer cutover 前编译基线）
  - `runtime-core`: 65 passed
- `CARGO_TARGET_DIR=/tmp/lime-refactor-v2-verify-target cargo test --manifest-path ember-rs/Cargo.toml -p model-provider --lib`
  - result: pass
  - 118 passed，新增 finish reason、truncated stream 和 invalid tool JSON 回归。
- `CARGO_TARGET_DIR=/tmp/lime-refactor-v2-verify-target cargo test --manifest-path ember-rs/Cargo.toml -p agent-runtime provider_turn`
  - result: pass
  - 3 passed，覆盖工具续轮、并行工具调用和 canonical provider error。
- `CARGO_TARGET_DIR=/tmp/lime-refactor-v2-verify-target cargo check --manifest-path ember-rs/Cargo.toml -p lime-agent`
  - result: pass
- `git diff --check`
  - result: pass
