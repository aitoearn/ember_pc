...
- `cargo test --manifest-path ember-rs/Cargo.toml -p app-server-protocol`
  - protocol unit 48/48
  - schema fixture 1/1
- `cargo test --manifest-path ember-rs/Cargo.toml -p app-server server_request --lib`
  - 9/9
...
- `cargo check --manifest-path ember-rs/Cargo.toml -p app-server-protocol -p app-server`
  - pass，warning-free
