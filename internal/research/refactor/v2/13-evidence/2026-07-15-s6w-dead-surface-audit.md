...
2. `ember-rs/crates/thread-store/src/subagent_tree.rs` 只由自身测试和
...
3. `ember-rs/crates/agent/src/team_runtime_governor.rs` 只由自身测试与 crate root 的
   module/re-export 引用；在 `ember-rs/crates` 内没有外部 consumer。current team/runtime
...
- `rg` 在 `ember-rs/crates` 中只找到 `subagent_tree` 的 crate root module 行与自身文件，
