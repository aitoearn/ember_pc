...
- `cargo test --manifest-path ember-rs/Cargo.toml -p agent-runtime provider_turn --lib`：
  11/11，覆盖跨 Turn、跨 sampling attempt identity；完整 related Rust 结果见 S2o3 evidence。
- `cargo test --manifest-path ember-rs/Cargo.toml -p app-server --lib agent_control`：13/13。
