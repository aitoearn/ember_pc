...
- 最终 `cargo check --manifest-path ember-rs/Cargo.toml -p app-server --lib`：协调进程复用 `/tmp/lime-s4z-app-server-check` 在最终源码哈希上通过。原 owner 的 `/tmp/lime-s4x-final-verify` 首次尝试曾因 ENOSPC 失败，但未删除共享缓存，且该环境阻塞已由后续独立 target 复验关闭。
