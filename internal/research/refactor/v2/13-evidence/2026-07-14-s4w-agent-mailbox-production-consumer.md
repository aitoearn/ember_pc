...
CARGO_TARGET_DIR=/tmp/lime-s4w-check cargo check --manifest-path ember-rs/Cargo.toml \
  -p app-server --lib
  passed
...
RUST_MIN_STACK=8388608 CARGO_TARGET_DIR=/tmp/lime-s4w-structure cargo test \
  --manifest-path ember-rs/Cargo.toml -p app-server agent_mailbox_delivery --lib
