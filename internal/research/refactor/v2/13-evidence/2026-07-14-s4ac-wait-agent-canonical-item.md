...
- `cargo test --manifest-path ember-rs/Cargo.toml -p lime-agent canonical_wait_agent --lib`：1/1。
- `cargo test --manifest-path ember-rs/Cargo.toml -p lime-agent protocol:: --lib`：19/19。
- `cargo test --manifest-path ember-rs/Cargo.toml -p lime-agent current_provider_turn --lib`：13/13。
- `cargo check --manifest-path ember-rs/Cargo.toml -p lime-agent --lib`：通过。
