...
```text
cargo test --manifest-path ember-rs/Cargo.toml -p lime-mcp elicitation --lib
# 20 passed

cargo test --manifest-path ember-rs/Cargo.toml -p app-server mcp_elicitation --lib
# 7 passed

cargo test --manifest-path ember-rs/Cargo.toml -p tool-runtime mcp_connection --lib
# 8 passed

cargo test --manifest-path ember-rs/Cargo.toml -p app-server session_scope_rejects_turn_thread_mismatch --lib
# passed

cargo test --manifest-path ember-rs/Cargo.toml -p app-server delete_session_closes_exact_runtime_owner_before_removal --lib
# passed

cargo test --manifest-path ember-rs/Cargo.toml -p lime-mcp session_cancel_keeps_other_session_and_forwarded_waiter_is_adapter_owned --lib
# passed
