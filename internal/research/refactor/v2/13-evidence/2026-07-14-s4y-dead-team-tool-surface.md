...
- 删除 `ember-rs/crates/tool-runtime/src/collab_agent.rs` 与 `collab_agent/**`。
...
- `cargo test --manifest-path ember-rs/Cargo.toml -p tool-runtime --lib -q`：249/249。
- `cargo test --manifest-path ember-rs/Cargo.toml -p lime-agent agent_tools::catalog --lib -q`：9/9。
- `cargo test --manifest-path ember-rs/Cargo.toml -p lime-agent current_provider_turn --lib -q`：13/13。
- `cargo test --manifest-path ember-rs/Cargo.toml -p lime-core tool_calling --lib -q`：23/23。
