...
```text
CARGO_TARGET_DIR=/tmp/lime-s4x-final-check cargo test --manifest-path ember-rs/Cargo.toml \
...
CARGO_TARGET_DIR=/tmp/lime-s4x-final-verify cargo test --manifest-path ember-rs/Cargo.toml \
...
CARGO_TARGET_DIR=/tmp/lime-s4x-final-verify cargo check --manifest-path ember-rs/Cargo.toml \
  -p app-server --lib -q
  passed
