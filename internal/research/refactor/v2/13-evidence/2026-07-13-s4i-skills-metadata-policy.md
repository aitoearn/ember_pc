...
```text
cargo test --manifest-path ember-rs/Cargo.toml -p tool-runtime skill_ --lib
PASS: 20 tests

cargo test --manifest-path ember-rs/Cargo.toml -p tool-runtime --lib
PASS: 260 tests

cargo check --manifest-path ember-rs/Cargo.toml -p app-server
PASS (6 existing dead-code warnings)

cargo test --manifest-path ember-rs/Cargo.toml -p lime-skills --lib
PASS: 65 tests

cargo test --manifest-path ember-rs/Cargo.toml -p app-server-protocol --lib
PASS: 41 tests

RUST_MIN_STACK=8388608 cargo test --manifest-path ember-rs/Cargo.toml \
