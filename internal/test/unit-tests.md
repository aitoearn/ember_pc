...
单元测试是测试金字塔的基础，覆盖最小的可测试单元。Ember 的单元测试主要针对：
- 协议转换器
- Provider 模块
- 工具函数
- 数据结构
...
*.contract.test.ts    # DevBridge / desktop-host mock / App Server / command catalog 契约
*.integration.test.ts # 文件系统、子进程、本地 server、多模块流程
*.e2e.test.ts         # Vitest 内显式 E2E / smoke 测试
*.live.test.ts        # 真实 Provider / 真实联网，默认跳过
```

`*.live.test.ts` 归入 E2E 层，但默认不运行。确需运行时必须显式设置 `EMBER_ALLOW_LIVE_PROVIDER_SMOKE=1` 或 `EMBER_REAL_API_TEST=1`。

显式后缀不能降低风险层级。`*.unit.test.*` 只有在测试不触碰 React/jsdom、DevBridge、Desktop Host、App Server、文件系统、子进程、网络、Playwright 等外部边界时才会进入 `test:unit`；一旦出现这些边界，分类器会自动提升到 component / contract / integration / e2e。
...
2. 给这些纯逻辑补 `*.unit.test.ts`，不要依赖 React render、jsdom、真实 timer、DevBridge、Desktop Host、App Server、文件系统或网络。
