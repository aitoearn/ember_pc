...
> 面向 Ember `P3-6 Replay 样本沉淀` 的 current 事实源  
> 目标：把 replay 样本、grader 合同、仓库固定任务集与 nightly 摘要，收口到一条可执行主链。

## 先给结论

Ember 当前不直接把“真实模型重放平台”一次做完，而是先固定四件事：
...
如果没有固定 manifest 和 runner，Ember 当前的 replay 样本会停留在“可以导出”，却还不能稳定回答：
...
| `ember`       | 持有产品层 handoff bundle、evidence pack、workspace `.ember/` 样本目录 | runner、fixture、nightly 摘要与工作区发现逻辑                   |

一句话：

**Codex 决定评估形状，Aster 决定运行时事实边界，Ember 负责把 replay case、grader 和 nightly 摘要落到 current 主链。**
...
3. **工作区 Replay 自动发现**
   - 扫描 `.ember/harness/sessions/*/replay`
...
node scripts/harness/replay-promote.mjs \
  --replay-dir ".ember/harness/sessions/session-123/replay" \
  --slug "pending-request-runtime"
```
...
4. 把样本复制到 `internal/test/harness-fixtures/replay/<slug>/`，并把人工审核摘要回写到 manifest case。
...
如果当前已经在 Ember 工作台里导出了 Replay 样本，也可以直接在 `HarnessStatusPanel` 的 Replay 区块点击：
...
这样做的目的不是把 promotion 内建进 Ember，而是把仓库已有的 current 主命令直接挂到工作台，避免用户还要自己重新拼 `session-id / slug / title`，并且在 promotion 后立刻补上统一 trend 入口。
...
- 本地默认把 history window 写到 `./.ember/harness/history`，并把 `summary / trend / cleanup / dashboard` 全部写到 `./.ember/harness/reports`
...
- `cleanup report` 在未显式传入 `--trend-input / --trend-history-dir` 时，会优先自动发现 `./.ember/harness/history`，再回退到 `./artifacts/history`
...
# 默认产物目录：
# ./.ember/harness/reports/harness-eval-summary.json
# ./.ember/harness/reports/harness-eval-summary.md
# ./.ember/harness/reports/harness-eval-trend.json
# ./.ember/harness/reports/harness-eval-trend.md
# ./.ember/harness/reports/harness-cleanup-report.json
# ./.ember/harness/reports/harness-cleanup-report.md
# ./.ember/harness/reports/harness-dashboard.html

# 如需覆盖 dashboard 产物路径
node scripts/harness/eval-history-record.mjs \
  --history-dir "./.ember/harness/history" \
