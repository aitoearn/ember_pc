...
> 面向 Ember 当前桌面端产品形态的测试入口与索引

## 概述

Ember 当前是一个本地优先的 Electron Desktop Host + App Server JSON-RPC 桌面应用，而不是单一前端项目或单一 API 服务。legacy desktop adapter 只用于兼容和退役守卫，不再作为 current 交付事实源。
...
├── testing-strategy-2026.md     # 当前 Ember 主测试策略
...
├── ember-agent-qc-rollout-plan.md # Ember 样本产品落地计划
└── ember-agent-qc-current-blockers.md # 当前 P0 qcloop 阻断记录
```
...
| 文档                                                                                     | 说明                         | 适用场景                                                 |
| ---------------------------------------------------------------------------------------- | ---------------------------- | -------------------------------------------------------- |
| [testing-strategy-2026.md](testing-strategy-2026.md)                                     | 当前 Ember 测试体系建设建议   | 建立分层门禁、规划演进                                   |
...
| [../tests/ember-agent-qc-rollout-plan.md](../tests/ember-agent-qc-rollout-plan.md)         | Ember 落地计划                | 分阶段构建 qcloop 证据链、GUI/Runtime 深测、release gate |
| [../tests/ember-agent-qc-current-blockers.md](../tests/ember-agent-qc-current-blockers.md) | 当前 P0 阻断记录             | 真实 qcloop fail evidence、root cause、关闭条件          |
...
默认会把完整 artifact 套件写到 `./.ember/harness/reports/`：
...
```bash
# Rust
cargo clippy --manifest-path "ember-rs/Cargo.toml"
