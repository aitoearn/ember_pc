...
2. **SessionConfigBuilder** (ember-rs/crates/agent/src/aster_state_support.rs)
   - 用于构建 aster Agent 的会话配置
   - 当前字段：id、max_turns、system_prompt、include_context_trace
   - 缺少上下文压缩相关配置

3. **AsterAgentWrapper** (ember-rs/src/agent/aster_agent.rs)
...
- 当前内置路径：`ember-rs/crates/aster-rust/`
- 相关文件：
