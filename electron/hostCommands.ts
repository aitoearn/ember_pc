...
export class ElectronHostCommands {
  ...
  async invoke(
    ...
    switch (command) {
      ...
      case "device_automation_ensure_sidecar":
        return await deviceAutomationRuntime.ensure(readEnsureRuntimeParams(args));
      case "device_automation_get_sidecar_status":
        return await deviceAutomationRuntime.ensure();
      case "device_automation_list_devices":
        return await deviceAutomationRuntime.listDevices(readListDevicesParams(args));
      case "device_automation_capture_screenshot":
        return await deviceAutomationRuntime.captureScreenshot(readScreenshotParams(args));
      case "device_automation_send_navigation": {
        const navParams = readNavigationParams(args);
      ...
      case "device_automation_prepare_ai_session":
        return await deviceAutomationRuntime.prepareAiSession(
          readPrepareAiSessionParams(args),
        );
      case "device_automation_submit_ai_task":
        return await deviceAutomationRuntime.submitAiTask(readSubmitAiTaskParams(args));
      case "device_automation_poll_ai_task":
        return await deviceAutomationRuntime.pollAiTask(readPollAiTaskParams(args));
      case "device_automation_cancel_ai_task":
        return await deviceAutomationRuntime.cancelAiTask(
          readRequiredString(toRecord(args) ?? {}, "taskId"),
        );
      case "device_automation_send_tap": {
        const tapParams = readTapParams(args);
      ...
      case "device_automation_send_swipe": {
        const swipeParams = readSwipeParams(args);
      ...
      case "device_automation_scrcpy_reverse_tcp":
        return await deviceAutomationRuntime.reverseScrcpyTcp(
          readScrcpyReverseTcpParams(args),
        );
      case "device_automation_scrcpy_prepare_reverse":
        return await deviceAutomationRuntime.prepareScrcpyReverse(
          readScrcpyPrepareReverseParams(args),
        );
      case "device_automation_scrcpy_start":
        return await deviceAutomationRuntime.startScrcpy(readScrcpyStartParams(args));
      case "device_automation_scrcpy_connect":
        return await deviceAutomationRuntime.connectScrcpy(readScrcpyConnectParams(args));
      case "device_automation_scrcpy_launch":
        return await deviceAutomationRuntime.launchScrcpy(readScrcpyConnectParams(args));
      case "device_automation_scrcpy_prewarm":
        return deviceAutomationRuntime.prewarmScrcpy(readScrcpyPrewarmParams(args));
      case "device_automation_scrcpy_stop":
        return deviceAutomationRuntime.stopScrcpy(readScrcpyPrewarmParams(args));
      case "device_automation_scrcpy_teardown":
        return deviceAutomationRuntime.teardownScrcpy(readScrcpyTeardownParams(args));
      case "device_automation_stop_sidecar":
        await deviceAutomationRuntime.stop();
        return { stopped: true };
      case "device_automation_perf_list_apps":
        return deviceAutomationRuntime.listPerfApps(readPerfListAppsParams(args));
      case "device_automation_perf_start":
        return deviceAutomationRuntime.startPerfCollection(readPerfStartParams(args));
      case "device_automation_perf_stop":
        return deviceAutomationRuntime.stopPerfCollection(readPerfStopParams(args));
      case "device_automation_perf_get_status":
        return deviceAutomationRuntime.getPerfStatus();
      case "device_automation_monkey_start":
        return deviceAutomationRuntime.startMonkeyTest(readMonkeyStartParams(args));
      ...
      case "device_automation_perf_trace_stop":
        return deviceAutomationRuntime.stopPerfTraceCapture(
          readPerfTraceCaptureIdParams(args),
        );
      ...
      case "device_automation_perf_trace_analyze":
        return await deviceAutomationRuntime.analyzePerfTrace(readPerfTraceAnalyzeParams(args));
      case "device_automation_perf_trace_open_external":
        return deviceAutomationRuntime.openPerfTraceExternal(
          readPerfTraceOpenExternalParams(args),
        );
      case "device_automation_perf_trace_delete_local":
        return deviceAutomationRuntime.deletePerfTraceLocalFile(
          readPerfTraceDeleteLocalParams(args),
        );
      case "ui_agent_start":
        return await this.#startUiAgent(args);
      case "ui_agent_cancel": {
        const taskId = readRequiredString(toRecord(args) ?? {}, "taskId");
        const deviceId = readString(toRecord(args) ?? {}, "deviceId") ?? "";
      ...
      case "device_flow_record_manual_start":
        return deviceFlowRecordRuntime.start(readManualFlowRecordStartParams(args));
      case "device_flow_record_manual_stop":
        return deviceFlowRecordRuntime.stop(readManualFlowRecordStopParams(args));
      case "device_flow_replay_start":
        return await this.#startDeviceFlowReplay(args);
      case "device_flow_replay_cancel":
        return deviceFlowReplayRuntime.cancel(
          readRequiredString(toRecord(args) ?? {}, "runId"),
        );
   ...
   * 启动 UI Agent 任务。
   *
   * 凭证流（不经渲染层）：渲染层只传 providerId + model；Host 经
   * modelProvider/read 解析 baseUrl，再经 modelProviderKey/next 取明文 apiKey，
   * 注入自包含 sidecar（scripts/ui-agent/index.mjs）。
   */
  async #startUiAgent(args: HostArgs): Promise<{ taskId: string }> {
    const request = readRequest(args);
    const taskId = readRequiredString(request, "taskId");
    const instruction = readRequiredString(request, "instruction");
    const serial = readString(request, "serial") ?? "";
    const deviceId = readString(request, "deviceId") ?? serial;
    const model = readRequiredString(request, "model");

    // providerId 缺省时回退到默认 provider
    let providerId = readString(request, "providerId")?.trim() ?? "";
    if (!providerId) {
      providerId = await this.#getDefaultProvider();
    }
    if (!providerId) {
      throw new Error("未配置可用的模型 Provider，无法启动 UI Agent");
    }

    const providerResponse = await this.#appServerRequest<{
      provider?: Record<string, unknown> | null;
    }>(METHOD_MODEL_PROVIDER_READ, { providerId });
    const provider = toRecord(providerResponse.provider);
    const baseUrl =
      readString(provider, "api_host") ?? readString(provider, "apiHost") ?? "";
    if (!baseUrl) {
      throw new Error(`Provider ${providerId} 未配置 api_host（baseUrl）`);
    }

    const keyResponse = await this.#appServerRequest<{
      apiKey?: string | null;
      ...
      return uiAgentRuntime.start(
        {
          ...
          apiKey,
          model,
          maxSteps: readNumber(request, "maxSteps") ?? undefined,
          memoryWindow: readNumber(request, "memoryWindow") ?? undefined,
          packageName: readString(request, "packageName") ?? undefined,
          userNote: readString(request, "userNote") ?? undefined,
        },
        (channel, payload) => {
          ...
          if (
            ...
            ((payload as { type?: string }).type === "done" ||
              (payload as { type?: string }).type === "error" ||
              (payload as { type?: string }).type === "exit")
          ) {
            deviceActivityLock.release(deviceId, taskId);
          }
        },
  ...
  async #startDeviceFlowReplay(args: HostArgs): Promise<{ runId: string }> {
    const request = readRequest(args);
    ...
    const serial = readString(request, "serial") ?? "";
    const flow = toRecord(request.flow) as unknown as TestFlow | undefined;
    if (!flow) {
    ...
    let baseUrl = "";
    let apiKey = "";
    let model = readString(request, "model") ?? "";
  ...
  #getBrowserConnectorSettings(): Record<string, unknown> {
    ...
    return {
      ...
      install_dir: path.join(installRoot, "Ember Browser Connector"),
...
function readListDevicesParams(args: HostArgs): { force?: boolean } {
...
function readScreenshotParams(args: HostArgs): {
  platform: string;
  deviceId: string;
} {
  const record = toRecord(args) ?? {};
  return {
    platform: readRequiredString(record, "platform"),
    deviceId: readRequiredString(record, "deviceId"),
  };
}

function readPerfListAppsParams(args: HostArgs): {
  platform: string;
  deviceId: string;
} {
  return readScreenshotParams(args);
}

function readPerfStartParams(args: HostArgs): {
  platform: "android";
  deviceId: string;
  packageName: string;
  ...
  if (platform !== "android") {
    throw new Error("首期仅支持 Android 性能采集");
  }
  const metricsRaw = record.metrics;
  if (!Array.isArray(metricsRaw) || metricsRaw.length === 0) {
    throw new Error("metrics 不能为空");
  }
  const metrics = metricsRaw.map((value) => {
    if (value !== "cpu" && value !== "memory" && value !== "fps") {
      throw new Error("metrics 含非法指标");
    }
    return value;
  });
  const intervalMsRaw = record.intervalMs;
  if (typeof intervalMsRaw !== "number" || !Number.isFinite(intervalMsRaw)) {
    throw new Error("intervalMs 必须是数字");
  }
  const allowed = new Set([500, 1000, 2000, 5000]);
  if (!allowed.has(intervalMsRaw)) {
    throw new Error("intervalMs 必须是 500/1000/2000/5000 之一");
  }
  return {
    platform: "android",
    deviceId: readRequiredString(record, "deviceId"),
...
function readPerfStopParams(args: HostArgs): { sessionId: string } {
  const record = toRecord(args) ?? {};
  return {
    sessionId: readRequiredString(record, "sessionId"),
  };
}

function readStringListOptional(value: unknown): string[] | undefined {
...
function readExploreConfigOptional(
  value: unknown,
): import("../src/features/device-automation/explore/types").ExploreConfig | undefined {
  if (!value || typeof value !== "object") {
    return undefined;
  }
  const record = value as Record<string, unknown>;
  return {
    actWhitelist: readStringListOptional(record.actWhitelist) ?? [],
    actBlacklist: readStringListOptional(record.actBlacklist) ?? [],
    blockWidgetXpaths: readStringListOptional(record.blockWidgetXpaths) ?? [],
    blockTreeXpaths: readStringListOptional(record.blockTreeXpaths) ?? [],
  };
}

function readExploreRulesOptional(
  value: unknown,
): import("../src/features/device-automation/explore/types").ExploreRule[] | undefined {
  if (!Array.isArray(value)) {
    return undefined;
  }
  return value as import("../src/features/device-automation/explore/types").ExploreRule[];
}
...
function readMonkeyStartParams(args: HostArgs): {
  ...
  const throttleMsRaw = record.throttleMs;
  const seedRaw = record.seed;
  const runningMinutesRaw = record.runningMinutes;
  const profilePeriodRaw = record.profilePeriod;
  const modeRaw = record.mode;
  const fastbotAgentRaw = record.fastbotAgent;
  return {
    deviceId: readRequiredString(record, "deviceId"),
    packageName: readRequiredString(record, "packageName"),
    mode:
    ...
    throttleMs:
      typeof throttleMsRaw === "number" && Number.isFinite(throttleMsRaw)
        ...
        : undefined,
    seed:
      typeof seedRaw === "number" && Number.isFinite(seedRaw) ? seedRaw : undefined,
    runningMinutes:
      typeof runningMinutesRaw === "number" && Number.isFinite(runningMinutesRaw)
        ? runningMinutesRaw
        : undefined,
    ignoreCrashes:
      typeof record.ignoreCrashes === "boolean" ? record.ignoreCrashes : undefined,
    ignoreTimeouts:
      typeof record.ignoreTimeouts === "boolean"
        ? record.ignoreTimeouts
        : undefined,
    profilePeriod:
      typeof profilePeriodRaw === "number" && Number.isFinite(profilePeriodRaw)
        ? profilePeriodRaw
        : undefined,
    takeScreenshots:
      typeof record.takeScreenshots === "boolean"
        ? record.takeScreenshots
        : undefined,
    fastbotAgent:
      fastbotAgentRaw === "double-sarsa" || fastbotAgentRaw === "sarsa"
        ? fastbotAgentRaw
        : undefined,
    deviceOutputRoot:
      typeof record.deviceOutputRoot === "string"
        ? record.deviceOutputRoot.trim()
        : undefined,
...
function readPerfTraceStartParams(args: HostArgs): {
  ...
  const record = toRecord(args) ?? {};
  const presetId = readRequiredString(record, "presetId");
  if (
    ...
    throw new Error("presetId 非法");
  }
  const configOverride = readString(record, "configOverride") ?? undefined;
  const linkedSessionId = readString(record, "linkedSessionId") ?? undefined;
  return {
    deviceId: readRequiredString(record, "deviceId"),
    packageName: readRequiredString(record, "packageName"),
    localTracesDir: readRequiredString(record, "localTracesDir"),
    presetId,
...
function readPerfTraceCaptureIdParams(args: HostArgs): { captureId: string } {
  const record = toRecord(args) ?? {};
  return {
    captureId: readRequiredString(record, "captureId"),
  };
}
...
function readPerfTraceAnalyzeParams(args: HostArgs): {
  ...
  let timeRange: { startNs: number; endNs: number } | undefined;
  if (timeRangeRaw && typeof timeRangeRaw === "object") {
    const rangeRecord = timeRangeRaw as Record<string, unknown>;
    const startNs = rangeRecord.startNs;
    const endNs = rangeRecord.endNs;
...
function readPerfTraceOpenExternalParams(args: HostArgs): {
  localPath: string;
  target: "perfetto_ui";
} {
  const record = toRecord(args) ?? {};
  ...
  if (target !== "perfetto_ui") {
    throw new Error("target 仅支持 perfetto_ui");
  }
  return {
    localPath: readRequiredString(record, "localPath"),
    target: "perfetto_ui",
...
function readPerfTraceDeleteLocalParams(args: HostArgs): { localPath: string } {
  const record = toRecord(args) ?? {};
  return {
    localPath: readRequiredString(record, "localPath"),
  };
...
function readNavigationParams(args: HostArgs): {
  action: "back" | "home";
  platform: string;
  deviceId: string;
} {
  ...
  if (action !== "back" && action !== "home") {
    throw new Error("action 必须是 back 或 home");
  }
  return {
    action,
    platform: readRequiredString(record, "platform"),
...
function readPrepareAiSessionParams(args: HostArgs): {
  deviceId: string;
  deviceSerial: string;
  ...
  return {
    deviceId: readRequiredString(record, "deviceId"),
    deviceSerial: readRequiredString(record, "deviceSerial"),
    mode:
      mode === "classic" || mode === "layered" ? mode : undefined,
  };
}

function readSubmitAiTaskParams(args: HostArgs): {
  sessionId: string;
  message: string;
} {
  const record = toRecord(args) ?? {};
...
function readPollAiTaskParams(args: HostArgs): {
  taskId: string;
  afterSeq?: number;
} {
  const record = toRecord(args) ?? {};
  const afterSeqRaw = record.afterSeq;
  return {
    taskId: readRequiredString(record, "taskId"),
    afterSeq:
      typeof afterSeqRaw === "number" && Number.isFinite(afterSeqRaw)
        ? afterSeqRaw
        : undefined,
  };
}

function readTapParams(args: HostArgs): {
  platform: string;
  deviceId: string;
  x: number;
  y: number;
...
function readManualFlowRecordStartParams(args: HostArgs): {
  recordId: string;
  deviceId: string;
  serial: string;
  screenWidth?: number;
  ...
  const record = toRecord(args) ?? {};
  const screenWidth = readNumber(record, "screenWidth");
  const screenHeight = readNumber(record, "screenHeight");
  return {
    recordId: readRequiredString(record, "recordId"),
    deviceId: readRequiredString(record, "deviceId"),
    serial: readString(record, "serial") ?? "",
    screenWidth:
...
function readManualFlowRecordStopParams(args: HostArgs): { recordId: string } {
  const record = toRecord(args) ?? {};
  return { recordId: readRequiredString(record, "recordId") };
}

function readSwipeParams(args: HostArgs): {
  platform: string;
  deviceId: string;
  x1: number;
  y1: number;
  ...
  return {
    platform: readRequiredString(record, "platform"),
    deviceId: readRequiredString(record, "deviceId"),
    x1: readRequiredNumber(record, "x1"),
    y1: readRequiredNumber(record, "y1"),
    x2: readRequiredNumber(record, "x2"),
    y2: readRequiredNumber(record, "y2"),
  };
}

function readScrcpyPrepareReverseParams(args: HostArgs): {
  deviceId: string;
  remote: string;
} {
  const record = toRecord(args) ?? {};
  return {
    deviceId: readRequiredString(record, "deviceId"),
    remote: readRequiredString(record, "remote"),
  };
}

function readScrcpyTeardownParams(args: HostArgs): {
  deviceId: string;
  remote: string;
  killServer?: boolean;
} {
  ...
  return {
    deviceId: readRequiredString(record, "deviceId"),
    remote: readRequiredString(record, "remote"),
    killServer: typeof record.killServer === "boolean" ? record.killServer : undefined,
  };
}

function readScrcpyReverseTcpParams(args: HostArgs): {
  deviceId: string;
  remote: string;
  localPort: number;
} {
  ...
  return {
    deviceId: readRequiredString(record, "deviceId"),
    remote: readRequiredString(record, "remote"),
    localPort: readRequiredNumber(record, "localPort"),
  };
}

function readScrcpyStartParams(args: HostArgs): {
  deviceId: string;
  scid: string;
  maxSize?: number;
  videoBitRate?: number;
  ...
  return {
    deviceId: readRequiredString(record, "deviceId"),
    scid: readRequiredString(record, "scid"),
    maxSize: readOptionalNumber(record, "maxSize"),
    videoBitRate: readOptionalNumber(record, "videoBitRate"),
    audio: typeof record.audio === "boolean" ? record.audio : undefined,
  };
}

function readScrcpyPrewarmParams(args: HostArgs): { deviceId: string } {
  const record = toRecord(args) ?? {};
  return {
    deviceId: readRequiredString(record, "deviceId"),
  };
...
function readScrcpyConnectParams(args: HostArgs): {
  deviceId: string;
  remote: string;
  localPort: number;
  scid: string;
...
function readEnsureRuntimeParams(args: HostArgs): {
  warmDevice?: { platform: string; deviceId: string };
} {
  const record = toRecord(args) ?? {};
  const warmDeviceRecord = toRecord(record.warmDevice);
  if (!warmDeviceRecord) {
    return {};
  }
  const platform = warmDeviceRecord.platform;
  const deviceId = warmDeviceRecord.deviceId;
  if (typeof platform !== "string" || typeof deviceId !== "string") {
    return {};
  }
  return {
    warmDevice: { platform, deviceId },
  };
}
...
function buildAgentAppRuntimeTaskMessage(
  ...
  return [
    ...
    "",
    "Runtime Boundary:",
    "- 请在 Ember AgentRuntime 主链中完成这个 App 业务任务。",
